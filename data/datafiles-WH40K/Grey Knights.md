# Grey Knights

## Units

### Brother-Captain
**Points:** 90 | **M:** 5" | **T:** 5 | **SV:** 2+ | **W:** 6 | **LD:** 6+ | **OC:** 1
**Keywords:** Brother-Captain, Terminator, Grey Knights, Psyker, Infantry, Imperium, Character

**Abilities:**

#### Leader (Abilities)
- **Description:** This model can be attached to the following units:
■ Brotherhood Terminator Squad
■ Paladin Squad

#### Hammerhand (Psychic) (Abilities)
- **Description:** While this model is leading a unit, melee weapons equipped by models in that unit have the [Lethal Hits] ability.

#### Eye of Judgement (Psychic) (Abilities)
- **Description:** Each time this model makes an attack, you can re-roll the Wound roll.

---

### Brother-Captain Stern [Legends]
**Points:** 90 | **M:** 5" | **T:** 5 | **SV:** 2+ | **W:** 6 | **LD:** 6+ | **OC:** 1
**Keywords:** Epic Hero, Character, Infantry, Imperium, Psyker, Grey Knights, Terminator, Brother-Captain Stern

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Nemesis force sword | Melee | Melee | 5 | 2+ | 6 | -2 | 2 | Psychic |

**Abilities:**

#### Leader (Abilities)
- **Description:** This model can be attached to the following units:
■ Brotherhood Terminator Squad
■ Paladin Squad

#### Exemplar of the Silvered Host (Abilities)
- **Description:** While this model is leading a unit, each time a model in that unit makes a melee attack, on a Critical Wound, the target suffers 1 mortal wound in addition to any normal damage.

#### Strands of Fate (Psychic) (Abilities)
- **Description:** The first time this model is destroyed, roll one D6 at the end of the phase. On a 2+, set this model back up on the battlefield as close as possible to where it was destroyed and not within Engagement Range of any enemy units, with its full wounds remaining.

---

### Brotherhood Champion
**Points:** 70 | **M:** 6" | **T:** 4 | **SV:** 2+ | **W:** 4 | **LD:** 6+ | **OC:** 1
**Keywords:** Character, Infantry, Psyker, Imperium, Grey Knights, Brotherhood Champion

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Nemesis force weapon | Melee | Melee | 5 | 2+ | 6 | -2 | 2 | Precision, Psychic |

**Abilities:**

#### Leader (Abilities)
- **Description:** This model can be attached to the following units:
■ Purgation Squad
■ Strike Squad

#### Clarion of Haste (Psychic) (Abilities)
- **Description:** While this model is leading a unit, that unit is eligible to declare a charge in a turn in which it Advanced.

#### Inspiring Exemplar (Abilities)
- **Description:** Each time this model destroys a Character model in the Fight phase, you gain 1CP and, until the end of the battle, add 1 to the Attacks characteristic of its Nemesis force weapon.

---

### Brotherhood Chaplain
**Points:** 65 | **M:** 5" | **T:** 5 | **SV:** 2+ | **W:** 5 | **LD:** 5+ | **OC:** 1
**Keywords:** Brotherhood Chaplain, Grey Knights, Terminator, Psyker, Imperium, Infantry, Character

**Abilities:**

#### Zealous Path (Abilities)
- **Description:** While this model is leading a unit, you can re-roll Charge rolls made for that unit.

#### Leader (Abilities)
- **Description:** This model can be attached to the following units:
■ Brotherhood Terminator Squad
■ Paladin Squad

---

### Brotherhood Librarian
**Points:** 80 | **M:** 5" | **T:** 5 | **SV:** 2+ | **W:** 5 | **LD:** 6+ | **OC:** 1
**Keywords:** Brotherhood Librarian, Grey Knights, Imperium, Terminator, Infantry, Character, Psyker

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Nemesis force weapon | Melee | Melee | 4 | 2+ | 6 | -1 | 2 | Psychic |
| Combi-weapon | Ranged | 24" | 1 | 4+ | 4 | 0 | 1 | Anti-infantry 4+, Devastating Wounds, Rapid Fire 1 |

**Abilities:**

#### Haloed in Soulfire (Psychic) (Abilities)
- **Description:** While this model is leading a unit, that unit can only be selected as the target of a ranged attack if the attacking model is within 18".

#### Leader (Abilities)
- **Description:** This model can be attached to the following units:
- Brotherhood Terminator Squad
- Paladin Squad

#### Sanctic Hood (Abilities)
- **Description:** While this model is leading a unit, models in that unit have the Feel No Pain 4+ ability against Psychic Attacks.

---

### Brotherhood Techmarine
**Points:** 70 | **M:** 6" | **T:** 4 | **SV:** 2+ | **W:** 4 | **LD:** 6+ | **OC:** 1
**Keywords:** Character, Infantry, Psyker, Imperium, Grey Knights, Brotherhood Techmarine

**Abilities:**

#### Leader (Abilities)
- **Description:** This model can be attached to the following units:
■ Strike Squad
■ Purgation Squad
■ Purifier Squad

#### Guardians of the Machine (Abilities)
- **Description:** Each time an enemy unit ends a Charge move within Engagement Range of one or more **Grey Knights Vehicle** models from your army and within 6" of this model's unit, you can target this unit with the Heroic Intervention Stratagem for 0CP, and can do so even if you have already targeted a different unit with that Stratagem this phase.

#### Blessing of the Omnissiah (Abilities)
- **Description:** In your Command phase, you can select one friendly **Grey Knights Vehicle model within 3" of this model. That model regains up to D3 lost wounds and, until the start of your next Command phase, each time that Vehicle** model makes an attack, add 1 to the Hit roll. Each model can only be selected for this ability once per turn.

#### Techmarine (Abilities)
- **Description:** While this model is within 3" of one or more friendly **Grey Knights Vehicle** units, this model has the Lone Operative ability.

---

### Brotherhood Terminator Squad
**Points:** 150 | **M:** 5" | **T:** 5 | **SV:** 2+ | **W:** 3 | **LD:** 6+ | **OC:** 2
**Keywords:** Battleline, Infantry, Grenades, Psyker, Imperium, Grey Knights, Terminator, Brotherhood Terminator Squad

**Abilities:**

#### Force Edge (Psychic) (Abilities)
- **Description:** Each time a model in this unit makes a melee attack that targets a unit (excluding **Vehicles** or **Monsters**), improve the Armour Penetration characteristic of that attack by 1.

#### Ancient's Banner (Abilities)
- **Description:** Add 1 to the Objective Control characteristic of models in the bearer’s unit.

#### Apothecary's narthecium (Abilities)
- **Description:** In your Command phase, if the bearer is not destroyed, you can return 1 destroyed model (excluding **Characters**) to the bearer’s unit.

---

### Castellan Crowe
**Points:** 90 | **M:** 6" | **T:** 4 | **SV:** 2+ | **W:** 5 | **LD:** 6+ | **OC:** 1
**Keywords:** Castellan Crowe, Grey Knights, Psyker, Imperium, Epic Hero, Infantry, Character

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Purifying Flame | Ranged | 18" | 3 | 2+ | 4 | -2 | 1 | Anti-Infantry 2+, Ignores Cover, Psychic |
| Black Blade of Antwyr | Melee | Melee | 5 | 2+ | 6 | -2 | 2 | Devastating Wounds, Precision |

**Abilities:**

#### Champion of the Order of Purifiers (Psychic) (Abilities)
- **Description:** While this model is leading a unit, add 1 to the Attacks characteristic of Purifying Flame weapons equipped by that unit.

#### Foesight (Psychic) (Abilities)
- **Description:** Each time this model makes an attack that targets a **Character** unit, you can re-roll the Hit roll.

#### Leader (Abilities)
- **Description:** This model can be attached to the following unit:
■ Purifier Squad

---

### Champion of Titan [Crucible]
**Points:** 90 | **M:** 6" | **T:** 4 | **SV:** 2+ | **W:** 4 | **LD:** 6+ | **OC:** 1
**Keywords:** Character, Infantry, Crucible, Psyker, Grenades, Imperium, Grey Knights, Champion of Titan

**Abilities:**

#### Might of Titan (Abilities)
- **Description:** Once per battle, at the start of a phase, this model can use this ability. If it does, until the end of the phase, add 3 to the Attacks and Strength characteristics of melee weapons equipped by this model.

#### Leader (Abilities)
- **Description:** This model can be attached to the following units:
- Purgation Squad
- Strike Squad

---

### Dreadknight Champion [Crucible]
**Points:** 210 | **M:** 8" | **T:** 8 | **SV:** 2+ | **W:** 13 | **LD:** 6+ | **OC:** 4
**Keywords:** Character, Crucible, Psyker, Imperium, Grey Knights, Vehicle, Walker, Dreadknight Champion

**Abilities:**

#### Surge of Wrath (Abilities)
- **Description:** Each time this model makes a melee attack that targets a Monster or Vehicle unit, you can re-roll the Hit roll, you can re-roll the Wound roll and you can re-roll the Damage roll.

#### Damaged: 1-4 Wounds Remaining (Abilities)
- **Description:** When Damaged, subtract 1 from Hit rolls when it makes attacks.

---

### Grand Master
**Points:** 95 | **M:** 5" | **T:** 5 | **SV:** 2+ | **W:** 7 | **LD:** 6+ | **OC:** 1
**Keywords:** Infantry, Character, Grenades, Psyker, Grey Knights, Terminator, Grand Master, Imperium

**Abilities:**

#### Might of Titan (Psychic) (Abilities)
- **Description:** Once per battle, at the start of the Fight phase, this model can use this ability. If it does, until the end of the phase, add 3 to the Attacks and Strength characteristics of melee weapons equipped by this model.

#### Leader (Abilities)
- **Description:** This model can be attached to the following units:
■ Brotherhood Terminator Squad
■ Paladin Squad

---

### Grand Master in Nemesis Dreadknight
**Points:** 225 | **M:** 8" | **T:** 8 | **SV:** 2+ | **W:** 13 | **LD:** 6+ | **OC:** 4
**Keywords:** Character, Vehicle, Walker, Psyker, Imperium, Grand Master in Nemesis Dreadknight, Grey Knights

**Abilities:**

#### Surge of Wrath (Psychic) (Abilities)
- **Description:** Each time this model makes a melee attack that targets a Monster or Vehicle unit, you can re-roll the Hit roll, you can re-roll the Wound roll and you can re-roll the Damage roll.

---

### Grand Master Voldus
**Points:** 110 | **M:** 5" | **T:** 5 | **SV:** 2+ | **W:** 7 | **LD:** 6+ | **OC:** 1
**Keywords:** Epic Hero, Character, Infantry, Psyker, Imperium, Grey Knights, Terminator, Grand Master Voldus

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Malleus Argyrum | Melee | Melee | 5 | 2+ | 10 | -2 | 3 | Psychic |
| Searing Purity | Ranged | 12" | D3+1 | 2+ | 12 | -2 | 2 | Devastating Wounds, Psychic |

**Abilities:**

#### Sanctuary (Psychic) (Abilities)
- **Description:** While this model is leading a unit, each time an attack targets that unit, subtract 1 from the Hit roll.

#### Hammer Aflame (Psychic) (Abilities)
- **Description:** Each time this model’s unit is selected to fight, you can select one enemy unit within Engagement Range of this model’s unit and roll one D6: on a 2-3, that enemy unit suffers 1 mortal wound; on a 4-5, that enemy unit suffers D3 mortal wounds; on a 6+, that enemy unit suffers D3+3 mortal wounds.

#### Leader (Abilities)
- **Description:** This model can be attached to the following units:
■ Brotherhood Terminator Squad
■ Paladin Squad

---

### Grey Knights Dreadnought [Legends]
**Points:** 160 | **M:** 6" | **T:** 9 | **SV:** 2+ | **W:** 8 | **LD:** 6+ | **OC:** 3
**Keywords:** Dreadnought, Grey Knights, Walker, Vehicle, Psyker, Smoke, Imperium

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| ➤ Missile launcher - frag | Ranged | 48" | D6 | 3+ | 4 | 0 | 1 | Blast |
| ➤ Missile launcher - krak | Ranged | 48" | 1 | 3+ | 9 | -2 | D6 | - |

**Abilities:**

#### Wisdom of the Ancients (Aura) (Abilities)
- **Description:** While a friendly Grey Knights Infantry unit is within 6" of this model, each time a model in that unit makes an attack, re-roll a Hit roll of 1 and re-roll a Wound roll of 1.

---

### Grey Knights Relic Razorback [Legends]
**Points:** 95 | **M:** 12" | **T:** 9 | **SV:** 3+ | **W:** 10 | **LD:** 6+ | **OC:** 2
**Keywords:** Grey Knights, Dedicated Transport, Vehicle, Transport, Smoke, Imperium, Relic Razorback

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Twin Psycannon | Ranged | 24" | 3 | 3+ | 8 | -1 | 2 | Psychic, Twin-linked |

**Abilities:**

#### Fire Support (Abilities)
- **Description:** In your Shooting phase, after this model has shot, select one enemy unit it scored one or more hits against this phase. Until the end of the phase, each time a friendly model that disembarked from this Transport this turn makes an attack that targets that enemy unit, you can re-roll the Wound roll.

---

### Grey Knights Thunderhawk Gunship
**Points:** 805 | **M:** 20+" | **T:** 12 | **SV:** 2+ | **W:** 30 | **LD:** 6+ | **OC:** 0
**Keywords:** Thunderhawk Gunship, Grey Knights, Titanic, Vehicle, Fly, Transport, Aircraft, Imperium

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Lascannon | Ranged | 48" | 1 | 3+ | 12 | -3 | D6+1 | - |
| Armored Hull | Melee | Melee | 6 | 4+ | 8 | 0 | 1 | - |
| Thunderhawk heavy cannon | Ranged | 48" | D6+6 | 3+ | 10 | -2 | 3 | Blast |
| Turbo-laser destructor | Ranged | 96" | D3+1 | 3+ | 20 | -4 | D6+6 | Blast |
| Hellstrike missile battery | Ranged | 72" | 4 | 3+ | 8 | -2 | 3 | Anti-Fly 4+ |

**Abilities:**

#### Damaged: 1-10 Wounds Remaining (Abilities)
- **Description:** While this model has 1-10 wounds remaining, each time this model makes an attack, subtract 1 from the Hit roll.

#### Aerial Assault (Abilities)
- **Description:** Each time a unit with the Deep Strike ability disembarks from this model after it has made a Normal move, that unit is still eligible to declare a charge this turn.

#### Thunderhawk cluster bombs (Abilities)
- **Description:** Each time the bearer ends a Normal move, you can select one enemy unit it moved over during that move and roll six D6: for each 3+, that unit suffers 1 mortal wound.

---

### Interceptor Squad
**Points:** 125 | **M:** 12" | **T:** 4 | **SV:** 2+ | **W:** 2 | **LD:** 6+ | **OC:** 1
**Keywords:** Infantry, Grenades, Imperium, Fly, Psyker, Grey Knights, Interceptor Squad

**Abilities:**

#### Personal Teleporters (Abilities)
- **Description:** In your Shooting phase, after this unit has shot, if it is not within Engagement Range of one or more enemy units, it can make a Normal move of up to 6" as if it were your Movement phase. If it does, until the end of the turn, this unit is not eligible to declare a charge.

---

### Kaldor Draigo [Legends]
**Points:** 125 | **M:** 5" | **T:** 5 | **SV:** 2+ | **W:** 7 | **LD:** 6+ | **OC:** 1
**Keywords:** Epic Hero, Infantry, Character, Grenades, Psyker, Terminator, Imperium, Kaldor Draigo, Grey Knights

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Scourging | Ranged | 18" | 3 | 2+ | 6 | -2 | 2 | Anti-Daemon 2+, Ignores Cover, Psychic |
| The Titansword | Melee | Melee | 6 | 2+ | 8 | -4 | 3 | Anti-Daemon 2+, Psychic |

**Abilities:**

#### Untouchable Purity (Abilities)
- **Description:** While this model is leading a unit, models in that unit have the Feel No Pain 4+ ability against mortal wounds

#### One With the Warp (Psychic) (Abilities)
- **Description:** Once per battle, when this model’s unit declares a charge in the same turn it was set up on the battlefield using the Deep Strike or Teleport Assault ability, add 3 to the Charge roll.

#### Leader (Abilities)
- **Description:** This model can be attached to the following units:
■ Brotherhood Terminator Squad
■ Paladin Squad

#### Supreme Commander (Abilities)
- **Description:** If this model is in your army, it must be your WARLORD.

---

### Land Raider
**Points:** 220 | **M:** 10" | **T:** 12 | **SV:** 2+ | **W:** 16 | **LD:** 6+ | **OC:** 5
**Keywords:** Vehicle, Smoke, Transport, Imperium, Land Raider, Grey Knights

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Godhammer lascannon | Ranged | 48" | 2 | 3+ | 12 | -3 | D6+1 | - |

**Abilities:**

#### Assault Ramp (Abilities)
- **Description:** Each time a unit disembarks from this model after it has made a Normal move, that unit is still eligible to declare a charge this turn.

---

### Land Raider Banisher
**Points:** 255 | **M:** 10" | **T:** 12 | **SV:** 2+ | **W:** 16 | **LD:** 6+ | **OC:** 5
**Keywords:** Transport, Vehicle, Smoke, Imperium, Land Raider Banisher, Grey Knights

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Twin psycannon | Ranged | 24" | 3 | 3+ | 8 | -1 | 2 | Psychic, Sustained Hits 1, Twin-linked |

**Abilities:**

#### Assault Ramp (Abilities)
- **Description:** Each time a unit disembarks from this model after it has made a Normal move, that unit is still eligible to declare a charge this turn.

#### Land Raider Banisher (Abilities)
- **Description:** This model has a transport capacity of 12 Grey Knights Infantry models. Each Terminator model takes up the space of 2 models.

---

### Land Raider Crusader
**Points:** 220 | **M:** 12" | **T:** 12 | **SV:** 2+ | **W:** 16 | **LD:** 6+ | **OC:** 5
**Keywords:** Vehicle, Transport, Smoke, Imperium, Grenades, Land Raider Crusader, Grey Knights

**Abilities:**

#### Assault Ramp (Abilities)
- **Description:** Each time a unit disembarks from this model after it has made a Normal move, that unit is still eligible to declare a charge this turn.

---

### Land Raider Redeemer
**Points:** 270 | **M:** 12" | **T:** 12 | **SV:** 2+ | **W:** 16 | **LD:** 6+ | **OC:** 5
**Keywords:** Land Raider Redeemer, Grey Knights, Grenades, Imperium, Vehicle, Transport, Smoke

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Flamestorm cannon | Ranged | 12" | D6+3 | N/A | 6 | -2 | 2 | Ignores Cover, Torrent |

**Abilities:**

#### Assault Ramp (Abilities)
- **Description:** Each time a unit disembarks from this model after it has made a Normal move, that unit is still eligible to declare a charge this turn.

---

### Nemesis Dreadknight
**Points:** 210 | **M:** 8" | **T:** 8 | **SV:** 2+ | **W:** 13 | **LD:** 6+ | **OC:** 4
**Keywords:** Nemesis Dreadknight, Vehicle, Walker, Psyker, Imperium, Grey Knights

**Abilities:**

#### Indomitable Spirit (Psychic) (Abilities)
- **Description:** This model is eligible to shoot and declare a charge in a turn in which it Advanced or Fell Back.

---

### Paladin Squad
**Points:** 180 | **M:** 5" | **T:** 5 | **SV:** 2+ | **W:** 3 | **LD:** 6+ | **OC:** 1
**Keywords:** Infantry, Grenades, Imperium, Psyker, Grey Knights, Terminator, Paladin Squad

**Abilities:**

#### Attuned Onslaught (Psychic) (Abilities)
- **Description:** Each time this unit makes a Charge move, until the end of the turn, add 1 to the Damage characteristic of melee weapons equipped by **Paladin Squad** models in this unit.

#### Ancient's Banner (Abilities)
- **Description:** Add 1 to the Objective Control characteristic of models in the bearer’s unit.

#### Apothecary's narthecium (Abilities)
- **Description:** In your Command phase, if the bearer is not destroyed, you can return 1 destroyed model (excluding **Characters**) to the bearer’s unit.

---

### Purgation Squad
**Points:** 115 | **M:** 6" | **T:** 4 | **SV:** 2+ | **W:** 2 | **LD:** 6+ | **OC:** 1
**Keywords:** Purgation Squad, Psyker, Imperium, Grenades, Infantry, Grey Knights

**Abilities:**

#### Righteous Persecution (Abilities)
- **Description:** In your Shooting phase, after this unit has shot, select one enemy unit (excluding **Monsters** or **Vehicles**) hit by one or more of those attacks; until the start of your next turn, that enemy unit is pinned. While a unit is pinned, subtract 2 from that unit's Move characteristic and subtract 2 from Charge rolls made for it.

---

### Purifier Squad
**Points:** 125 | **M:** 6" | **T:** 4 | **SV:** 2+ | **W:** 2 | **LD:** 6+ | **OC:** 1
**Keywords:** Purifier Squad, Grey Knights, Psyker, Imperium, Grenades, Infantry

**Abilities:**

#### Sanctity of Purpose (Abilities)
- **Description:** Each time a model in this unit makes an attack, re-roll a Wound roll of 1. If the target is within range of an objective marker, you can re-roll the Wound roll instead.

---

### Razorback
**Points:** 85 | **M:** 12" | **T:** 9 | **SV:** 3+ | **W:** 10 | **LD:** 6+ | **OC:** 2
**Keywords:** Razorback, Grey Knights, Dedicated Transport, Vehicle, Transport, Smoke, Imperium

**Abilities:**

#### Fire Focus (Abilities)
- **Description:** In your Shooting phase, after this model has shot, select one enemy unit hit by one or more of those attacks. Until the end of the turn, each time a friendly model that disembarked from this **Transport** this turn makes an attack that targets that enemy unit, improve the Armour Penetration characteristic of that attack by 1. The same enemy unit can only be affected by this ability once per turn.

---

### Rhino
**Points:** 80 | **M:** 12" | **T:** 9 | **SV:** 3+ | **W:** 10 | **LD:** 6+ | **OC:** 2
**Keywords:** Rhino, Grey Knights, Transport, Vehicle, Dedicated Transport, Imperium, Smoke

**Abilities:**

#### Truesilver Aegis (Psychic) (Abilities)
- **Description:** While a friendly **Grey Knights** unit is wholly within 6" of this model, models in that unit have the Feel No Pain 6+ ability against mortal wounds.

---

### Servitors [Legends]
**Points:** 50 | **M:** 6" | **T:** 4 | **SV:** 4+ | **W:** 1 | **LD:** 8+ | **OC:** 0
**Keywords:** Infantry, Imperium, Servitors, Grey Knights

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Servo-arm | Melee | Melee | 1 | 5+ | 6 | -2 | 3 | - |
| Heavy bolter | Ranged | 36" | 3 | 4+ | 5 | -1 | 2 | Heavy, Sustained Hits 1 |
| Multi-melta | Ranged | 18" | 2 | 4+ | 9 | -4 | D6 | Heavy, Melta 2 |
| ➤ Plasma cannon - standard | Ranged | 36" | D3 | 4+ | 7 | -2 | 1 | Blast, Heavy |
| ➤ Plasma cannon - supercharge | Ranged | 36" | D3 | 4+ | 8 | -3 | 2 | Blast, Hazardous, Heavy |

**Abilities:**

#### Invulnerable Save (6+) (Abilities)
- **Description:** This unit has a 6+ Invulnerable Save

#### Mindlock (Abilities)
- **Description:** While a Brotherhood Techmarine model is leading this unit, improve the Ballistic Skill and Weapon Skill characteristics of ranged and melee weapons equipped by Servitor models in this unit by 1

#### Servitor Retinue (Abilities)
- **Description:** At the start of the Declare Battle Formations step, this unit can join one other unit from your army that is being led by a Brotherhood Techmarine (a unit cannot have more than one Servitors unit joined to it). If it does, until the end of the battle, every model in this unit counts as being part of that Bodyguard unit, and that Bodyguard unit’s Starting Strength is increased accordingly.

#### Retinue (Abilities)
- **Description:** While a Brotherhood Techmarine model is leading this unit, models in this unit have the Deep Strike and Teleport Assault abilities

---

### Stormhawk Interceptor
**Points:** 160 | **M:** 20+" | **T:** 9 | **SV:** 3+ | **W:** 10 | **LD:** 6+ | **OC:** 0
**Keywords:** Stormhawk Interceptor, Grey Knights, Vehicle, Aircraft, Fly, Smoke, Imperium

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Las-talon | Ranged | 36" | 2 | 3+ | 10 | -3 | D6+1 | - |
| Icarus stormcannon | Ranged | 48" | 6 | 3+ | 7 | -1 | 2 | Anti-Fly 2+ |

**Abilities:**

#### Interceptor (Abilities)
- **Description:** Each time this model makes a ranged attack that targets a unit that can **Fly**, add 1 to the Hit roll.

---

### Stormraven Gunship
**Points:** 280 | **M:** 20+" | **T:** 10 | **SV:** 3+ | **W:** 14 | **LD:** 6+ | **OC:** 0
**Keywords:** Stormraven Gunship, Grey Knights, Aircraft, Vehicle, Transport, Fly, Imperium

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Armoured hull | Melee | Melee | 6 | 4+ | 8 | 0 | 1 | - |
| Stormstrike missile launcher | Ranged | 48" | 1 | 3+ | 10 | -3 | D6+2 | - |
| ➤ Twin heavy plasma cannon - standard | Ranged | 36" | 3 | 3+ | 7 | -2 | 2 | Blast, Twin-linked |
| ➤ Twin heavy plasma cannon - supercharge | Ranged | 36" | 3 | 3+ | 8 | -3 | 3 | Blast, Hazardous, Twin-linked |
| Twin multi-melta | Ranged | 18" | 2 | 3+ | 9 | -4 | D6 | Melta 2, Twin-linked |

**Abilities:**

#### Armoured Resilience (Abilities)
- **Description:** Each time an attack is allocated to this model, subtract 1 from the Damage characteristic of that attack.

---

### Stormtalon Gunship
**Points:** 170 | **M:** 20+" | **T:** 9 | **SV:** 3+ | **W:** 10 | **LD:** 6+ | **OC:** 0
**Keywords:** Stormtalon Gunship, Grey Knights, Aircraft, Vehicle, Fly, Imperium

**Abilities:**

#### Strafing Run (Abilities)
- **Description:** Each time this model makes a ranged attack that targets a unit that cannot **Fly**, add 1 to the Hit roll

---

### Strike Squad
**Points:** 120 | **M:** 6" | **T:** 4 | **SV:** 2+ | **W:** 2 | **LD:** 6+ | **OC:** 2
**Keywords:** Strike Squad, Grey Knights, Psyker, Imperium, Grenades, Battleline, Infantry

**Abilities:**

#### Sanctifying Ritual (Psychic) (Abilities)
- **Description:** At the end of your Command phase, if this unit is within range of an objective marker that you control, that objective marker remains under you control until your opponent's Level of Control over that objective marker is greater than yours at the end of a phase.

---

### Venerable Daemon Slayer [Crucible]
**Points:** 175 | **M:** 8" | **T:** 9 | **SV:** 2+ | **W:** 8 | **LD:** 6+ | **OC:** 3
**Keywords:** Character, Crucible, Psyker, Imperium, Grey Knights, Vehicle, Walker, Smoke, Venerable Daemon Slayer

**Abilities:**

#### Guidance of the Ancients (Abilities)
- **Description:** In your Shooting phase, after this unit has shot, select one unit hit by one or more of those attacks. Until the end of the phase, each time a Grey Knights model from your army makes an attack that targets that unit, add 1 to the Hit roll.

---

### Venerable Dreadnought
**Points:** 140 | **M:** 8" | **T:** 9 | **SV:** 2+ | **W:** 8 | **LD:** 6+ | **OC:** 3
**Keywords:** Venerable Dreadnought, Grey Knights, Walker, Vehicle, Psyker, Smoke, Imperium, Character

**Abilities:**

#### Guidance of the Ancients (Psychic) (Abilities)
- **Description:** In your Shooting phase, after this unit has shot, select one enemy unit hit by one or more of those attacks. Until the end of the phase, each time a **Grey Knights** model from your army makes an attack which targets that unit, add 1 to the Hit roll.

---

## Points Table

| Unit | Points |
| --- | ---: |
| Servitors [Legends] | 50 |
| Brotherhood Chaplain | 65 |
| Brotherhood Champion | 70 |
| Brotherhood Techmarine | 70 |
| Brotherhood Librarian | 80 |
| Rhino | 80 |
| Razorback | 85 |
| Brother-Captain | 90 |
| Brother-Captain Stern [Legends] | 90 |
| Castellan Crowe | 90 |
| Champion of Titan [Crucible] | 90 |
| Grand Master | 95 |
| Grey Knights Relic Razorback [Legends] | 95 |
| Grand Master Voldus | 110 |
| Purgation Squad | 115 |
| Strike Squad | 120 |
| Interceptor Squad | 125 |
| Kaldor Draigo [Legends] | 125 |
| Purifier Squad | 125 |
| Venerable Dreadnought | 140 |
| Brotherhood Terminator Squad | 150 |
| Grey Knights Dreadnought [Legends] | 160 |
| Stormhawk Interceptor | 160 |
| Stormtalon Gunship | 170 |
| Venerable Daemon Slayer [Crucible] | 175 |
| Paladin Squad | 180 |
| Dreadknight Champion [Crucible] | 210 |
| Nemesis Dreadknight | 210 |
| Land Raider | 220 |
| Land Raider Crusader | 220 |
| Grand Master in Nemesis Dreadknight | 225 |
| Land Raider Banisher | 255 |
| Land Raider Redeemer | 270 |
| Stormraven Gunship | 280 |
| Grey Knights Thunderhawk Gunship | 805 |
