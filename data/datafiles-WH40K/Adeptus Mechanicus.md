# Adeptus Mechanicus

## Units

### Archaeopter Fusilave
**Points:** 160 | **M:** 20+" | **T:** 9 | **SV:** 3+ | **W:** 10 | **LD:** 7+ | **OC:** 0
**Keywords:** Archaeopter Fusilave, Vehicle, Aircraft, Fly, Imperium, Adeptus Mechanicus, Skitarii

**Abilities:**

#### Bomb Rack (Abilities)
- **Description:** Each time this model finishes a Normal move, you can select one enemy unit it moved across and roll six D6: for each 4+, that unit suffers 1 mortal wound.

#### Chaff Launcher (Abilities)
- **Description:** The bearer has the Smoke keyword.

#### Command Uplink (Abilities)
- **Description:** Each time you select the bearer’s unit as the target of a Stratagem, roll one D6: on a 5+, you gain 1CP.

---

### Archaeopter Stratoraptor
**Points:** 185 | **M:** 20+" | **T:** 9 | **SV:** 3+ | **W:** 10 | **LD:** 7+ | **OC:** 0
**Keywords:** Archaeopter Stratoraptor, Skitarii, Vehicle, Aircraft, Fly, Imperium, Adeptus Mechanicus

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Cognis heavy stubber | Ranged | 36" | 3 | 4+ | 4 | 0 | 1 | Rapid Fire 3, Sustained Hits 1 |
| Heavy phosphor blaster | Ranged | 36" | 3 | 4+ | 6 | -1 | 2 | Ignores Cover |
| Twin cognis lascannon | Ranged | 48" | 1 | 4+ | 12 | -3 | D6+1 | Sustained hits 1, Twin-linked |

**Abilities:**

#### Strafing Run (Abilities)
- **Description:** Each time this model makes a ranged attack that targets an enemy unit (excluding units that can Fly), add 1 to the Hit roll.

#### Chaff Launcher (Abilities)
- **Description:** The bearer has the Smoke keyword.'

#### Command Uplink (Abilities)
- **Description:** Each time you select the bearer’s unit as the target of a Stratagem, roll one D6: on a 5+, you gain 1CP.

---

### Archaeopter Transvector
**Points:** 150 | **M:** 20+" | **T:** 9 | **SV:** 3+ | **W:** 10 | **LD:** 7+ | **OC:** 0
**Keywords:** Archaeopter Transvector, Adeptus Mechanicus, Vehicle, Aircraft, Fly, Imperium, Skitarii, Transport

**Abilities:**

#### Aerial Deployment (Abilities)
- **Description:** If this model starts the game in Hover mode and in Strategic Reserves, it can be set up in the Reinforcements step of your first, second or third Movement phase, regardless of any mission rules.

#### Chaff Launcher (Abilities)
- **Description:** The bearer has the Smoke keyword.'

#### Command Uplink (Abilities)
- **Description:** Each time you select the bearer’s unit as the target of a Stratagem, roll one D6: on a 5+, you gain 1CP.

---

### Belisarius Cawl
**Points:** 210 | **M:** 8" | **T:** 8 | **SV:** 2+ | **W:** 10 | **LD:** 6+ | **OC:** 3
**Keywords:** Belisarius Cawl, Epic Hero, Monster, Character, Tech-Priest, Adeptus Mechanicus, Imperium, Cult Mechanicus

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Arc Scourge | Melee | Melee | 4 | 2+ | 5 | -1 | 1 | Anti-Vehicle 4+, Devastating Wounds, Extra Attacks |
| Cawl's Omnissian axe | Melee | Melee | 4 | 2+ | 8 | -2 | 2 | - |
| Mechadendrite hive | Melee | Melee | 2D6 | 3+ | 4 | 0 | 1 | Extra Attacks |
| Solar atomiser | Ranged | 18" | 3 | 2+ | 14 | -4 | D6 | Melta 3 |

**Abilities:**

#### Canticles of the Omnissiah (Abilities)
- **Description:** At the start of your Command phase, select one of the abilities in the Canticles of the Omnissiah section. Until the start of your next Command phase, this model has that ability.
Invocation of Machine Vengeance (Aura): At the start of your Command phase, select one unit from your opponent’s army. Until the start of your next Command phase, that enemy unit is your Machine Vengeance target. Each time a model in a friendly Adeptus Mechanicus unit makes an attack that targets your Machine Vengeance target, you can re‑roll the Hit roll.
Mantra of Discipline: This model has the Battleline keyword and has the following ability:
Binharic Courage (Aura): While a friendly Adeptus Mechanicus unit is within 6" of this model, add 1 to the Objective Control characteristic of models in that unit and each time you take a Battle-shock or Leadership test for that unit, add 1 to that test.’
Shroudpsalm (Aura): While a friendly Adeptus Mechanicus unit is within 6" of this model, that unit has the Stealth ability.

#### Mechanicus Bodyguard (Abilities)
- **Description:** While this model is within 3" of one or more other friendly **Adeptus Mechanicus** units, this model has the Lone Operative ability.

#### Self-repair Mechanisms (Abilities)
- **Description:** At the start of your Command phase, this model regains up to D3 lost wounds.

#### Supreme Commander (Abilities)
- **Description:** If this model is in your army, it must be your Warlord.

---

### Cohort Commander [Crucible]
**Points:** 45 | **M:** 6" | **T:** 3 | **SV:** 4+ | **W:** 3 | **LD:** 6+ | **OC:** 1
**Keywords:** Character, Adeptus Mechanicus, Infantry, Imperium, Crucible, Skitarii, Marshal, Cohort Commander

**Abilities:**

#### Invulnerable Save (5+) (Abilities)
- **Description:** This model has a 5+ invulnerable save.

#### Leader (Abilities)
- **Description:** This model can be attached to the following units: **Skitarii Rangers, Skitarii Vanguard**

#### Prioritisation Protocol (Abilities)
- **Description:** Melee weapons equipped by this model have [Precision].

---

### Corpuscarii Electro-Priests
**Points:** 65 | **M:** 6" | **T:** 3 | **SV:** 7+ | **W:** 1 | **LD:** 7+ | **OC:** 1
**Keywords:** Electro-Priests, Corpuscarii, Imperium, Infantry, Adeptus Mechanicus, Cult Mechanicus

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Electrostatic gauntlets | Ranged | 12" | 3 | 3+ | 5 | 0 | 1 | Pistol, Sustained Hits 2 |
| Electrostatic gauntlets | Melee | Melee | 3 | 4+ | 5 | 0 | 1 | Sustained Hits 2 |

**Abilities:**

#### Electro-shock (Abilities)
- **Description:** In your Shooting phase, after this unit has shot, select one enemy unit (excluding **Monsters** and **Vehicles**) hit by one or more of those attacks. Until the end of your opponent’s next turn, that enemy unit is shocked. While a unit is shocked, subtract 2" from its Move characteristic and subtract 2 from Advance and Charge rolls made for it.

---

### Cybernetica Datasmith
**Points:** 35 | **M:** 6" | **T:** 4 | **SV:** 2+ | **W:** 3 | **LD:** 7+ | **OC:** 1
**Keywords:** Cybernetica Datasmith, Tech-Priest, Infantry, Character, Imperium, Adeptus Mechanicus, Legio Cybernetica

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Power fist | Melee | Melee | 2 | 3+ | 8 | -2 | 2 | - |

**Abilities:**

#### Leader (Abilities)
- **Description:** This model must be attached to the following unit:
■ **Kastelan Robots**
You must attach this model to a Kastelan Robots unit, even if one or more other Cybernetica Datasmith models havealready been attached to it. In such cases, if that bodyguard unit is destroyed, the Leader units attached to it become separate units with their original Starting Strengths. If you cannot attach this model to a Kastelan Robots unit, this model cannot be deployed and counts as having been destroyed during the first battle round. While attached to a Kastelan Robots unit, this model loses the INFANTRY keyword.

#### Battle Protocols (Abilities)
- **Description:** At the start of the battle, if this model is leading a **Kastelan Robots** unit, that unit enters Aegis Protocols (see below). In your Command phase, if this model is leading a **Kastelan Robots** unit, you can select one protocol from those listed below for that unit to enter. Once a unit enters a protocol, it remains in that protocol until it enters a different one
- Protector Protocol: Add 2 to the Attacks characteristic of ranged weapons equipped by **Kastelan Robot** models in that unit.
- Conqueror Protocol: Add 2 to the Attacks characteristic of melee weapons equipped by **Kastelan Robot** models in that unit.
- Aegis Protocol: Add 1 to the Toughness characteristic of **Kastelan Robot** models in that unit.

#### Invulnerable Save (Abilities)
- **Description:** This model has a 5+ invulnerable save.

---

### Fulgurite Electro-Priests
**Points:** 70 | **M:** 6" | **T:** 3 | **SV:** 7+ | **W:** 1 | **LD:** 7+ | **OC:** 1
**Keywords:** Electro-Priests, Fulgurite, Infantry, Imperium, Adeptus Mechanicus, Cult Mechanicus

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Electroleech stave | Melee | Melee | 2 | 3+ | 6 | -1 | 2 | Devastating Wounds |

**Abilities:**

#### Electro-Infusion (Abilities)
- **Description:** While a **Character** model is leading this unit, each time an attack targets this unit, subtract 1 from the Wound roll.

---

### Hastarii Exterminator
**Points:** 0 | **M:** 5" | **T:** 4 | **SV:** 3+ | **W:** 2 | **LD:** 7+ | **OC:** 1
**Keywords:** Adeptus Mechanicus

---

### Hastarii Exterminator Alpha
**Points:** 0 | **M:** 5" | **T:** 4 | **SV:** 3+ | **W:** 2 | **LD:** 7+ | **OC:** 1
**Keywords:** Adeptus Mechanicus

---

### Hastarii Exterminators
**Points:** 135 | **M:** 5" | **T:** 4 | **SV:** 3+ | **W:** 2 | **LD:** 7+ | **OC:** 1
**Keywords:** Imperium, Adeptus Mechanicus, Infantry, Skitarii, Hastarii, Exterminators

**Abilities:**

#### Attached Unit (Abilities)
- **Description:** If a Character unit from your army can be attached to a Skitarii Rangers unit, it can be attached to this unit instead

#### Broad‑spectrum Targeting Augurs (Abilities)
- **Description:** Each time a model in this unit makes an attack with an eradication caster that targets a unit (excluding Monster and Vehicle units), that attack has the **[SUSTAINED HITS 1]** ability.

---

### Hastarii Fusilier Alpha
**Points:** 0 | **M:** 5" | **T:** 4 | **SV:** 3+ | **W:** 2 | **LD:** 7+ | **OC:** 1
**Keywords:** Adeptus Mechanicus

---

### Hastarii Fusiliers
**Points:** 145 | **M:** 5" | **T:** 4 | **SV:** 3+ | **W:** 2 | **LD:** 7+ | **OC:** 1
**Keywords:** Imperium, Adeptus Mechanicus, Infantry, Skitarii, Hastarii, Fusiliers

**Abilities:**

#### Attached Unit (Abilities)
- **Description:** If a Character unit from your army can be attached to a Skitarii Rangers unit, it can be attached to this unit instead

#### Monocular Targeting Helms (Abilities)
- **Description:** Each time a model in this unit makes an attack with a neutron fusil against a Monster or Vehicle unit, that attack has the **[IGNORES COVER]** ability.

---

### Ironstrider Alpha [Crucible]
**Points:** 80 | **M:** 10" | **T:** 7 | **SV:** 3+ | **W:** 7 | **LD:** 7+ | **OC:** 2
**Keywords:** Character, Adeptus Mechanicus, Imperium, Crucible, Skitarii, Vehicle, Walker, Smoke, Ironstrider Alpha

**Abilities:**

#### Invulnerable Save (5+) (Abilities)
- **Description:** This model has a 5+ invulnerable save.

#### Leader (Abilities)
- **Description:** This model can be attached to the following units: **Ironstrider Ballistarii, Sydonian Dragoons with Taser Lances, Sydonian Dragoons with Radium Jezzails**

#### Vector of the Omnissiah's Wrath (Abilities)
- **Description:** Ranged weapons equipped by this model have [Ignores Cover].

---

### Ironstrider Ballistarii
**Points:** 0 | **M:** 10" | **T:** 7 | **SV:** 3+ | **W:** 7 | **LD:** 7+ | **OC:** 2
**Keywords:** Ironstrider Ballistarii, Vehicle, Smoke, Skitarii, Adeptus Mechanicus, Walker, Imperium

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Twin cognis autocannon | Ranged | 48" | 4 | 4+ | 9 | -1 | 3 | Sustained Hits 1, Twin-linked |
| Twin cognis lascannon | Ranged | 48" | 2 | 4+ | 12 | -3 | D6+1 | Sustained Hits 1, Twin-linked |

**Abilities:**

#### Elevated Strider (Abilities)
- **Description:** This unit is eligible to shoot in a turn in which it Fell Back or Advanced, and you can re-roll Desperate Escape tests taken for models in this unit.

#### Broad Spectrum Data-tether (Abilities)
- **Description:** Each time you select this unit as the target of a Stratagem, roll one D6: on a 5+, you gain 1CP.

---

### Ironstrider Ballistarius (Twin cognis autocannon)
**Points:** 85 | **M:** 10" | **T:** 7 | **SV:** 3+ | **W:** 7 | **LD:** 7+ | **OC:** 2
**Keywords:** Adeptus Mechanicus

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Twin cognis autocannon | Ranged | 48" | 4 | 4+ | 9 | -1 | 3 | Sustained Hits 1, Twin-linked |

---

### Ironstrider Ballistarius (Twin cognis lascannon)
**Points:** 85 | **M:** 10" | **T:** 7 | **SV:** 3+ | **W:** 7 | **LD:** 7+ | **OC:** 2
**Keywords:** Adeptus Mechanicus

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Twin cognis lascannon | Ranged | 48" | 2 | 4+ | 12 | -3 | D6+1 | Sustained Hits 1, Twin-linked |

---

### Kastelan Robot
**Points:** 0 | **M:** 6" | **T:** 9 | **SV:** 2+ | **W:** 7 | **LD:** 7+ | **OC:** 2
**Keywords:** Adeptus Mechanicus

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Close combat weapon | Melee | Melee | 3 | 4+ | 6 | 0 | 1 | - |
| Twin Kastelan phosphor blaster | Ranged | 24" | 3 | 4+ | 6 | -1 | 1 | Ignores Cover, Twin-linked |
| Twin Kastelan fist | Melee | Melee | 4 | 4+ | 12 | -2 | 3 | Twin-linked |
| Kastelan fist | Melee | Melee | 4 | 4+ | 12 | -2 | 3 | - |
| Kastelan phosphor blaster | Ranged | 24" | 3 | 4+ | 6 | -1 | 1 | Ignores Cover |
| Incendine combustor | Ranged | 12" | D6 | N/A | 6 | -1 | 1 | Ignores Cover, Torrent |
| Heavy phosphor blaster | Ranged | 36" | 3 | 4+ | 6 | -1 | 2 | Ignores Cover |

---

### Kastelan Robots
**Points:** 180 | **M:** 6" | **T:** 9 | **SV:** 2+ | **W:** 7 | **LD:** 7+ | **OC:** 2
**Keywords:** Kastelan Robots, Adeptus Mechanicus, Imperium, Vehicle, Walker, Legio Cybernetica

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Close combat weapon | Melee | Melee | 3 | 4+ | 6 | 0 | 1 | - |
| Twin Kastelan phosphor blaster | Ranged | 24" | 3 | 4+ | 6 | -1 | 1 | Ignores Cover, Twin-linked |
| Twin Kastelan fist | Melee | Melee | 4 | 4+ | 12 | -2 | 3 | Twin-linked |
| Kastelan fist | Melee | Melee | 4 | 4+ | 12 | -2 | 3 | - |
| Kastelan phosphor blaster | Ranged | 24" | 3 | 4+ | 6 | -1 | 1 | Ignores Cover |
| Incendine combustor | Ranged | 12" | D6 | N/A | 6 | -1 | 1 | Ignores Cover, Torrent |
| Heavy phosphor blaster | Ranged | 36" | 3 | 4+ | 6 | -1 | 2 | Ignores Cover |

**Abilities:**

#### Robotic Bodyguard (Abilities)
- **Description:** While a **Cybernetica Datasmith** model is leading this unit, that model has the Feel No Pain 4+ ability.

#### Repulsor Grid (Abilities)
- **Description:** Each time a ranged attack is allocated to a **Kastelan Robot** model in this unit, on an unmodified saving throw of 6, the attacking unit suffers 1 mortal wound after it has finished making its attacks.

---

### Kataphron Breachers
**Points:** 160 | **M:** 5" | **T:** 7 | **SV:** 3+ | **W:** 3 | **LD:** 7+ | **OC:** 1
**Keywords:** Breachers, Kataphron, Infantry, Adeptus Mechanicus, Imperium, Cult Mechanicus

**Abilities:**

#### Breaching Command (Abilities)
- **Description:** Each time a model in this unit makes an attack, re-roll a Hit roll of 1. While this unit is within 6” of one or more friendly **Adeptus Mechanicus Battleline** units, you can re-roll the Hit roll instead.

#### Invulnerable Save (Abilities)
- **Description:** Models in this unit have a 6+ invulnerable save.

---

### Kataphron Destroyers
**Points:** 105 | **M:** 5" | **T:** 6 | **SV:** 3+ | **W:** 3 | **LD:** 7+ | **OC:** 1
**Keywords:** Destroyers, Kataphron, Infantry, Adeptus Mechanicus, Imperium, Cult Mechanicus

**Abilities:**

#### Sentinel Directive (Abilities)
- **Description:** Each time you target this unit with the Fire Overwatch Stratagem, hits are scored on unmodified Hit rolls of 5+ when resolving that Stratagem.

#### Invulnerable Save (Abilities)
- **Description:** Models in this unit have a 6+ invulnerable save.

---

### Magos [Crucible]
**Points:** 70 | **M:** 6" | **T:** 4 | **SV:** 2+ | **W:** 4 | **LD:** 7+ | **OC:** 1
**Keywords:** Character, Adeptus Mechanicus, Infantry, Imperium, Cult Mechanicus, Magos, Tech-Priest, Crucible

**Abilities:**

#### Invulnerable Save (5+) (Abilities)
- **Description:** This model has a 5+ invulnerable save.

#### Leader (Abilities)
- **Description:** This model can be attached to the following units: **Electro-Priests, Kataphrons, Skitarii Rangers, Skitarii Vanguard**

#### Technoarcane Zealotry (Abilities)
- **Description:** While this model's unit is in range of one or more objective markers, improve this model's Leadership characteristic by 1.

---

### Onager Dunecrawler
**Points:** 155 | **M:** 8" | **T:** 10 | **SV:** 2+ | **W:** 11 | **LD:** 7+ | **OC:** 3
**Keywords:** Onager Dunecrawler, Imperium, Vehicle, Walker, Skitarii, Adeptus Mechanicus, Smoke

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Cognis heavy stubber | Ranged | 36" | 3 | 4+ | 4 | 0 | 1 | Rapid Fire 3, Sustained Hits 1 |
| Dunecrawler legs | Melee | Melee | 3 | 4+ | 6 | 0 | 1 | - |
| ➤ Eradication beamer - dissipated | Ranged | 36" | 3D3 | 4+ | 9 | -2 | 2 | Blast, Sustained Hits 1 |
| ➤ Eradication beamer - focused | Ranged | 18" | 3D3 | 4+ | 10 | -3 | 3 | Blast, Sustained Hits 1 |
| Daedalus missile launcher | Ranged | 48" | 2 | 4+ | 10 | -2 | D6+1 | Anti-Fly 2+ |
| Icarus array | Ranged | 48" | 6 | 4+ | 8 | -1 | 2 | Anti-Fly 4+, Twin-linked |
| Cognis heavy stubber | Ranged | 36" | 3 | 4+ | 4 | 0 | 1 | Rapid Fire 3, Sustained Hits 1 |
| Neutron laser | Ranged | 48" | 3 | 4+ | 16 | -4 | D6+2 | Heavy |
| Twin Onager heavy phosphor blaster | Ranged | 36" | 12 | 4+ | 6 | -1 | 2 | Ignores Cover, Twin-linked |

**Abilities:**

#### Emanatus Forcefield (Aura) (Abilities)
- **Description:** While a friendly **Adeptus Mechanicus Battleline** model is wholly within 6" of this model, that **Battleline** model has a 4+ invulnerable save against ranged attacks.

#### Invulnerable Save (Abilities)
- **Description:** 4+

#### Damaged: 1-4 wounds remaining (Abilities)
- **Description:** While this mdel has 1-4 wounds remaining, each time this model makes an attack, subtract 1 from the Hit roll.

#### Scuttling Walker (Abilities)
- **Description:** Each time this model makes a Normal, Advance or Fall Back move, it can move through friendly **Monster** and **Vehicle** models and sections of terrain features that are 4" or less in height.

#### Broad spectrum data-tether (Abilities)
- **Description:** The bearer loses the SMOKE keyword, but each time you target the bearer with a Stratagem, roll one D6: on a 5+, you gain 1CP.

---

### Pteraxii Skystalker Alpha
**Points:** 0 | **M:** 12" | **T:** 4 | **SV:** 4+ | **W:** 2 | **LD:** 7+ | **OC:** 1
**Keywords:** Adeptus Mechanicus

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Flechette blaster | Ranged | 12" | 5 | 4+ | 3 | 0 | 1 | Pistol |
| Taser goad | Melee | Melee | 3 | 4+ | 6 | 0 | 1 | Sustained Hits 2 |

---

### Pteraxii Skystalkers
**Points:** 75 | **M:** 12" | **T:** 4 | **SV:** 4+ | **W:** 2 | **LD:** 7+ | **OC:** 1
**Keywords:** Skystalkers, Imperium, Adeptus Mechanicus, Infantry, Fly, Jump Pack, Grenades, Skitarii, Pteraxii

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Flechette blaster | Ranged | 12" | 5 | 4+ | 3 | 0 | 1 | Pistol |
| Taser goad | Melee | Melee | 3 | 4+ | 6 | 0 | 1 | Sustained Hits 2 |
| Close combat weapon | Melee | Melee | 2 | 4+ | 3 | 0 | 1 | - |
| Flechette carbine | Ranged | 18" | 6 | 4+ | 3 | 0 | 1 | - |

**Abilities:**

#### Ride the Thermals (Abilities)
- **Description:** In your Shooting phase, after this unit has shot, if it is not within Engagement Range of one or more enemy units, it can do one of the following:
- Make a Normal move of up to 6".
- Make a Normal move of up to 12", provided every model in this unit ends that move wholly within 6" of one or more friendly **Adeptus Mechanicus Battleline** units.
In either case, if it does, until the end of the turn, this unit is not eligible to declare a charge.

---

### Pteraxii Sterylizor Alpha
**Points:** 0 | **M:** 12" | **T:** 4 | **SV:** 4+ | **W:** 2 | **LD:** 7+ | **OC:** 1
**Keywords:** Adeptus Mechanicus

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Flechette blaster | Ranged | 12" | 5 | 4+ | 3 | 0 | 1 | Pistol |
| Taser goad | Melee | Melee | 3 | 4+ | 6 | 0 | 1 | Sustained Hits 2 |
| Pteraxii talons | Melee | Melee | 2 | 4+ | 4 | 0 | 1 | - |

---

### Pteraxii Sterylizors
**Points:** 80 | **M:** 12" | **T:** 4 | **SV:** 4+ | **W:** 2 | **LD:** 7+ | **OC:** 1
**Keywords:** Sterylizors, Imperium, Infantry, Fly, Jump Pack, Skitarii, Adeptus Mechanicus, Pteraxii

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Flechette blaster | Ranged | 12" | 5 | 4+ | 3 | 0 | 1 | Pistol |
| Taser goad | Melee | Melee | 3 | 4+ | 6 | 0 | 1 | Sustained Hits 2 |
| Pteraxii talons | Melee | Melee | 2 | 4+ | 4 | 0 | 1 | - |
| Pteraxii talons | Melee | Melee | 2 | 4+ | 4 | 0 | 1 | - |
| Phosphor torch | Ranged | 12" | D6 | N/A | 4 | 0 | 1 | Ignores Cover, Torrent |

**Abilities:**

#### Searing Conflagaration (Abilities)
- **Description:** Each time a model in this unit makes an attack with a phosphor torch that targets an enemy unit within range of an objective marker, re-roll a Wound roll of 1. If this unit is also within 6" of one or more friendly **Adeptus Mechanicus Battleline** units, each time such an attack targets such a unit, you can re-roll the Wound roll instead.

---

### Secutarii Hoplites [Legends]
**Points:** 65 | **M:** 6" | **T:** 3 | **SV:** 5+ | **W:** 1 | **LD:** 7+ | **OC:** 1
**Keywords:** Secutarii Hoplites, Infantry, Imperium, Skitarii, Adeptus Mechanicus

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Alpha close combat weapon | Melee | Melee | 4 | 4 | 5 | -1 | 1 | - |
| Arc lance | Melee | Melee | 2 | 4+ | 5 | -1 | 1 | Anti-Vehicle 4+, Devastating Wounds, Lance |
| Arc lance | Ranged | 12" | 1 | 4+ | 5 | -1 | 1 | Anti-Vehicle 4+, Devastating Wounds |
| Arc lance | Melee | Melee | 2 | 4+ | 5 | -1 | 1 | Anti-Vehicle 4+, Devastating Wounds, Lance |
| Arc lance | Ranged | 12" | 1 | 4+ | 5 | -1 | 1 | Anti-Vehicle 4+, Devastating Wounds |
| Arc lance | Melee | Melee | 2 | 4+ | 5 | -1 | 1 | Anti-Vehicle 4+, Devastating Wounds, Lance |
| Arc lance | Ranged | 12" | 1 | 4+ | 5 | -1 | 1 | Anti-Vehicle 4+, Devastating Wounds |
| Arc lance | Melee | Melee | 2 | 4+ | 5 | -1 | 1 | Anti-Vehicle 4+, Devastating Wounds, Lance |
| Arc lance | Ranged | 12" | 1 | 4+ | 5 | -1 | 1 | Anti-Vehicle 4+, Devastating Wounds |

**Abilities:**

#### Invulnerable Save (Abilities)
- **Description:** Models in this unit have a 4+ invulnerable save.

#### Titan Guard (Abilities)
- **Description:** You can target this unit with the Heroic Intervention Stratagem for 0CP, and can do so even if you have already targeted a different unit with that Stratagem this phase. When oing so, if there are one or more friendly **Adeptus Titanicus** units on the battlefield, you can re-roll the subsequent charge roll.

#### Secutarii (Abilities)
- **Description:** If a model from your army with the Leader ability can be attached to a **Skitarii Vanguard** unit, it can be attached to this unit instead.

#### Enhanced data-tether (Abilities)
- **Description:** Each time you select the bearer’s unit as the target of a Stratagem, roll one D6: on a 5+, you gain 1CP.

#### Omnispex (Abilities)
- **Description:** Ranged weapons equipped by models in the bearer’s unit have the IGNORES COVER ability.

---

### Secutarii Peltasts [Legends]
**Points:** 50 | **M:** 6" | **T:** 3 | **SV:** 5+ | **W:** 1 | **LD:** 7+ | **OC:** 1
**Keywords:** Infantry, Adeptus Mechanicus, Secutarii Peltasts, Skitarii, Imperium

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Alpha close combat weapon | Melee | Melee | 4 | 4 | 5 | -1 | 1 | - |
| Close combat weapon | Melee | Melee | 1 | 4+ | 3 | 0 | 1 | - |
| ➤ Galvanic caster - flechette | Ranged | 18" | 5 | 4+ | 4 | 0 | 1 | Assault |
| ➤ Galvanic caster - hammershot | Ranged | 24" | 2 | 4+ | 5 | -1 | 1 | Heavy |
| ➤ Galvanic caster - flechette | Ranged | 18" | 5 | 4+ | 4 | 0 | 1 | Assault |
| ➤ Galvanic caster - hammershot | Ranged | 24" | 2 | 4+ | 5 | -1 | 1 | Heavy |
| Close combat weapon | Melee | Melee | 1 | 4+ | 3 | 0 | 1 | - |
| ➤ Galvanic caster - flechette | Ranged | 18" | 5 | 4+ | 4 | 0 | 1 | Assault |
| ➤ Galvanic caster - hammershot | Ranged | 24" | 2 | 4+ | 5 | -1 | 1 | Heavy |
| Close combat weapon | Melee | Melee | 1 | 4+ | 3 | 0 | 1 | - |
| ➤ Galvanic caster - flechette | Ranged | 18" | 5 | 4+ | 4 | 0 | 1 | Assault |
| ➤ Galvanic caster - hammershot | Ranged | 24" | 2 | 4+ | 5 | -1 | 1 | Heavy |
| Close combat weapon | Melee | Melee | 1 | 4+ | 3 | 0 | 1 | - |

**Abilities:**

#### Invulnerable Save (Abilities)
- **Description:** Models in this unit have a 6+ invulnerable save.

#### Blind Barrage (Abilities)
- **Description:** In your Shooting phase, after this unit has shot, select one enemy Infantry unit hit by one or more of those attacks. Until the start of your next turn, while this unit is on the battlefield, that enemy unit is blinded. While a unit is blinded, each time a model in that unit makes an attack, subtract 1 from the Hit roll.

#### Secutarii (Abilities)
- **Description:** If a model from your army with the Leader ability can be attached to a **Skitarii Vanguard** unit, it can be attached to this unit instead.

#### Enhanced data-tether (Abilities)
- **Description:** Each time you select the bearer’s unit as the target of a Stratagem, roll one D6: on a 5+, you gain 1CP.

#### Omnispex (Abilities)
- **Description:** Ranged weapons equipped by models in the bearer’s unit have the IGNORES COVER ability.

---

### Serberys Raiders
**Points:** 60 | **M:** 12" | **T:** 4 | **SV:** 4+ | **W:** 2 | **LD:** 7+ | **OC:** 2
**Keywords:** Serberys Raiders, Imperium, Adeptus Mechanicus, Skitarii, Mounted

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Galvanic carbine | Ranged | 18" | 3 | 4+ | 4 | 0 | 1 | Devastating Wounds |
| Cavalry sabre & clawed limbs | Melee | Melee | 4 | 4+ | 4 | -1 | 1 | - |
| Cavalry sabre & clawed limbs | Melee | Melee | 4 | 4+ | 4 | -1 | 1 | - |
| Galvanic carbine | Ranged | 18" | 3 | 4+ | 4 | 0 | 1 | Devastating Wounds |
| Cavalry sabre & clawed limbs | Melee | Melee | 4 | 4+ | 4 | -1 | 1 | - |
| Galvanic carbine | Ranged | 18" | 3 | 4+ | 4 | 0 | 1 | Devastating Wounds |

**Abilities:**

#### Tactica Obliqua (Abilities)
- **Description:** Once per turn, when an enemy unit ends a Normal, Advance or Fall Back move within 9" of this unit, if this unit is not within Engagement Range of one or more enemy units, it can do one of the following:
- Make a Normal move of up to D6".
- Make a Normal move of up to 6", provided every model in this unit ends that move wholly within 6" of one or more friendly **Adeptus Mechanicus Battleline** units.

#### Enhanced data-tether (Abilities)
- **Description:** Each time you select the bearer’s unit as the target of a Stratagem, roll one D6: on a 5+, you gain 1CP.

---

### Serberys Sulphurhounds
**Points:** 55 | **M:** 12" | **T:** 4 | **SV:** 4+ | **W:** 2 | **LD:** 7+ | **OC:** 2
**Keywords:** Serberys Sulphurhounds, Imperium, Adeptus Mechanicus, Skitarii, Mounted

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Cavalry arc maul | Melee | Melee | 1 | 4+ | 5 | -1 | 1 | Anti-Vehicle 4+, Devastating Wounds, Extra Attacks |
| Clawed limbs | Melee | Melee | 4 | 4+ | 4 | 0 | 1 | - |
| Clawed limbs | Melee | Melee | 4 | 4+ | 4 | 0 | 1 | - |
| Phosphor pistol | Ranged | 12" | 1 | 4+ | 4 | 0 | 1 | Ignores Cover, Pistol |
| Phosphor blast carbine | Ranged | 18" | D6 | 4+ | 6 | 0 | 1 | Blast, Ignores cover |
| Phosphor pistol | Ranged | 12" | 1 | 4+ | 4 | 0 | 1 | Ignores Cover, Pistol |
| Clawed limbs | Melee | Melee | 4 | 4+ | 4 | 0 | 1 | - |

**Abilities:**

#### Line-breakers (Abilities)
- **Description:** Each time this unit ends a Charge move, select one enemy unit within Engagement Range of it and roll one D6 for each model in this unit that is within Engagement Range of that enemy unit, adding 2 to the result if this unit started its Charge move within 6" of one or more friendly Adeptus Mechanicus Battleline units. For each 4+, that enemy unit suffers 1 mortal wound.

---

### Servitor Battleclade
**Points:** 60 | **M:** 6" | **T:** 4 | **SV:** 4+ | **W:** 2 | **LD:** 7+ | **OC:** 1
**Keywords:** Adeptus Mechanicus, Infantry, Imperium, Cult Mechanicus, Servitor Battleclade

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Mechanicus pistol | Ranged | 12" | 1 | 4+ | 6 | -1 | 1 | Devastating Wounds, Pistol |
| Dataspikes | Melee | Melee | 4 | 4+ | 4 | 0 | 1 | - |
| Heavy arc rifle | Ranged | 30" | 1 | 4+ | 8 | -2 | 3 | Anti-VEHICLE 4+, Rapid Fire 1 |
| Heavy bolter | Ranged | 36" | 3 | 4+ | 5 | -1 | 2 | Heavy, Sustained Hits 1 |
| Incendine igniter | Ranged | 12" | D6 | N/A | 3 | 0 | 1 | Ignores Cover, Torrent |
| Meltagun | Ranged | 12" | 1 | 4+ | 9 | -4 | D6 | Melta 2 |
| Phosphor blaster | Ranged | 24" | 1 | 4+ | 4 | 0 | 1 | Ignores Cover, Rapid Fire 1 |

**Abilities:**

#### Network Override (Abilities)
- **Description:** While this unit contains one or more **Tech-Priest** models, this unit is: 
- Eligible to perform an Action in a turn in which it Advanced. 
- Eligible to shoot in a turn in which it started an Action.

#### Invulnerable Save (Abilities)
- **Description:** Models in this unit have a 6+ invulnerable save.

#### Attached Unit (Abilities)
- **Description:** If a **Character** unit from your army with the Leader ability can be attached to a **Kataphron Breachers** unit, it can be attached to this unit instead.

---

### Servitors [Legends]
**Points:** 50 | **M:** 6" | **T:** 3 | **SV:** 4+ | **W:** 1 | **LD:** 8+ | **OC:** 0
**Keywords:** Adeptus Mechanicus, Infantry, Imperium, Cult Mechanicus, Servitors

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Servitor's servo arm | Melee | Melee | 1 | 5+ | 8 | -2 | 3 | - |
| Heavy bolter | Ranged | 36" | 3 | 6+ | 5 | -1 | 2 | Heavy, Sustained Hits 1 |
| Multi-melta | Ranged | 18" | 2 | 6+ | 9 | -4 | D6 | Heavy, Melta 2 |
| ➤ Plasma cannon - Standard | Ranged | 36" | D3 | 6+ | 7 | -2 | 1 | Heavy, Blast |
| ➤ Plasma cannon - supercharge | Ranged | 36" | D3 | 6+ | 8 | -3 | 2 | Hazardous, Heavy, Blast |

**Abilities:**

#### Invulnerable Save (Abilities)
- **Description:** Models in this unit have a 6+ invulnerable save.

#### Mindlock (Abilities)
- **Description:** While a TECH-PRIEST model is leading this unit, improve the Ballistic Skill and Weapon Skill characteristics of ranged and melee weapons equipped by SERVITOR models in this unit by 1.

#### Servitor Retinue (Abilities)
- **Description:** At the start of the Declare Battle Formations step, this unit can join one other unit from your army that is being led by a TECH-PRIEST ENGINSEER model (a unit cannot have more than one SERVITORS unit joined to it). If it does, until the end of the battle, every model in this unit counts as being part of that Bodyguard unit, and that Bodyguard unit’s Starting Strength is increased accordingly.

---

### Sicarian Infiltrators
**Points:** 75 | **M:** 8" | **T:** 4 | **SV:** 4+ | **W:** 2 | **LD:** 7+ | **OC:** 1
**Keywords:** Imperium, Adeptus Mechanicus, Skitarii, Infantry, Sicarian, Infiltrators

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Power weapon | Melee | Melee | 4 | 4+ | 4 | -2 | 1 | - |
| Stubcarbine | Ranged | 12" | 3 | 4+ | 4 | 0 | 1 | Pistol |
| Flechette blaster | Ranged | 12" | 5 | 4+ | 3 | 0 | 1 | Pistol |
| Taser goad | Melee | Melee | 3 | 4+ | 6 | -1 | 1 | Sustained Hits 2 |
| Power weapon | Melee | Melee | 4 | 4+ | 4 | -2 | 1 | - |
| Stubcarbine | Ranged | 12" | 3 | 4+ | 4 | 0 | 1 | Pistol |
| Taser goad | Melee | Melee | 3 | 4+ | 6 | -1 | 1 | Sustained Hits 2 |
| Flechette blaster | Ranged | 12" | 5 | 4+ | 3 | 0 | 1 | Pistol |

**Abilities:**

#### Neurostatic Interference (Aura) (Abilities)
- **Description:** While an enemy unit is within 6" of this unit, each time a Battle-shock or Leadership test is taken for that unit, subtract 1 from that test. While this unit is within 6" of one or more friendly **Adeptus Mechanicus Battleline** units, subtract 2 from that test instead.

---

### Sicarian Ruststalkers
**Points:** 80 | **M:** 8" | **T:** 4 | **SV:** 4+ | **W:** 2 | **LD:** 7+ | **OC:** 1
**Keywords:** Ruststalkers, Imperium, Adeptus Mechanicus, Infantry, Skitarii, Sicarian

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Transonic razor & chordclaw | Melee | Melee | 3 | 4+ | 5 | -2 | 2 | Anti-Infantry 3+, Precision |
| Transonic blades | Melee | Melee | 5 | 4+ | 5 | -1 | 1 | Devastating Wounds, Precision |
| Transonic blades & chordclaw | Melee | Melee | 5 | 4+ | 5 | -2 | 1 | Anti-Infantry 3+, Devastating Wounds, Precision |
| Transonic razor & chordclaw | Melee | Melee | 3 | 4+ | 5 | -2 | 2 | Anti-Infantry 3+, Precision |

**Abilities:**

#### Optimised Gait (Abilities)
- **Description:** Add 1 to Advance and Charge rolls made for this unit. While this unit is within 6" of one or more friendly **Adeptus Mechanicus Battleline** units, add 2 to Advance and Charge rolls made for this unit instead.

---

### Skitarii Marshal
**Points:** 35 | **M:** 6" | **T:** 3 | **SV:** 4+ | **W:** 3 | **LD:** 6 | **OC:** 1
**Keywords:** Marshal, Imperium, Adeptus Mechanicus, Skitarii, Character, Infantry

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Control stave | Melee | Melee | 4 | 3+ | 6 | -1 | 1 | - |

**Abilities:**

#### Leader (Abilities)
- **Description:** This model can be attached to the following units:
- **Skitarii Rangers**
- **Skitarii Vanguard**

#### Control Edict (Abilities)
- **Description:** While this model is leading a unit, each time a model in that unit makes an attack, you can re-roll the Hit roll.

#### Servo-skull Uplink (Abilities)
- **Description:** Once per battle, at the start of any phase, you can select one friendly **Skitarii** unit that is Battle-shocked and within 6" of this model. That unit is no longer Battle-shocked.

#### Invulnerable Save (Abilities)
- **Description:** This model has a 5+ invulnerable save.

---

### Skitarii Rangers
**Points:** 85 | **M:** 6" | **T:** 3 | **SV:** 4+ | **W:** 1 | **LD:** 7+ | **OC:** 2
**Keywords:** Rangers, Imperium, Adeptus Mechanicus, Infantry, Skitarii, Battleline

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Galvanic rifle | Ranged | 30" | 2 | 4+ | 4 | 0 | 1 | - |
| Alpha combat weapon | Melee | Melee | 2 | 4+ | 5 | -1 | 1 | - |
| Transuranic arquebus | Ranged | 36" | 1 | 4+ | 7 | -2 | D3 | Heavy, Precision |
| Arc rifle | Ranged | 30" | 1 | 4+ | 8 | -1 | D3 | Anti-Vehicle 4+, Devastating Wounds, Rapid Fire 1 |
| ➤ Plasma caliver - supercharge | Ranged | 30" | 2 | 4+ | 8 | -3 | 2 | Hazardous |
| ➤ Plasma caliver - standard | Ranged | 30" | 2 | 4+ | 7 | -2 | 1 | - |
| Galvanic rifle | Ranged | 30" | 2 | 4+ | 4 | 0 | 1 | - |
| Galvanic rifle | Ranged | 30" | 2 | 4+ | 4 | 0 | 1 | - |
| Galvanic rifle | Ranged | 30" | 2 | 4+ | 4 | 0 | 1 | - |

**Abilities:**

#### Objective Scouted (Abilities)
- **Description:** At the end of your Command phase, if this unit is within range of an objective marker you control, that objective marker remains under your control, even if you have no models within range of it, until your opponent controls it at the start or end of any turn.

#### Enhanced data-tether (Abilities)
- **Description:** Each time you select the bearer’s unit
as the target of a Stratagem, roll one D6: on a 5+, you gain 1CP.

#### Omnispex (Abilities)
- **Description:** Ranged weapons equipped by models in the bearer’s unit have the IGNORES COVER ability.

---

### Skitarii Vanguard
**Points:** 95 | **M:** 6" | **T:** 3 | **SV:** 4+ | **W:** 1 | **LD:** 7+ | **OC:** 2
**Keywords:** Vanguard, Imperium, Adeptus Mechanicus, Infantry, Skitarii, Battleline

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Radium carbine | Ranged | 18" | 3 | 4+ | 3 | 0 | 1 | Anti-Infantry 4+ |
| Alpha combat weapon | Melee | Melee | 2 | 4+ | 5 | -1 | 1 | - |
| Transuranic arquebus | Ranged | 36" | 1 | 4+ | 7 | -2 | D3 | Heavy, Precision |
| Arc rifle | Ranged | 30" | 1 | 4+ | 8 | -1 | D3 | Anti-Vehicle 4+, Devastating Wounds, Rapid Fire 1 |
| ➤ Plasma caliver - supercharge | Ranged | 30" | 2 | 4+ | 8 | -3 | 2 | Hazardous |
| ➤ Plasma caliver - standard | Ranged | 30" | 2 | 4+ | 7 | -2 | 1 | - |
| Radium carbine | Ranged | 18" | 3 | 4+ | 3 | 0 | 1 | Anti-Infantry 4+ |
| Radium carbine | Ranged | 18" | 3 | 4+ | 3 | 0 | 1 | Anti-Infantry 4+ |
| Radium carbine | Ranged | 18" | 3 | 4+ | 3 | 0 | 1 | Anti-Infantry 4+ |

**Abilities:**

#### Rad-saturation (Aura) (Abilities)
- **Description:** While an enemy unit (excluding **Vehicle** units) is within 3" of this unit, subtract 1 from the Objective Control characteristic of models in that unit.

#### Enhanced data-tether (Abilities)
- **Description:** Each time you select the bearer’s unit as the target of a Stratagem, roll one D6: on a 5+, you gain 1CP.

#### Omnispex (Abilities)
- **Description:** Ranged weapons equipped by models in the bearer’s unit have the IGNORES COVER ability.

---

### Skorpius Disintegrator
**Points:** 165 | **M:** 10" | **T:** 10 | **SV:** 2+ | **W:** 12 | **LD:** 7+ | **OC:** 3
**Keywords:** Skorpius Disintegrator, Imperium, Adeptus Mechanicus, Skitarii, Vehicle, Smoke

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Cognis heavy stubber | Ranged | 36" | 3 | 4+ | 4 | 0 | 1 | Rapid Fire 3, Sustained Hits 1 |
| Disruptor missile launcher | Ranged | 36" | 3 | 4+ | 9 | -2 | D6 | Twin-linked |
| Belleros energy cannon | Ranged | 36" | 2D6 | 4+ | 7 | -2 | 1 | Blast, Indirect Fire |
| Ferrumite cannon | Ranged | 48" | 3 | 4+ | 12 | -3 | D6+1 | - |

**Abilities:**

#### Broad spectrum data-tether (Abilities)
- **Description:** Each time you select the bearer as the target of a Stratagem, roll one D6: on a 5+, you gain 1CP.

#### Blistering Salvoes (Abilities)
- **Description:** Each time this model makes an attack with a belleros energy cannon that targets an **Infantry** unit, add 1 to the Hit roll. Each time this model makes an attack with a ferrumite cannon that targets a **Monster** or **Vehicle** unit, add 1 to the Hit roll.

#### Damaged: 1-4 wounds remaining (Abilities)
- **Description:** While this model has 1-4 wounds remaining, each time this model makes an attack, subtract 1 from the Hit roll.

---

### Skorpius Dunerider
**Points:** 85 | **M:** 12" | **T:** 9 | **SV:** 3+ | **W:** 11 | **LD:** 7+ | **OC:** 2
**Keywords:** Skorpius Dunerider, Imperium, Adeptus Mechanicus, Vehicle, Dedicated Transport, Transport, Skitarii, Smoke

**Abilities:**

#### Broad spectrum data-tether (Abilities)
- **Description:** Each time you select the bearer as the target of a Stratagem, roll one D6: on a 5+, you gain 1CP.

#### Fire Support (Abilities)
- **Description:** In your Shooting phase, after this model has shot, select one enemy unit it scored one or more hits against this phase. Until the end of the phase, each time a friendly model that disembarked from this **Transport** this turn makes an attack that targets that enemy unit, you can re-roll the Wound roll.

---

### Sydonian Dragoon with taser lance
**Points:** 65 | **M:** 10" | **T:** 7 | **SV:** 3+ | **W:** 7 | **LD:** 7+ | **OC:** 2
**Keywords:** Adeptus Mechanicus

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Phosphor serpenta | Ranged | 18" | 1 | 4+ | 5 | -1 | 2 | Ignores Cover, Pistol |
| Taser lance | Melee | Melee | 4 | 4+ | 7 | -2 | 2 | Anti-Walker 2+, Lance, Sustained Hits 2 |

---

### Sydonian Dragoons with radium jezzails
**Points:** 55 | **M:** 10" | **T:** 7 | **SV:** 3+ | **W:** 7 | **LD:** 7+ | **OC:** 2
**Keywords:** Imperium, Adeptus Mechanicus, Skitarii, Vehicle, Walker, Sydonian, Dragoons with Radium Jezzails, Smoke

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Phosphor serpenta | Ranged | 18" | 1 | 4+ | 5 | -1 | 2 | Ignores Cover, Pistol |
| Radium jezzail | Ranged | 36" | 1 | 4+ | 5 | -2 | 3 | Anti-Infantry 3+, Heavy, Precision |

**Abilities:**

#### Focused Hunters (Abilities)
- **Description:** At the start of the battle, select one unit from your opponent’s army. Until the end of the battle, each time a model in this unit makes an attack that targets that unit, you can re-roll the Hit roll.

#### Broad Spectrum Data-tether (Abilities)
- **Description:** Each time you select the bearer as the target of a Stratagem, roll one D6: on a 5+, you gain 1CP.

---

### Sydonian Dragoons with taser lances
**Points:** 0 | **M:** 10" | **T:** 7 | **SV:** 3+ | **W:** 7 | **LD:** 7+ | **OC:** 2
**Keywords:** Imperium, Adeptus Mechanicus, Skitarii, Vehicle, Walker, Sydonian, Dragoons with Taser Lances, Smoke

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Phosphor serpenta | Ranged | 18" | 1 | 4+ | 5 | -1 | 2 | Ignores Cover, Pistol |
| Taser lance | Melee | Melee | 4 | 4+ | 7 | -2 | 2 | Anti-Walker 2+, Lance, Sustained Hits 2 |

**Abilities:**

#### Dynamic Efficiency (Abilities)
- **Description:** This unit is eligible to declare a charge in a turn in which it Advanced or Fell Back, and you can re-roll Desperate Escape tests taken for models in this unit.

#### Broad Spectrum Data-tether (Abilities)
- **Description:** Each time you select the bearer as the target of a Stratagem, roll one D6: on a 5+, you gain 1CP.

---

### Sydonian Skatros
**Points:** 50 | **M:** 8" | **T:** 4 | **SV:** 4+ | **W:** 4 | **LD:** 7+ | **OC:** 1
**Keywords:** Infantry, Imperium, Skitarii, Sydonian, Adeptus Mechanicus, Character, Skatros

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Sydonian Feet | Melee | Melee | 3 | 4+ | 3 | 0 | 1 | - |
| Radium Jezzail | Ranged | 36" | 1 | 3+ | 5 | -2 | 3 | Heavy, Precision, Anti-Infantry 3+ |
| Skatros transuranic arquebus | Ranged | 36" | 1 | 3+ | 7 | -2 | D3 | Heavy, Precision, Anti-Monster 4+, Anti-Vehicle 4+ |

**Abilities:**

#### Invulnerable Save (Abilities)
- **Description:** This model has a 5+ invulnerable save.

#### Dread Snipers (Abilities)
- **Description:** In your Shooting phase, after this model has shot, select one enemy unit hit by one or more of those attacks. That unit must take a Battle-shock test.

#### Sydonian Sentinel (Abilities)
- **Description:** This model cannot be your **Warlord**.

#### Achillan Eye (Abilities)
- **Description:** Each time this model makes an attack with a radium jezzail that targets an Infantry unit, you can re-roll the Wound roll. Each time this model makes an attack with a Skatros transuranic arquebus that targets a Monster or Vehicle unit, you can re-roll the Wound roll.

---

### Tech-Priest Dominus
**Points:** 65 | **M:** 6" | **T:** 4 | **SV:** 2+ | **W:** 4 | **LD:** 7+ | **OC:** 1
**Keywords:** Dominus, Imperium, Adeptus Mechanicus, Tech-Priest, Character, Infantry, Cult Mechanicus

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Omnissian axe | Melee | Melee | 4 | 3+ | 6 | -2 | 2 | - |
| Macrostubber | Ranged | 12" | 5 | 3+ | 4 | 0 | 1 | Pistol |
| Phosphor serpenta | Ranged | 18" | 1 | 3+ | 5 | -1 | 2 | Ignores Cover, Pistol |
| ➤ Eradication ray - dissipated | Ranged | 24" | D3 | 3+ | 6 | -1 | 1 | - |
| ➤ Eradication ray - focused | Ranged | 12" | D3 | 3+ | 6 | -2 | 2 | - |
| Volkite blaster | Ranged | 24" | 3 | 3+ | 5 | 0 | 2 | Devastating Wounds |

**Abilities:**

#### Leader (Abilities)
- **Description:** This model can be attached to the following units:
■ Corpuscarii Electro-Priests
■ Fulgurite Electro-Priests
■ Kataphron Breachers
■ Kataphron Destroyers
■ Skitarii Rangers
■ Skitarii Vanguard

#### Invulnerable Save (Abilities)
- **Description:** This model has a 5+ invulnerable save.

#### Lord of the Machine Cult (Abilities)
- **Description:** While this model is leading a unit, models in that unit have the Feel No Pain 5+ ability. If that unit has the **Electro-Priests** keyword, models in that unit have the Feel No Pain 4+ ability instead

#### Data-spike (Abilities)
- **Description:** At the start of the Fight phase, you can select one enemy **Vehicle** unit within Engagement Range of this model’s unit and roll one D6: on a 4+, that enemy unit suffers D6 mortal wounds and, until the end of the phase, the Weapon Skill characteristic of melee weapons equipped by that enemy unit is worsened by 1.

---

### Tech-Priest Enginseer
**Points:** 55 | **M:** 6" | **T:** 4 | **SV:** 3+ | **W:** 3 | **LD:** 7+ | **OC:** 1
**Keywords:** Enginseer, Imperium, Adeptus Mechanicus, Tech-Priest, Character, Infantry, Cult Mechanicus

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Servo arm | Melee | Melee | 1 | 4+ | 6 | -2 | 2 | Extra Attacks |
| Omnissian axe | Melee | Melee | 3 | 4+ | 6 | -2 | 2 | - |

**Abilities:**

#### Enginseer (Abilities)
- **Description:** While this model is within 3" of one or more friendly **Adeptus Mechanicus Vehicle** units, unless it is leading a unit, this model has the Lone Operative ability.

#### Omnissiah's Blessing (Abilities)
- **Description:** In your Command phase, select one friendly Adeptus Mechanicus model within 3" of this model. That model regains up to D3 lost wounds and, if it is a Vehicle model, until the start of your next Command phase, that model has the Feel No Pain 5+ ability. Each model can only be selected for this ability once per Command phase

#### Vengeance for the Omnissiah (Abilities)
- **Description:** If a friendly **Adeptus Mechanicus Vehicle** model is destroyed within 12" of this model, until the end of the battle, this model’s Omnissian axe has an Attacks characteristic of 6.

#### Invulnerable Save (Abilities)
- **Description:** This model has a 5+ invulnerable save.

#### Leader (Abilities)
- **Description:** This model can be attached to the following units:
■ Corpuscarii Electro-Priests
■ Fulgurite Electro-Priests
■ Kataphron Breachers
■ Kataphron Destroyers
■ Servitors
■ Skitarii Rangers
■ Skitarii Vanguard

---

### Tech-Priest Manipulus
**Points:** 60 | **M:** 6" | **T:** 4 | **SV:** 2+ | **W:** 4 | **LD:** 7+ | **OC:** 1
**Keywords:** Manipulus, Imperium, Adeptus Mechanicus, Tech-Priest, Character, Infantry, Cult Mechanicus

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Magnarail lance | Ranged | 36" | 1 | 3+ | 7 | -2 | 3 | Heavy |
| Transonic cannon | Ranged | 12" | D6 | N/A | 4 | 0 | 2 | Devastating Wounds, Torrent |
| Omnissian staff | Melee | Melee | 4 | 3+ | 6 | -1 | 2 | - |

**Abilities:**

#### Invulnerable Save (Abilities)
- **Description:** This model has a 5+ invulnerable save.

#### Leader (Abilities)
- **Description:** This model can be attached to the following units:
■ Corpuscarii Electro-Priests
■ Fulgurite Electro-Priests
■ Kataphron Breachers
■ Kataphron Destroyers
■ Skitarii Rangers
■ Skitarii Vanguard

#### Galvanic Field (Abilities)
- **Description:** While this model is leading a unit, weapons equipped by models in that unit have the [Lethal Hits] ability.

#### Defend the Divine Work (Abilities)
- **Description:** Once per battle, at the start of any phase, this model can use this ability. If it does, until the end of the phase, all models in this model’s unit have a 4+ invulnerable save.

---

### Technoarcheologist
**Points:** 45 | **M:** 6" | **T:** 4 | **SV:** 3+ | **W:** 3 | **LD:** 7+ | **OC:** 1
**Keywords:** Technoarcheologist, Imperium, Adeptus Mechanicus, Tech-Priest, Character, Infantry, Cult Mechanicus

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Servo-arc claw | Melee | Melee | 3 | 4+ | 8 | -2 | 2 | Anti-Vehicle 4+, Devastating Wounds |

**Abilities:**

#### Invulnerable Save (Abilities)
- **Description:** This model has a 5+ invulnerable save.

#### Seekers of Divine Arcana (Abilities)
- **Description:** While this model is leading a unit, add 1 to the Objective Control characteristic of models in that unit.

#### Cogitative Instincts (Abilities)
- **Description:** Enemy units that are set up on the battlefield as Reinforcements cannot be set up within 12" horizontally of this model.

#### Leader (Abilities)
- **Description:** This model can be attached to the following units:
■ Corpuscarii Electro-Priests
■ Fulgurite Electro-Priests
■ Kataphron Breachers
■ Kataphron Destroyers
■ Skitarii Rangers
■ Skitarii Vanguard

---

### Terrax-Pattern Termite [Legends]
**Points:** 200 | **M:** 8" | **T:** 10 | **SV:** 3+ | **W:** 14 | **LD:** 6+ | **OC:** 2
**Keywords:** Vehicle, Imperium, Transport, Dedicated Transport, Terrax-pattern Termite

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| Termite drill | Melee | Melee | 6 | 4+ | 14 | -2 | D3+3 | Anti-Vehicle 3+ |
| Terrax melta cutter | Ranged | 12" | 5 | 4+ | 9 | -4 | D6 | Melta 2 |
| Combi bolter | Ranged | 24" | 2 | 4+ | 4 | 0 | 1 | Rapid Fire 2 |
| Heavy flamer | Ranged | 12" | D6 | N/A | 5 | -1 | 1 | Ignores Cover, Torrent |
| Twin volkite charger | Ranged | 18" | 2 | 4+ | 5 | 0 | 2 | Devastating Wounds, Twin-linked |

**Abilities:**

#### Damaged: 1-5 wounds remaining (Abilities)
- **Description:** While this mdel has 1-5 wounds remaining, each time this model makes an attack, subtract 1 from the Hit roll.

#### Termite Assault (Abilities)
- **Description:** This model must start the battle in Reserves, but neither it nor any units embarked within it are counted towards any limits placed on the maximum number of Reserves units you can start the battle with. This model can be set up in the Reinforcements step of your first, second or third Movement phase, regardless of any mission rules. Any units embarked within this model can disembark after it has been set up on the battlefield, and if they do they must be set up more than 9" away from all enemy models.

#### Deadly Demise (Abilities)
- **Description:** D3

---

### Thulia Ghuld
**Points:** 210 | **M:** 8" | **T:** 8 | **SV:** 2+ | **W:** 10 | **LD:** 6+ | **OC:** 3
**Keywords:** Epic Hero, Monster, Character, Tech-Priest, Adeptus Mechanicus, Imperium, Cult Mechanicus, Thulia Ghuld

**Weapons:**
| Weapon | Type | Range | A | BS/WS | S | AP | D | Keywords |
|--------|------|-------|---|-------|---|----|---|----------|
| ➤ Rod of the War Forge - strike | Melee | Melee | 6 | 2+ | 10 | -2 | 3 | - |
| ➤ Rod of the War Forge - sweep | Melee | Melee | 12 | 2+ | 6 | -1 | 1 | - |
| ➤ Jericho-class conversion resonator - shockwave | Ranged | 24" | D6+2 | 2+ | 6 | -2 | 1 | Blast, Sustained Hits 1 |
| ➤ Jericho-class conversion resonator - titanic impact | Ranged | 24" | 2 | 2+ | 12 | -3 | D6+2 | Devastating Wounds |

**Abilities:**

#### Icon of War (Abilities)
- **Description:** **Fanatical Devotion:** You can select one friendly Skitarii or Thulia Ghuld unit within 6" of this model; until the start of your next Command phase, that unit is eligible to shoot and declare a charge in a turn in which it Advanced.
**Adaptive Tactics:** You can select one friendly Skitarii or Thulia Ghuld unit within 6" of this model; until the start of your next Command phase, that unit is eligible to shoot and declare a charge in a turn in which it Fell Back.
**The Fires of Mars:** You can select one friendly Skitarii or Thulia Ghuld unit within 6" of this model; until the start of your next Command phase, the Conqueror Imperative and Protector Imperative are both active for that unit.

#### Mechanicus Bodyguard (Abilities)
- **Description:** While this model is within 3" of one or more other friendly **Adeptus Mechanicus** units, this model has the Lone Operative ability.

#### Secutor of Olympus (Abilities)
- **Description:** At the start of your Shooting phase, select one enemy Vehicle unit within 12" of this model and roll one D6: on a 2+, that enemy unit suffers D3+1 mortal wounds.

#### Supreme Commander (Abilities)
- **Description:** If this model is in your army, it must be your Warlord.

#### Rod of the War Forge (Abilities)
- **Description:** In your Command phase, select one of the abilities in the Icon of War section. Until the start of your next Command phase, this model has that ability.

#### Cybernetic Augmentation (Abilities)
- **Description:** This model can move through terrain features, but cannot end a move within a wall, a floor, etc. This model can be set up or end a move on any floor level of Ruins, but if that level is not the ground floor, it can only do so if its base does not overhang the floor at that level.

---

## Points Table

| Unit | Points |
| --- | ---: |
| Hastarii Exterminator | 0 |
| Hastarii Exterminator Alpha | 0 |
| Hastarii Fusilier Alpha | 0 |
| Ironstrider Ballistarii | 0 |
| Kastelan Robot | 0 |
| Pteraxii Skystalker Alpha | 0 |
| Pteraxii Sterylizor Alpha | 0 |
| Sydonian Dragoons with taser lances | 0 |
| Cybernetica Datasmith | 35 |
| Skitarii Marshal | 35 |
| Cohort Commander [Crucible] | 45 |
| Technoarcheologist | 45 |
| Secutarii Peltasts [Legends] | 50 |
| Servitors [Legends] | 50 |
| Sydonian Skatros | 50 |
| Serberys Sulphurhounds | 55 |
| Sydonian Dragoons with radium jezzails | 55 |
| Tech-Priest Enginseer | 55 |
| Serberys Raiders | 60 |
| Servitor Battleclade | 60 |
| Tech-Priest Manipulus | 60 |
| Corpuscarii Electro-Priests | 65 |
| Secutarii Hoplites [Legends] | 65 |
| Sydonian Dragoon with taser lance | 65 |
| Tech-Priest Dominus | 65 |
| Fulgurite Electro-Priests | 70 |
| Magos [Crucible] | 70 |
| Pteraxii Skystalkers | 75 |
| Sicarian Infiltrators | 75 |
| Ironstrider Alpha [Crucible] | 80 |
| Pteraxii Sterylizors | 80 |
| Sicarian Ruststalkers | 80 |
| Ironstrider Ballistarius (Twin cognis autocannon) | 85 |
| Ironstrider Ballistarius (Twin cognis lascannon) | 85 |
| Skitarii Rangers | 85 |
| Skorpius Dunerider | 85 |
| Skitarii Vanguard | 95 |
| Kataphron Destroyers | 105 |
| Hastarii Exterminators | 135 |
| Hastarii Fusiliers | 145 |
| Archaeopter Transvector | 150 |
| Onager Dunecrawler | 155 |
| Archaeopter Fusilave | 160 |
| Kataphron Breachers | 160 |
| Skorpius Disintegrator | 165 |
| Kastelan Robots | 180 |
| Archaeopter Stratoraptor | 185 |
| Terrax-Pattern Termite [Legends] | 200 |
| Belisarius Cawl | 210 |
| Thulia Ghuld | 210 |
