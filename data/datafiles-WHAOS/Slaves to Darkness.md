# Slaves to Darkness

## Army Rules

### Daemon Prince

#### Airborne Horror (Passive)
- **Effect:** If this unit has **Wings**, it has a Move characteristic of 10" and has Fly.

#### The Mounted Skulls of the Fallen (Passive)
- **Effect:** If this unit has a **Trophy Rack**, its **Hellforged Weapons** also have Anti-**Hero (+1 Rend)**.

### Anvil of Apotheosis

#### Oath of Supremacy (Activated)
- **Timing:** Once Per Battle, Any Charge Phase
- **Effect:** If this unit is wholly within enemy territory and charged this phase, for the rest of the battle, add 1 to hit rolls for combat attacks made by friendly Darkoath units while they are wholly within 12" of this unit.

#### Myrmidon Rage (Passive)
- **Effect:** While this unit is damaged, add 2 to the Attacks characteristic of its melee weapons.

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of its **Mighty Chaotic Beast's Talons** is 4.

#### The Bringer of Fire (Activated)
- **Timing:** Your Movement Phase
- **Declare:** Pick a terrain feature that has not yet been set ablaze within this Hero's combat range to be the target.
- **Effect:** That terrain feature is now set ablaze. Add 1 to this unit's control score for the rest of the battle (this effect is cumulative). In addition, for the rest of the battle, each time a unit (friend or enemy) ends a move within 3" of a terrain feature that has been set ablaze, roll a D3. On a 2+, inflict an amount of mortal damage on that unit equal to the roll.

#### Victor of 100 Duels (Activated)
- **Timing:** Any Combat Phase
- **Declare:** Pick an enemy Hero in combat with this unit to be the target.
- **Effect:** Your opponent must decide whether the target will accept or refuse this unit's challenge:
***Accept:*** For the rest of the phase, when this unit and the target are picked to use a Fight ability, all of their attacks must target the other unit. In addition, for the rest of the phase, you can re-roll hit rolls for this unit's combat attacks.
***Refuse:*** The target cannot use Fight abilities for the rest of the phase. In addition, you gain 1 command point.

#### Betrayed by the False God (Passive)
- **Effect:** You can re-roll hit rolls for this unit's combat attacks that target enemy Order units.

#### Rampant Mutation (Activated)
- **Timing:** Your Hero Phase
- **Effect:** This unit must use this ability in each of your hero phases. Roll a dice and apply the corresponding effect until the start of your next turn:
**1 *Rotting Skin:*** Inflict 1 mortal damage on this unit. Subtract 1 from the control scores of enemy units while they are within 12" of this unit.
**2 *Brittle Scales:*** Add 1 to the Rend characteristic of combat attacks that target this unit.
**3 *Bestial Tongue:*** You lose 1 command point. This unit cannot use commands.
**4 *Sprouting Tentacles:*** This unit's Move characteristic is 2D6".
**5 *Extra Eyeball:*** Add 1 to hit rolls for this unit's combat attacks.
**6 *Erupting Spikes:*** Each time this unit finishes a pile-in move, pick an enemy unit within 1" of this unit to be the target. Inflict D3 mortal damage on the target.

#### Reckless Arrogance (Passive)
- **Effect:** This unit cannot use the 'All-out Defence' command.

#### Spurned by the Gods (Passive)
- **Effect:** This unit cannot be picked as a target for the 'Eye of the Gods' ability.

#### Blood of Molten Varanite (Passive)
- **Effect:** Roll a dice each time a damage point inflicted by a combat attack is allocated to this unit. On a 4+, inflict 1 mortal damage on the attacking unit after the Fight ability has been resolved.

#### Carve a Bloody Path (Activated)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Declare:** Pick a terrain feature that has not yet been set ablaze within this Hero's combat range to be the target.
- **Effect:** When this unit uses the 'Power Through' command, inflict an additional D3+ 1 mortal damage on the target and add 3" to the distance this unit can move as part of that ability.
- **Keywords:** Rampage

#### Blood-fuelled Menace (Activated)
- **Timing:** Once Per Battle, Any Combat Phase
- **Effect:** This unit can use 2 Fight abilities this phase. After the first is used, however, this unit has Strike-last for the rest of the turn.

#### Glimpse the Future (Activated)
- **Timing:** Deployment Phase
- **Effect:** Roll a dice. On a 1-4, the result corresponds to the battle round in which this unit will be inspired. On a 5+, pick a number. The number you picked corresponds to the battle round in which this unit will be inspired. While this unit is inspired, it has Strike-first.

#### The Grandfather's Blessing (Activated)
- **Timing:** Start of Any Turn
- **Effect:** Heal (D3) this unit.

#### The Thrill of Battle (Passive)
- **Effect:** Each time you make a charge roll for this unit, for each dice that was an unmodified 6, add 1 to the Attacks characteristic of this unit's melee weapons for the rest of the battle. Units can be affected by this ability multiple times and the effects are cumulative.

### Paths

#### Daemonic Pact (Activated)
- **Timing:** Once Per Battle, Any Combat Phase
- **Effect:** Roll a D3. Inflict an amount of mortal damage on this unit equal to the roll. Then, pick 1 of this unit's melee weapons and add the result to its Attacks characteristic for the rest of the turn.

#### Marked for Greatness (Passive)
- **Effect:** This unit has Ward (6+). In addition, each time an unmodified ward roll of 6 is made for this unit, **Heal (1)** this unit.

#### Dark Immortality (Activated)
- **Timing:** Once Per Battle, Your Movement Phase
- **Declare:** This unit can use this ability if it has been destroyed.
- **Effect:** Roll a dice. On a 6+, set up a replacement unit on the battlefield more than 9" from all enemy units.

#### Not to Be Denied (Activated)
- **Timing:** Any Combat Phase
- **Effect:** If this unit is within 6" of any enemy units but not in combat, this unit can make a move of D6".

#### Exhilarating Speed (Activated)
- **Timing:** Reaction: You declared a Charge ability for this unit
- **Effect:** Change one of the dice in the charge roll to a 6.

#### Molten Goreblade (Passive)
- **Effect:** Each time a hit roll for a combat attack made for this unit scores a critical hit, you can pick an enemy unit within 8" of this unit to be the target. If you do so, inflict 1 mortal damage on the target after that Fight ability has been resolved.

#### Fated Champion (Activated)
- **Timing:** Deployment Phase
- **Effect:** Roll 3 dice. During the battle, instead of making a roll from the list below for this unit, you can spend 1 of these dice The roll you would have made is replaced with the roll on the dice you spend:
• Hit Rolls
• Wound rolls
• Save rolls

#### Bountiful Flesh (Passive)
- **Effect:** Subtract 2 from the Damage characteristic of weapons used for attacks that target this unit.

#### Conquerors for Chaos (Passive)
- **Effect:** Each time this unit uses a Fight ability, add 1 to this unit's control score for this unit for the rest of the battle (this effect is cumulative).

#### Aspiring Warriors (Activated)
- **Timing:** Your Hero Phase
- **Effect:** For the rest of the turn, this unit has Ward (6+)

#### The Dread Hosts March (Activated)
- **Timing:** Once Per Battle, Your Charge Phase
- **Effect:** For the rest of the phase, add 1 to the number of dice rolled when making charge rolls for this unit, to a maximum of 3.

#### Bulwarks of Black Iron (Passive)
- **Effect:** This unit's melee weapons have **Anti-charge (+1 Rend)**

#### Ferrium Plate (Passive)
- **Effect:** Each phase, the first damage point inflicted on this unit is ignored.

#### Bringers of Ruin (Activated)
- **Timing:** Once Per Battle, Any Combat Phase
- **Effect:** Add 1 to the Attacks characteristic of this unit's melee weapons for the rest of the turn.

#### Godmarked Warriors (Activated)
- **Timing:** Deployment Phase
- **Effect:** Pick one of the **Pledge to Chaos** keywords for this unit to have for the rest of the battle.

### Battle Traits

#### Pledge to Chaos (Activated)
- **Timing:** Once Per Turn (Army), Your Hero Phase
- **Declare:** Pick a friendly non-Unique Slaves to Darkness unit that does not already have one of the keywords below to be the target.
- **Effect:** Pick 1 of the following **Pledge to Chaos** keywords:
• Pledged to Khorne: 
• Pledged to Tzeentch:
• Pledged to Nurgle:
• Pledged to Slaanesh:
The target has that keyword for the rest of the battle.

#### Eye of the Gods (Activated)
- **Timing:** Once Per Battle (Army), Deployment Phase
- **Declare:** Pick a friendly non-Unique Warriors of Chaos or Darkoath Hero to be the target.

#### Dark Apotheosis (Activated)
- **Timing:** Once Per Battle (Army), Any Hero Phase
- **Declare:** Pick a friendly unit with 8 or more **Dark Apotheosis** points to be the target.
- **Effect:** Pick 1 of the following.
***Aura of Chaos:* Heal(X)** the target, where **X** is the number of damage points allocated to the target. In addition, the target has Ward (5+) for the rest of the battle.
***Daemonhood:*** Set up a **Daemon Prince** within 1" of the target. Then, remove the target from the battlefield. If the target is your general, the **Daemon Prince** becomes your general instead and is added to your general’s regiment. If the target has any enhancements, the **Daemon Prince** has those enhancements instead. If the target has a **Pledge to Chaos** keyword, the **Daemon Prince** has the same keyword. Otherwise, pick 1 **Pledge to Chaos** keyword for the **Daemon Prince** to have for the rest of the battle If any Manifestations that were summoned by the target are on the battlefield, those Manifestations are removed from play.

#### Pledged to Tzeentch (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Declare:** Pick a friendly Pledged to Tzeentch unit to be the target.
- **Effect:** Roll 2D6. Then pick a point within a number of inches of the target equal to the roll. Remove the target from the battlefield and set it up again on the battlefield wholly within 6" of that point and more than 9" from all enemy units.

#### Pledged to Khorne (Passive)
- **Effect:** Add 1 to the Attacks characteristic of melee weapons used by friendly Pledged to Khorne units.

#### Pledged to Nurgle (Passive)
- **Effect:** Friendly Pledged to Nurgle units have Ward (6+). If a unit already has Ward (6+), it has Ward (5+) instead.

#### Pledged to Slaanesh (Activated)
- **Timing:** Your Charge Phase
- **Effect:** For the rest of the turn, when making charge rolls for friendly Pledged to Slaanesh units, you can roll 1 additional dice (to a maximum of 3), but if you do, you must discard 1 dice of your choice.

### Artefacts of Power

#### The Conquerer's Crown (Passive)
- **Effect:** Subtract 5 from the control scores of enemy Infantry units while they are in combat with this unit.

#### Realmwarper's Twist-rune (Activated)
- **Timing:** Once Per Battle, Your Hero Phase
- **Declare:** Pick a terrain feature within 12" of this unit to be the target.
- **Effect:** If the target does not have the ‘Obscuring’ terrain ability, it gains that ability until the start of your next turn. Then, roll a dice for each model (friendly and enemy) within 1" of the target. For each 5+, inflict 1 mortal damage on that model’s unit.

#### Infernal Puppet (Activated)
- **Timing:** Once Per Battle, Start of Any Turn
- **Declare:** Pick a visible enemy Wizard within 24" of this unit to be the target.
- **Effect:** For the rest of the turn, each time the target is picked to use a Spell ability, inflict D3 mortal damage on the target as a reaction. If the target is destroyed by this ability, do not resolve the effect of that Spell ability.

#### The Varanite Shackle (Passive)
- **Effect:** Friendly non-Warriors of Chaos Slaves to Darkness units have Ward (6+) while they are wholly within 12" of this unit.

#### Amulet of Chaos (Activated)
- **Timing:** End of Any Turn
- **Declare:** Pick a visible enemy unit within 12" of this unit to be the enemy target. Then, you can pick a friendly Darkoath Infantry unit in combat with that enemy unit to be the friendly target.
- **Effect:** Roll a D3. On a 2+:
• Inflict an amount of mortal damage on the enemy target equal to the roll.
• If there is a friendly target, return a number of slain models to it equal to the roll.

#### Head of the Unworthy (Passive)
- **Effect:** If a friendly Slaves to Darkness Hero with any **Dark Apotheosis** points is destroyed, give this unit 8 **Dark Apotheosis** points.

### Battle Formations

#### Rally the Tribes (Command)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Cost:** 1
- **Declare:** If there is a friendly Darkoath Hero on the battlefield, pick a friendly non-Hero non-Unique Darkoath unit that has been destroyed to be the target.
- **Effect:** Set up a replacement unit with half the number of models from the target unit (rounding up) on the battlefield, wholly within 6" of a battlefield edge and more than 3" from all enemy units.

#### Feral Ruin (Activated)
- **Timing:** Once Per Turn (Army), Reaction: You declared a Fight ability for a friendly **Daemon Prince** unit
- **Declare:** The **Daemon Prince** unit using that Fight ability.
- **Effect:** Pick a friendly non-Hero Slaves to Darkness Monster or Beast unit that has not used a Fight ability this turn and is wholly within 12" of this unit to be the target. The target can be picked to use a Fight ability immediately after the Fight ability used by this unit has been resolved.

#### Ironclad Onslaught (Activated)
- **Timing:** Once Per Turn (Army), End of Your Turn
- **Declare:** Pick an objective that is contested by any friendly units to be the target.
- **Effect:** Place a **defiled token** next to the target objective. Then, roll a D3 for each enemy unit contesting an objective that has a friendly **defiled token**. On a 2+, inflict an amount of mortal damage on that enemy unit equal to the roll.

#### Devotees of Ruin (Activated)
- **Timing:** Once Per Turn (Army), Your Hero Phase
- **Declare:** Pick a friendly Warriors of Chaos unit. Then, pick up to 2 visible friendly Darkoath units that do not have a **Pledge to Chaos** keyword and are wholly within 12" of that Warriors of Chaos unit to be the targets.
- **Effect:** Until the start of your next turn, the targets have the same **Pledge to Chaos** keyword as that Warriors of Chaos unit.

#### Paragons of Darkness (Passive)
- **Effect:** Add 2 to the Attacks characteristic of melee weapons used by friendly non-Monster Slaves to Darkness Heroes.

### Heroic Traits

#### Favoured of the Pantheon (Passive)
- **Effect:** If this unit is picked as the target of the 'Eye of the Gods' ability, it gains 3 **Dark Apotheosis** points.

#### Deathmonger (Activated)
- **Timing:** Once Per Battle, Any Combat Phase
- **Effect:** This unit can use 2 Fight abilities this phase. After the first is used, however, this unit has Strike-last for the rest of the turn.

#### Radiance of Dark Glory (Activated)
- **Timing:** Any Hero Phase
- **Declare:** Pick each damaged friendly unit wholly within 12" of this unit to be the targets.
- **Effect:** Roll a dice for each target. On a 3+, **Heal (1)** the target. **Heal (3)** the target instead if it is a Monster.

### Ensorcelled Banners

#### The Eroding Icon (Passive)
- **Keywords:** Ensorcelled Banner
- **Effect:** This unit has the Pledged to Nurgle keyword.
In addition, while this unit includes any standard bearers, attacks that target this unit cannot score critical hits (treat them as regular hits instead).

#### The Banner of Rage (Passive)
- **Keywords:** Ensorcelled Banner
- **Effect:** This unit has the Pledged to Khorne keyword.
In addition, while this unit includes any standard bearers, add 1 to wound rolls for this unit's combat attacks.

#### The Banner of Screaming Flesh (Passive)
- **Keywords:** Ensorcelled Banner
- **Effect:** This unit has the Pledged to Slaanesh keyword.
In addition, while this unit includes any standard bearers, if this unit charged this turn, for the rest of the turn, add 1 to the Attacks characteristic of this unit's melee weapons.

#### The Blasted Standard (Passive)
- **Keywords:** Ensorcelled Banner
- **Effect:** This unit has the Pledged to Tzeentch keyword.
In addition, while this unit includes any standard bearers, this unit has Ward (4+) against damage inflicted by shooting attacks.

#### The Dread Banner (Passive)
- **Keywords:** Ensorcelled Banner
- **Effect:** While this unit includes any standard bearers, enemy units cannot use commands while they are in combat with this unit.

## Subfactions

### Legion of the First Prince

**Battle Traits**

#### Daemonic Reinforcements (Command)
- **Timing:** Once Per Turn (Army), Any Movement Phase
- **Cost:** 1
- **Declare:** Pick a friendly non-Hero Legion of the First Prince Daemon Infantry or Cavalry unit that has been destroyed to be the target.
- **Effect:** Set up a replacement unit with half the number of models from the target unit (rounding up) wholly within 12" of a friendly **Be'lakor** and more than 9" from all enemy units.

#### Storm of Blackened Blades (Activated)
- **Timing:** Once Per Battle (Army), Your Combat Phase
- **Declare:** Pick a friendly **Eternus** that is in combat and up to 2 friendly non-Hero non-Daemon Legion of the First Prince units wholly within 12" of that **Eternus** to be the targets.
- **Effect:** Add 1 to the Attacks characteristic of the targets' melee weapons for the rest of the turn.

#### Bestow Favour (Activated)
- **Timing:** Once Per Turn (Army), Your Hero Phase
- **Declare:** If a friendly **Be'lakor** is on the battlefield, pick a non-Hero Legion of the First Prince unit to be the target.
- **Effect:** Until the start of your next turn:
• Add 5 to the target's control score.
• If the target is a Daemon unit, add 1 to save rolls for the target.
• If the target is a non-Daemon unit, it has Ward (6+).

#### First-damned Prince (Passive)
- **Effect:** While a friendly **Be'lakor** is within the combar range of any friendly non-Hero Legion of the First Prince Daemon units:
• That friendly **Be'lakor** has Ward (4+).
• Each time you make a successful ward roll for that friendly **Be'lakor**, allocate 1 damage point to a friendly non-**Hero Legion of the First Prince Daemon** unit within **Be'lakor**'s combat range after the damage sequence for this unit has been resolved (ward rolls cannot be made for those damage points).

**Artefacts of Power**

#### Black Ritual Dagger (Activated)
- **Timing:** Once Per Turn, Reaction: You declared a Spell* ability for a friendly unit wholly within 12" of this unit.
- **Effect:** Allocate 1 damage point to this unit. Then, you can re-roll the casting roll.

**Heroic Traits**

#### Dread Marshal (Activated)
- **Timing:** Any Charge Phase
- **Declare:** Pick up to 3 friendly units wholly within 12" of this unit to be the targets.
- **Effect:** Add 1 to charge rolls for the targets for the rest of the turn.

### The Swords of Chaos

**Battle Traits**

#### Demands of the Dark Gods (Activated)
- **Timing:** Once Per Turn (Army), Start of Battle Round
- **Effect:** Pick 1 of the following effects:
***Demand of Khorne:*** For the rest of the battle round, add 1 to the Rend characteristic of melee weapons used by friendly Swords of Chaos units.
***Demand of Tzeentch:*** Pick 1 visible enemy unit within 9" of a friendly Swords of Chaos unit that is not in combat and roll a D6. On a 3+, inflict an amount of mortal damage on that enemy unit equal to the roll.
***Demand of Nurgle:*** **Heal (D3)** each friendly Swords of Chaos unit. Then pick a friendly Swords of Chaos unit that healed any damage points this turn. Until the start of the next battle round, that unit has Ward (6+).
***Demand of Slaanesh:*** Add 2" to the Move characteristic of friendly Swords of Chaos units until the start of the next battle round

#### Knights of the First Circle (Activated)
- **Timing:** Deployment Phase
- **Declare:** If there are more friendly Swords of Chaos units on the battlefield than there are set up in reserve, pick a friendly Swords of Chaos unit that has not been deployed.
- **Effect:** Set up that unit in reserve **in the Ruinous Skies**. It has now been deployed.
- **Keywords:** Deploy

#### Descent from Ruinous Skies (Activated)
- **Timing:** Your Movement Phase
- **Declare:** Pick a friendly Swords of Chaos unit that is **in the Ruinous Skies**.
- **Effect:** Set up that unit anywhere on the battlefield more than 9" from all enemy units

**First Circle Titles**

#### Betrayers of the Anvilking (Passive)
- **Effect:** This unit has Ward (6+).

#### Tamers of Haradh's Torment (Passive)
- **Effect:** Add 1 to the Damage characteristic of this unit’s **Companion** weapons.

#### The Blackstorm Apostates (Passive)
- **Effect:** Add 3" to this unit’s Move characteristic.

#### The Grimroot Order (Passive)
- **Effect:** Add 1 to this unit’s Health characteristic.

#### The Hounds Apocalyptus (Passive)
- **Effect:** While this unit is wholly within enemy territory, add 1 to the Attacks characteristic of this unit’s melee weapons.

### Tribes of the Snow Peaks

**Battle Traits**

#### Pledges to the Dark Gods (Passive)
- **Effect:** Each time a friendly Snow Peaks unit uses 1 of the following abilities and the effect of that ability is resolved, gain 1 **oath point**.
• ‘Oath of Bloodshed’
• ‘Oath of Murder’
• ‘Oath of Supremacy’
• ‘Oath of the Marauder’
• ‘Oath of the Raider’
• ‘Oath of Conquest’
• ‘Oath of Dark Sacrifice’
**Designer’s Note:** *The abilities listed above are abilities on Darkoath warscrolls. By using those abilities (i.e. completing the oaths), the player gains* **oath points***, which can then be spent to use the* Oath *abilities in these Army of Renown rules.*

#### Hunger of Shesh'shan (Activated)
- **Timing:** Once Per Phase (Army), Your Combat Phase
- **Declare:** If you have 1 or more **oath points**, pick a friendly Snow Peaks unit that has not used an Oath ability this turn to be the target.
- **Effect:** Spend 1 **oath point**. For the rest of the turn, add 1 to the Attacks characteristic of the target’s melee weapons
- **Keywords:** Oath

#### Daemonfire Weapons (Activated)
- **Timing:** Once Per Phase (Army), Your Combat Phase
- **Declare:** If you have 1 or more **oath points**, pick a friendly Snow Peaks unit that has not used an Oath ability this turn to be the target.
- **Effect:** Spend 1 **oath point**. For the rest of the turn, add 1 to the Rend characteristic of the target’s melee weapons.
- **Keywords:** Oath

#### Messengers of the Gods (Activated)
- **Timing:** Once Per Phase (Army), End of Any Turn
- **Declare:** If you have 1 or more **oath points**, pick a friendly Snow Peaks **Darkoath Marauders** or **Fellriders** unit that has been destroyed and has not used an Oath ability this turn to be the target
- **Effect:** Spend 2 **oath points**. Set up a replacement unit with half the number of models from the target unit (rounding up) on the battlefield, wholly within 6" of a battlefield edge and more than 3" from all enemy units.
- **Keywords:** Oath

#### Shroud of the Pale Elk (Activated)
- **Timing:** Once Per Phase (Army), Reaction: Opponent declared an Attack ability
- **Declare:** A friendly Snow Peaks unit targeted by that Attack ability, if you have 1 or more **oath points**.
- **Effect:** If that friendly Snow Peaks unit has not used an Oath ability this turn, spend 1 **oath point**. For the rest of the turn, that friendly unit has Ward (4+).
- **Keywords:** Oath

#### Rage of Arkhar (Activated)
- **Timing:** Once Per Phase (Army), Your CombatPhase
- **Declare:** If you have 1 or more **oath points**, pick a friendly Snow Peaks unit that has not used an Oath ability this turn to be the target.
- **Effect:** Spend 2 **oath points**. The target can use 2 Fight abilities this phase. After the first is used, however, the target has Strike-last for the rest of the turn.
- **Keywords:** Oath

#### Speed of the Blood Crow (Activated)
- **Timing:** Once Per Phase (Army), Your Charge Phase
- **Declare:** If you have 1 or more **oath points**, pick a friendly Snow Peaks unit that has not used an Oath ability this turn to be the target.
- **Effect:** Spend 1 **oath point**. For the rest of the turn, the target can use Charge abilities even if it used a Run ability in the same turn.
- **Keywords:** Oath

**Heroic Traits**

#### Oath of Kinship (Passive)
- **Effect:** While this unit is within the combat range of another friendly Snow Peaks unit, add 1 to hit rolls for this unit’s attacks.

**Artefacts of Power**

#### Grand Offering (Activated)
- **Timing:** Once Per Battle (Army), Any Movement Phase
- **Declare:** Pick a friendly Snow Peaks **Wilderfiend** within 9" of this unit to be the target
- **Effect:** Give the target D6 **sacrifice points**.

## Units

### Abraxia, Spear of the Everchosen
**Points:** 250 | **Move:** 10" | **Health:** 14 | **Save:** 3+ | **Control:** 5
**Keywords:** UNIQUE, HERO, MONSTER, SLAVES TO DARKNESS, CHAOS, WARRIORS OF CHAOS, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Gorbolga | Melee | - | 5 | 3+ | 3+ | 2 | 2 | Anti‑Hero (+1 Rend), Charge (+1 Damage) |
| The Thanatorg’s Claws and Fangs | Melee | - | 6 | 4+ | 3+ | 1 | 2 | Companion |

**Abilities:**

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of **the Thanatorg's Claws and Fangs** is 4.

#### Warlord of the First Circle (Passive)
- **Effect:** If a friendly **Varanguard** unit uses the ‘Relentless Killers’ ability while it is wholly within 12" of this unit, that unit does not have Strike-last as a result of that ability.

#### Gorbolga the Accurs'd (Activated)
- **Timing:** Any Combat Phase
- **Effect:** If this unit is in combat, roll a dice and apply the corresponding effect:
**1 *Uncontrollable Power:*** Allocate 1 damage point to this unit.
**2 *The Spear Slumbers:*** No effect.
**3-5 *White-hot Varanite:*** Add 1 to the Damage characteristic of this unit’s **Gorbolga** for the rest of the turn.
**6 *Rampant Mutation:*** The effect of ‘White-hot Varanite’ applies and this unit’s **Gorbolga** has **Crit (Mortal)** for the rest of the turn.

#### Blood of Molten Varanite (Passive)
- **Effect:** Roll a dice each time a damage point inflicted by a combat attack is allocated to this unit. On a 4+, inflict 1 mortal damage on the attacking unit after the Fight ability has been resolved.

#### Roar of Domination (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy Hero in combat with this unit to be the target.
- **Effect:** Roll a dice. On a 3+, subtract 1 from the Attacks characteristic of the target’s melee weapons for the rest of the turn.
- **Keywords:** Rampage

---

### Abraxia, Spear of the Everchosen (Scourge of Ghyran)
**Points:** 300 | **Move:** 10" | **Health:** 14 | **Save:** 3+ | **Control:** 5
**Keywords:** UNIQUE, HERO, MONSTER, SLAVES TO DARKNESS, CHAOS, WARRIORS OF CHAOS, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Gorbolga the Accurs'd | Melee | - | 5 | 3+ | 3+ | 2 | 3 | Anti‑Hero (+1 Rend), Charge (+1 Damage) |
| The Thanatorg’s Vicious Claws | Melee | - | 6 | 4+ | 2+ | 2 | 2 | Companion |

**Abilities:**

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of **The Thanatorg’s Vicious Claws** is 4.

#### Mutative Spear (Activated)
- **Timing:** End of Any Turn
- **Declare:** Pick an enemy unit that had any damage points allocated to it this turn by this unit’s combat attacks to be the target.
- **Effect:** The target has a maximum control score of 1 for the rest of the turn.

#### Champion of the Undivided (Activated)
- **Timing:** Your Combat Phase
- **Declare:** Pick a friendly non-Unique Slaves to Darkness unit wholly within 12" of this unit and that does not have a **Pledge to Chaos** keyword to be the target.
- **Effect:** For the rest of the turn, add 1 to the Rend characteristic of melee weapons used by the target while they are contesting an objective you do not control.

#### Unending Will to Conquer (Activated)
- **Timing:** End of Any Turn

#### Unstoppable Invasion (Activated)
- **Timing:** Once Per Turn (Army), Any Charge Phase
- **Declare:** If this unit charged this phase, pick an enemy unit within 1" of it to be the target.
- **Effect:** Inflict D3 mortal damage on the target. Then, roll 2D6. This unit can move a distance up to the value of the roll. It can pass through models in the target unit but must end that move in combat.
- **Keywords:** Rampage

---

### Archaon, the Everchosen
**Points:** 810 | **Move:** 14" | **Health:** 25 | **Save:** 3+ | **Control:** 10
**Keywords:** UNIQUE, HERO, MONSTER, WARD (5+), FLY, WARMASTER, WIZARD (2), SLAVES TO DARKNESS, DAEMON, WARRIORS OF CHAOS, CHAOS

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| The Slayer of Kings | Melee | - | 4 | 2+ | 3+ | 2 | 3 | Anti-Hero (+1 Rend) |
| Dorghar’s Heads | Melee | - | 3 | 3+ | 2+ | 1 | 5 | Companion |
| Dorghar’s Claws and Tails | Melee | - | 6 | 4+ | 2+ | 2 | 3 | Companion |

**Abilities:**

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of **Dorghar’s Claws and Tails** is 4.

#### The Slayer of Kings (Passive)
- **Effect:** Each time this unit uses a Fight ability, if the unmodified wound roll for 2 or more attacks made with **The Slayer of Kings** that target the same Hero is 6, that Hero is automatically destroyed.

#### The Eye of Sheerian (Activated)
- **Timing:** Once Per Battle, Your Hero Phase
- **Effect:** Roll a dice. This roll replaces the priority roll of the next battle round. On a 1-3, your opponent must take the first turn of that battle round. On a 4-6, you must take the first turn of that battle round. This ability cannot be used if there is an enemy **Archaon** on the battlefield.

#### Three-headed Titan (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Effect:** Pick 1 of the following effects:
***Filth-spewer:*** Pick an enemy unit within 6" of this unit and roll a number of dice equal to the number of models in that unit, to a maximum of 7. For each 3+, inflict 1 mortal damage on that unit.
***Skull-gorger:*** Pick an enemy unit in combat with this unit and roll a dice. If the roll exceeds that unit’s Health characteristic, 1 model in that unit is slain and you can **Heal (X)** this unit, where **X** is that unit’s Health characteristic.
***Spell-eater:*** Pick a Manifestation within 12" of this unit and roll a dice. On a 2+, it is banished and removed from play. In addition, inflict D3 mortal damage on the unit that summoned it.
- **Keywords:** Rampage

---

### Athol Khul
**Points:** 150 | **Move:** 5" | **Health:** 6 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, SLAVES TO DARKNESS, INFANTRY, CHAOS, DARKOATH, UNIQUE, PLEDGED TO KHORNE, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Spear of the Khul | Melee | - | 6 | 4+ | 3+ | 1 | 2 | Crit (Mortal) |

**Abilities:**

#### Tribal Warleader (Activated)
- **Timing:** Once Per Battle, End of Any Turn
- **Effect:** If this destroyed an enemy Hero this turn, this unit has Ward (4+) for the rest of the battle.

#### Oath of Murder (Activated)
- **Timing:** Once Per Battle, Your Charge Phase
- **Effect:** For the rest of the turn, friendly units that do not already have a Pledge to Chaos keyword have the Pledged to Khorne while they are wholly within 12" of this unit.

---

### Be'lakor, the Dark Master
**Points:** 450 | **Move:** 14" | **Health:** 14 | **Save:** 4+ | **Control:** 10
**Keywords:** UNIQUE, HERO, MONSTER, FLY, WARMASTER, WIZARD (2), SLAVES TO DARKNESS, DAEMON, WARD (6+), CHAOS

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Blade of Shadows | Melee | - | 8 | 3+ | 3+ | 2 | 2 | Crit (Auto-wound) |
| Fell Claw and Spearing Tail | Melee | - | 2 | 2+ | 2+ | 2 | 2 | Charge (+1 Damage) |

**Abilities:**

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of its **Blade of Shadows** is 6.

#### Shadow Form (Passive)
- **Effect:** Ignore modifiers to save rolls for this unit (positive and negative).

#### The Dark Master (Activated)
- **Timing:** Once Per Battle, Enemy Hero Phase
- **Declare:** Pick a visible enemy unit to be the target.
- **Effect:** Until the start of your next turn, each time the target is picked to use an ability, roll a dice as a reaction. On a 3+, that ability has no effect.

#### Enfeeble Foe (Spell)
- **Timing:** Your Hero Phase
- **Casting Value:** 6
- **Declare:** Pick a visible enemy unit within 18" of this unit to be the target, then make a casting roll of 2D6.
- **Effect:** Until the start of your next turn: 
• Subtract 1 from hit rolls from the target's combat attacks.
• The target’s combat attacks cannot score critical hits (treat them as regular hits instead).
- **Keywords:** Spell

#### Deadly Trap (Activated)
- **Timing:** Once Per Turn (Army), Enemy Combat Phase
- **Declare:** Pick this unit and up to 2 friendly Slaves to Darkness units within this unit’s combat range to be the targets.
- **Effect:** Roll a dice for each target. On a 4+, that target has Strike-first for the rest of the turn.
- **Keywords:** Rampage

---

### Centaurion Marshal
**Points:** 120 | **Move:** 8" | **Health:** 10 | **Save:** 4+ | **Control:** 2
**Keywords:** HERO, SLAVES TO DARKNESS, CAVALRY, CHAOS

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Mauling Spear, Skull Bludgeon and Varanspire Gladius | Melee | - | 6 | 3+ | 3+ | 1 | 2 | Charge (+1 Damage) |

**Abilities:**

#### Marshal of the Legions (Activated)
- **Timing:** You declared a Fight ability for this unit
- **Effect:** Pick a friendly non-Hero Daemon or **Chaos Legionnaires** unit that has not used a Fight ability this turn and is within this unit’s combat range to be the target. The target can be picked to use a Fight ability immediately after the Fight ability used by this unit has been resolved. If it is picked to do so, add 1 to hit rolls for the target’s attacks for the rest of the turn.

---

### Chaos Chariot
**Points:** 90 | **Move:** 10" | **Health:** 7 | **Save:** 4+ | **Control:** 2
**Keywords:** CHAOS, SLAVES TO DARKNESS, WARRIORS OF CHAOS, WAR MACHINE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Chaos Steeds’ Trampling Hooves | Melee | - | 4 | 5+ | 3+ | - | 1 | Companion |
| Lashing Whip | Melee | - | 2 | 4+ | 4+ | - | 1 | - |
| Chaos Greatblade | Melee | - | 3 | 4+ | 3+ | 1 | 2 | - |
| Chaos War-flail | Melee | - | 6 | 3+ | 3+ | - | 1 | - |

**Abilities:**

#### Swift Death (Activated)
- **Timing:** Any Charge Phase
- **Declare:** If this unit charged this phase, pick an enemy unit within 1" of it to be the target.
- **Effect:** Roll a D3. On a 2+, inflict an amount of mortal damage on the target equal to the roll.

---

### Chaos Chosen
**Points:** 270 | **Move:** 5" | **Health:** 3 | **Save:** 3+ | **Control:** 1
**Keywords:** CHAOS, SLAVES TO DARKNESS, INFANTRY, WARRIORS OF CHAOS, STANDARD BEARER (1/5), MUSICIAN (1/5), CHAMPION

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Soul Splitter | Melee | - | 3 | 3+ | 3+ | 1 | 2 | Crit (Mortal) |

**Abilities:**

#### Heralds of Ruination (Activated)
- **Timing:** Once Per Battle, Any Combat Phase
- **Effect:** This unit can use 2 Fight abilities this phase. After the first is used, however, this unit has Strike-last for the rest of the turn.

---

### Chaos Furies
**Points:** 120 | **Move:** 12" | **Health:** 1 | **Save:** 6+ | **Control:** 1
**Keywords:** CHAOS, SLAVES TO DARKNESS, DAEMON, FLY, WARD (6+), INFANTRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Razor-sharp Dagger and Claws | Melee | - | 3 | 4+ | 4+ | 1 | 1 | - |

**Abilities:**

#### Sneaky Little Devils (Activated)
- **Timing:** Any Combat Phase
- **Effect:** If this unit is in combat, roll a dice. On a 3+, this unit can immediately use the ‘Retreat’ ability as if it were your movement phase without any mortal damage being inflicted on it.

---

### Chaos Knights
**Points:** 250 | **Move:** 10" | **Health:** 4 | **Save:** 3+ | **Control:** 1
**Keywords:** CHAOS, SLAVES TO DARKNESS, WARRIORS OF CHAOS, STANDARD BEARER (1/5), MUSICIAN (1/5), CAVALRY, CHAMPION

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Warsteed’s Hooves | Melee | - | 2 | 5+ | 3+ | - | 1 | Companion |
| Cursed Lance | Melee | - | 3 | 3+ | 3+ | 1 | 1 | Charge (+1 Damage) |

**Abilities:**

#### Impaling Charge (Passive)
- **Effect:** If this unit charged this turn, for the rest of the turn, add 1 to the Rend characteristic of its **Cursed Lances**.

---

### Chaos Legionnaires
**Points:** 80 | **Move:** 5" | **Health:** 1 | **Save:** 4+ | **Control:** 1
**Keywords:** CHAOS, SLAVES TO DARKNESS, INFANTRY, CHAMPION (1/8), UNDIVIDED

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Darkiron Weapons | Melee | - | 2 | 3+ | 4+ | 1 | 1 | - |

**Abilities:**

#### Sow Confusion (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Declare:** Pick an enemy unit within 6" of this unit to be the target.
- **Effect:** Roll a dice. On a 4+, the target cannot use commands for the rest of the turn.

---

### Chaos Lord
**Points:** 100 | **Move:** 5" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, SLAVES TO DARKNESS, INFANTRY, WARRIORS OF CHAOS, CHAOS

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Reaperblade | Melee | - | 5 | 3+ | 3+ | 1 | 2 | Crit (Mortal) |

**Abilities:**

#### Glory in Battle (Activated)
- **Timing:** Reaction: You declared a Fight ability for this unit
- **Effect:** Pick a friendly non-Hero Warriors of Chaos Infantry unit that has not used a Fight ability this turn and is within this unit’s combat range to be the target. The target can be picked to use a Fight ability immediately after the Fight ability used by this unit has been resolved. If it is picked to do so, add 1 to wound rolls for the target’s attacks for the rest of the turn.

---

### Chaos Lord on Daemonic Mount
**Points:** 140 | **Move:** 10" | **Health:** 8 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, SLAVES TO DARKNESS, CAVALRY, WARRIORS OF CHAOS, CHAOS

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Cursed Weapons | Melee | - | 5 | 3+ | 3+ | 2 | 2 | Charge (+1 Damage) |
| Daemonic Mount’s Mighty Hooves | Melee | - | 3 | 5+ | 3+ | - | 1 | Companion |

**Abilities:**

#### The Knights of Chaos (Activated)
- **Timing:** You declared a Fight ability for this unit
- **Effect:** Pick a friendly non-Hero Warriors of Chaos Cavalry unit that has not used a Fight ability this turn and is within this unit’s combat range to be the target. The target can be picked to use a Fight ability immediately after the Fight ability used by this unit has been resolved. If it is picked to do so, add 1 to wound rolls for the target’s attacks for the rest of the turn.

---

### Chaos Lord on Karkadrak
**Points:** 190 | **Move:** 9" | **Health:** 10 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, SLAVES TO DARKNESS, CAVALRY, WARRIORS OF CHAOS, CHAOS

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Hexed Weapons | Melee | - | 5 | 3+ | 3+ | 1 | 2 | Charge (+1 Damage), Crit (Mortal) |
| Karkadrak’s Claws and Horn | Melee | - | 4 | 4+ | 3+ | 1 | 2 | Companion |

**Abilities:**

#### Drive the Legions Forward (Activated)
- **Timing:** Once Per Battle (Army), Any Charge Phase
- **Declare:** Pick this unit and up to 2 friendly non-Hero non-Cavalry Warriors of Chaos units wholly within 12" of this unit to be the targets.
- **Effect:** Add 2 to charge rolls for the targets for the rest of the turn.

#### Brutish Rampage (Activated)
- **Timing:** Any Charge Phase
- **Declare:** If this unit charged this phase, pick up to 3 enemy units within 1" of it to be the targets.
- **Effect:** Roll a D3 for each target. On a 2+, inflict an amount of mortal damage on the target equal to the roll.

---

### Chaos Lord on Manticore
**Points:** 260 | **Move:** 12" | **Health:** 12 | **Save:** 4+ | **Control:** 5
**Keywords:** HERO, SLAVES TO DARKNESS, MONSTER, FLY, CHAOS, WARRIORS OF CHAOS, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Daemonbound Weapons | Melee | - | 5 | 3+ | 3+ | 1 | 2 | Crit (Mortal) |
| Manticore’s Claws and Fangs | Melee | - | 6 | 4+ | 2+ | 1 | 2 | Anti-Monster (+1 Rend), Companion |

**Abilities:**

#### Iron-willed Overlord (Passive)
- **Effect:** Add 2 to the control scores of friendly Warriors of Chaos units while they are wholly within 12" of this unit.

#### Apex Predator (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy Monster in combat with this unit to be the target.
- **Effect:** Add 1 to hit rolls for this unit’s combat attacks that target that Monster for the rest of the turn. This ability also affects **Companion** weapons.
- **Keywords:** Rampage

---

### Chaos Sorcerer Lord
**Points:** 120 | **Move:** 5" | **Health:** 5 | **Save:** 4+ | **Control:** 2
**Keywords:** HERO, SLAVES TO DARKNESS, WIZARD (1), INFANTRY, WARRIORS OF CHAOS, CHAOS

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Sorcerer Staff and Runeblade | Melee | - | 3 | 4+ | 3+ | 1 | D3 | - |

**Abilities:**

#### Oracular Visions (Activated)
- **Timing:** Once Per Turn (Army), Your Hero Phase
- **Declare:** Pick a friendly Warriors of Chaos unit wholly within 12" of this unit to be the target and roll a dice.
- **Effect:** On a 3+, the target has Ward (5+) until the start of your next turn.

---

### Chaos Sorcerer Lord on Manticore
**Points:** 280 | **Move:** 12" | **Health:** 12 | **Save:** 4+ | **Control:** 5
**Keywords:** HERO, SLAVES TO DARKNESS, MONSTER, WIZARD (1), FLY, CHAOS, WARRIORS OF CHAOS, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Sorcerous Reaping Staff | Melee | - | 3 | 3+ | 3+ | 1 | D3 | - |
| Manticore’s Claws and Fangs | Melee | - | 6 | 4+ | 2+ | 1 | 2 | Anti-Monster (+1 Rend), Companion |

**Abilities:**

#### Oracular Visions (Activated)
- **Timing:** Once Per Turn (Army), Your Hero Phase
- **Declare:** Pick a friendly Warriors of Chaos unit wholly within 12" of this unit to be the target and roll a dice.
- **Effect:** On a 3+, the target has Ward (5+) until the start of your next turn.

#### Venomous Tail Strike (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy Monster in combat with this unit to be the target.
- **Effect:** Roll a dice. On a 3+, the target has Strike-last for the rest of the turn.
- **Keywords:** Rampage

---

### Chaos Spawn
**Points:** 60 | **Move:** 2D6" | **Health:** 5 | **Save:** 5+ | **Control:** 1
**Keywords:** SLAVES TO DARKNESS, CHAOS, BEAST

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Freakish Mutations | Melee | - | 2D6 | 5+ | 4+ | - | 1 | Crit (2 Hits), Companion |

**Abilities:**

#### Drawn to Power (Passive)
- **Effect:** While this unit is wholly within 12" of a friendly **Daemon Prince**, you can re-roll the random characteristic rolls for this unit’s Move and Attacks characteristics.

---

### Chaos Warriors
**Points:** 180 | **Move:** 5" | **Health:** 2 | **Save:** 3+ | **Control:** 1
**Keywords:** CHAOS, SLAVES TO DARKNESS, INFANTRY, WARRIORS OF CHAOS, STANDARD BEARER (1/10), MUSICIAN (1/10), CHAMPION

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Rune-etched Weapons | Melee | - | 2 | 3+ | 3+ | 1 | 1 | Crit (Auto-wound) |

**Abilities:**

#### Bringers of Desolation (Passive)

---

### Chaos Warshrine
**Points:** 250 | **Move:** 8" | **Health:** 14 | **Save:** 4+ | **Control:** 5
**Keywords:** HERO, SLAVES TO DARKNESS, CHAOS, PRIEST (1), WAR MACHINE, WARRIORS OF CHAOS, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Sacrificial Blade | Melee | - | 4 | 3+ | 3+ | 1 | 2 | - |
| Flailing Fists | Melee | - | 6 | 4+ | 3+ | - | 2 | Companion |

**Abilities:**

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of its **Flailing Fists** is 4.

#### Favour of Chaos (Prayer)
- **Timing:** Your Hero Phase
- **Chanting Value:** 4
- **Declare:** Pick a visible friendly Slaves to Darkness unit wholly within 12" of this unit to be the target. Then, make a chanting roll of D6.
- **Effect:** Apply 1 of the following effects to the target for the rest of the turn:
• Undivided: When the target uses the ‘Rally’ command, you can make 3 additional rally rolls of D6.
• Khorne: Add 1 to hit rolls for the target’s attacks.
• Tzeentch: The target has Ward (5+).
• Nurgle: Add 1 to wound rolls for the target’s attacks.
• Slaanesh: Add 1 to the number of dice rolled when making charge rolls for the target, to a maximum of 3. 
If the chanting roll was 8+, you can pick up to 2 eligible units to be the targets instead of 1.
- **Keywords:** Prayer

#### Protection of the Dark Gods (Passive)
- **Effect:** Friendly Warriors of Chaos units have Ward (6+) while they are wholly within 12" of this unit.

---

### Corvus Cabal
**Points:** 100 | **Move:** 6" | **Health:** 1 | **Save:** 6+ | **Control:** 1
**Keywords:** CHAOS, SLAVES TO DARKNESS, INFANTRY, UNDIVIDED, CHAMPION (1/9), Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Raven Darts | Ranged | 10" | 1 | 4+ | 4+ | - | 1 | Shoot in Combat |
| Hooked Weapons | Melee | - | 2 | 4+ | 4+ | - | 1 | Crit (2 Hits) |

**Abilities:**

#### Strike from the Shadows (Activated)
- **Timing:** Your Movement Phase
- **Declare:** Pick this unit if it is lurking in the shadows.
- **Effect:** Set up this unit anywhere on the battlefield more than 9" from all enemy units. Then, add 1 to charge rolls for this unit for the rest of the turn.

#### Shadow-lurkers (Activated)
- **Timing:** Deployment Phase
- **Declare:** Pick this unit if it has not been deployed.
- **Effect:** Set up this unit in reserve **lurking in the shadows**. It has now been deployed.
- **Keywords:** Deploy

---

### Cypher Lords
**Points:** 100 | **Move:** 6" | **Health:** 1 | **Save:** 6+ | **Control:** 1
**Keywords:** CHAOS, SLAVES TO DARKNESS, INFANTRY, UNDIVIDED, Legends, CHAMPION (1/8)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Throwing Stars and Chakrams | Ranged | 10" | 1 | 4+ | 4+ | - | 1 | Shoot in Combat |
| Exotic Blades | Melee | - | 2 | 3+ | 4+ | - | 1 | - |

**Abilities:**

#### Acrobatic Leaps (Passive)
- **Effect:** This unit has Strike-first if it charged in the same turn.

#### Shattered Gloom Globes (Activated)
- **Timing:** Once Per Battle, Any Hero Phase
- **Effect:** This unit cannot be targeted by shooting attacks for the rest of the turn.

---

### Daemon Prince
**Points:** 250 | **Move:** 8" | **Health:** 10 | **Save:** 3+ | **Control:** 5
**Keywords:** HERO, INFANTRY, WARD (6+), CHAOS, SLAVES TO DARKNESS, DAEMON

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Hellforged Weapons | Melee | - | 6 | 3+ | 3+ | 1 | 3 | Crit (Mortal) |

**Abilities:**

#### Immortal Champion (Activated)
- **Timing:** Any Combat Phase
- **Effect:** Roll a dice. On a 4+, this unit has Strike-first for the rest of the turn.

#### Airborne Horror (Passive)
- **Effect:** If this unit has **Wings**, it has a Move characteristic of 10" and has Fly.

#### The Mounted Skulls of the Fallen (Passive)
- **Effect:** If this unit has a **Trophy Rack**, its **Hellforged Weapons** also have Anti-**Hero (+1 Rend)**.

---

### Darkoath Chieftain
**Points:** 80 | **Move:** 5" | **Health:** 5 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, SLAVES TO DARKNESS, INFANTRY, CHAOS, DARKOATH

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Cursed Broadsword | Melee | - | 5 | 4+ | 3+ | 1 | 2 | Crit (Mortal) |

**Abilities:**

#### Tribal Warleader (Activated)
- **Timing:** Reaction: You declared a Fight ability for this unit
- **Effect:** Pick a friendly non-Hero Darkoath Infantry unit that has not used a Fight ability this turn and is within this unit’s combat range to be the target. The target can be picked to use a Fight ability immediately after the Fight ability used by this unit has been resolved. If it is picked to do so, add 1 to the Attacks characteristic of the target’s melee weapons for the rest of the turn.

#### Oath of Murder (Activated)
- **Timing:** Once Per Battle, End of Any Turn
- **Effect:** If this unit destroyed an enemy Hero or Monster this turn, this unit has Strike-first for the rest of the battle.

---

### Darkoath Chieftain on Warsteed
**Points:** 100 | **Move:** 12" | **Health:** 7 | **Save:** 4+ | **Control:** 2
**Keywords:** HERO, SLAVES TO DARKNESS, CHAOS, DARKOATH, CAVALRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Darkoath Cursed Weapon | Melee | - | 5 | 4+ | 3+ | 1 | 2 | Crit (Mortal) |
| Warsteed’s Hooves | Melee | - | 2 | 5+ | 3+ | - | 1 | Companion |

**Abilities:**

#### Cavalry Warleader (Passive)
- **Effect:** You can re-roll charge rolls for friendly Darkoath Cavalry units while they are wholly within 12" of this unit.

#### Oath of Murder (Activated)
- **Timing:** Once Per Battle, End of Any Turn
- **Effect:** If this unit destroyed an enemy Hero or Monster this turn, this unit has Strike-first for the rest of the battle.

---

### Darkoath Fellriders
**Points:** 140 | **Move:** 12" | **Health:** 3 | **Save:** 5+ | **Control:** 1
**Keywords:** CHAOS, SLAVES TO DARKNESS, STANDARD BEARER (1/5), MUSICIAN (1/5), CAVALRY, CHAMPION, DARKOATH

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Warsteed’s Hooves | Melee | - | 2 | 5+ | 3+ | - | 1 | Companion |
| Fellrider Blades | Melee | - | 2 | 4+ | 3+ | - | 1 | Charge (+1 Damage) |
| Marauder Javelin | Ranged | 10" | 1 | 4+ | 3+ | - | D3 | - |
| Marauder Javelin | Melee | - | 2 | 4+ | 4+ | 1 | 1 | - |

**Abilities:**

#### Swift Attackers (Passive)
- **Effect:** Subtract 1 from hit rolls for shooting attacks that target this unit.

#### Oath of the Raider (Activated)
- **Timing:** Once Per Battle, Any Charge Phase
- **Effect:** If this unit charged this phase and is only in combat with 1 enemy unit, and that enemy unit is only in combat with this unit, add 1 to the Rend characteristic of this unit’s weapons for the rest of the battle.

---

### Darkoath Marauders
**Points:** 80 | **Move:** 5" | **Health:** 1 | **Save:** 5+ | **Control:** 1
**Keywords:** CHAOS, SLAVES TO DARKNESS, INFANTRY, STANDARD BEARER (1/10), MUSICIAN (1/10), DARKOATH, CHAMPION

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Raider Weapons | Melee | - | 2 | 4+ | 3+ | - | 1 | - |

**Abilities:**

#### Oath of the Marauder (Activated)
- **Timing:** Once Per Battle, Any Charge Phase
- **Effect:** If this unit charged this phase and the unmodified charge roll was 8+, add 1 to the Rend characteristic of this unit’s weapons for the rest of the battle.

#### Glorious Death (Passive)
- **Effect:** While this unit is wholly within 12" of any friendly Darkoath Heroes, each time a model in this unit is slain by a combat attack and that model was in combat with the attacking unit, roll a dice. On a 5+, inflict 1 mortal damage on the attacking unit after the Fight ability has been resolved.

---

### Darkoath Savagers
**Points:** 90 | **Move:** 5" | **Health:** 1 | **Save:** 5+ | **Control:** 1
**Keywords:** CHAOS, SLAVES TO DARKNESS, INFANTRY, DARKOATH, CHAMPION (1/10)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Darkoath Weapons | Melee | - | 2 | 4+ | 3+ | - | 1 | Crit (Mortal) |

**Abilities:**

#### Oath of Conquest (Activated)
- **Timing:** Once Per Battle, Any Hero Phase

---

### Darkoath Warqueen
**Points:** 80 | **Move:** 5" | **Health:** 5 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, SLAVES TO DARKNESS, INFANTRY, CHAOS, DARKOATH, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Rune-etched Axe | Melee | - | 5 | 4+ | 3+ | 1 | 2 | Crit (Mortal) |

**Abilities:**

#### Ruin-blessed Conqueror (Activated)
- **Timing:** Once Per Turn (Army), Any Hero Phase
- **Declare:** Pick an objective that this unit is contesting.
- **Effect:** Roll a dice. On a 3+, for the rest of the turn, other friendly Darkoath units have Ward (5+) while they are contesting that objective.

#### Oath of Supremacy (Activated)
- **Timing:** Once Per Battle, Any Charge Phase
- **Effect:** If this unit is wholly within enemy territory and charged this phase, for the rest of the battle, add 1 to hit rolls for combat attacks made by friendly Darkoath units while they are wholly within 12" of this unit.

---

### Darkoath Wilderfiend
**Points:** 130 | **Move:** 8" | **Health:** 9 | **Save:** 5+ | **Control:** 1
**Keywords:** SLAVES TO DARKNESS, CHAOS, DARKOATH, BEAST, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Razor-sharp Claws | Melee | - | 6 | 4+ | 3+ | 1 | 2 | Companion |
| Flesh-tearing Maw | Melee | - | 3 | 3+ | 3+ | 2 | D3 | Companion |

**Abilities:**

#### Feed on Flesh (Passive)
- **Effect:** Each time a model (friendly or enemy) is slain within 12" of this unit, give this unit a **sacrifice point**, to a maximum of 6. If there are more than 1 friendly **Darkoath Wilderfiend** units within 12" of the slain model, only 1 of them can be given a **sacrifice point** for that slain model.

#### Primal Sorcery (Activated)
- **Timing:** Your Hero Phase
- **Effect:** Pick 1 of the following effects:
***Mind Shroud:*** Pick an enemy unit within 12" of this unit to be the target and roll a number of dice equal to the number of **sacrifice points** this unit has. If any of the rolls are a 5+, the target cannot use commands for the rest of the turn.
***Warping Balefire:*** Pick an enemy unit within 12" of this unit to be the target and roll a number of dice equal to the number of **sacrifice points** this unit has. For each 3+, inflict 1 mortal damage on the target.
***Dark Might: Heal (X)*** this unit, where X is the number of **sacrifice points** this unit has.
After applying the effect you picked, reset this unit’s **sacrifice points** to 0.

---

### Eternus, Blade of the First Prince
**Points:** 180 | **Move:** 9" | **Health:** 9 | **Save:** 3+ | **Control:** 2
**Keywords:** UNIQUE, HERO, SLAVES TO DARKNESS, CAVALRY, CHAOS

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Death Glaive and Skull Flail | Melee | - | 6 | 3+ | 3+ | 1 | 2 | Charge (+1 Damage) |
| Drakocephalus’s Fangs | Melee | - | 3 | 4+ | 3+ | - | 1 | Companion |

**Abilities:**

#### Network of Spies (Activated)
- **Timing:** Reaction: Opponent declared a command for a unit within 12" of this unit
- **Effect:** Roll a dice. Add 1 to the roll if the enemy unit is in combat with a friendly **Chaos Legionnaires** or **Chaos Furies** unit. On a 5+, that command has no effect. The command still counts as having been used and the command points spent to use the command are still lost.

#### Veins of Black Lightning (Activated)
- **Timing:** Any Movement Phase
- **Declare:** This unit can use this ability if it has been destroyed.
- **Effect:** Roll 2D6. Add 1 to the roll if a friendly **Be’lakor** is on the battlefield. On an 8+, set up a replacement unit on the battlefield more than 9" from all enemy units.
- **Keywords:** Core

---

### Exalted Hero of Chaos
**Points:** 90 | **Move:** 5" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, SLAVES TO DARKNESS, INFANTRY, WARRIORS OF CHAOS, CHAOS

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Blades of the Dark Champion | Melee | - | 5 | 3+ | 3+ | 1 | 2 | Anti-Hero (+1 Rend), Anti-Monster (+1 Rend) |

**Abilities:**

#### Glory-seeker (Passive)
- **Effect:** Add 1 to the Damage characteristic of this unit’s melee weapons for attacks that target Heroes or Monsters.

---

### Fomoroid Crusher
**Points:** 120 | **Move:** 6" | **Health:** 8 | **Save:** 5+ | **Control:** 2
**Keywords:** SLAVES TO DARKNESS, CHAOS, MONSTER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Hurled Terrain | Ranged | 12" | D3 | 4+ | 2+ | 1 | 2 | - |
| Crushing Fists and Swinging Masonry | Melee | - | 6 | 4+ | 2+ | 1 | D3 | Crit (2 Hits) |

**Abilities:**

#### Cursed Destroyers (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick a terrain feature within 1" of this unit, then pick each other unit (friendly and enemy) within 3" of that terrain feature to be the targets.
- **Effect:** Roll a D3 for each target. On a 2+, inflict an amount of mortal damage on the target equal to the roll.
- **Keywords:** Rampage

---

### Gaunt Summoner
**Points:** 160 | **Move:** 5" | **Health:** 5 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, SLAVES TO DARKNESS, WIZARD (2), DAEMON, INFANTRY, CHAOS, PLEDGED TO TZEENTCH, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Warptongue Blade | Melee | - | 3 | 3+ | 3+ | 1 | 2 | - |
| Changestaff | Ranged | 12" | 3 | 4+ | 3+ | 1 | D3 | Crit (Mortal) |

**Abilities:**

#### Minions of the Silver Tower (Activated)
- **Timing:** Deployment Phase
- **Declare:** Pick a friendly unit that has not been deployed to be the target
- **Effect:** The target is set up in reserve **in a Silver Tower**. It has now been deployed.
- **Keywords:** Deploy

#### Book of Profane Secrets (Activated)
- **Timing:** Your Movement Phase
- **Declare:** Pick a friendly unit **in a Silver Tower** to be the target.
- **Effect:** Set up the target on the battlefield wholly within 12" of this unit and more than 9" from all enemy units.

#### Divert Realmgate (Spell)
- **Timing:** Your Hero Phase
- **Casting Value:** 7
- **Declare:** Make a casting roll of 2D6
- **Effect:** Units set up this turn using this unit’s ‘Book of Profane Secrets’ ability can be set up wholly within 18" of this unit and more than 7" from all enemy units instead of the distances in the ability.
- **Keywords:** Spell

---

### Gaunt Summoner on Disc of Tzeentch
**Points:** 190 | **Move:** 14" | **Health:** 7 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, SLAVES TO DARKNESS, CAVALRY, WIZARD (2), FLY, DAEMON, CHAOS, PLEDGED TO TZEENTCH, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Warptongue Blade | Melee | - | 3 | 3+ | 3+ | 1 | 2 | - |
| Changestaff | Ranged | 12" | 3 | 4+ | 3+ | 1 | D3 | Crit (Mortal) |
| Disc’s Teeth and Horns | Melee | - | 2 | 4+ | 3+ | 1 | D3 | Companion |

**Abilities:**

#### Minions of the Silver Tower (Activated)
- **Timing:** Deployment Phase
- **Declare:** Pick a friendly unit that has not been deployed to be the target
- **Effect:** The target is set up in reserve **in a Silver Tower**. It has now been deployed.
- **Keywords:** Deploy

#### Book of Profane Secrets (Activated)
- **Timing:** Your Movement Phase
- **Declare:** Pick a friendly unit **in a Silver Tower** to be the target.
- **Effect:** Set up the target on the battlefield wholly within 12" of this unit and more than 9" from all enemy units.

#### Arcane Imprisonment (Spell)
- **Timing:** Your Hero Phase
- **Casting Value:** 7
- **Declare:** Pick an enemy Hero in combat with this unit to be the target, then make a casting roll of 2D6.
- **Effect:** If the unmodified casting roll exceeds the target’s Health characteristic, it is automatically destroyed. For the rest of the battle, that unit cannot be picked to be the target of an ability that allows a replacement unit to be set up.
- **Keywords:** Spell

---

### Godsworn Hunt
**Points:** 110 | **Move:** 5" | **Health:** 2 | **Save:** 6+ | **Control:** 1
**Keywords:** INFANTRY, UNIQUE, Legends, CHAMPION, UNDIVIDED, DARKOATH, SLAVES TO DARKNESS

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Godsworn Weapons | Melee | - | 2 | 4+ | 3+ | - | 1 | Crit (Mortal) |
| Godsworn Weapons | Melee | - | 2 | 4+ | 3+ | - | 1 | Crit (Mortal) |
| Godsworn Weapons | Melee | - | 2 | 4+ | 3+ | - | 1 | Crit (Mortal) |
| Godsworn Weapons | Melee | - | 2 | 4+ | 3+ | - | 1 | Crit (Mortal) |
| Godsworn Weapons | Melee | - | 2 | 4+ | 3+ | - | 1 | Crit (Mortal) |
| Hunting Bow | Ranged | 20" | 2 | 4+ | 4+ | - | 1 | Crit (Auto-wound) |
| Godsworn Weapons | Melee | - | 2 | 4+ | 3+ | - | 1 | Crit (Mortal) |

**Abilities:**

#### Oath of Conquest (Activated)
- **Timing:** Once Per Battle, Any Hero Phase

#### Theddra Skull-scryer (Passive)
- **Effect:** While this unit’s **Theddra Skull-scryer** is on the battlefield, it has Wizard (1).

---

### Gorebeast Chariot
**Points:** 100 | **Move:** 9" | **Health:** 8 | **Save:** 4+ | **Control:** 2
**Keywords:** CHAOS, SLAVES TO DARKNESS, WARRIORS OF CHAOS, WAR MACHINE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Gorebeast’s Crushing Fists | Melee | - | 4 | 4+ | 4+ | - | 2 | Companion |
| Lashing Whip | Melee | - | 2 | 4+ | 4+ | - | 1 | - |
| Chaos Greatblade | Melee | - | 3 | 4+ | 3+ | 1 | 2 | - |
| Chaos War-flail | Melee | - | 6 | 3+ | 3+ | - | 1 | - |

**Abilities:**

#### Brutal Momentum (Passive)
- **Effect:** When this unit uses the ‘Power Through’ command, inflict an additional D3 mortal damage on the target and add 3" to the distance this unit can move as part of that ability

---

### Gunnar Brand
**Points:** 180 | **Move:** 5" | **Health:** 6 | **Save:** 5+ | **Control:** 2
**Keywords:** UNIQUE, HERO, SLAVES TO DARKNESS, CHAOS, INFANTRY, DARKOATH, GUNNAR'S OATHSWORN

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Dual Axes | Melee | - | 10 | 4+ | 3+ | 1 | 1 | Crit (Mortal) |

**Abilities:**

#### Carve a Path to Glory (Activated)
- **Timing:** Any Combat Phase
- **Declare:** This unit can only use this ability if it is in combat with an enemy Hero.
- **Effect:** This unit can use 2 Fight abilities this phase. After the first is used, however, this unit has Strike-last for the rest of the turn. When using the second Fight ability, this unit must pick an enemy Hero to be the target of all of its attacks.

#### Oath of Bloodshed (Activated)
- **Timing:** Once Per Battle, End of Any Turn
- **Declare:** This unit can only use this ability if any enemy Heroes were slain by this unit’s combat attacks this turn.
- **Effect:** Friendly Gunnar’s Oathsworn units have Ward (4+) for the rest of the battle.

---

### Horns of Hashut
**Points:** 120 | **Move:** 6" | **Health:** 1 | **Save:** 4+ | **Control:** 1
**Keywords:** CHAOS, SLAVES TO DARKNESS, INFANTRY, UNDIVIDED, Legends, CHAMPION (1/10)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Forge Weapons | Melee | - | 2 | 4+ | 4+ | 1 | 1 | - |
| Flamehurler | Ranged | 8" | D6 | 2+ | 3+ | - | 1 | Anti-Infantry (+1 Rend), Shoot in Combat |

**Abilities:**

#### Stampede of Iron (Activated)
- **Timing:** Any Charge Phase
- **Declare:** If this unit charged this phase, pick an enemy unit within 1" of it to be the target.
- **Effect:** Roll a dice for each model in this unit that is within the target’s combat range. For each 5+, inflict 1 mortal damage on the target. If 3 or more mortal damage is inflicted on the target by this ability, subtract 1 from hit rolls for the target’s combat attacks for the rest of the turn.

---

### Iron Golem
**Points:** 100 | **Move:** 4" | **Health:** 1 | **Save:** 4+ | **Control:** 1
**Keywords:** CHAOS, SLAVES TO DARKNESS, INFANTRY, UNDIVIDED, Legends, CHAMPION (1/8), STANDARD BEARER (1/8)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Legion Weapons | Melee | - | 2 | 4+ | 4+ | 1 | 1 | - |
| Bolas | Ranged | 8" | 1 | 4+ | 4+ | - | 1 | Shoot in Combat |
| Legion Weapons | Melee | - | 2 | 4+ | 4+ | 1 | 1 | - |
| Bolas | Ranged | 8" | 1 | 4+ | 4+ | - | 1 | Shoot in Combat |

**Abilities:**

#### Ogor Breacher (Passive)
- **Effect:** While this unit includes an **Ogor Breacher**, ignore the first damage point that would be allocated to this unit in each phase.

#### Iron Resilience (Passive)
- **Effect:** Add 1 to save rolls for this unit if it has not used a Move ability in the same turn.

---

### Khagra's Ravagers
**Points:** 170 | **Move:** 6" | **Health:** 3 | **Save:** 3+ | **Control:** 1
**Keywords:** INFANTRY, UNIQUE, Legends, CHAMPION, SLAVES TO DARKNESS, WARRIORS OF CHAOS, CHAOS

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Hellforged Weapons | Melee | - | 3 | 3+ | 3+ | 1 | 1 | Crit (Auto-wound) |
| Hellforged Weapons | Melee | - | 3 | 3+ | 3+ | 1 | 1 | Crit (Auto-wound) |
| Hellforged Weapons | Melee | - | 3 | 3+ | 3+ | 1 | 1 | Crit (Auto-wound) |
| Hellforged Weapons | Melee | - | 3 | 3+ | 3+ | 1 | 1 | Crit (Auto-wound) |

**Abilities:**

#### Ravagers of the Mortal Realms (Passive)

#### Zarshia Bittersoul (Passive)
- **Effect:** While this unit’s **Zarshia Bittersoul** is on the battlefield, it has Wizard (1).

---

### Legion of the First Prince Beasts of Nurgle
**Points:** 120 | **Move:** 7" | **Health:** 8 | **Save:** 5+ | **Control:** 1
**Keywords:** WARD (5+), CHAOS, DAEMON, BEAST, SLAVES TO DARKNESS, LEGION OF THE FIRST PRINCE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Filthy Claws and Slobbering Maw | Melee | - | 5 | 4+ | 3+ | 1 | D3 | Companion |

**Abilities:**

#### Attention Seekers (Activated)
- **Timing:** Any Charge Phase
- **Declare:** If this unit is not in combat, pick the closest enemy unit to it to be the target. If 2 or more enemy units are tied to be the closest, you can pick which is the target. Then, make a charge roll of 2D6.
- **Effect:** This unit can move a distance up to the value of the charge roll. During that move, this unit can move into combat and can pass through models in the target unit, but it must end that move within 1" of the target. Then, inflict D3 mortal damage on the target. If it does so, this unit has charged.
- **Keywords:** Core, Move, Charge

---

### Legion of the First Prince Bloodcrushers
**Points:** 150 | **Move:** 8" | **Health:** 5 | **Save:** 4+ | **Control:** 2
**Keywords:** WARD (6+), CHAOS, DAEMON, CHAMPION, MUSICIAN (1/3), STANDARD BEARER (1/3), CAVALRY, SLAVES TO DARKNESS, LEGION OF THE FIRST PRINCE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Bloodcrusher Hellblade | Melee | - | 2 | 3+ | 3+ | 1 | 1 | Anti-Infantry (+1 Rend), Crit (Mortal) |
| Juggernaut’s Brazen Hooves | Melee | - | 2 | 4+ | 3+ | 1 | D3 | Companion |

**Abilities:**

#### Slaughterous Charge (Activated)
- **Timing:** Any Charge Phase
- **Declare:** If this unit charged this turn, pick an enemy unit within 1" of it to be the target.
- **Effect:** Make a crushing charge roll of D3. On a 2+, inflict an amount of mortal damage on the target equal to the roll.

---

### Legion of the First Prince Bloodletters
**Points:** 170 | **Move:** 5" | **Health:** 2 | **Save:** 5+ | **Control:** 1
**Keywords:** WARD (6+), CHAOS, DAEMON, INFANTRY, CHAMPION, STANDARD BEARER (1/10), MUSICIAN (1/10), SLAVES TO DARKNESS, LEGION OF THE FIRST PRINCE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Hellblade | Melee | - | 2 | 3+ | 3+ | 1 | 1 | Crit (Mortal) |

**Abilities:**

#### The Thinning Veil (Activated)
- **Timing:** End of Any Turn
- **Effect:** If this unit is in combat, you can return D3 slain models to this unit.

---

### Legion of the First Prince Fiends
**Points:** 140 | **Move:** 10" | **Health:** 4 | **Save:** 5+ | **Control:** 1
**Keywords:** DAEMON, WARD (6+), BEAST, CHAOS, CHAMPION, SLAVES TO DARKNESS, LEGION OF THE FIRST PRINCE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Deadly Pincers and Barbed Stinger | Melee | - | 4 | 4+ | 3+ | 1 | D3 | Anti-Monster (+1 Rend), Companion |

**Abilities:**

#### Soporific Musk (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a dice. On a 4+, the target has Strike-last for the rest of the turn.

#### Disruptive Song (Passive)
- **Effect:** Subtract 1 from casting rolls and unbinding rolls for enemy Wizards while they are within 12" of this unit.

---

### Legion of the First Prince Flamers of Tzeentch
**Points:** 120 | **Move:** 9" | **Health:** 2 | **Save:** 5+ | **Control:** 1
**Keywords:** INFANTRY, CHAOS, DAEMON, WARD (6+), FLY, CHAMPION, SLAVES TO DARKNESS, LEGION OF THE FIRST PRINCE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Flaming Maws | Melee | - | 3 | 3+ | 4+ | - | 1 | - |
| Warping Flames | Ranged | 12" | 3 | 3+ | 4+ | - | D3 | Anti-Infantry (+1 Rend), Wyrdflame |

**Abilities:**

#### Wyrdflame of Tzeentch (Activated)
- **Timing:** Once Per Turn (Army), Your Shooting Phase
- **Declare:** Pick an enemy unit to be the target.
- **Effect:** For the rest of the turn, add 1 to wound rolls for this unit’s shooting attacks that target that unit. In addition, if every shooting attack made by this unit this phase targets that enemy unit, inflict D3 mortal damage on the target after the Shoot ability has been resolved.

---

### Legion of the First Prince Hellflayer
**Points:** 130 | **Move:** 10" | **Health:** 7 | **Save:** 4+ | **Control:** 2
**Keywords:** CHAOS, DAEMON, WARD (6+), WAR MACHINE, LEGION OF THE FIRST PRINCE, SLAVES TO DARKNESS

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Axle Blades | Melee | - | 4 | 4+ | 2+ | 2 | D3 | Anti-Infantry (+1 Rend), Companion |
| Steeds’ Poisoned Tongues | Melee | - | 4 | 3+ | 4+ | - | 1 | Companion |
| Flensing Whips and Piercing Claws | Melee | - | 4 | 3+ | 4+ | 1 | 1 | - |

**Abilities:**

#### Soulscent (Activated)
- **Timing:** Any Movement Phase
- **Declare:** Pick up to 3 enemy units that this unit passed across this phase to be the targets.
- **Effect:** Roll a D3 for each target. On a 2+, inflict an amount of mortal damage on the target equal to the roll. If any models are slain by this ability, this unit has Ward (5+) for the rest of the turn.

#### Threshing Doom (Passive)
- **Effect:** When this unit moves, it can pass through models in enemy Infantry units and can pass through the combat ranges of enemy Infantry units, but it cannot end a move in combat unless specified in the ability used.

---

### Legion of the First Prince Plaguebearers
**Points:** 140 | **Move:** 4" | **Health:** 2 | **Save:** 6+ | **Control:** 1
**Keywords:** WARD (5+), CHAOS, DAEMON, CHAMPION, INFANTRY, STANDARD BEARER (1/10), MUSICIAN (1/10), SLAVES TO DARKNESS, LEGION OF THE FIRST PRINCE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Plaguesword | Melee | - | 1 | 4+ | 3+ | - | 1 | Crit (Mortal) |

**Abilities:**

#### Cloud of Flies (Passive)
- **Effect:** Subtract 1 from hit rolls for shooting attacks that target this unit.

---

### Legion of the First Prince Screamers of Tzeentch
**Points:** 80 | **Move:** 14" | **Health:** 3 | **Save:** 5+ | **Control:** 1
**Keywords:** CHAOS, DAEMON, WARD (6+), BEAST, FLY, SLAVES TO DARKNESS, LEGION OF THE FIRST PRINCE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Lamprey Bite | Melee | - | 3 | 4+ | 3+ | 1 | 1 | Anti-Monster (+1 Rend), Companion |

**Abilities:**

#### Slashing Fins (Activated)
- **Timing:** Your Movement Phase
- **Declare:** Pick an enemy unit that this unit passed across this phase to be the target.
- **Effect:** Roll a dice for each model in this unit that passed across the target. For each 4+, inflict 1 mortal damage on the target.

---

### Mindstealer Sphiranx
**Points:** 140 | **Move:** 10" | **Health:** 10 | **Save:** 5+ | **Control:** 5
**Keywords:** SLAVES TO DARKNESS, CHAOS, MONSTER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Shredding Claws | Melee | - | 6 | 4+ | 4+ | 1 | 2 | - |

**Abilities:**

#### Dominate Mind (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy unit within 6" of this unit to be the target.
- **Effect:** Roll a dice. If the roll exceeds the target’s Control characteristic, the target has Strike-last for the rest of the turn.
- **Keywords:** Rampage

---

### Mutalith Vortex Beast
**Points:** 160 | **Move:** 10" | **Health:** 14 | **Save:** 4+ | **Control:** 5
**Keywords:** SLAVES TO DARKNESS, CHAOS, MONSTER, DAEMON, PLEDGED TO TZEENTCH

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Crushing Claws | Melee | - | 4 | 4+ | 2+ | 1 | D3 | Companion |
| Betentacled Maw | Melee | - | 12 | 4+ | 4+ | - | 1 | Crit (Auto-wound), Companion |

**Abilities:**

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of its **Betentacled Maw** is 8.

#### Aura of Mutation (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick a visible enemy unit within 12" of this unit to be the target.
- **Effect:** Roll a dice and apply the corresponding effect below. If this unit is wholly within 12" of a friendly Pledged to Tzeentch Wizard, you can roll 2 dice instead, pick either result, then apply the corresponding effect below.
**1 *Untouched:*** No effect.
**2 *Hideous Disfigurement:*** Subtract 3 from the target’s control score until the start of your next turn.
**3 *Troggbrains:*** The target cannot use commands for the rest of the turn.
**4 *Gift of Mutations:*** Subtract 1" from the target’s Move characteristic for the rest of the battle.
**5 *Horrific Absorption:*** Inflict D6 mortal damage on the target. Then, **Heal (D6)** this unit.
**6 *Transmogrification:*** Pick any other effect.
- **Keywords:** Rampage

#### Mutant Regeneration (Activated)
- **Timing:** Any Hero Phase
- **Effect:** **Heal (D3)** this unit.

---

### Ogroid Myrmidon
**Points:** 120 | **Move:** 6" | **Health:** 8 | **Save:** 4+ | **Control:** 2
**Keywords:** HERO, SLAVES TO DARKNESS, INFANTRY, CHAOS

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Gladiator Spear | Melee | - | 6 | 3+ | 2+ | 1 | 3 | Anti-Monster (+1 Rend) |
| Gladiator Spear | Ranged | 10" | 1 | 3+ | 3+ | 1 | D3 | - |

**Abilities:**

#### Myrmidon Rage (Passive)
- **Effect:** While this unit is damaged, add 2 to the Attacks characteristic of its melee weapons.

#### Pit Master (Activated)
- **Timing:** Reaction: You declared a Fight ability for this unit
- **Effect:** Pick a friendly **Ogroid Theridons** unit that has not used a Fight ability this turn and is within this unit’s combat range to be the target. The target can be picked to use a Fight ability immediately after the Fight ability used by this unit has been resolved. If it is picked to do so, add 1 to hit rolls for the target’s attacks for the rest of the turn.

---

### Ogroid Theridons
**Points:** 160 | **Move:** 6" | **Health:** 5 | **Save:** 4+ | **Control:** 2
**Keywords:** CHAOS, SLAVES TO DARKNESS, INFANTRY, MUSICIAN (1/3), STANDARD BEARER (1/3), CHAMPION

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Goroan Weapons | Melee | - | 4 | 4+ | 2+ | 1 | 2 | Anti-Infantry (+1 Rend) |

**Abilities:**

#### Unleashed Savagery (Activated)
- **Timing:** Once Per Battle, Any Combat Phase
- **Effect:** Add 1 to the Damage characteristic of this unit’s melee weapons for the rest of the turn.

---

### Raptoryx
**Points:** 100 | **Move:** 10" | **Health:** 2 | **Save:** 6+ | **Control:** 1
**Keywords:** CHAOS, SLAVES TO DARKNESS, BEAST

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Razor-sharp Beak and Talons | Melee | - | 3 | 4+ | 4+ | - | 1 | Companion |

**Abilities:**

#### Crazed Flock (Passive)
- **Effect:** If this unit charged this turn, for the rest of the turn, add 1 to the Attacks characteristic of its melee weapons. This ability also affects Companion weapons.

---

### Scions of the Flame
**Points:** 120 | **Move:** 5" | **Health:** 1 | **Save:** 5+ | **Control:** 1
**Keywords:** CHAOS, SLAVES TO DARKNESS, INFANTRY, UNDIVIDED, Legends, CHAMPION (1/8)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Scion Weapons | Melee | - | 2 | 4+ | 4+ | 1 | 1 | Anti-Infantry (+1 Rend) |
| Flameburst Pots and Infernal Fires | Ranged | 10" | 1 | 4+ | 4+ | - | 1 | Anti-Infantry (+1 Rend) |

**Abilities:**

#### All Shall Burn (Passive)
- **Effect:** Add 1 to the Damage characteristic of this unit’s **Flameburst Pots and Infernal Fires** for attacks that target War Machines or Infantry units with 10 or more models.

---

### Slaughterbrute
**Points:** 200 | **Move:** 10" | **Health:** 14 | **Save:** 4+ | **Control:** 5
**Keywords:** SLAVES TO DARKNESS, CHAOS, MONSTER, DAEMON, PLEDGED TO KHORNE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Mighty Jaws | Melee | - | 2 | 4+ | 2+ | 1 | 3 | Companion |
| Razor-tipped Claws | Melee | - | 8 | 4+ | 2+ | 1 | 2 | Anti-Infantry (+1 Rend), Companion |

**Abilities:**

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of its **Razor-tipped Claws** is 5.

#### Rampaging Charge (Activated)
- **Timing:** Once Per Turn (Army), Any Charge Phase
- **Declare:** If this unit charged this phase, pick an enemy unit within 1" of it to be the target.
- **Effect:** Roll a D3. On a 2+, inflict an amount of mortal damage on the target equal to the roll. If the target unit is Infantry, double the amount of mortal damage inflicted.
- **Keywords:** Rampage

#### Sigils of Domination (Activated)
- **Timing:** Deployment Phase
- **Declare:** Pick a friendly Slaves to Darkness Hero within this unit’s combat range to be this unit’s master.
- **Effect:** For the rest of the battle, while this unit is within its master’s combat range:
• Add 1 to the number of dice rolled when making charge rolls for this unit, to a maximum of 3.
• This unit can ignore the effects of the **Companion** weapon ability on its weapons.

---

### Soul Grinder
**Points:** 330 | **Move:** 10" | **Health:** 16 | **Save:** 4+ | **Control:** 5
**Keywords:** SLAVES TO DARKNESS, CHAOS, DAEMON, MONSTER, WARD (6+), Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Harvester Cannon | Ranged | 12" | 6 | 4+ | 3+ | 1 | 1 | Companion |
| Piston-driven Legs and Daemonic Blade or Claw | Melee | - | 8 | 4+ | 2+ | 1 | D3 | Companion |
| Hellforged Claw | Melee | - | 1 | 4+ | 2+ | 2 | D6 | Crit (Mortal), Companion |
| Phlegm Bombardment | Ranged | 12" | 1 | 4+ | 3+ | 1 | 3 | Companion |

**Abilities:**

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of its **Piston-driven Legs and Daemonic Blade or Claw** is 6.

#### Piston-driven Carnage (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy Infantry unit in combat with this unit to be the target.
- **Effect:** Roll a D3. On a 2+, inflict an amount of mortal damage on the target equal to the roll. Then, this unit can move 6". During that move, it can pass through models in the target unit but must end that move in combat.
- **Keywords:** Rampage

#### Implacable Advance (Passive)
- **Effect:** This unit can use Charge abilities even if it used a Run ability in the same turn.

---

### Spire Tyrants
**Points:** 110 | **Move:** 5" | **Health:** 1 | **Save:** 5+ | **Control:** 1
**Keywords:** CHAOS, SLAVES TO DARKNESS, INFANTRY, UNDIVIDED, Legends, CHAMPION (1/9)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Gladiator Weapons | Melee | - | 2 | 4+ | 4+ | 1 | 1 | Anti-Hero (+1 Rend) |

**Abilities:**

#### Fight for Glory (Passive)
- **Effect:** The first time each turn that this unit is picked to use a Fight ability in the combat phase, if this unit is wholly within 12" of any friendly Slaves to Darkness Heroes, add 1 to the Attacks characteristic of this unit’s melee weapons for the rest of the turn.

---

### Splintered Fang
**Points:** 110 | **Move:** 6" | **Health:** 1 | **Save:** 5+ | **Control:** 1
**Keywords:** CHAOS, SLAVES TO DARKNESS, INFANTRY, UNDIVIDED, Legends, CHAMPION (1/9)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Poisoned Weapons | Melee | - | 2 | 4+ | 4+ | 1 | 1 | Crit (Mortal) |

**Abilities:**

#### Slithering Serpents (Activated)
- **Timing:** End of Any Turn
- **Declare:** If this unit’s **Serpents** are on the battlefield, pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a D3. On a 1, remove this unit’s **Serpents** from the battlefield. On a 2+, inflict an amount of mortal damage on the target equal to the roll.

#### Paralysing Strikes (Passive)
- **Effect:** Each time this unit uses a Fight ability, if its attacks score 2 or more critical hits against the same enemy unit, that unit has Strike-last for the rest of the turn.

---

### Tarantulos Brood
**Points:** 150 | **Move:** 6" | **Health:** 1 | **Save:** 6+ | **Control:** 1
**Keywords:** CHAOS, SLAVES TO DARKNESS, INFANTRY, UNDIVIDED, Legends, CHAMPION (1/13)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Brood Weapons | Melee | - | 2 | 4+ | 4+ | - | 1 | - |
| Envenomed Projectiles | Ranged | 10" | 1 | 4+ | 4+ | 1 | 1 | Crit (Auto-wound) |
| Venomous Bites | Melee | - | 4 | 5+ | 5+ | - | 1 | Crit (Mortal) |

**Abilities:**

#### Creeping Summons (Activated)
- **Timing:** End of Any Turn
- **Effect:** Roll a dice. On a 2+, you can return D3 slain **Spider Swarms** to this unit.

---

### The Gnarlspirit Pack
**Points:** 110 | **Move:** 5" | **Health:** 2 | **Save:** 6+ | **Control:** 1
**Keywords:** INFANTRY, UNIQUE, Legends, CHAMPION, UNDIVIDED, DARKOATH, SLAVES TO DARKNESS, CHAOS

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Gnarlspirit Weapons | Melee | - | 3 | 4+ | 3+ | 1 | 1 | - |
| Gnarlspirit Weapons | Melee | - | 3 | 4+ | 3+ | 1 | 1 | - |
| Gnarlspirit Weapons | Melee | - | 3 | 4+ | 3+ | 1 | 1 | - |
| Gnarlspirit Weapons | Melee | - | 3 | 4+ | 3+ | 1 | 1 | - |

**Abilities:**

#### Oath of Dark Sacrifice (Activated)
- **Timing:** Once Per Battle, Any Hero Phase
- **Effect:** If an unmodified casting roll for this unit this phase was 8+, this unit has Ward (5+) for the rest of the battle.

#### Sarrakkar Blackwing (Passive)
- **Effect:** While this unit’s **Sarrakkar Blackwing** is on the battlefield, it has Wizard (1).

---

### The Unmade
**Points:** 110 | **Move:** 6" | **Health:** 1 | **Save:** 6+ | **Control:** 1
**Keywords:** CHAOS, SLAVES TO DARKNESS, INFANTRY, UNDIVIDED, Legends, CHAMPION (1/9)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Maiming Weapons | Melee | - | 2 | 4+ | 4+ | - | 1 | Anti-Infantry (+1 Rend) |
| Nightmare Sickles | Melee | - | 6 | 4+ | 3+ | 1 | 1 | Anti-Infantry (+1 Rend) |

**Abilities:**

#### Frozen in Fear (Passive)
- **Effect:** Enemy units cannot use the ‘Redeploy’ or ‘Rally’ commands while they are within 12" of this unit.

---

### Untamed Beasts
**Points:** 110 | **Move:** 6" | **Health:** 1 | **Save:** 6+ | **Control:** 1
**Keywords:** CHAOS, SLAVES TO DARKNESS, INFANTRY, UNDIVIDED, Legends, CHAMPION (1/9)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Ghurish Hunting Weapons | Melee | - | 2 | 4+ | 4+ | - | 1 | Crit (2 Hits) |
| Ghurish Hunting Weapons | Melee | - | 2 | 4+ | 4+ | - | 1 | Crit (2 Hits) |

**Abilities:**

#### Unleash the Beast (Activated)
- **Timing:** Deployment Phase
- **Effect:** This unit can use the ‘Normal Move’ ability as if it were your movement phase.

#### Jagged Harpoon (Activated)
- **Timing:** End of Any Turn
- **Declare:** If this unit includes a **First Fang**, pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a D3. If the target is a Beast or Monster, roll a dice instead. On a 2+, inflict an amount of mortal damage on the target equal to the roll.

---

### Varanguard
**Points:** 310 | **Move:** 10" | **Health:** 5 | **Save:** 3+ | **Control:** 2
**Keywords:** CAVALRY, CHAOS, SLAVES TO DARKNESS, WARRIORS OF CHAOS

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Daemonforged Blades | Melee | - | 3 | 3+ | 3+ | 2 | 2 | Charge (+1 Damage), Crit (Mortal) |
| Steed’s Tearing Fangs | Melee | - | 3 | 4+ | 3+ | 1 | 1 | Companion |

**Abilities:**

#### Relentless Killers (Activated)
- **Timing:** Once Per Battle, Any Combat Phase
- **Effect:** This unit can use 2 Fight abilities this phase. After the first is used, however, this unit has Strike-last for the rest of the turn.

---

## Points Table

| Unit | Points |
| --- | ---: |
| Chaos Spawn | 60 |
| Chaos Legionnaires | 80 |
| Darkoath Chieftain | 80 |
| Darkoath Marauders | 80 |
| Darkoath Warqueen | 80 |
| Legion of the First Prince Screamers of Tzeentch | 80 |
| Chaos Chariot | 90 |
| Darkoath Savagers | 90 |
| Exalted Hero of Chaos | 90 |
| Chaos Lord | 100 |
| Corvus Cabal | 100 |
| Cypher Lords | 100 |
| Darkoath Chieftain on Warsteed | 100 |
| Gorebeast Chariot | 100 |
| Iron Golem | 100 |
| Raptoryx | 100 |
| Godsworn Hunt | 110 |
| Spire Tyrants | 110 |
| Splintered Fang | 110 |
| The Gnarlspirit Pack | 110 |
| The Unmade | 110 |
| Untamed Beasts | 110 |
| Centaurion Marshal | 120 |
| Chaos Furies | 120 |
| Chaos Sorcerer Lord | 120 |
| Fomoroid Crusher | 120 |
| Horns of Hashut | 120 |
| Legion of the First Prince Beasts of Nurgle | 120 |
| Legion of the First Prince Flamers of Tzeentch | 120 |
| Ogroid Myrmidon | 120 |
| Scions of the Flame | 120 |
| Darkoath Wilderfiend | 130 |
| Legion of the First Prince Hellflayer | 130 |
| Chaos Lord on Daemonic Mount | 140 |
| Darkoath Fellriders | 140 |
| Legion of the First Prince Fiends | 140 |
| Legion of the First Prince Plaguebearers | 140 |
| Mindstealer Sphiranx | 140 |
| Athol Khul | 150 |
| Legion of the First Prince Bloodcrushers | 150 |
| Tarantulos Brood | 150 |
| Gaunt Summoner | 160 |
| Mutalith Vortex Beast | 160 |
| Ogroid Theridons | 160 |
| Khagra's Ravagers | 170 |
| Legion of the First Prince Bloodletters | 170 |
| Chaos Warriors | 180 |
| Eternus, Blade of the First Prince | 180 |
| Gunnar Brand | 180 |
| Chaos Lord on Karkadrak | 190 |
| Gaunt Summoner on Disc of Tzeentch | 190 |
| Slaughterbrute | 200 |
| Abraxia, Spear of the Everchosen | 250 |
| Chaos Knights | 250 |
| Chaos Warshrine | 250 |
| Daemon Prince | 250 |
| Chaos Lord on Manticore | 260 |
| Chaos Chosen | 270 |
| Chaos Sorcerer Lord on Manticore | 280 |
| Abraxia, Spear of the Everchosen (Scourge of Ghyran) | 300 |
| Varanguard | 310 |
| Soul Grinder | 330 |
| Be'lakor, the Dark Master | 450 |
| Archaon, the Everchosen | 810 |
