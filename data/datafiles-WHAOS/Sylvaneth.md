# Sylvaneth

## Army Rules

### Anvil of Apotheosis

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of its **Razor-like Mandibles** is 3.

#### Verdant Healer (Activated)
- **Timing:** Your Hero Phase
- **Declare:** Pick a visible friendly Sylvaneth unit wholly within 12" of this unit to be the target.
- **Effect:** Pick 1 of the following effects:
• **Heal (D3)** the target.
• The target has Ward (6+) until the start of your next turn.

#### Noble Guardian (Passive)
- **Effect:** Add 2 to the Attacks characteristic of this unit's melee weapons while it is wholly within the **creeping overgrowth**.

#### Gnarled Spirit (Passive)
- **Effect:** Subtract 3 from the control scores of enemy units while they are in combat with this unit.

#### Rootwalker (Activated)
- **Timing:** Enemy Movement Phase
- **Effect:** If this unit is more than 9" from all enemy units, it can immediately use the 'Normal Move' ability as if it were your movement phase.

#### Tormented Memories (Activated)
- **Timing:** End of Any Turn
- **Declare:** You must use this ability if this unit is contesting an objective you do not control. Pick each other friendly unit contesting the same objective as this unit to be the targets.
- **Effect:** Roll a D3 for each target. On a 2+, inflict an amount of mortal damage on the target equal to the roll.

#### Old Barkflesh (Passive)
- **Effect:** Each time this unit uses a Move ability, roll a dice. On a 4+, inflict 1 mortal damage on it after that ability has been resolved.

#### Slow to Awaken (Passive)
- **Effect:** Halve this unit's Move characteristic until the start of the second battle round.

#### Cautious Spirit (Passive)
- **Effect:** While this unit is wholly outside the **creeping overgrowth**, it can only use Move and Attack abilities.

#### Slashing Wings (Activated)
- **Timing:** Your Movement Phase
- **Effect:** This unit can move a distance up to your Move characteristic. It can pass through models during that move. Then, you can pick an enemy unit that this unit passed across during that move to be the target. Roll a dice for each model in the target unit. For each 5+, inflict 1 mortal damage on the target.
- **Keywords:** Move

#### Strike and Fade (Activated)
- **Timing:** Once Per Turn (Army). End of Any Turn
- **Effect:** If this unit used a Fight ability this turn and is wholly within the **creeping overgrowth**, remove it from the battlefield and set it up again on the battlefield more than 9" from all enemy units and wholly within the **creeping overgrowth**.

#### Charge of the Spites (Activated)
- **Timing:** Once Per Turn (Army), Any Charge Phase
- **Declare:** If this unit charged this phase, pick up to 3 visible enemy units within 6" of this unit to be the targets.
- **Effect:** Roll a D3 for each target. On a 2+, inflict an amount of mortal damage on the target equal to the roll.
- **Keywords:** Rampage

#### Seismic Stomps (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** If this unit charged this turn, pick each enemy unit in combat with this unit to be the targets.
- **Effect:** Roll a dice for each target. On a 3+, subtract 1 from hit rolls for the target's combat attacks for the rest of the turn.
- **Keywords:** Rampage

#### Snatching Vines (Activated)
- **Timing:** Once Per Turn (Army), Your Shooting Phase
- **Declare:** Pick an enemy Infantry Hero within 9" of this unit to be the target.
- **Effect:** Roll a dice. On a 3+, your opponent must remove the target from the battlefield and set it up again within 1" of this unit.

#### Shambling Overgrowth (Passive)
- **Effect:** You can measure the **creeping overgrowth** unit from this unit.

### Paths

#### Natural Regeneration (Passive)
- **Effect:** When using the 'Endless Growth' ability, do not roll a D3 for this unit. Instead, **Heal (3)** this unit.

#### Song of Spite (Passive)
- **Effect:** Enemy units cannot use commands while they are in combat with this unit if this unit charged in the same turn.

#### Territorial (Passive)
- **Effect:** Add 1 to hit rolls for this unit's combat attacks while it is wholly within the **creeping overgrowth**.

#### Jade Exemplar (Passive)
- **Effect:** Add 1 to casting rolls and banishment rolls for this unit while it is wholly within the **creeping overgrowth**.

#### Backscarred (Activated)
- **Timing:** Once Per Turn, Reaction: This unit was picked to be the target of a non-Core ability
- **Effect:** Roll a dice. On a 4+, that ability has no effect on this unit.

#### Bountiful Radiance (Passive)
- **Effect:** You can measure the **creeping overgrowth** from this unit.

#### Song of the Land (Activated)
- **Timing:** Your Hero Phase
- **Effect:** Give every objective and terrain feature within 18" of this unit an **overgrown token**. You can measure the creeping overgrowth from objectives that have a friendly overgrown token.

#### Trick of the Dark (Activated)
- **Timing:** Your Movement Phase
- **Declare:** Pick a friendly **Awakened Wyldwood** to be the target.
- **Effect:** Remove the target from the battlefield and set it up again wholly within 6" of this unit, more than 1" from all enemy units and other terrain features and more than 3" from all objectives. Then, remove this unit from the battlefield and set it up again wholly within the **creeping overgrowth** and more than 9" from all enemy units.

#### Entangling Vines (Passive)
- **Effect:** Enemy non-Monster units cannot use Retreat abilities while they are in combat with this unit.

#### Envoys of the Everqueen (Passive)
- **Effect:** This unit has Ward (6+) while it is wholly within the **creeping overgrowth**.

#### Spirit-step (Activated)
- **Timing:** Any Movement Phase
- **Effect:** If this unit is in combat, it can make a pile-in move. Then, you can pick an enemy unit in combat with this unit and roll a D3. On a 2+, inflict an amount of mortal damage on the enemy unit equal to the roll.

#### Shimmergloom Moths (Activated)
- **Timing:** Reaction: Opponent declared a Spell ability
- **Effect:** If this unit was picked to be a target of that spell, roll a dice. On a 4+, ignore the effect of that spell on this unit.

#### Carpet of Thorns (Activated)
- **Timing:** Any Movement Phase
- **Declare:** Pick a visible enemy unit within 12" of this unit to be the target.
- **Effect:** Subtract D3 from charge rolls for the target for the rest of the turn.

#### The Old Stag's Roar (Passive)
- **Effect:** Add the number of enemy units that have been destroyed this battle to this unit's Move characteristic, to a maximum of +4".

#### Ceaseless Growth (Passive)
- **Effect:** When using the 'Endless Growth' ability, add **X** to the D3 roll for this unit, where **X** is the number of terrain features within 12" of this unit that have a friendly **overgrown token**.

#### Sacred Pursuit (Activated)
- **Timing:** Once Per Turn (Army), Your Charge Phase
- **Effect:** If this unit is not in combat and has not used a Run or Retreat ability this turn, remove it from the battlefield and set it up again wholly within the **creeping overgrowth** and more than 9" from all enemy units.

### Battle Traits

#### Endless Growth (Activated)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Declare:** Pick each friendly Sylvaneth unit that is wholly within the **creeping overgrowth** to be the targets.
- **Effect:** For each target, roll a D3 and pick 1 of the following effects:
• **Heal (X)** the target, where **X** is equal to the roll.
• Return a number of slain models to it with a combined Health characteristic of up to the value of the roll.

#### The Creeping Overgrowth (Passive)
- **Effect:** The **creeping overgrowth** is measured from each terrain feature that has a friendly **overgrown token**. The range of the **creeping overgrowth** is based on the number of friendly, **overgrown tokens** on the battlefield as shown:
• 0-2 **overgrown tokens**: 6"
• 3-5 **overgrown tokens**: 9"
• 6-8 **overgrown tokens**: 12"
• 9+ **overgrown tokens**: Whole battlefield
***Designer's Note**: Awakened Wyldwoods are your main way of generating overgrown tokens. Some unit abilities allow you to too.*

#### Fury of The Forest (Command)
- **Timing:** Enemy Combat Phase
- **Cost:** 1
- **Declare:** Pick a friendly Sylvaneth unit wholly within the **creeping overgrowth** and that is in combat with only 1 enemy unit to use this ability.
- **Effect:** The unit using this ability has Strike-first for the rest of the turn.

#### The Land Awakens (Command)
- **Timing:** End of Your Turn
- **Cost:** 1
- **Declare:** Pick a friendly non-Hero Revenant Infantry unit or friendly **Dryads** unit that has been destroyed to be the target.
- **Effect:** Setup a replacement unit with half the number of models from the target unit (rounding up) wholly within **creeping overgrowth** and more than 9" from all enemy units.

#### Reclaimed By Nature (Activated)
- **Timing:** Once Per Turn (Army), Your Hero Phase
- **Effect:** If there are fewer than 3 friendly **Awakened Wyldwoods** on the battlefield, you can set up an **Awakened Wyldwood** on the battlefield more than 3" from all objectives and enemy units, more than 12" from all other friendly **Awakened Wyldwoods** and more than 1" from all other terrain features.

#### Creeping Dread (Activated)
- **Timing:** Once Per Turn (Army), Start of Any Turn
- **Declare:** Pick an enemy unit within the **creeping overgrowth** to be the target.
- **Effect:** For the rest of the turn, the first time your opponent declares a command for the target while it is not wholly within 12" of a Hero in their army, unless your opponent spends 1 additional command point to use that command, the command has no effect, it still counts as being used and the command points spent to use the command are still lost.

### Artefacts of Power

#### Seed of Rebirth (Passive)
- **Effect:** The first time this unit would be destroyed, before removing it from play, roll a die. On a 3+, this unit is not destroyed and any remaining damage points inflicted on it have no effect. **Heal (D3)** this unit.

#### Amberglade Nectar (Activated)
- **Timing:** Once Per Battle, Any Combat Phase
- **Declare:** Pick a visible enemy unit within 6" of this unit to be the target.
- **Effect:** For the rest of the turn, add 1 to the Damage characteristic of **Companion** weapons used by friendly Sylvaneth units for attacks that target that enemy unit.

#### Wychwood Glaive (Activated)
- **Timing:** End of Any Turn
- **Declare:** Pick an enemy unit that had any damage points allocated to it by this unit's combat attacks this turn to be the target.
- **Effect:** Subtract 1 from save rolls for the target for the rest of the battle. This unit can use this ability even if it has been destroyed.

#### Acorn of the Ages (Passive)
- **Effect:** While they are wholly within 12" of this unit, friendly **Awakened Wyldwoods** have Ward (5+).

#### Ranu's Lamentiri (Activated)
- **Timing:** Once Per Turn (Army), Reaction: Opponent declared a Spell ability for a unit within 30" of this unit
- **Effect:** Make an unbinding roll of 2D6. If the roll exceeds the casting roll for the spell, the spell is unbound and its effect is not resolved. Then, inflict an amount of mortal damage on the caster equal to the difference between the casting roll and the unbinding roll, to a maximum of 6.
- **Keywords:** Unbind

#### Glamourweave (Passive)
- **Effect:** Each phase, you can re-roll 1 hit roll, 1 wound roll and 1 save roll for this unit.

### Battle Formations

#### Nature's Titans (Passive)
- **Effect:** When using the 'Endless Growth' ability, do not roll a D3 for Monster targets. Instead **Heal (3)** each of those targets.

#### Sacred Quarry (Activated)
- **Timing:** Once Per Turn (Army), Your Hero Phase
- **Declare:** Pick a friendly Sylvaneth unit to be the target.
- **Effect:** Until the start of your next turn, add 1 to hit rolls for the target's attacks while it is wholly within 12" of the enemy general or an enemy Warmaster.

#### Memories Of The Fallen (Activated)
- **Timing:** Once Per Turn (Army), Any Hero Phase
- **Declare:** For each friendly Sylvaneth unit or friendly **Awakened Wyldwood** that was destroyed in the previous turn, you can pick a friendly Sylvaneth unit to be the target.
- **Effect:** Add 1 to the Attacks characteristic of each target's melee weapons for the rest of the turn.

#### Fear of The Deepwoods (Passive)
- **Effect:** In the 'Creeping Dread' ability, replace 'while it is not wholly within 12" of a Hero in their army' with 'while it is not wholly within 6" of a Hero in their army'.

#### Walk the Hidden Paths (Activated)
- **Timing:** Once Per Turn (Army), End of Your Turn
- **Declare:** Pick a friendly Sylvaneth unit wholly within 12" of a friendly **Awakened Wyldwood** to be the target.
- **Effect:** Remove the target from the battlefield and set it up again wholly within 12" of another friendly **Awakened Wyldwood** and more than 9" from all enemies.

#### Spiteful Dusk (Passive)
- **Effect:** The first time each phase an enemy unit within the **creeping overgrowth** uses a command, inflict D3 mortal damage on that unit after that command has been resolved. If that command was a reaction, instead inflict D3 mortal damage on that unit after the ability that command was a reaction to has been resolved.

### Heroic Traits

#### Spellsinger (Passive)
- **Effect:** If this unit is a **Wizard**, add 1 to casting rolls for this unit.
If this unit is not a **Wizard**, it can use the 'Unbind' ability as if it has Wizard(1).

#### Spirit of The Dark Forest (Activated)
- **Timing:** Once Per battle, Enemy Hero Phase
- **Declare:** Pick a visible enemy **Hero** to be the target.
- **Effect:** Halve the target's Move characteristic the rest of the turn.

#### Realmroot Guide (Activated)
- **Timing:** Once Per Battle, Your Movement Phase
- **Declare:** This unit can only use this ability if it is wholly within the **creeping overgrowth**. Pick this unit and up to 1 other friendly Sylvaneth unit. within this unit's combat range to be the targets.
- **Effect:** Remove the targets from the battlefield and set them up again wholly within the **creeping overgrowth** and more than 9" from all enemy units.

## Subfactions

### Lords of the Clan

**Battle Traits**

#### Dormant Heroes (Activated)
- **Timing:** Deployment Phase
- **Declare:** If there are more Lords of the Clan units on the battlefield than there are in reserve, pick a friendly Lords of the Clan Monster unit that has not been deployed.
- **Effect:** Set up that unit in reserve as a **dormant forest spirit**. It has now been deployed.
- **Keywords:** Deploy

#### Elder Authority (Activated)
- **Timing:** Once Per Turn (Army), Deployment Phase
- **Declare:** You can only use this ability if there is a friendly **Awakened Wyldwood** on the battlefield.
- **Effect:** You can set up up to 2 additional **Awakened Wyldwoods** on the battlefield (an Awakened Wyldwood is a single tree miniature with a crescent-shaped base). The first can be set up anywhere on the battlefield more than 3" from all units, objectives, and other terrain features. The second can be set up anywhere on the battlefield wholly outside of enemy territory and more than 3" from all units, objectives, and other terrain features.
**Designer's Note:** *The original* **Awakened Wyldwood** *is set up in Step 1 of the deployment phase using the 'Deploy Faction Terrain' ability. This ability allows you to set up a second and third such terrain feature and is used in Step 3 of the deployment phase, after armies have been deployed.*

#### Our Reach is Long (Passive)
- **Effect:** The **creeping overgrowth** (referenced by other abilities** extends 6" from each terrain feature that has a friendly **overgrown token**.
Increase this range by 1" for each friendly Lords of the Clan unit that is wholly outside friendly territory.

#### Roused to Fury (Command)
- **Timing:** Enemy Movement Phase
- **Cost:** 1
- **Declare:** Pick an enemy unit within 9" of a friendly **Awakened Wyldwood** to be the target.
- **Effect:** Pick 1 of the following units:
• A friendly Lords of the Clan unit that is in reserve as a **dormant forest spirit**.
• A friendly Lords of the Clan Monster that has been destroyed.
Set up the unit you picked, or a replacement unit if you picked a destroyed unit, within 9" of the target, wholly within 6" of that friendly **Awakened Wyldwood** and not in combat. If you set up a replacement unit, allocate 6 damage points to it (ward rolls cannot be made for those damage points).
Then, remove that friendly **Awakened Wyldwood** from the battlefield.

**Artefacts of Power**

#### Rejuvenating Companions (Activated)
- **Timing:** End of Any Turn
- **Effect:** **Heal (3)** this unit.

**Heroic Traits**

#### Ancient Might (Passive)
- **Effect:** This unit can use Charge abilities even if it is in combat. If the charge roll is 3 or less when it does so, it does not count as having charged.

### Soulpod Guardians

**Battle Traits**

#### The Sacred Groves (Activated)
- **Timing:** Once Per Battle (Army), Deployment Phase
- **Declare:** You must use this ability.
- **Effect:** Set up 3 **Awakened Wyldwoods** (an Awakened Wyldwood is a single tree miniature with a crescent-shaped base) wholly within friendly territory, more than 1" from all other terrain features and more than 3" from all objectives. The tips of each crescent-shaped base must touch, forming a circle.
Once you have done so, for the rest of the battle, the 3 tree miniatures join to form 1 **Soulpod Grove** terrain feature. The area inside the circle formed by the trees is part of the terrain feature.
- **Keywords:** Deploy Terrain

#### Jade Haven (Passive)
- **Effect:** The Health characteristic of your **Soulpod Grove** is 18. The **creeping overgrowth** extends 12" from your **soulpod grove**.

#### Vengeance for the Lost (Passive)
- **Effect:** If your **Soulpod Grove** has been demolished this battle, add 2 to the Attacks characteristic of friendly Soulpod Guardians units' melee weapons.

#### Heart of the Forest (Passive)
- **Effect:** You cannot allocate more than 10 damage points per phase to a friendly Soulpod Guardians unit while it is wholly within 12" of your **Soulpod Grove** (remaining damage points inflicted on it in that phase have no effect).

**Artefacts of Power**

#### Clarionbuds (Activated)
- **Timing:** Once Per Battle, Your Movement Phase
- **Declare:** If there is an enemy unit wholly within 12" of your **Soulpod Grove**, pick up to 3 friendly Soulpod Guardians units to be the targets.
- **Effect:** Remove the targets from the battlefield and set them up again wholly within 12" of your **Soulpod Grove** and more than 6" from all enemy units.

**Heroic Traits**

#### Devoted Protector (Passive)
- **Effect:** If this unit is destroyed while it is wholly within 12" of your **Soulpod Grove**, **Heal (10)** your **Soulpod Grove**.

### The Evergreen Hunt

**Battle Traits**

#### Song of the Hunt (Passive)
- **Effect:** The following cumulative effects apply to friendly Evergreen Hunt units depending on the number of chords you have:
**1 chord – *Simple:*** Add 1 to run rolls and charge rolls for friendly units wholly within the same quarter of the battlefield as the quarry.
**2 chords – *Tuneful:*** Add 1 to hit rolls and wound rolls for friendly units’ combat attacks that target an enemy unit wholly within the same quarter of the battlefield as the quarry.
**3-5 chords – *Melodic:*** Add 1 to the Attacks characteristic of melee weapons used by friendly units while they are in combat with the quarry.
**6+ chords – *Mellifluous:*** While a friendly unit is in combat with the quarry, the quarry has Strike-last.

#### Abundant Growth (Activated)
- **Timing:** Once Per Turn (Army), Your Hero Phase
- **Declare:** Pick each friendly Evergreen Hunt unit that is wholly within 3" of a terrain feature to be the targets.
- **Effect:** **Heal (1)** each target.

#### Opening Horn Blast (Activated)
- **Timing:** Once Per Battle Round (Army), Start of Battle Round
- **Declare:** You must use this ability at the start of the battle round.
- **Effect:** Your chords are reset to 0. Then, you gain 1 chord:
• If a friendly **Belthanos** is on the battlefield.
• For each friendly Evergreen Hunt unit that is wholly within the same large quarter of the battlefield as the quarry.
• For each quarry destroyed so far during the battle.

#### Rhythm of the Chase (Activated)
- **Timing:** Once Per Battle Round (Army), Start of Battle Round
- **Declare:** If there is no **quarry** on the battlefield, pick an enemy unit on the battlefield to be the target.
- **Effect:** The target is the **quarry** for the rest of the battle.

#### A Prize Quarry is Sighted (Command)
- **Timing:** Once Per Battle (Army), Your Hero Phase
- **Cost:** 1
- **Declare:** Pick an enemy unit within 9" of a friendly Evergreen Hunt unit to be the target.
- **Effect:** The target becomes the quarry for the rest of the battle. If there was already a quarry on the battlefield when you declared this ability, that unit is no longer the quarry.

#### Merciful Strike (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick a friendly Evergreen Hunt Monster that has not used a Rampage ability this turn to use this ability, then pick an enemy unit in combat with it to be the target.
- **Effect:** Roll a dice and add the number of damage points the target has. If the result exceeds the target’s Health characteristic, 1 model in the target unit is slain.

#### Nature Aetheric (Activated)
- **Timing:** Once Per Battle (Army), Deployment Phase
- **Declare:** Pick a terrain feature that is within friendly territory to be the target.
- **Effect:** For the rest of the battle, friendly units are
wholly within the **creeping overgrowth** while they are wholly within 6" of the target.

#### Questing Tendrils (Activated)
- **Timing:** Once Per Turn (Army), Start of Any Turn
- **Declare:** Pick the terrain feature that is closest to the quarry to be the target.
- **Effect:** For the rest of the turn, friendly units are wholly within the creeping overgrowth while they are wholly within 6" of the target.

**Heroic Traits**

#### Sapwood Leader (Passive)
- **Effect:** If you pick this unit to be a target of the ‘Abundant Growth’ ability, **Heal (3)** this unit instead of **Heal (1)**.

**Artefacts of Power**

#### Heartwood Hunting Horn (Activated)
- **Timing:** Once Per Battle (Army), Any Hero Phase
- **Effect:** You gain 1 chord.

## Units

### Alarielle the Everqueen
**Points:** 600 | **Move:** 14" | **Health:** 16 | **Save:** 4+ | **Control:** 10
**Keywords:** UNIQUE, HERO, MONSTER, FLY, WARMASTER, ORDER, SYLVANETH, WIZARD (3), WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Spear of Kurnoth | Melee | - | 5 | 3+ | 3+ | 2 | 2 | Charge (+1 Damage) |
| Spear of Kurnoth | Ranged | 12" | 1 | 2+ | 3+ | 2 | 4 | - |
| Wardroth Beetle's Antlers | Melee | - | 6 | 4+ | 2+ | 2 | 4 | Companion |

**Abilities:**

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of its **Wardroth Beetle's Antlers** is 4.

#### Metamorphosis (Spell)
- **Timing:** Your Hero Phase
- **Casting Value:** 7
- **Declare:** Pick a visible enemy unit within 12" of this unit to be the target, then make a casting roll of 2D6.
- **Effect:** Inflict 2D3 mortal damage on the target. If the target would be destroyed by this spell, before removing any models, pick the closest terrain feature to the target that does not have a friendly **overgrown token** and give it a friendly **overgrown token**. If all terrain features on the battlefield already have a friendly **overgrown token**, inflict D3 mortal damage on each enemy unit within 6" of the target instead.
- **Keywords:** Spell

#### Lifebloom (Activated)
- **Timing:** Once Per Turn (Army), Any Hero Phase
- **Effect:** **Heal (2D6)** this unit.

#### Living Battering Ram (Activated)
- **Timing:** Once Per Turn (Army), Any Charge Phase
- **Declare:** If this unit charged this turn, pick a visible enemy unit within 1" of it to be the target.
- **Effect:** Roll a dice. On a 3+, inflict an amount of mortal damage on the target equal to the roll.
- **Keywords:** Rampage

#### The Goddess Of Life (Command)
- **Timing:** Once Per Battle, Your Movement Phase
- **Cost:** 2
- **Declare:** This unit can use this ability even if it has been destroyed.
- **Effect:** Set up a replacement unit with 8 damage points allocated to it (ward rolls cannot be made for those damage points) wholly within the **creeping overgrowth** and more than 9" from all enemy units.

#### Seasons Of War (Activated)
- **Timing:** Start of Battle Round
- **Declare:** You must use this ability.
- **Effect:** Pick 1 of the following effects to apply for the rest of the battle round. You cannot pick the effect that you picked in the previous battle round.
***The Burgeoning***: Each time a friendly **Awakened Wyldwood** uses the 'Ever Growing' ability, you can pick any terrain feature your opponent does not control to be the target rather than the closest.
***The Reaping***: Add 1 to casting rolls and chanting rolls for friendly Sylvaneth units while they are wholly within 12" of this unit.
***The Dwindling***: Friendly Sylvaneth units can use Charge abilities even if they used a Retreat ability in the same turn. In addition, no mortal damage is inflicted on those units when they use Retreat abilities.
***Everdusk***: Each time you use the 'Creeping Dread' ability this battle round, you can pick up to 3 targets instead of 1.

---

### Anvil of Apotheosis: Forest Elder Hero
**Points:** 0 | **Move:** 6" | **Health:** 14 | **Save:** 3+ | **Control:** 5
**Keywords:** HERO, ORDER, SYLVANETH, MONSTER, FOREST ELDER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Sweeping Branches | Melee | - | 5 | 4+ | 3+ | 1 | 2 | Anti-charge (+1 Rend) |
| Strangleroots | Ranged | 10" | 3 | 3+ | 2+ | 1 | 2 | Shoot in Combat |
| Kurnoth Javelin | Ranged | 12" | 1 | 3+ | 2+ | 2 | D6 | Anti-Monster (+1 Rend) |
| Huge Impaling Talons | Melee | - | 2 | 4+ | 2+ | 2 | 3 | Crit (Mortal) |
| Swarm of Spites | Melee | - | 10 | 4+ | 4+ | - | 1 | Crit (Mortal), Companion |

**Abilities:**

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of its **Sweeping Branches** is 3.

#### Verdant Healer (Activated)
- **Timing:** Your Hero Phase
- **Declare:** Pick a visible friendly Sylvaneth unit wholly within 12" of this unit to be the target.
- **Effect:** Pick 1 of the following effects:
• **Heal (D3)** the target.
• The target has Ward (6+) until the start of your next turn.

#### Noble Guardian (Passive)
- **Effect:** Add 2 to the Attacks characteristic of this unit's melee weapons while it is wholly within the **creeping overgrowth**.

#### Gnarled Spirit (Passive)
- **Effect:** Subtract 3 from the control scores of enemy units while they are in combat with this unit.

#### Rootwalker (Activated)
- **Timing:** Enemy Movement Phase
- **Effect:** If this unit is more than 9" from all enemy units, it can immediately use the 'Normal Move' ability as if it were your movement phase.

#### Tormented Memories (Activated)
- **Timing:** End of Any Turn
- **Declare:** You must use this ability if this unit is contesting an objective you do not control. Pick each other friendly unit contesting the same objective as this unit to be the targets.
- **Effect:** Roll a D3 for each target. On a 2+, inflict an amount of mortal damage on the target equal to the roll.

#### Old Barkflesh (Passive)
- **Effect:** Each time this unit uses a Move ability, roll a dice. On a 4+, inflict 1 mortal damage on it after that ability has been resolved.

#### Slow to Awaken (Passive)
- **Effect:** Halve this unit's Move characteristic until the start of the second battle round.

#### Cautious Spirit (Passive)
- **Effect:** While this unit is wholly outside the **creeping overgrowth**, it can only use Move and Attack abilities.

#### Slashing Wings (Activated)
- **Timing:** Your Movement Phase
- **Effect:** This unit can move a distance up to your Move characteristic. It can pass through models during that move. Then, you can pick an enemy unit that this unit passed across during that move to be the target. Roll a dice for each model in the target unit. For each 5+, inflict 1 mortal damage on the target.
- **Keywords:** Move

#### Strike and Fade (Activated)
- **Timing:** Once Per Turn (Army). End of Any Turn
- **Effect:** If this unit used a Fight ability this turn and is wholly within the **creeping overgrowth**, remove it from the battlefield and set it up again on the battlefield more than 9" from all enemy units and wholly within the **creeping overgrowth**.

#### Charge of the Spites (Activated)
- **Timing:** Once Per Turn (Army), Any Charge Phase
- **Declare:** If this unit charged this phase, pick up to 3 visible enemy units within 6" of this unit to be the targets.
- **Effect:** Roll a D3 for each target. On a 2+, inflict an amount of mortal damage on the target equal to the roll.
- **Keywords:** Rampage

#### Seismic Stomps (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** If this unit charged this turn, pick each enemy unit in combat with this unit to be the targets.
- **Effect:** Roll a dice for each target. On a 3+, subtract 1 from hit rolls for the target's combat attacks for the rest of the turn.
- **Keywords:** Rampage

#### Snatching Vines (Activated)
- **Timing:** Once Per Turn (Army), Your Shooting Phase
- **Declare:** Pick an enemy Infantry Hero within 9" of this unit to be the target.
- **Effect:** Roll a dice. On a 3+, your opponent must remove the target from the battlefield and set it up again within 1" of this unit.

#### Shambling Overgrowth (Passive)
- **Effect:** You can measure the **creeping overgrowth** unit from this unit.

---

### Arch-Revenant
**Points:** 130 | **Move:** 12" | **Health:** 6 | **Save:** 4+ | **Control:** 2
**Keywords:** HERO, ORDER, SYLVANETH, FLY, INFANTRY, REVENANT

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Revenant’s Glaive and Tail Pincers | Melee | - | 5 | 3+ | 4+ | 1 | 2 | - |

**Abilities:**

#### Fight And Fly (Activated)
- **Timing:** Reaction: You declared a Fight ability for this unit
- **Effect:** If this unit charged this turn, this unit can move up to 2D6" after that Fight ability has been resolved. It cannot end that move in combat.

#### Gossamid Commander (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** If this unit charged this turn, pick a visible friendly **Gossamid Archers** unit that is not in combat and is wholly within 18" of this unit to be the target.
- **Effect:** The target can immediately use a Shoot ability as if it were your shooting phase but all of the attacks made as part of that Shoot ability must target an enemy unit that is in combat with this unit.

---

### Belthanos, First Thorn of Kurnoth
**Points:** 290 | **Move:** 12" | **Health:** 14 | **Save:** 3+ | **Control:** 5
**Keywords:** UNIQUE, HERO, MONSTER, ORDER, SYLVANETH, FLY, KURNOTHI

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Kurnoth Glaive | Melee | - | 5 | 3+ | 3+ | 2 | 2 | Charge (+1 Damage) |
| Carnelian Greatspite’s Mandibles | Melee | - | 4 | 4+ | 2+ | 2 | 3 | Companion |

**Abilities:**

#### Huntmaster's Strike (Activated)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Effect:** If this unit charged this turn, remove it from the battlefield and set it up again more than 6" from all enemy units and wholly within the **creeping overgrowth**.

#### Kurnothi War-horn (Passive)
- **Effect:** Add 2" to the Move characteristic of friendly Kurnothi units while they are wholly within 12" of this unit.

#### Rhythm of the Chase (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a D3. On a 2+, inflict an amount of mortal damage on the target equal to the roll, then you can remove this unit from the battlefield and set it up again on the battlefield within 1" of the target.
- **Keywords:** Rampage

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of its **Carnelian Greatspite’s Mandibles** is 3.

---

### Branchwych
**Points:** 100 | **Move:** 6" | **Health:** 5 | **Save:** 5+ | **Control:** 2
**Keywords:** ORDER, SYLVANETH, HERO, WIZARD (1), INFANTRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Greenwood Scythe and Bittergrub | Melee | - | 3 | 3+ | 4+ | 1 | D3 | - |

**Abilities:**

#### Slay The Trespassers! (Activated)
- **Timing:** Once Per Turn (Army), Your Hero Phase
- **Declare:** Pick an enemy unit that is within the **creeping overgrowth** or that is visible to this unit and within friendly territory to be the target.
- **Effect:** Rolla dice. On a 3+, add 1 to wound rolls for combat attacks that target that enemy unit for the rest of the turn.

---

### Dryads
**Points:** 90 | **Move:** 6" | **Health:** 1 | **Save:** 5+ | **Control:** 1
**Keywords:** CHAMPION, ORDER, INFANTRY, SYLVANETH

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Wracking Talons | Melee | - | 2 | 3+ | 4+ | - | 1 | - |

**Abilities:**

#### One With The Forest (Passive)
- **Effect:** Subtract 1 from hit rolls and wound rolls for attacks that target this unit while it is wholly within the **creeping overgrowth**.

---

### Drycha Hamadreth
**Points:** 250 | **Move:** 10" | **Health:** 12 | **Save:** 3+ | **Control:** 5
**Keywords:** UNIQUE, HERO, MONSTER, ORDER, SYLVANETH, WIZARD (1), FOREST ELDER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Slashing Talons | Melee | - | 5 | 3+ | 2+ | 2 | 2 | Crit (2 Hits) |
| Swarm of Spites | Melee | - | 20 | 4+ | 4+ | - | 1 | Crit (Mortal), Companion |

**Abilities:**

#### Terror Of The Deep Woods (Passive)
- **Effect:** Enemy units cannot use Run or Retreat abilities while they are within 9" of this unit.

#### Enrage The Spites (Spell)
- **Timing:** Your Hero Phase
- **Casting Value:** 7
- **Declare:** Pick up to 3 enemy units that are visible to and within 12" of this unit or that are within the **creeping overgrowth** to be the targets, then make a casting roll of 2D6.
- **Effect:** Inflict D3 mortal damage on each target.
- **Keywords:** Spell

#### Primal Terror (Activated)
- **Timing:** Your Charge Phase
- **Effect:** If this unit is not in combat and has not used a Run or **Retreat** ability this turn, make a charge roll of 2D6. Then, remove this unit from the battlefield and set it up again within 1/2" of an enemy unit and wholly within **X**" of a terrain feature that has a friendly **overgrown token**, where **X** is equal to the charge roll. If you do so, this unit has charged.
- **Keywords:** Core, Move, Charge, Rampage

---

### Drycha Hamadreth (Scourge of Ghyran)
**Points:** 300 | **Move:** 10" | **Health:** 12 | **Save:** 3+ | **Control:** 5
**Keywords:** UNIQUE, HERO, MONSTER, ORDER, SYLVANETH, WIZARD (1)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Slashing Talons | Melee | - | 10 | 3+ | 2+ | 2 | 2 | Crit (2 Hits) |

**Abilities:**

#### Strength From Spite (Passive)
- **Effect:** Ward rolls cannot be made for damage points inflicted by this unit’s attacks.

#### Twisted Spirit-Song (Spell)
- **Timing:** Your Hero Phase
- **Casting Value:** 6
- **Declare:** Pick a visible friendly Sylvaneth unit wholly within 12" of this unit to be the target. Then, make a casting roll of 2D6.
- **Effect:** Until the start of your next turn, add 1 to wound rolls for the target’s combat attacks, including those made with **Companion** weapons.
In addition, if the target is non-Kurnothi, add 1 to the Attacks characteristics of the target’s melee weapons.
- **Keywords:** Spell

#### Vicious Spiteswarms (Activated)
- **Timing:** Reaction: Opponent declared a Charge ability for a unit within 18" of this unit
- **Effect:** Your opponent must pick 1 of the following: 
***Avoid the Swarm:*** Subtract D3 from the charge roll. In addition, if your opponent re-rolls that charge roll, subtract D3 from that roll.
***Charge Through the Swarm:*** Subtract 1 from hit rolls for that unit’s attacks for the rest of the turn, and add 1 to hit rolls for attacks that target that unit for the rest of the turn.
- **Keywords:** Rampage

---

### Gossamid Archers
**Points:** 120 | **Move:** 12" | **Health:** 2 | **Save:** 5+ | **Control:** 1
**Keywords:** CHAMPION, ORDER, INFANTRY, FLY, SYLVANETH, REVENANT

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Gossamid Bow | Ranged | 12" | 2 | 3+ | 4+ | 1 | 1 | Crit (Auto-wound), Shoot in Combat |
| Cruel Talons | Melee | - | 1 | 3+ | 4+ | - | 1 | - |

**Abilities:**

#### Forest Fighters (Passive)
- **Effect:** This unit has a coherency range of 2".

#### Zephyrspites (Activated)
- **Timing:** Reaction: You declared a Shoot ability for this unit
- **Effect:** After that Shoot ability has been resolved, this unit can move up to D6". It cannot move into combat during any part of that move.

---

### Grove Guardian
**Points:** 220 | **Move:** 4" | **Health:** 7 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, ORDER, SYLVANETH, FOREST ELDER, INFANTRY, PRIEST (2)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Grove Sickle or Shears | Melee | - | 3 | 3+ | 4+ | 1 | D3 | - |
| Guardian Spites | Melee | - | 6 | 4+ | 4+ | 1 | D3 | Crit (Mortal), Companion |

**Abilities:**

#### Soulshriek (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick each friendly non-Hero Revenant unit wholly within 12" of and visible to this unit to be the targets.
- **Effect:** For the rest of the phase, each time a model in a target unit is slain by a combat attack, roll a dice. On a 5+, inflict 1 mortal damage on the attacking unit.

#### Regenesis (Prayer)
- **Timing:** Your Hero Phase
- **Chanting Value:** 4
- **Declare:** Pick a friendly Sylvaneth unit that is wholly within the **creeping overgrowth** or that is visible to and wholly within 12" of this unit to be the target, then make a chanting roll of D6.
- **Effect:** If the chanting roll was 6+, you can pick another eligible unit to be a second target. If a target is a non-Hero Revenant Infantry or **Dryads** unit, return up to D3 models to the target unit. Otherwise, **Heal (3)** that target
- **Keywords:** Prayer

---

### Kurnoth Hunters with Kurnoth Greatbows
**Points:** 200 | **Move:** 6" | **Health:** 5 | **Save:** 4+ | **Control:** 2
**Keywords:** CHAMPION, ORDER, INFANTRY, SYLVANETH, KURNOTHI

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Kurnoth Greatbow | Ranged | 18" | 2 | 3+ | 3+ | 1 | 2 | Anti-Cavalry (+1 Rend) |
| Vicious Claws | Melee | - | 3 | 3+ | 3+ | - | 1 | - |

**Abilities:**

#### Hunter's Endurance (Passive)
- **Effect:** Subtract 1 from the Rend characteristic of weapons used for attacks that target this unit while it is wholly within the **creeping overgrowth**.

#### Steady Aim (Activated)
- **Timing:** Once Per Turn (Army), Your Shooting Phase
- **Effect:** If this unit has not used a Move ability this turn and was not set up this turn, add 1 to hit rolls for this unit's shooting attacks for the rest of the turn.

---

### Kurnoth Hunters with Kurnoth Greatswords
**Points:** 200 | **Move:** 6" | **Health:** 5 | **Save:** 4+ | **Control:** 2
**Keywords:** CHAMPION, ORDER, INFANTRY, SYLVANETH, KURNOTHI

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Kurnoth Greatsword | Melee | - | 4 | 3+ | 3+ | 1 | 2 | Crit (Mortal) |

**Abilities:**

#### Hunter's Endurance (Passive)
- **Effect:** Subtract 1 from the Rend characteristic of weapons used for attacks that target this unit while it is wholly within the **creeping overgrowth**.

#### Crashing Impact (Activated)
- **Timing:** Any Combat Phase
- **Declare:** If this unit charged this turn, pick an enemy unit that is in combat with this unit to be the target.
- **Effect:** Roll a dice for each model in this unit that is in combat with the target. For each 3+, inflict 1 mortal damage on the target.

---

### Kurnoth Hunters with Kurnoth Scythes
**Points:** 190 | **Move:** 6" | **Health:** 5 | **Save:** 4+ | **Control:** 2
**Keywords:** CHAMPION, ORDER, INFANTRY, SYLVANETH, KURNOTHI

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Kurnoth Scythe | Melee | - | 3 | 3+ | 3+ | 1 | 3 | Anti-charge (+1 Rend) |

**Abilities:**

#### Hunter's Endurance (Passive)
- **Effect:** Subtract 1 from the Rend characteristic of weapons used for attacks that target this unit while it is wholly within the **creeping overgrowth**.

#### Tanglethorn Thicket (Activated)
- **Timing:** Any Combat Phase
- **Declare:** Pick an enemy unit that charged this turn and is in combat with this unit to be the target.
- **Effect:** Roll a dice for each model in this unit that is in combat with the target. For each 3+, inflict 1 mortal damage on the target.

---

### Kurnoth's Heralds
**Points:** 110 | **Move:** 12" | **Health:** 3 | **Save:** 5+ | **Control:** 1
**Keywords:** ORDER, UNIQUE, Legends, CHAMPION, KURNOTHI, CAVALRY, SYLVANETH

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Hunter Weapons | Melee | - | 3 | 3+ | 4+ | 1 | 1 | Charge (+1 Damage) |
| Kurnothi Bow | Ranged | 18" | 3 | 3+ | 4+ | 1 | 1 | - |
| Hunter Weapons | Melee | - | 3 | 3+ | 4+ | 1 | 1 | Charge (+1 Damage) |
| Hunter Weapons | Melee | - | 3 | 3+ | 4+ | 1 | 1 | Charge (+1 Damage) |

**Abilities:**

#### Graceful Leaps (Passive)
- **Effect:** Each time this unit makes a pile-in move:
• Add 3" to the distance this unit can move.
• This unit can pass through models in enemy units.

---

### Revenant Seekers
**Points:** 180 | **Move:** 12" | **Health:** 5 | **Save:** 4+ | **Control:** 2
**Keywords:** CAVALRY, CHAMPION, STANDARD BEARER (1/3), MUSICIAN (1/3), FLY, ORDER, SYLVANETH, REVENANT

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Seeker’s Sickle | Melee | - | 3 | 3+ | 4+ | 1 | 1 | Crit (Mortal) |
| Dragonspite's Mandibles | Melee | - | 3 | 4+ | 3+ | 1 | 2 | Companion |

**Abilities:**

#### Deafening Drone (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** If this unit did not charge this turn, pick an enemy unit in combat with it to be the target.
- **Effect:** Subtract 1 from the Attacks characteristic of the target's melee weapons for the rest of the turn.

#### Forest Fighters (Passive)
- **Effect:** This unit has a coherency range of 2".

---

### Revenant Seekers (Scourge of Ghyran)
**Points:** 240 | **Move:** 12" | **Health:** 5 | **Save:** 4+ | **Control:** 2
**Keywords:** CAVALRY, CHAMPION, STANDARD BEARER (1/3), MUSICIAN (1/3), FLY, ORDER, SYLVANETH

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Scourge Sickles | Melee | - | 3 | 3+ | 4+ | 1 | 2 | Crit (Mortal) |
| Sharpened Mandibles | Melee | - | 3 | 4+ | 3+ | 1 | 1 | Charge (+1 Damage), Companion |

**Abilities:**

#### Lamentiri Life Energies (Activated)
- **Timing:** Once Per Turn (Army), Any Hero Phase
- **Declare:** Pick a visible friendly Sylvaneth unit wholly within 12" of this unit to be the target.
- **Effect:** Roll a dice. On a 3+, the target has Ward (5+) for the rest of the turn.

#### Forest Fighters (Passive)
- **Effect:** This unit has a coherency range of 2".

---

### Skaeth's Wild Hunt
**Points:** 90 | **Move:** 6" | **Health:** 2 | **Save:** 5+ | **Control:** 1
**Keywords:** ORDER, INFANTRY, UNIQUE, Legends, CHAMPION, MUSICIAN (1/5), KURNOTHI, SYLVANETH

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Tracker Weapons | Melee | - | 2 | 3+ | 4+ | 1 | 1 | - |
| Wild Hunt Ranged Weapons | Ranged | 12" | 2 | 3+ | 4+ | 1 | 1 | - |
| Tracker Weapons | Melee | - | 2 | 3+ | 4+ | 1 | 1 | - |
| Wild Hunt Ranged Weapons | Ranged | 12" | 2 | 3+ | 4+ | 1 | 1 | - |
| Tracker Weapons | Melee | - | 2 | 3+ | 4+ | 1 | 1 | - |
| Tracker Weapons | Melee | - | 2 | 3+ | 4+ | 1 | 1 | - |
| Tracker Weapons | Melee | - | 2 | 3+ | 4+ | 1 | 1 | - |

**Abilities:**

#### Karthaen (Passive)
- **Effect:** While this unit’s **Karthaen, Huntcaller**, is on the battlefield, it has Wizard (1).

#### Fleet of Foot (Passive)
- **Effect:** This unit can use Shoot and/or Charge abilities even if it used a Run or Retreat ability in the same turn.

---

### Spirit of Durthu
**Points:** 320 | **Move:** 6" | **Health:** 14 | **Save:** 3+ | **Control:** 5
**Keywords:** HERO, ORDER, SYLVANETH, MONSTER, FOREST ELDER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Verdant Blast | Ranged | 12" | 5 | 4+ | 3+ | 1 | 2 | - |
| Guardian Sword | Melee | - | 5 | 3+ | 2+ | 2 | 4 | Anti-Monster (+1 Rend) |
| Massive Impaling Talons | Melee | - | 2 | 4+ | 2+ | 2 | 3 | Crit (Mortal) |

**Abilities:**

#### Groundshaker (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy non-Fly Infantry or Cavalry unit in combat with this unit to be the target.
- **Effect:** Roll a dice, On a 3+, the target has Strike-last for the rest of the turn.
- **Keywords:** Rampage

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of its **Guardian Sword** is 3.

#### Guardian Of The Glades (Activated)
- **Timing:** Once Per Turn (Army), Reaction: You declared a Charge ability for this unit
- **Effect:** Add 2 to the charge roll but this unit must end that charge move within 1/2" of an enemy unit that is in combat with another friendly Sylvaneth Hero.

---

### Spite-Revenants
**Points:** 80 | **Move:** 6" | **Health:** 2 | **Save:** 5+ | **Control:** 1
**Keywords:** CHAMPION, ORDER, SYLVANETH, INFANTRY, REVENANT

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Cruel Talons and Fangs | Melee | - | 3 | 3+ | 4+ | - | 1 | Crit (Mortal) |

**Abilities:**

#### Shadowcreep (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Effect:** If this unit is wholly within the **creeping overgrowth**, remove this unit from the battlefield and set it up again wholly within 3" of a terrain feature and more than 9" from all enemy units. Then add 1 to charge rolls for this unit for the rest of the turn.

---

### Spiterider Lancers
**Points:** 220 | **Move:** 12" | **Health:** 5 | **Save:** 4+ | **Control:** 2
**Keywords:** CAVALRY, CHAMPION, STANDARD BEARER (1/3), MUSICIAN (1/3), FLY, ORDER, SYLVANETH, REVENANT

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Spiterider Lance | Melee | - | 3 | 3+ | 4+ | 1 | 1 | Charge (+1 Damage), Crit (2 Hits) |
| Dragonspite's Mandibles | Melee | - | 3 | 4+ | 3+ | 1 | 2 | Companion |

**Abilities:**

#### Strike And Fade (Activated)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Effect:** If this unit charged this turn, remove it from the battlefield and set it up again more than 9" from all enemy units and wholly within the **creeping overgrowth**.

#### Forest Fighters (Passive)
- **Effect:** This unit has a coherency range of 2".

---

### The Lady of Vines
**Points:** 280 | **Move:** 6" | **Health:** 10 | **Save:** 4+ | **Control:** 5
**Keywords:** UNIQUE, HERO, MONSTER, ORDER, SYLVANETH, WIZARD (2), WARD (5+), FOREST ELDER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Kurnotheal’s Wrath | Ranged | 12" | 1 | 2+ | 2+ | 2 | D6 | - |
| Kurnotheal’s Wrath and Lashing Vines | Melee | - | 6 | 3+ | 3+ | 2 | 2 | Crit (Mortal) |

**Abilities:**

#### Verdian Crown (Activated)
- **Timing:** Once Per Battle, Any Hero Phase
- **Declare:** Pick a terrain feature within 18" of this unit that does not have a friendly **overgrown token** to be the target.
- **Effect:** Give the target a friendly **overgrown token**

#### Protection Of The Everqueen (Spell)
- **Timing:** Your Hero Phase
- **Casting Value:** 7
- **Declare:** Make a casting roll of 2D6. Add 1 to the roll if this unit is wholly within the **creeping overgrowth**.
- **Effect:** Until the start of your next turn, friendly Sylvaneth units have Ward (5+) while they are wholly within 12" of this unit.
- **Keywords:** Spell

#### Hands Of The Everqueen (Activated)
- **Timing:** Once Per Turn (Army), Any Hero Phase
- **Declare:** Pick a terrain feature within 18" of this unit that has a friendly **overgrown token** to be the target.
- **Effect:** The next time this unit uses a non-Summon Spell ability this phase, when picking targets for that spell, you can measure range and visibility from the target terrain feature instead of from this unit, and your opponent can measure range and visibility to the target terrain feature instead of to this unit for the purposes of the ‘Unbind’ ability.
- **Keywords:** Rampage

---

### The Twistweald
**Points:** 120 | **Move:** 6" | **Health:** 2 | **Save:** 5+ | **Control:** 1
**Keywords:** INFANTRY, CHAMPION (1/8), ORDER, SYLVANETH, REVENANT

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Twistroot Weapons | Melee | - | 3 | 3+ | 4+ | 1 | 1 | - |
| Warden’s Bow | Ranged | 18" | 2 | 3+ | 4+ | 1 | 1 | - |
| Twistroot Weapons | Melee | - | 3 | 3+ | 4+ | 1 | 1 | - |
| Voracious Swarm | Ranged | 10" | 6 | 3+ | 4+ | 1 | 1 | Shoot in Combat |
| Twistroot Weapons | Melee | - | 3 | 3+ | 4+ | 1 | 1 | - |

**Abilities:**

#### Latching Paraspites (Activated)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** The target has the Infested keyword for the rest of the battle. Infested enemy units cannot move more than 6" when using non-Charge Move abilities.

#### Pain-Induced Frenzy (Passive)
- **Effect:** This unit's melee weapons have **Crit (2 Hits)** if it charged in the same turn.

---

### Tree-Revenants
**Points:** 110 | **Move:** 6" | **Health:** 2 | **Save:** 5+ | **Control:** 1
**Keywords:** CHAMPION, ORDER, SYLVANETH, INFANTRY, STANDARD BEARER (1/5), MUSICIAN (1/5), REVENANT

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Revenant Blade | Melee | - | 3 | 3+ | 4+ | 1 | 1 | - |

**Abilities:**

#### Waypiper's Dance (Passive)
- **Effect:** While this unit is in combat and has not charged this turn, add 1 to the Damage characteristic of this unit's melee weapons.

---

### Treelord
**Points:** 260 | **Move:** 6" | **Health:** 14 | **Save:** 3+ | **Control:** 5
**Keywords:** ORDER, SYLVANETH, MONSTER, FOREST ELDER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Strangleroots | Ranged | 10" | 3 | 3+ | 2+ | 1 | 2 | Shoot in Combat |
| Sweeping Blows | Melee | - | 5 | 4+ | 2+ | 1 | 2 | Anti-charge (+1 Rend) |
| Massive Impaling Talons | Melee | - | 2 | 4+ | 2+ | 2 | 3 | Crit (Mortal) |

**Abilities:**

#### Strangling Grasp (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick a visible enemy unit within 10" of this unit to be the target
- **Effect:** Roll a dice. If the roll exceeds the target's Health characteristic, 1 model in the target unit is slain.
- **Keywords:** Rampage

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of its **Sweeping Blows** is 3.

#### Lash And Tangle (Activated)
- **Timing:** Once Per Turn (Army), Any Shooting Phase
- **Declare:** Pick an enemy unit that had any damage points allocated to it this turn by attacks made with this unit’s **Strangleroots** to be the target.
- **Effect:** Roll a dice. Add 1 to the roll if the target is within the **creeping overgrowth**. On a 3+, subtract 1 from hit rolls for the target's attacks until the start of your next turn.

---

### Treelord Ancient
**Points:** 280 | **Move:** 6" | **Health:** 14 | **Save:** 3+ | **Control:** 5
**Keywords:** ORDER, SYLVANETH, MONSTER, HERO, FOREST ELDER, WIZARD (2)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Doom Tendril Staff | Ranged | 18" | 4 | 4+ | 2+ | 1 | D3 | - |
| Sweeping Blows | Melee | - | 5 | 4+ | 2+ | 1 | 2 | Anti-charge (+1 Rend) |
| Massive Impaling Talons | Melee | - | 2 | 4+ | 2+ | 2 | 3 | Crit (Mortal) |

**Abilities:**

#### Eldest Of Elders (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick each friendly Forest Elder within this unit's combat range to be the targets.
- **Effect:** Roll a dice for each target, On a 3+, add 1 to hit rolls for that target's combat attack until the start of your next turn.
- **Keywords:** Rampage

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of its **Sweeping Blows** is 3.

#### Awaken Slumbering Spirits (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Declare:** Pick a visible terrain feature that has a friendly **overgrown token** and is within 12" of this unit. Then, pick a friendly non-Unique Kurnothi or Treelord unit that has been destroyed to be the target.
- **Effect:** Set up a replacement unit with 1 model from the target unit wholly within 6" of that terrain feature and more than 9" from all enemy units. Then, remove the friendly **overgrown token** from that terrain feature. If the target was a **Treelord**, allocate 10 damage points to the replacement unit (ward rolls cannot be made for those damage points).

---

### Warsong Revenant
**Points:** 150 | **Move:** 6" | **Health:** 7 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, ORDER, SYLVANETH, FLY, INFANTRY, WARD (6+), PRIEST (1), REVENANT

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Spirit Falchion and Spearing Vines | Melee | - | 5 | 3+ | 3+ | 1 | D3 | - |

**Abilities:**

#### Wyldwood Revenants (Activated)
- **Timing:** Once Per Turn (Army), Reaction: You made an unmodified chanting roll of 1 for a **Sylvaneth Priest** wholy within 12" of this unit.
- **Effect:** Re-roll that chanting roll.

#### Alarielle's Song (Passive)
- **Effect:** Friendly Sylvaneth units have Ward (6+) while they are wholly within 12" of this unit. Subtract 1 from ward rolls for enemy units while they are within 12" of this unit.

---

### Ylthari's Guardians
**Points:** 140 | **Move:** 5" | **Health:** 3 | **Save:** 5+ | **Control:** 1
**Keywords:** ORDER, INFANTRY, UNIQUE, Legends, CHAMPION, MUSICIAN (1/5), SYLVANETH

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Enchanted Glade Weapons | Melee | - | 3 | 3+ | 4+ | 1 | 1 | Crit (Auto-wound) |
| Revenant Bow | Melee | - | 1 | 3+ | 4+ | - | 1 | - |
| Revenant Bow | Ranged | 18" | 2 | 3+ | 4+ | 1 | 1 | Crit (Auto-wound) |
| Enchanted Glade Weapons | Melee | - | 3 | 3+ | 4+ | 1 | 1 | Crit (Auto-wound) |
| Enchanted Glade Weapons | Melee | - | 3 | 3+ | 4+ | 1 | 1 | Crit (Auto-wound) |

**Abilities:**

#### Ylthari (Passive)
- **Effect:** While this unit’s **Ylthari** is on the battlefield, it has Wizard (1).

#### Bitterness of the Soul (Passive)
- **Effect:** Add 1 to the Damage characteristic of this unit’s **Enchanted Glade Weapons** for each model in this unit that is slain.
*Designer’s Note: Models that have been slain and returned to this unit do not count as being slain for the purposes of this ability.*

---

## Points Table

| Unit | Points |
| --- | ---: |
| Anvil of Apotheosis: Forest Elder Hero | 0 |
| Spite-Revenants | 80 |
| Dryads | 90 |
| Skaeth's Wild Hunt | 90 |
| Branchwych | 100 |
| Kurnoth's Heralds | 110 |
| Tree-Revenants | 110 |
| Gossamid Archers | 120 |
| The Twistweald | 120 |
| Arch-Revenant | 130 |
| Ylthari's Guardians | 140 |
| Warsong Revenant | 150 |
| Revenant Seekers | 180 |
| Kurnoth Hunters with Kurnoth Scythes | 190 |
| Kurnoth Hunters with Kurnoth Greatbows | 200 |
| Kurnoth Hunters with Kurnoth Greatswords | 200 |
| Grove Guardian | 220 |
| Spiterider Lancers | 220 |
| Revenant Seekers (Scourge of Ghyran) | 240 |
| Drycha Hamadreth | 250 |
| Treelord | 260 |
| The Lady of Vines | 280 |
| Treelord Ancient | 280 |
| Belthanos, First Thorn of Kurnoth | 290 |
| Drycha Hamadreth (Scourge of Ghyran) | 300 |
| Spirit of Durthu | 320 |
| Alarielle the Everqueen | 600 |
