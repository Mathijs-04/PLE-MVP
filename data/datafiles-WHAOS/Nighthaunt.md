# Nighthaunt

## Army Rules

### Anvil of Apotheosis

#### Stolen Hours (Passive)
- **Effect:** Each time this unit uses a Fight ability, after that Fight ability has been resolved, **Heal (X)** this unit, where X is the number of damage points allocated to enemy units by combat attacks made as part of that Fight ability.

#### Sinister Judgement (Activated)
- **Timing:** Once Per Battle (Army), Your Hero Phase
- **Declare:** Pick a visible enemy unit within 18" of this unit to be the target.
- **Effect:** Roll a dice. On a 2+, the target is **judged** for the rest of the turn. While the target is **judged**, negative modifiers to its control score are doubled.

#### Fright Train (Activated)
- **Timing:** Any Charge Phase
- **Declare:** This unit can use this ability if it charged this phase. Pick each enemy unit within 1" of this unit to be the targets.
- **Effect:** Roll a dice. On a 2+, ignore positive modifiers to save rolls for the targets for the rest of the turn.

#### Cthonic Juggernaut (Activated)
- **Timing:** Any Charge Phase
- **Declare:** This unit can use this ability if it charged this phase.
- **Effect:** Roll a D3. On a 2+, add the roll to the Attacks characteristic of this unit's melee weapons for the rest of the turn.

#### Eyes Torn Open (Passive)
- **Effect:** You can re-roll hit rolls for this unit's combat attacks that target Order units. In addition, you can re-roll wound rolls for this unit's combat attacks that target Stormcast Etetrnals units.

#### Nihilistic Gourmand (Activated)
- **Timing:** Once Per Turn (Army), Any Hero Phase
- **Declare:** Pick a friendly Nighthaunt unit to be the target.
- **Effect:** For the rest of the turn, while the target is within this unit's combat range, each time a model in the target unit is slain, **Heal (1)** this unit.

#### Awful Resemblance (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a dice. On a 3+, for the rest of the turn:
• This unit cannot be picked to be the target of pile-in moves made by the target.
• Other than the **Companion** weapon ability, weapon abilities for attacks that target this unit have no effect.

#### Flaking Soul (Activated)
- **Timing:** End of Any Turn
- **Declare:** This unit must use this ability at the end of each turn.
- **Effect:** Roll a number of dice equal to this unit's Health characteristic. For each 6, allocate 1 damage point to this unit (ward rolls cannot be made for those damage points).

#### Bringer of False Hope (Passive)
- **Effect:** Visible friendly Nighthaunt units within 6" of this unit cannot be healed and slain models cannot be returned to those units.

#### Deluded Worshipper (Passive)
- **Effect:** Visible friendly Nighthaunt units within 6" of this unit cannot use commands.

#### Spectral Blades (Activated)
- **Timing:** Any Charge Phase
- **Declare:** If this unit charged this phase, pick an enemy unit within 1" of it to be the target.
- **Effect:** Roll a D3. On a 2+, inflict an amount of mortal damage on the target equal to the roll.

#### The Soul of a Coward (Activated)
- **Timing:** Your Movement Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a D3. On a 2+, this unit immediately use the 'Retreat' ability without any mortal damage being inflicted on it. Then, inflict an amount of mortal damage on the target equal to the roll.

#### Reckless Reaper (Activated)
- **Timing:** Your Charge Phase
- **Effect:** For the rest of the turn, add 1 to the number of dice rolled when making charge rolls for this unit, to a maximum of 3.

#### Devour Magic (Activated)
- **Timing:** Reaction: Your opponent declared a Spell ability
- **Effect:** If a friendly Nighthaunt unit wholly within 12" of rhis unit was picked to be the target of that spell, this unit can use the 'Unbind' ability as if it had Wizard (1). If the spell is unbound, **Heal (D3)** this unit.

#### It Tolls for Thee (Activated)
- **Timing:** Once Per Turn (Army), Enemy Movement Phase
- **Declare:** Pick a visible enemy unit within 18" of this unit to be the target.
- **Effect:** For the rest of the turn, each time the target is picked to use a Core ability, roll a dice as a reaction. On a 4+, inflict D3 mortal damage on the target.

#### Eternal Horror (Passive)
- **Effect:** Other than the **Companion** weapon ability, weapon abilities for attacks made by enemy units have no effect while they are in combat with this unit.

#### Chill Aura (Activated)
- **Timing:** Once Per Battle, Reaction: Opponent declared a charge ability for a unit within 6" of this unit
- **Effect:** Discard the highest dice from that charge roll.

#### Dread Harvester (Passive)
- **Effect:** Ward rolls cannot be made for damage points inflicted by this unit's combat attacks. This also affects **Companion** weapons.

### Paths

#### Justice of Death (Activated)
- **Timing:** Once Per Turn (Army), Any Hero Phase
- **Declare:** Pick an enemy unit within 18" of this unit to be an **accused** unit for the rest of the battle.
- **Effect:** The next time that **accused** unit uses a Core ability, inflict D3 mortal damage on it after that ability is resolved.

#### Marked for Capture (Activated)
- **Timing:** Once Per Turn (Army), Any Hero Phase
- **Declare:** Pick an enemy unit within 18" of this unit to be an **accused** unit for the rest of the battle.
- **Effect:** If that **accused** unit is destroyed while it is in combat with this unit, add 5 to this unit's control score for the rest of the battle.

#### Walking Nightmare (Activated)
- **Timing:** Once Per Battle, Enemy Hero Phase
- **Declare:** Pick an enemy unit that is in combat with this unit to be the target.
- **Effect:** The target must use a Retreat ability in the following movement phase if able.

#### Malignant Prosecutor (Passive)
- **Effect:** Enemy units in combat with this unit cannot use commands.

#### Shackled Gaoler (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy unit within 6" of this unit, and each **accused** enemy unit within 12" of this unit to be the targets.
- **Effect:** For each target, after it makes a pile-in move this phase, roll 2 D3s. For each 2+, inflict an amount of mortal damage on the target equal to the roll.

#### Grim Judge (Activated)
- **Timing:** Once Per Turn (Army), Any Hero Phase
- **Declare:** Pick an enemy unit in combat with this unit, and each **accused** enemy unit within 12" of this unit, to be the targets.
- **Effect:** Ward rolls cannot be made for the targets for the rest of the turn.

#### Pronounce Sentence (Activated)
- **Timing:** Once Per Battle (Army), Any Hero Phase
- **Effect:** For the rest of the turn, **accused** units' Path abilities are ignored.

#### Soul-splitting Wail (Activated)
- **Timing:** Once Per Battle, End of Any Turn
- **Effect:** Inflict **X** mortal damage on each enemy unit within 6" of this unit, where **X** is equal to the totalled negative modifiers to that enemy unit's control score.

#### Wrathful Scourging (Passive)
- **Effect:** Add 1 to run rolls and charge rolls for this unit.

#### Fresh from Nagashizzar (Passive)
- **Effect:** Ignore negative modifiers to this unit's control score.

#### Dutiful Pentients (Activated)
- **Timing:** Reaction: You declared a command for this unit
- **Effect:** Return 1 slain model to this unit.

#### Blades of Black Souls (Activated)
- **Timing:** Once Per Battle (Army), Any Combat Phase
- **Effect:** Add 1 to the Rend characteristic of this unit's weapons for the rest of the turn.

#### Spectral Lampreys (Passive)
- **Effect:** Subtract 1 from wound rolls for melee attacks that target this unit.

#### Dark Flames (Activated)
- **Timing:** End of Any Turn
- **Effect:** Roll a D3 for each enemy unit in combat with this unit. Allocate a number of damage points to that enemy unit equal to the roll (ward rolls cannot be made for those damage points).

#### Immortal Horrors (Passive)
- **Effect:** Subtract 5 from the control scores of enemy units while they are in combat with this unit.

### Battle Traits

#### Ethereal (Passive)
- **Effect:** Ignore negative modifiers to save rolls for friendly Nighthaunt units, excluding **Nagash**.

#### Ethereal Translocation (Command)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Cost:** 1
- **Declare:** Pick a friendly Nighthaunt unit that is not in combat to use this ability.
- **Effect:** Remove that unit from the battlefield and set it up again on the battlefield more than 9" from all enemy units.

#### Mounting Dread (Passive)
- **Effect:** Subtract the current battle round number from the control scores of enemy units while they are in combat with any friendly Nighthaunt units.

#### Sepulchral Apparitions (Activated)
- **Timing:** Once Per Battle (Army), Deployment Phase
- **Effect:** If there is a friendly **Nexus of Grief** on the battlefield, you can set up up to 2 additional **Nexuses of Grief** on the battlefield. Each must be set up wholly outside enemy territory, more than 3" from all objectives and other terrain features, and more than 12" from all other friendly **Nexuses of Grief**.
**Designer’s Note:** *The original* **Nexus of Grief** *is set up in Step 1 of the deployment phase using the ‘Deploy Faction Terrain’ ability. This ability allows you to set up a second and third such terrain feature and is used in Step 3 of the deployment phase, after armies have been deployed.*

### Artefacts of Power

#### Lightshard of the Harvest Moon (Activated)
- **Timing:** Once Per Battle, Any Combat Phase
- **Effect:** If this unit is in combat, add 1 to the Attacks characteristic of melee weapons used by friendly Nighthaunt units while they are wholly within 12" of this unit for the rest of the turn.

#### Mirror of Echoing Failures (Activated)
- **Timing:** Reaction: Opponent declared a Spell or Prayer ability for a Wizard or Priest within 12" of this unit.
- **Declare:** Pick each enemy Wizard and **Priest** within 12" of this unit to be the targets.
- **Effect:** Roll a D3. On a 2+, for the rest of the phase:
• Subtract 1 from casting rolls and chanting rolls for the targets.
• Each time any of the targets fails to cast a spell or chant a prayer, inflict 1 mortal damage on the target.

#### Brazier of Nagashizzar (Activated)
- **Timing:** Reaction: You declared the 'Redeploy' command for a friendly Nighthaunt Infantry unit wholly within 12" of this unit.
- **Effect:** If you roll a 1-3 when determining the distance that unit can move, you can use a value of 4 instead.

#### Stave of Suffering (Passive)
- **Effect:** While this unit is within 12" of any damaged enemy units or enemy units that have had any models slain, add 1 to casting rolls for friendly Nighthaunt units while they are wholly within 12" of this unit.

#### Tombstone of the Penitent (Passive)
- **Effect:** Add 1 to the Attacks characteristic of this unit’s melee weapons for each non-Faction Terrain terrain feature that is being contested by friendly units, to a maximum of 8.

#### Amulet of Waking Nightmares (Activated)
- **Timing:** Your Hero Phase
- **Declare:** Pick an enemy Hero within 18" of this unit to be the target.
- **Effect:** Roll a dice. On a 3+, for the rest of the turn, the target cannot be affected by the ‘Guarded Hero’ ability or the ‘Obscuring’ terrain ability.

### Battle Formations

#### Vanish and Reappear (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Declare:** Pick a friendly Nighthaunt unit that is in combat to use this ability.
- **Effect:** Roll a dice. The unit using this ability can move a distance up to its Move characteristic added to the roll. It can pass through other models and the combat ranges of enemy units, and it can end that move in combat.
- **Keywords:** Core, Move

#### There Is No Escape (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Declare:** Pick a friendly Nighthaunt unit to be the target.
- **Effect:** For the rest of the turn:
• The target unit can still use Charge and Shoot abilities even if it has used a Retreat ability in the same turn.
• The target can use the 'Retreat' ability without any mortal damage being inflicted on it.

#### Lethal Rancour (Activated)
- **Timing:** Once Per Turn (Army), Enemy Hero Phase
- **Declare:** Pick up to 2 friendly Nighthaunt units to be the targets.
- **Effect:** The targets' melee weapons have **Anti-charge (+1 Rend)** for the rest of the turn.

#### Petrify (Activated)
- **Timing:** Once Per Turn (Army), Enemy Hero Phase
- **Declare:** Pick a friendly Nighthaunt Hero to be the target.
- **Effect:** For the rest of the turn, enemy units cannot use Retreat abilities while they are in combat with the target.

#### Greedy for Souls (Activated)
- **Timing:** Once Per Turn (Army), End of Enemy Turn
- **Declare:** Pick a friendly **Nexus of Grief** to be the target.
- **Effect:** For the rest of the turn, the target has a Move characteristic of 3". It can immediately move up to 3" and can move into combat but cannot end that move on a terrain feature or within 3" of an objective.

#### Blades of Eternal Rust (Activated)
- **Timing:** Once Per Turn (Army), Enemy Hero Phase
- **Declare:** Pick an enemy unit that is in combat with any friendly Nighthaunt units to be the target.
- **Effect:** Subtract 1 from save rolls for the target for the rest of the turn.

### Heroic Traits

#### Deathly Possessor (Activated)
- **Timing:** Any Combat Phase
- **Declare:** Pick an enemy Hero in combat with this unit to be the target, then pick another enemy unit within the target's combat range to be the victim.
- **Effect:** Roll a dice. On a 3+, pick 1 of the target's melee weapons. Immediately resolve combat attacks for the target with that weapon against the victim.

#### Ruler of the Spectral Hosts (Activated)
- **Timing:** Once Per Turn (Army), Your Hero Phase
- **Declare:** Pick up to 3 friendly non-Hero Nighthaunt units wholly within 12" of this unit to be the targets.
- **Effect:** Roll a D3 for each target. On a 2+:
• If the target is damaged, **Heal (X)** the target, where **X** is an amount equal to the roll.
• If the target is not damaged, return a number of slain models to it with a combined Health characteristic of up to X, where X is an amount equal to the roll.

#### Shadowy Aura (Activated)
- **Timing:** Reaction: You declared the 'Redeploy' command for a friendly Nighthaunt unit wholly within 12" of this unit
- **Effect:** For the rest of the turn, subtract 1 from hit rolls for attacks that target this unit.

## Subfactions

### Legions of Nagash [LEGENDS]

### The Clattering Procession

**Battle Traits**

#### The Speed of Death (Activated)
- **Timing:** Once Per Turn (Army), End of Your Turn
- **Declare:** Pick a friendly Clattering Procession unit that used a Charge or Retreat ability this turn, is not in combat, and is wholly within 9" of a battlefield edge to use this ability.
- **Effect:** Remove that unit from the battlefield and set it up again wholly within 9" of a battlefield edge and more than 3" from all enemy units.

#### Ethereal (Passive)
- **Effect:** Ignore negative modifiers to save rolls for friendly Nighthaunt units.

#### Spectral Swiftness (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Declare:** Pick a friendly Clattering Procession unit that is in combat to use this ability.
- **Effect:** That unit can move a distance up to its Move characteristic. It can pass through other models and the combat ranges of enemy units, but it cannot end that move in combat.
- **Keywords:** Core, Move, Retreat

#### Pitiless Reapers (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Declare:** Pick a friendly Clattering Procession unit that passed across any enemy units this phase to use this ability, then pick 1 of those enemy units to be the target.
- **Effect:** Roll a dice for each model in the Clattering Procession unit that passed across the target. Add 1 to the roll if any friendly **Clattering Procession Black Coaches** passed across the target this phase. For each 6+, inflict 1 mortal damage on the target.

**Artefacts of Power**

#### Talisman of the Nadir (Activated)
- **Timing:** Reaction: Opponent declared a Spell ability
- **Effect:** If a friendly Clattering Procession unit wholly within 12" of this unit was picked to be the target of that spell, this unit can use the 'Unbind' ability as if it had Wizard (1). Add 1 to the unbinding roll for that ability.

**Heroic Traits**

#### Silent Overseer (Activated)
- **Timing:** Your Charge Phase
- **Declare:** Pick up to 3 friendly Clattering Procession units wholly within 12" of this unit to be the targets.
- **Effect:** For the rest of the turn, the targets can use Charge abilities even if they used a Retreat ability in the same turn.

### The Eternal Nightmare

**Battle Traits**

#### Ethereal (Passive)
- **Effect:** Ignore negative modifiers to save rolls for friendly Nighthaunt units, excluding **Nagash**.

#### Gather Power (Passive)
- **Effect:** The following effects apply to the friendly **Nagash** during the first and second battle rounds, as shown:
**Battle Round 1:**
• His move characteristic is '-' and he cannot make pile-in moves.
• He has Ward (3+).
• He has a Control characteristic of 1.
• **Alakanash** and **Zefet-nebtar** each have an Attacks characteristic of 2 instead of 4.
**Battle Round 2:**
• His Move characteristic is 6".
• He cannot end a move more than 6" from the **ritual site**.
• He has Ward (4+).
• He has a Control characteristic of 5.
• **Alakanash** and **Zefet-nebtar** each have an Attacks characteristic of 2 instead of 4.

#### The Ritual Begins (Activated)
- **Timing:** Deployment Phase
- **Declare:** You must use this ability to send up the friendly Nagash. Pick a friendly **Nexus of Grief** wholly within friendly territory to be the **ritual site**.
- **Effect:** Set up the friendly **Nagash** within 1" of the **ritual site** and wholly within friendly territory. After you have done so, he has been **deployed**. That **Nexus of Grief** is the **ritual site** for the rest of the battle.
- **Keywords:** Deploy

#### Unholy Vessel (Passive)
- **Effect:** The **ritual site** has Ward (4+) and cannot use the 'Hungry Crypts' ability.

**Artefacts of Power**

#### The Seal of Nagash (Activated)
- **Timing:** Once Per Battle, Your Hero Phase
- **Effect:** Allocate 3 damage points to this unit (ward rolls cannot be made for those damage points). Then, **Heal (3)** the friendly **Nagash**.

**Heroic Traits**

#### Damned Vessel (Activated)
- **Timing:** Reaction: You declared a Spell ability for a friendly **Nagash** within 18" of this unit
- **Effect:** Measure the range and visibility of that Spell ability from this unit instead of that **Nagash**.

## Units

### Awlrach the Drowner
**Points:** 130 | **Move:** 10" | **Health:** 7 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, DEATH, UNIQUE, FLY, NIGHTHAUNT, WAR MACHINE, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Deathwood Oar | Melee | - | 4 | 4+ | 3+ | - | D3 | Crit (Auto-wound) |

**Abilities:**

#### Drifting on Spectral Tides (Activated)
- **Timing:** End of Any Turn
- **Effect:** Roll a dice. On a 3+, this unit can immediately use the 'Normal Move' or the 'Retreat' ability as if it were your movement phase.

#### Passage Through the Underworlds (Activated)
- **Timing:** Once Per Battle, Your Movement Phase
- **Declare:** If this unit is not in combat, pick a friendly non-Hero Nighthaunt unit that is not in combat and is wholly within 12" of this unit to be the target.
- **Effect:** Remove this unit and the target from the battlefield, then set them up again on the battlefield wholly within 6" of each other and more than 7" from all enemy units.

---

### Black Coach
**Points:** 220 | **Move:** 10" | **Health:** 12 | **Save:** 5+ | **Control:** 5
**Keywords:** DEATH, FLY, NIGHTHAUNT, WAR MACHINE, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Soulreach Grasp | Ranged | 10" | 2 | 4+ | 3+ | 2 | 2 | Crit (Auto-wound), Shoot In Combat |
| Wraith's Spectral Weapons | Melee | - | 5 | 3+ | 3+ | 1 | 2 | Anti-Infantry (+1 Rend), Crit (Auto-wound) |
| Relic Bearers’ Claws and Ethereal Steeds’ Hooves | Melee | - | 14 | 4+ | 4+ | - | 1 | Crit (Auto-wound), Companion |

**Abilities:**

#### Runaway Coach (Activated)
- **Timing:** Any Charge Phase
- **Declare:** If this unit charged this phase, pick an enemy unit within 1" of it to be the target.
- **Effect:** Roll a D3. On a 2+, inflict an amount of mortal damage equal on the target equal to the roll.

#### Nimbus of Power (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Effect:** If this unit is not in combat, remove it from the battlefield and set it up again on the battlefield more than 9" from all enemy units.
- **Keywords:** Core

#### Evocation of Death (Activated)
- **Timing:** End of Any Turn
- **Declare:** If any damage points were allocated to an enemy unit by this unit's combat attacks this turn and that enemy unit was destroyed this turn, pick a friendly non-Unique Nighthaunt Hero that has been destroyed.
- **Effect:** Set up a replacement unit wholly within 12" of this unit. The replacement unit can only be set up in combat with enemy units that are in combat with this unit.

---

### Black Coach (Scourge of Ghyran)
**Points:** 290 | **Move:** 10" | **Health:** 12 | **Save:** 5+ | **Control:** 5
**Keywords:** DEATH, FLY, NIGHTHAUNT, WAR MACHINE, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Wraiths' Spectral Weapons | Melee | - | 5 | 3+ | 3+ | 2 | 2 | Charge (+1 Damage), Crit (Auto-wound) |
| Black Coach's Iron-shod Wheels | Melee | - | 8 | 4+ | 3+ | 2 | 2 | Crit (Auto-wound), Companion |

**Abilities:**

#### Spectral Carriage (Activated)
- **Timing:** Your Charge Phase
- **Effect:** For the rest of the phase, add 1 to the number of dice rolled when making charge rolls for this unit, to a maximum of 3.

#### One Stop, No Return (Activated)
- **Timing:** Once Per Turn (Army), Any Hero Phase
- **Declare:** Pick an objective you do not control to be the target.
- **Effect:** For the rest of the turn, while enemy units are contesting the target objective, they cannot score critical hits while this unit is also contesting it (hit rolls that would be critical hits score hits instead).

#### Evocation of Death (Activated)
- **Timing:** Once Per Turn (Army), Any Hero Phase
- **Effect:** Pick 1 of the following to apply for the rest of the turn:
***Noble Blade:*** This unit’s melee weapons, including **Companion weapons**, have **Crit (Mortal)** instead of **Crit (Auto-wound)**.
***Unholy Grail:*** Pick a friendly Nighthaunt unit wholly within 12" of this unit. Each time that unit uses the ‘Rally’ command, you receive 3 additional rally points.
***Tome of Undead Lore:*** This unit has Wizard (1).

---

### Bladegheist Revenants
**Points:** 210 | **Move:** 8" | **Health:** 2 | **Save:** 6+ | **Control:** 1
**Keywords:** INFANTRY, DEATH, FLY, NIGHTHAUNT, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Tomb Greatblade | Melee | - | 2 | 3+ | 3+ | 1 | 1 | Crit (Auto-wound) |

**Abilities:**

#### Thrashing Desperation (Passive)
- **Effect:** While this unit contests an objective you control, enemy units cannot finish a charge move within 1/2" of a model in this unit unless the unmodified charge roll was 7 or more.

---

### Cairn Wraith
**Points:** 130 | **Move:** 8" | **Health:** 5 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, DEATH, FLY, NIGHTHAUNT, INFANTRY, WARD (5+), Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Cairnoch Scythe | Melee | - | 5 | 4+ | 3+ | 2 | 2 | Crit (Auto-wound) |

**Abilities:**

#### Eager Death-Dealers (Activated)
- **Timing:** Once Per Battle (Army), Any Combat Phase
- **Declare:** Pick a visible friendly **Grimghast Reapers** unit wholly within 12" of this unit to be the target.
- **Effect:** Add 1 to wound rolls for the target’s combat attacks for the rest of the turn.

---

### Chainghasts
**Points:** 90 | **Move:** 8" | **Health:** 3 | **Save:** 6+ | **Control:** 2
**Keywords:** INFANTRY, DEATH, CHAMPION, FLY, NIGHTHAUNT, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Ghastflails | Melee | - | 2 | 4+ | 3+ | 2 | 1 | Crit (Auto-wound) |
| Ghastflails | Ranged | 12" | 2 | 4+ | 3+ | 2 | 1 | Crit (Auto-wound) |

**Abilities:**

#### Another Link in the Chain (Activated)
- **Timing:** Any Combat Phase
- **Effect:** Roll a dice. Add 1 to the roll if this unit is wholly within 12" of any friendly **Spirit Torments**. On a 3+, for the rest of the turn, add 1 to hit rolls for combat attacks made by friendly Nighthaunt units while they are wholly within 12" of this unit.

---

### Chainrasps
**Points:** 100 | **Move:** 8" | **Health:** 1 | **Save:** 6+ | **Control:** 1
**Keywords:** INFANTRY, DEATH, CHAMPION, FLY, NIGHTHAUNT, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Malignant Weapon | Melee | - | 2 | 4+ | 5+ | - | 1 | Crit (Auto-wound) |
| Malignant Weapon | Melee | - | 2 | 4+ | 5+ | - | 1 | Crit (Auto-wound) |

**Abilities:**

#### Chilling Horde (Activated)
- **Timing:** Once Per Battle (Army), Deployment Phase
- **Declare:** Pick up to 3 friendly **Chainrasp** units to be the targets,
- **Effect:** Each target can move 6" but cannot end that move in combat.

---

### Craventhrone Guard
**Points:** 100 | **Move:** 8" | **Health:** 2 | **Save:** 6+ | **Control:** 1
**Keywords:** INFANTRY, DEATH, CHAMPION, FLY, NIGHTHAUNT, STANDARD BEARER (1/5), WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Wicked Sidearm | Melee | - | 1 | 4+ | 4+ | - | 1 | Crit (Auto-wound) |
| Soulhunter’s Crossbow | Ranged | 16" | 2 | 4+ | 3+ | 1 | 1 | Crit (Auto-wound) |

**Abilities:**

#### Black-hearted Lackeys (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a D3. On a 2+, this unit immediately uses the 'Retreat' ability without any mortal damage being inflicted on it. Then, inflict an amount of mortal damage on the target equal to the roll.

---

### Dreadblade Harrows
**Points:** 140 | **Move:** 12" | **Health:** 3 | **Save:** 6+ | **Control:** 1
**Keywords:** DEATH, FLY, NIGHTHAUNT, CAVALRY, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Dreadblade | Melee | - | 4 | 4+ | 3+ | 1 | 2 | Crit (Auto-wound) |
| Ethereal Steed’s Ghostly Hooves and Teeth | Melee | - | 3 | 5+ | 3+ | - | 1 | Crit (Auto-wound), Companion |

**Abilities:**

#### Curse of Loyalty (Passive)
- **Effect:** If this unit charged this turn, add 3 to the control scores of friendly Nighthaunt units while they are wholly within 12" of this unit.

#### Phantasmal Discorporation (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Effect:** Remove this unit from the battlefield and set it up again on the battlefield more than 9" from all enemy units.
- **Keywords:** Core

---

### Dreadscythe Harridans
**Points:** 200 | **Move:** 8" | **Health:** 2 | **Save:** 6+ | **Control:** 1
**Keywords:** INFANTRY, DEATH, FLY, NIGHTHAUNT, CHAMPION, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Scythed Limbs | Melee | - | 3 | 4+ | 4+ | 1 | 1 | Crit (Auto-wound) |

**Abilities:**

#### Harrowing Shrieks (Passive)
- **Effect:** Enemy units cannot be healed or have slain models returned to them while they are in combat with this unit.

---

### Glaivewraith Stalkers
**Points:** 100 | **Move:** 8" | **Health:** 2 | **Save:** 6+ | **Control:** 1
**Keywords:** INFANTRY, DEATH, FLY, NIGHTHAUNT, MUSICIAN (1/4), WARD (5+), Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Hunter’s Glaive | Melee | - | 2 | 4+ | 3+ | 1 | 1 | Charge (+1 Damage), Crit (Auto-wound) |

**Abilities:**

#### Unerring Hunters (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Effect:** For the rest of the turn, each time this unit makes a pile-in move, add 3" to the distance it can move.

---

### Grimghast Reapers
**Points:** 200 | **Move:** 8" | **Health:** 2 | **Save:** 6+ | **Control:** 1
**Keywords:** INFANTRY, DEATH, CHAMPION, FLY, NIGHTHAUNT, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Slasher Scythe | Melee | - | 2 | 4+ | 3+ | 1 | 1 | Anti-Infantry (+1 Rend), Crit (Auto-wound) |

**Abilities:**

#### Reaped Like Corn (Activated)
- **Timing:** Any Combat Phase
- **Declare:** Pick an enemy unit that is within the combat ranges of 5 or more models in this unit to be the target.
- **Effect:** Roll a number of dice equal to the number of models in the target unit. For each 6, inflict 1 mortal damage on the target. If the target unit has 20 or more models, inflict 1 mortal damage on the target for each 5+ instead.

---

### Guardian of Souls
**Points:** 110 | **Move:** 8" | **Health:** 5 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, DEATH, FLY, NIGHTHAUNT, INFANTRY, WIZARD (1), WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Chill Blade | Melee | - | 3 | 4+ | 3+ | 1 | 2 | Crit (Auto-wound) |

**Abilities:**

#### Nightmare Lantern (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick a visible friendly Nighthaunt unit wholly within 12" of this unit to be the target.
- **Effect:** Roll a dice. On a 3+, add 1 to wound rolls for the target's attacks for the rest of the turn.

---

### Hexwraiths
**Points:** 150 | **Move:** 12" | **Health:** 3 | **Save:** 6+ | **Control:** 1
**Keywords:** DEATH, CHAMPION, FLY, NIGHTHAUNT, CAVALRY, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Ethereal Steed’s Ghostly Hooves and Teeth | Melee | - | 2 | 5+ | 3+ | - | 1 | Crit (Auto-wound), Companion |
| Spectral Scythe | Melee | - | 2 | 4+ | 4+ | 1 | 1 | Charge (+1 Damage), Crit (Auto-wound) |

**Abilities:**

#### Spectral Hunters (Activated)
- **Timing:** Any Charge Phase
- **Declare:** If this unit charged this phase, pick an enemy unit within 1" of it to be the target.
- **Effect:** Roll a D3. On a 2+, inflict an amount of mortal damage on the target equal to the roll.

---

### Knight of Shrouds
**Points:** 100 | **Move:** 8" | **Health:** 5 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, DEATH, FLY, NIGHTHAUNT, INFANTRY, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Sword of Stolen Hours | Melee | - | 5 | 4+ | 3+ | 1 | 2 | Crit (Auto-wound) |

**Abilities:**

#### Stolen Hours (Passive)
- **Effect:** Each time this unit uses a Fight ability, after that Fight ability has been resolved, **Heal (X)** this unit, where X is the number of damage points allocated to enemy units by combat attacks made as part of that Fight ability.

#### Infantry Overseer (Activated)
- **Timing:** Reaction: You declared a Fight ability for this unit
- **Effect:** Pick a friendly non-Hero Nighthaunt Infantry unit that has not used a Fight ability this turn and is within this unit’s combat range to be the target. The target can be picked to use a Fight ability immediately after the Fight ability used by this unit has been resolved. If it is picked to do so, add 1 to hit rolls for the target’s attacks for the rest of the turn.

---

### Knight of Shrouds on Ethereal Steed
**Points:** 160 | **Move:** 12" | **Health:** 7 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, DEATH, FLY, NIGHTHAUNT, CAVALRY, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Ethereal Steed’s Ghostly Hooves and Teeth | Melee | - | 3 | 5+ | 3+ | - | 1 | Crit (Auto-wound), Companion |
| Sword of Stolen Hours | Melee | - | 5 | 4+ | 3+ | 1 | 2 | Crit (Auto-wound) |

**Abilities:**

#### Stolen Hours (Passive)
- **Effect:** Each time this unit uses a Fight ability, after that Fight ability has been resolved, **Heal (X)** this unit, where X is the number of damage points allocated to enemy units by combat attacks made as part of that Fight ability.

#### Cavalry Overseer (Activated)
- **Timing:** Reaction: You declared a Fight ability for this unit
- **Effect:** Pick a friendly non-Hero Nighthaunt Cavalry or War Machine unit that has not used a Fight ability this turn and is within this unit’s combat range to be the target. The target can be picked to use a Fight ability immediately after the Fight ability used by this unit has been resolved. If it is picked to do so, add 1 to hit rolls for the target’s attacks for the rest of the turn.

---

### Krulghast Cruciator
**Points:** 150 | **Move:** 8" | **Health:** 6 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, DEATH, FLY, NIGHTHAUNT, INFANTRY, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Talons and Flensing Knives | Melee | - | 4 | 4+ | 3+ | 1 | 2 | Crit (Auto-wound) |
| Phantasmal Torture | Ranged | 12" | 4 | 3+ | 3+ | 2 | 1 | Crit (Auto-wound), Shoot in Combat |

**Abilities:**

#### Empowered Through Excruciation (Passive)
- **Effect:** Ignore the first damage point that would be allocated to each friendly Nighthaunt unit wholly within 12" of this unit in each phase.

---

### Kurdoss Valentian, the Craven King
**Points:** 160 | **Move:** 8" | **Health:** 7 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, DEATH, UNIQUE, FLY, NIGHTHAUNT, INFANTRY, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Sepulchral Sceptre | Melee | - | 5 | 3+ | 3+ | 2 | 3 | Crit (Auto-wound) |

**Abilities:**

#### Hail Kurdoss! King of Nothing! (Activated)
- **Timing:** Your Charge Phase
- **Declare:** If this unit charged this phase, pick a friendly **Craventhrone Guard** unit that is not in combat to be the target.
- **Effect:** The target can immediately use a Shoot ability as if it were your shooting phase, but all of its attacks must target an enemy unit in combat with this unit. The target cannot use Charge abilities for the rest of the turn.

#### If I Cannot Rule, None Shall Rule! (Activated)
- **Timing:** Once Per Turn, Reaction: Opponent declared a command for a unit within 12" of this unit
- **Effect:** Roll a dice. On a 3+, unless your opponent spends 1 additional command point to use that command, the command has no effect, it still counts as having been used and the command points spent to use the command are still lost.

---

### Kurdoss Valentian, the Craven King (Scourge of Ghyran)
**Points:** 170 | **Move:** 8" | **Health:** 7 | **Save:** 5+ | **Control:** 5
**Keywords:** HERO, DEATH, UNIQUE, FLY, NIGHTHAUNT, INFANTRY, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Sepulchral Sceptre | Melee | - | 5 | 3+ | 3+ | 2 | 3 | Crit (Auto-wound) |

**Abilities:**

#### A Traitor's Legacy (Activated)
- **Timing:** Any Combat Phase
- **Declare:** If this unit is in combat, pick a visible friendly Nighthaunt unit wholly within 12" of this unit to be the target.
- **Effect:** Pick 1 of the following to apply for the rest of the turn:
***Regicide and Kinslaying:*** The target’s melee weapons have **Anti‑Hero (+1 Rend)**.
***Usurpation and Larceny:*** Add 5 to the target’s control score.
***Sedition and Betrayal:*** Enemy units cannot use commands while they
are in combat with the target.

#### Inescapable Mockery (Passive)
- **Effect:** Add 1 to hit rolls and wound rolls for this unit’s combat attacks that target an enemy unit that has a lower Control characteristic than this unit.

---

### Lady Olynder, Mortarch of Grief
**Points:** 320 | **Move:** 8" | **Health:** 7 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, DEATH, UNIQUE, FLY, WARMASTER, NIGHTHAUNT, WIZARD (2), INFANTRY, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Staff of Midnight | Melee | - | 6 | 4+ | 3+ | 1 | D3 | Crit (Auto-wound) |
| Horrifying Visage | Ranged | 12" | 1 | 2+ | 2+ | 3 | D6 | Crit (Auto-wound), Shoot in Combat |

**Abilities:**

#### No Rest for the Wicked (Activated)
- **Timing:** Once Per Battle, Your Hero Phase
- **Declare:** Pick any number of friendly Nighthaunt units on the battlefield to be the targets.
- **Effect:** For each target, you can return a number of slain models to that unit with a combined Health characteristic of up to D3+3.

#### Mortarch of Grief (Passive)
- **Effect:** If the unmodified hit roll or unmodified wound roll for an attack that targets this unit is 1-4, the attack fails and the attack sequence ends.

#### Grief-stricken (Spell)
- **Timing:** Your Hero Phase
- **Casting Value:** 6
- **Declare:** Pick a visible enemy unit within 18" of this unit to be the target, then make a casting roll of 2D6.
- **Effect:** Ignore positive modifiers to the target's control score and to hit rolls, wound rolls and save rolls for the target for the rest of the turn.
- **Keywords:** Spell

---

### Lord Executioner
**Points:** 170 | **Move:** 8" | **Health:** 5 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, DEATH, FLY, NIGHTHAUNT, INFANTRY, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Decapitating Greataxe | Melee | - | 5 | 3+ | 3+ | 2 | 2 | Anti-Hero (+1 Rend), Crit (Auto-wound) |

**Abilities:**

#### Ghastly Descent (Activated)
- **Timing:** End of Any Turn
- **Declare:** Pick an enemy Infantry or **Cavalry Hero** that had any damage points allocated to it this turn by this unit's attacks to be the target.
- **Effect:** Roll a dice. Add 1 to the roll if the target is a Sentenced unit. On a 5+, the target is automatically destroyed.

#### Staring Death in the Face (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a dice. On a 2+, subtract 1 from the Attacks characteristic of the target’s melee weapons for the rest of the turn. In addition, if the target is a Hero, subtract 5 from the target’s control score for the rest of the turn.

---

### Lord Vitriolic
**Points:** 120 | **Move:** 8" | **Health:** 5 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, DEATH, FLY, NIGHTHAUNT, INFANTRY, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Hurled Vial | Ranged | 10" | 1 | 3+ | 3+ | 2 | D6 | Crit (Auto-wound) |
| Culler's Sickle | Melee | - | 3 | 4+ | 3+ | 1 | D3 | Crit (Auto-wound) |

**Abilities:**

#### Spectral Alchemy (Activated)
- **Timing:** Once Per Turn (Army), Your Shooting Phase
- **Declare:** Pick a visible enemy unit within 10" of this unit to be the target.
- **Effect:** Roll a dice. On a 3+, pick 1 of the following effects or roll two dice and apply both corresponding effects in an order of your choosing, including duplicate effects:
**1 *Phantasmal Solvent:*** If the target has a Move characteristic of '-', inflict 6 mortal damage on the target.
**2 *Acidic Fug:*** Inflict D3 mortal damage on the target.
**3 *Fling Concoctions:*** Add 1 to the Attacks characteristic of this unit's **Hurled Vial** this phase but all attacks made by this unit this phase must target that enemy unit.
**4 *Choking Vapours:*** If the target is a Wizard, subtract 1 from its power level until the start of your next turn.
**5 *Fear-laced Hallucinogen:*** If the target is a Priest, remove D3 ritual points from it.
**6 *Unholy Prescription:*** Ward rolls cannot be made for the target for the rest of the turn.

---

### Myrmourn Banshees
**Points:** 100 | **Move:** 8" | **Health:** 2 | **Save:** 6+ | **Control:** 1
**Keywords:** INFANTRY, DEATH, FLY, NIGHTHAUNT, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Chill Dagger | Melee | - | 2 | 4+ | 3+ | 1 | D3 | Anti-Wizard (+1 Rend), Crit (Auto-wound) |

**Abilities:**

#### Spell-eaters (Activated)
- **Timing:** Reaction: Opponent declared a Spell ability
- **Effect:** If a friendly Nighthaunt unit wholly within 12" of this unit was picked to be the target of that spell, this unit can use the ‘Unbind’ ability as if it had Wizard (1). Add 1 to the unbinding roll for that ability. If the spell is unbound, inflict D3 mortal damage on the caster

---

### Nagash, Supreme Lord of the Undead
**Points:** 840 | **Move:** 10" | **Health:** 18 | **Save:** 3+ | **Control:** 10
**Keywords:** DEATH, UNIQUE, HERO, MONSTER, WIZARD (9), WARD (5+), FLY, WARMASTER, NIGHTHAUNT

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Alakanash, the Staff of Power | Melee | - | 4 | 3+ | 3+ | 2 | D6 | - |
| Zefet-nebtar, the Mortis Blade | Melee | - | 4 | 3+ | 3+ | 2 | 3 | - |

**Abilities:**

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, subtract 3 from its power level.

#### The Staff of Power (Passive)
- **Effect:** Add 2 to casting rolls for this unit while it has not miscast any spells this turn. If this unit miscasts a spell, ignore the restriction that would stop this unit from casting any more spells this turn.

#### Hand of Dust (Activated)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Declare:** Pick a visible enemy Hero or Monster in combat with this unit to be the target.
- **Effect:** Hide a dice in one of your hands or under one of two appropriate containers. Your opponent must pick one of your hands or containers. If they pick the one hiding the dice, this ability has no effect. If they pick the empty one, the target is automatically destroyed.
- **Keywords:** Rampage

#### Supreme Lord of the Undead (Activated)
- **Timing:** Once Per Battle, Your Hero Phase
- **Declare:** Pick a friendly non-Hero non-Unique Death unit that has been destroyed to be the target.
- **Effect:** Set up a replacement unit identical to the target wholly within 12" of this unit and more than 9" from all enemy units.

#### Invocation of Nagash (Spell)
- **Timing:** Your Hero Phase
- **Casting Value:** 7
- **Declare:** This unit can cast this spell more than once per phase. Pick a visible unit wholly within 18" of this unit that has not been picked to be the target of this spell this turn to be the target, then make a casting roll of 2D6.
- **Effect:** If the target is an enemy unit, inflict D3 mortal damage on it. If the target is a friendly Death unit, pick 1 of the following effects:
• Return a number of slain models to the target unit with a combined Health characteristic of up to 3.
• If the target was not set up this turn and is not in combat, it can move up to 3". It cannot move into combat.
- **Keywords:** Spell

---

### Pyregheists
**Points:** 180 | **Move:** 8" | **Health:** 2 | **Save:** 6+ | **Control:** 1
**Keywords:** INFANTRY, DEATH, CHAMPION (1/8), FLY, NIGHTHAUNT, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Flaming Weapons | Melee | - | 2 | 4+ | 4+ | - | 1 | Crit (Auto-wound) |
| Gout of Balefire | Ranged | 10" | 2 | 2+ | 4+ | 1 | 1 | Crit (Auto-wound), Shoot in Combat |

**Abilities:**

#### Light a Pyre (Activated)
- **Timing:** Once Per Turn (army), End of Any Turn
- **Declare:** If this unit is contesting an
objective, pick a visible enemy unit within
6" of this unit to be the target.
- **Effect:** Roll a D3. On a 2+:
• Inflict an amount of mortal damage on the target equal to the roll.
• Add a number equal to the roll to this unit’s control score for the rest of the turn.

---

### Reikenor the Grimhailer
**Points:** 200 | **Move:** 12" | **Health:** 7 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, DEATH, UNIQUE, FLY, NIGHTHAUNT, CAVALRY, WIZARD (1), WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Fellreaper | Melee | - | 5 | 3+ | 3+ | 2 | 2 | Charge (+1 Damage), Crit (Auto-wound) |
| Kyllaron’s Ghostly Hooves and Teeth | Melee | - | 3 | 5+ | 3+ | - | 1 | Crit (Auto-wound), Companion |

**Abilities:**

#### Corpse Candles (Activated)
- **Timing:** Any Hero Phase
- **Declare:** Pick either this unit or a visible enemy unit within 12" of this unit to be the target.
- **Effect:** If you picked this unit, allocate 1 damage point to this unit and add 1 to casting rolls for this unit for the rest of the turn (ward rolls cannot be made for that damage point). 
If you picked an enemy unit, roll a dice. On a 3+, inflict 1 mortal damage on that enemy unit and for the rest of the turn:
• Add 1 to casting rolls for this unit.
• Add 1 to this unit’s power level.

#### Grim Justice (Passive)
- **Effect:** Add 1 to hit rolls and wound rolls for attacks made with this unit’s **Fellreaper** if the target is a Priest or a Wizard.

#### Wraithstorm (Spell)
- **Timing:** Your Hero Phase
- **Casting Value:** 7
- **Declare:** Pick a visible enemy unit within 12" of this unit to be the target, then make a casting roll of 2D6.
- **Effect:** Inflict D3 mortal damage on the target. If any models are slain as a result of this ability, inflict a further D3 mortal damage on the target
- **Keywords:** Spell

---

### Scriptor Mortis
**Points:** 120 | **Move:** 8" | **Health:** 5 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, DEATH, FLY, NIGHTHAUNT, INFANTRY, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Malicious Quill | Melee | - | 3 | 4+ | 3+ | - | 1 | Crit (Auto-wound) |

**Abilities:**

#### Sentenced to Eternal Torment (Activated)
- **Timing:** Once Per Turn (Army), Your Hero Phase
- **Declare:** Pick a visible enemy unit within 18" of this unit to be the target.
- **Effect:** Roll a dice. On a 3+, inflict 1 mortal damage on the target and the target has the Sentenced keyword until the start of your next turn.

#### Judge, Jury and Executioner (Passive)
- **Effect:** Each time a friendly Nighthaunt Infantry unit uses a Fight ability, if all of its attacks target the same Sentenced enemy unit, that friendly unit's melee weapons have **Crit (Mortal)** for that Fight ability.

---

### Spirit Hosts
**Points:** 110 | **Move:** 8" | **Health:** 3 | **Save:** 6+ | **Control:** 1
**Keywords:** INFANTRY, DEATH, FLY, NIGHTHAUNT, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Spectral Claws and Daggers | Melee | - | 6 | 4+ | 4+ | - | 1 | Crit (Auto-wound) |

**Abilities:**

#### Drawn to War (Passive)
- **Effect:** Friendly Nighthaunt Infantry Heroes have Ward (4+) while they are wholly within this unit’s combat range.

---

### Spirit Torment
**Points:** 110 | **Move:** 8" | **Health:** 6 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, DEATH, FLY, NIGHTHAUNT, INFANTRY, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Shacklegheist Chains | Melee | - | 5 | 4+ | 3+ | 1 | 2 | Crit (Auto-wound) |

**Abilities:**

#### Captured Soul Energy (Passive)
- **Effect:** Each time a friendly Nighthaunt unit wholly within 12" of this unit uses the ‘Rally’ command, you can make 3 additional rally rolls of D6.

---

### The Headsmen's Curse
**Points:** 150 | **Move:** 8" | **Health:** 3 | **Save:** 5+ | **Control:** 1
**Keywords:** INFANTRY, UNIQUE, Legends, CHAMPION, DEATH, NIGHTHAUNT, FLY, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Terminus | Melee | - | 3 | 3+ | 3+ | 2 | D6 | Crit (Auto-wound) |
| Tool of Judgement | Melee | - | 3 | 4+ | 4+ | - | 1 | - |
| Tool of Judgement | Melee | - | 3 | 4+ | 4+ | - | 1 | - |
| Tool of Judgement | Melee | - | 3 | 4+ | 4+ | - | 1 | - |

**Abilities:**

#### Furious Indictment (Activated)
- **Timing:** Your Hero Phase
- **Declare:** Pick an enemy unit within 12" of this unit to be the target.
- **Effect:** Roll a dice. On a 3+, the target is **condemned** until the start of your next turn.
While a unit is **condemned**:
• Halve that unit’s Move characteristic.
• That unit cannot use Retreat abilities.
• Subtract 1 from the number of dice rolled when making charge rolls for that unit, to a minimum of 1.

---

### Thorns of the Briar Queen
**Points:** 140 | **Move:** 8" | **Health:** 2 | **Save:** 5+ | **Control:** 1
**Keywords:** INFANTRY, UNIQUE, Legends, DEATH, NIGHTHAUNT, FLY, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Malignant Weapon | Melee | - | 2 | 4+ | 5+ | - | 1 | Crit (Auto-wound) |
| Piercing Scream | Ranged | 12" | D3 | 4+ | 3+ | 2 | D3 | Shoot in Combat |
| Ancient Chill Dagger | Melee | - | 3 | 4+ | 3+ | 2 | D3 | Crit (Auto-wound) |
| Malignant Weapon | Melee | - | 2 | 4+ | 5+ | - | 1 | Crit (Auto-wound) |
| Malignant Weapon | Melee | - | 2 | 4+ | 5+ | - | 1 | Crit (Auto-wound) |
| Malignant Weapon | Melee | - | 2 | 4+ | 5+ | - | 1 | Crit (Auto-wound) |
| Malignant Weapon | Melee | - | 2 | 4+ | 5+ | - | 1 | Crit (Auto-wound) |
| Malignant Weapon | Melee | - | 2 | 4+ | 5+ | - | 1 | Crit (Auto-wound) |

**Abilities:**

#### Ghostly Howl (Activated)
- **Timing:** Once Per Turn (Army), Any Shooting Phase
- **Declare:** Pick an enemy unit that had any damage points allocated to it this turn by attacks made with this unit’s **Piercing Scream** to be the target.
- **Effect:** Roll a dice. On a 2+, the target cannot use commands until the start of your next turn.

#### The Briar Queen (Passive)
- **Effect:** While this unit’s **Briar Queen** is on the battlefield, it has Wizard (1).

---

### Tomb Banshee
**Points:** 130 | **Move:** 8" | **Health:** 5 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, DEATH, FLY, NIGHTHAUNT, INFANTRY, WARD (5+), Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Piercing Scream | Ranged | 12" | 2 | 4+ | 3+ | 2 | D3 | Shoot in Combat |
| Ancient Chill Dagger | Melee | - | 3 | 4+ | 3+ | 2 | D3 | Crit (Auto-wound) |

**Abilities:**

#### Cacophony of Sorrow (Passive)
- **Effect:** While there are any friendly **Myrmourn Banshees** units wholly within 12" of this unit, ignore positive modifiers to the control scores of enemy units within 12" of this unit.

#### Ghostly Howl (Activated)
- **Timing:** Once Per Battle(Army), Any Shooting Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Inflict D3 mortal damage on the target. If any models in the target unit are slain by this ability, the target cannot use commands for the rest of the turn.

---

## Points Table

| Unit | Points |
| --- | ---: |
| Chainghasts | 90 |
| Chainrasps | 100 |
| Craventhrone Guard | 100 |
| Glaivewraith Stalkers | 100 |
| Knight of Shrouds | 100 |
| Myrmourn Banshees | 100 |
| Guardian of Souls | 110 |
| Spirit Hosts | 110 |
| Spirit Torment | 110 |
| Lord Vitriolic | 120 |
| Scriptor Mortis | 120 |
| Awlrach the Drowner | 130 |
| Cairn Wraith | 130 |
| Tomb Banshee | 130 |
| Dreadblade Harrows | 140 |
| Thorns of the Briar Queen | 140 |
| Hexwraiths | 150 |
| Krulghast Cruciator | 150 |
| The Headsmen's Curse | 150 |
| Knight of Shrouds on Ethereal Steed | 160 |
| Kurdoss Valentian, the Craven King | 160 |
| Kurdoss Valentian, the Craven King (Scourge of Ghyran) | 170 |
| Lord Executioner | 170 |
| Pyregheists | 180 |
| Dreadscythe Harridans | 200 |
| Grimghast Reapers | 200 |
| Reikenor the Grimhailer | 200 |
| Bladegheist Revenants | 210 |
| Black Coach | 220 |
| Black Coach (Scourge of Ghyran) | 290 |
| Lady Olynder, Mortarch of Grief | 320 |
| Nagash, Supreme Lord of the Undead | 840 |
