# Helsmiths of Hashut

## Army Rules

### Anvil of Apotheosis

#### Extract Power (Activated)
- **Timing:** End of Your Turn
- **Effect:** If this unit is contesting an objective, a **Place of Power**, or a terrain feature and that objective, **Place of Power** or terrain feature has a friendly desolation token, give this unit 1 ritual point.

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of its **Horns and Hooves** is 5.

#### Daemonic Resilience (Passive)
- **Effect:** Apply the effect below that corresponds with the number of **daemonic power points** this unit has:
**1** This unit has Ward (6+).
**2** This unit has Ward (5+).
**3** This unit has Ward (4+) against damage inflicted by Spells, Prayers, and abilites used by Manifestations. Otherwise, it has Ward (5+).

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of its **Crushing Horns and Hooves** is 5.

#### Power Through Dark Pacts (Activated)
- **Timing:** Your Hero Phase
- **Declare:** You must use this ability in each of your hero phases.
- **Effect:** Roll a dice and apply the corresponding effect:
**1-3 *Sacrificial Demand:*** Pick another friendly unit within 12" of this unit. 1 model in that unit is slain. Otherwise, inflict D6 mortal damage on this unit.
**4+ *Forbidden Knowledge:*** Pick 1 of the following effects to apply until the start of your next turn:
• If this unit is a Wizard or Priest, add 1 to its power level.
• Add 1 to the Damage characteristic of this unit's melee weapons.
• Add 3 to the control scores of friendly units while they are wholly within 12" of this unit.

#### Ruthless Industrialist (Passive)
- **Effect:** During your Path to Glory campaign, keep a tally of the number of **desolation tokens** you give. Each time the tally reaches a multiple of 15, this unit gains the next rank on its Path.

#### Purveyor of Dark Mechanisms (Activated)
- **Timing:** Your Hero Phase
- **Declare:** Pick a friendly War Machine within this unit's combat range to be the target.
- **Effect:** **Heal (D3)** the target.

#### Rigid and Unmoving (Passive)
- **Effect:** While this unit is on the battlefield, friendly units cannot use the 'Redeploy' command.

#### Creeping Petrification (Passive)
- **Effect:** Subtract 2 from run rolls and charge rolls for this unit.
In addition, at the end of each battle, roll a dice. On a 1, give this unit 1 **stone-flesh point** and note this on your Path to Glory roster. When this unit has 3 **stone-flesh points**, you must remove this unit from your Path to Glory roster.

#### Chained by Obligation (Activated)
- **Timing:** Start of First Battle Round
- **Declare:** This unit must use this ability.
- **Effect:** Roll a dice, On a 1-3, your resources have been depleted before the battle to settle a debt. You lose 1 command point.

#### Body of Hardened Obsidian (Passive)
- **Effect:** For the purposes of the 'Daemonic Resilience' ability, this unit always counts as having 1 more **daemonic power point** than the number it actually has, to a maximum of 3.

#### Immolating Presence (Activated)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a dice. If the roll exceeds the target's Health characteristic, 1 model in the target unit is slain.
- **Keywords:** Rampage

#### Trample and Gore (Activated)
- **Timing:** Once Per Turn (Army), Any Charge Phase
- **Declare:** If this unit charged this phase, pick an enemy unit within 1" of it to be the target.
- **Effect:** Inflict D3 mortal damage on the target. Then, roll 2D6. This unit can move a distance up to the value of the roll. during that move, this unit can pass through models in the target unit but must end that move in combat.
- **Keywords:** Rampage

#### Hobgrot Advisor (Passive)
- **Effect:** This unit's **Hobgrot Advisor** is a token. While this unit's **Hobgrot Advisor** is on the battlefield, add 3 to the control scores of friendly Helsmiths of Hashut units wholly within 12" of this unit. If you make an unmodified save roll of 1 for this unit, remove this unit's **Hobgrod Advisor** from the battlefield, the attack fails, and the attack sequence ends.

#### Oracle of Hashut (Passive)
- **Effect:** This unit can chant prayers from the 'Prayers of the Scorched Sect' lore as if it had Priest (1) and it knows all the prayers from that lore.

### Paths

#### Unchained Fire (Activated)
- **Timing:** Once Per Battle, Any Combat Phase
- **Effect:** Add D3 to the Attacks characteristic of this unit's melee weapons for the rest of the turn.

#### Unyielding Tyrant (Passive)
- **Effect:** Add 2 to this unit's Health characteristic

#### Diabolical Conqueror (Passive)
- **Effect:** Add 2 to the control scores of friendly non-Hero Helsmiths of Hashut units while they are contesting the same objective as this unit.

#### Unholy Savant (Activated)
- **Timing:** Your Hero Phase
- **Effect:** Roll a dice. On a 3+, for the rest of the turn, add 1 to chanting rolls for friendly Helsmiths of Hashut Priests while they are wholly within 12" of this unit.

#### Ferocious Warlord (Activated)
- **Timing:** Any Charge Phase
- **Declare:** Pick this unit and up to 1 other friendly Helsmiths of Hashut unit wholly within 12" of this unit to be the targets.
- **Effect:** For the rest of the turn, add 1 to charge rolls for the targets.

#### Master of Bombardment (Command)
- **Timing:** Your Shooting Phase
- **Cost:** 1
- **Declare:** Pick up to 3 friendly Helsmiths of Hashut War Machines wholly within 12" of this unit to be the targets,
- **Effect:** Add 1 to the Attacks characteristic of the targets' ranged weapons for the rest of the turn.

#### Bone-Deep Corruption (Activated)
- **Timing:** Any Combat Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a dice. On a 3+:
• Inflict an amount of mortal damage on the target equal to the roll.
• If the roll exceeds the target's Health characteristic, it has Strike-last for the rest of the turn.

#### Marked by the Bull (Passive)
- **Effect:** This unit has Ward (5+). In addition, each time you make an unmodified ward roll of 6 for this unit, after all damage points have been allocated, **Heal (1)** this unit.

#### Disciplined March (Activated)
- **Timing:** Reaction: You declared a Run ability for this unit.
- **Effect:** Instead of making a run roll as part of that Run ability, you can use a value of 6 for the roll that cannot be modified.

#### Contemptuous Counter (Activated)
- **Timing:** Once Per Battle, Any Combat Phase
- **Declare:** Pick an enemy unit that charged this turn and is in combat with this unit to be the target,
- **Effect:** Roll a D3. On a 2+, inflict an amount of mortal damage on the target equal to the roll.

#### Infernal Reach (Passive)
- **Effect:** Add 3" to the Range characteristic of this unit's ranged weapons.

#### Prestigious Armoury (Passive)
- **Effect:** Add 1 to save rolls for this unit.

#### Scornful Bulwark (Passive)
- **Effect:** This unit's melee weapons have **Anti-charge (+1 Rend)**.

#### Smog Bombs (Activated)
- **Timing:** Once Per Battle, Reaction: This unit is targeted by an Attack ability
- **Effect:** Roll a dice. On a 2+, for the rest of the turn, if the unmodified hit roll for an attack that targets this unit is 1-4, the attack fails and the attack sequence ends.

#### Darksome Arsenal (Passive)
- **Effect:** This unit's melee weapons have **Crit (Mortal)**.

#### Favoured Retinue (Activated)
- **Timing:** Deployment Phase
- **Declare:** Pick a friendly Helsmiths of Hashut Infantry Hero to be the target.
- **Effect:** For the rest of the battle, while the target is within this unit's combat range, this unit and the target have Ward (5+).

### Battle Traits

#### Harness Daemonic Power (Activated)
- **Timing:** Once Per Turn (Army), Start of Your Turn
- **Effect:** You must use this ability at the start of each of your turns. Remove all **daemonic power points** from each friendly unit. Then, gain 1 **daemonic power point** for each friendly **desolation token** on the battlefield.
Then, allocate your **daemonic power points** to friendly non-Hobgrot Helsmiths of Hashut units. Each unit can have a maximum of 3 **daemonic power points**. Then, all unallocated **daemonic power points** are lost.
**Designer's Note:** *Non-*Hobgrot *units have abilities that become more powerful depending on how many daemonic power points they have. We recommend using coloured dice to track the number of points each units has.*

#### Reserves of Daemonic Power (Command)
- **Timing:** Once Per Turn (Army), Any Hero Phase
- **Cost:** 1
- **Declare:** Pick a friendly non-Hobgrot Helsmiths of Hashut unit that has no **daemonic power points** to be the target.
- **Effect:** If the total number of **daemonic power points** that friendly units have is 3 or fewer, give 2 **daemonic power points** to the target.
If the total number of **daemonic power points** that friendly units have is more than 3, give 1 **daemonic power point** to the target.

#### Leave the Land in Ruin (Activated)
- **Timing:** Once Per Turn (Army), Start of Any Turn
- **Declare:** Pick a terrain feature or objective that does not have a friendly **desolation token** and is contested by a friendly Helsmiths of Hashut unit that is not in combat to be the target.
- **Effect:** Give the target a **desolation token**.

### Artefacts of Power

#### Scroll of Petrification (Activated)
- **Timing:** Once Per Battle, Any Hero Phase
- **Declare:** Pick a friendly Helsmiths of Hashut unit wholly within 12" of this unit to be the target.
- **Effect:** For the rest of the turn:
• The target has Ward (2+).
• The target cannot use non-passive abilities or be picked to be the target of friendly abilities.
• Enemy units can ignore the target’s combat range for the purposes of movement and charging.
• Enemy units can ignore the target for the purposes of setting up on the battlefield but cannot be set up on the target.

#### Crucible of Spite (Activated)
- **Timing:** Once Per Battle, Your Hero Phase
- **Effect:** Give up to 3 **daemonic power points** to this unit. However, you cannot allocate or give any more **daemonic power points** to this unit for the rest of the battle.

#### Gauntlets of Punishment (Activated)
- **Timing:** Your Shooting Phase
- **Declare:** Pick a visible enemy unit within 12" of this unit to be the target.
- **Effect:** Roll a D3. On a 2+, inflict an amount of mortal damage on the target equal to the roll.

### Battle Formations

#### Amassed Legions (Passive)
- **Effect:** Each time a friendly Helsmiths of Hashut Infantry unit with 3 **daemonic power points** uses the 'Rally' command, you can make 3 additional rally rolls of D6.

#### Experimental Munitions (Passive)
- **Effect:** Add 1 to the Attacks characteristic of ranged weapons used by friendly Helsmiths of Hashut War Machines while they have 3 **daemonic power points**.

#### Bulls of the Ziggurat (Passive)
- **Effect:** Add 2" to the Move characteristic of friendly Helsmiths of Hashut Cavalry and Monster units while they have 3 **daemonic power points**.

#### Arcane Dominance (Passive)
- **Effect:** Add 1 to casting rolls, unbinding rolls, banishment rolls, and chanting rolls for friendly Helsmiths of Hashut Wizards and Priests while they have 3 **daemonic power points**.

#### Domination Hexes (Passive)
- **Effect:** Double the Health characteristic of friendly non-Hobgrot Helsmiths of Hashut units for the purposes of the ‘Power Through’ command.
**Designer’s Note:** *The effect of this ability applies both when you use ‘Power Through’ and when your opponent uses ‘Power Through’.*

#### Smog and Smoke (Passive)
- **Effect:** Subtract 1 from hit rolls for shooting attacks that target friendly Helsmiths of Hashut Infantry or Cavalry units while they are wholly within 9" of a friendly Helsmiths of Hashut War Machine.

### Heroic Traits

#### Servile Automaton (Activated)
- **Timing:** Deployment Phase
- **Declare:** Pick a friendly Helsmiths of Hashut Automaton unit within this unit's combat range to be the target.
- **Effect:** For the rest of the battle, the target is this unit's bodyguard and the following effects apply:
• While this unit is in combat and its bodyguard is within its combat range, its bodyguard has Strike-first.
• If this unit is Infantry, it has Ward (5+) while its bodyguard is within its combat range.

#### 'Fire, You Worms!' (Activated)
- **Timing:** Enemy Charge Phase
- **Declare:** Pick a friendly Helsmiths of Hashut Infantry unit that is not in combat and is wholly within 12" of this unit to be the target.
- **Effect:** The target can immediately use the 'Shoot' ability as if it were your shooting phase. All attacks must target the same enemy unit, and the enemy unit picked to be the target of the attacks must have charged this phase. If the unmodified hit roll for any of those attacks is 1-5, the attack fails and the attack sequence ends.

#### An Eye for Weakness (Activated)
- **Timing:** Your Hero Phase
- **Declare:** Pick a visible enemy unit within 18" of this unit to be the target.
- **Effect:** Until the start of your next turn, add 1 to wound rolls for combat attacks made by friendly Helsmiths of Hashut units that target that enemy unit.

### Accursed Devices

#### Greed Pistons (Passive)
- **Effect:** When making charge rolls for this unit, add 1 to the number of dice rolled, to a maximum of 3, then remove 1 dice of your choice and use the remaining dice as the charge roll.

#### Breath of Contempt (Activated)
- **Timing:** End of Any Turn
- **Declare:** Pick up to 3 enemy units in combat with this unit to be the targets.
- **Effect:** Roll a D3 for each target. On a 2+, inflict an amount of mortal damage on the target equal to the roll.

#### Overdrive Switch (Activated)
- **Timing:** Once Per Turn (Army), Reaction: You declared an Attack ability for this unit
- **Effect:** For the rest of the phase:
• Add 1 to hit rolls for this unit’s attacks, including those made with **Companion** weapons, that target an enemy unit within 9" of it.
• Each time an unmodified hit roll for an attack made by this unit is 1, allocate 1 damage point to this unit after the Attack ability has been resolved (ward rolls cannot be made for those damage points).

## Subfactions

### Taar's Grand Forgehost

**Battle Traits**

#### Rising Fire (Passive)
- **Effect:** The following cumulative effects apply to friendly Grand Forgehost units in each battle round after the first.
**Battle Round 2:**
***Daemonic Weaponry:*** Those units' weapons have **Crit (2 Hits)**.
**Battle Round 3:**
***Harnessed Sorcery:*** Add 1 to casting rolls and chanting rolls for those units.
**Battle Round 4:**
***Channeled Power:*** Attacks made by those units score critical hits on unmodified hit rolls of 5+.
**Battle Round 5:**
***Arcane Dominance:*** Add 1 to the power level of those units if they are a Wizard or Priest.

#### Dread Ranks Unbroken (Command)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Cost:** 1
- **Declare:** Pick a friendly non-Hero Grand Forgehost Infantry unit that has been destroyed to be the target.
- **Effect:** Set up a replacement unit with half the number of models from the target unit (rounding up) wholly within 6" of the battlefield edge and more than 9" from all enemy units.

#### Skilled Skirmishers (Passive)
- **Effect:** Friendly Grand Forgehost War Machines have Ward (6+).

**Artefacts of Power**

#### Talisman of Obsidian (Passive)
- **Effect:** Subtract 1 from hit rolls for attacks that target this unit.

**Heroic Traits**

#### Ruthless Overseer (Passive)
- **Effect:** Each time a friendly Grand Forgehost unit wholly within 12" of this unit uses the 'Rally' command, you can make 3 additional rally rolls of D6.

### Ziggurat Stampede

**Battle Traits**

#### Let the Realms Tremble (Passive)
- **Effect:** Add **X** to charge rolls for friendly Ziggurat Stampede units, where **X** is the number of friendly Ziggurat Stampede units that have already charged this phase.

#### Run Roughshod (Command)
- **Timing:** Your Movement Phase
- **Cost:** 1
- **Declare:** Pick a friendly Ziggurat Stampede unit in combat to be the target.
- **Effect:** For the rest of the turn:
• The target can use Shoot and/or Charge abilities even if it used a Retreat ability in the same turn.
• No mortal damage is inflicted on the target by Retreat abilities.

#### 'Break them, One and All...' (Passive)
- **Effect:** Any number of friendly Ziggurat Stampede units can use the 'Power Through' command in the same turn.

**Artefacts of Power**

#### Visage of the Great Bull (Passive)
- **Effect:** In your charge phase, add 1 to the number of dice rolled when making charge rolls for this unit, to a maximum of 3.

**Heroic Traits**

#### Raging Animus (Passive)
- **Effect:** Each time you make an unmodified save roll of 1 for a combat attack that targets this unit, inflict D3 mortal damage on the attacking unit after the Fight ability has been resolved.

## Units

### Anointed Sentinels
**Points:** 130 | **Move:** 10" | **Health:** 4 | **Save:** 4+ | **Control:** 1
**Keywords:** CHAOS, HELSMITHS OF HASHUT, DUARDIN, CHAMPION, CAVALRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Thrice-cursed Glaive | Melee | - | 3 | 3+ | 3+ | 1 | 2 | Anti-Cavalry (+1 Rend), Charge (+1 Damage) |

**Abilities:**

#### Zealous Counter-Attack (Activated)
- **Timing:** Reaction: You declared the 'Counter-charge' command for this unit
- **Effect:** This unit has Strike-first for he rest of the turn.

#### Crush the Unworthy (Passive)
- **Effect:** Add 1 to charge rolls for each **daemonic power point** it has.

---

### Ashen Elder
**Points:** 120 | **Move:** 4" | **Health:** 6 | **Save:** 4+ | **Control:** 2
**Keywords:** HERO, CHAOS, HELSMITHS OF HASHUT, DUARDIN, INFANTRY, PRIEST (1)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Black Hammer of Hashut | Melee | - | 3 | 3+ | 4+ | 1 | D3 | - |

**Abilities:**

#### Extract Power (Activated)
- **Timing:** End of Your Turn
- **Effect:** If this unit is contesting an objective, a **Place of Power** or a terrain feature and that objective, **Place of Power** or terrain feature has a friendly desolation token, give this unit 1 ritual point.

#### Stoked Fanaticism (Passive)
- **Effect:** Ignore the first damage point allocated to each friendly non-Hobgrot Helsmiths of Hashut unit each phase while it is wholly within 6" of this unit. Add 6" to the range of this ability for each **daemonic power point** this unit has

---

### Bull Centaurs
**Points:** 190 | **Move:** 10" | **Health:** 4 | **Save:** 4+ | **Control:** 1
**Keywords:** CHAOS, HELSMITHS OF HASHUT, DUARDIN, CHAMPION, CAVALRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Brazen Mauls | Melee | - | 4 | 3+ | 3+ | 1 | 2 | Charge (+1 Damage), Crit (Mortal) |

**Abilities:**

#### Bull Charge (Activated)
- **Timing:** Any Charge Phase
- **Declare:** If this unit charged this turn, pick a visible enemy unit within 1" of it to be the target.
- **Effect:** Roll a dice for each model in this unit. Add the number of **daemonic power points** this unit has. For each 6+, inflict 1 mortal damage on the target.

#### Crush the Unworthy (Passive)
- **Effect:** Add 1 to charge rolls for each **daemonic power point** it has.

---

### Daemonsmith
**Points:** 100 | **Move:** 4" | **Health:** 6 | **Save:** 4+ | **Control:** 2
**Keywords:** HERO, CHAOS, HELSMITHS OF HASHUT, DUARDIN, WIZARD (1), INFANTRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Darkiron Talon | Melee | - | 3 | 3+ | 4+ | 1 | D3 | - |
| Hurled Daemonfire | Ranged | 18" | 3 | 4+ | 4+ | 1 | D3 | Crit (2 Hits), Shoot in Combat |

**Abilities:**

#### Molten Mending (Activated)
- **Timing:** Once Per Turn (Army), Your Hero Phase
- **Declare:** Pick a friendly Helsmiths of Hashut War Machine wholly within 6" of this unit to be the target. Add 6" to the range of this ability for each **daemonic power point** this unit has.
- **Effect:** **Heal (D3+X)** the target, where **X** is the number of **daemonic power points** this unit has.

---

### Daemonsmith on Infernal Taurus
**Points:** 290 | **Move:** 10" | **Health:** 15 | **Save:** 3+ | **Control:** 5
**Keywords:** HERO, CHAOS, WAR MACHINE, MONSTER, FLY, HELSMITHS OF HASHUT, DUARDIN, WIZARD (1)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Infernal Staff | Melee | - | 3 | 4+ | 3+ | 1 | D3 | - |
| Hurled Daemonfire | Ranged | 18" | 3 | 4+ | 4+ | 1 | D3 | Crit (2 Hits), Shoot in Combat |
| Horns and Hooves | Melee | - | 6 | 4+ | 2+ | 2 | 3 | Charge (+1 Damage), Companion |

**Abilities:**

#### Unholy Stampede (Passive)
- **Effect:** While a friendly Helsmiths of Hashut Cavalry unit is wholly within 12" of this unit:
• That unit can use a Retreat ability and still use Charge abilities later in the turn.
• No mortal damage is inflicted on that unit by Retreat abilities.

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of its **Horns and Hooves** is 4.

#### Immolating Presence (Activated)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a dice. If the roll exceeds the target's Health characteristic, 1 model in the target unit is slain.
- **Keywords:** Rampage

#### Daemonic Resilience (Passive)
- **Effect:** Apply the effect below that corresponds with the number of **daemonic power points** this unit has:
**1** This unit has Ward (6+).
**2** This unit has Ward (5+).
**3** This unit has Ward (4+) against damage inflicted by Spells, Prayers, and abilites used by Manifestations. Otherwise, it has Ward (5+).

---

### Deathshrieker Rocket Battery
**Points:** 140 | **Move:** 4" | **Health:** 8 | **Save:** 3+ | **Control:** 2
**Keywords:** CHAOS, HELSMITHS OF HASHUT, WAR MACHINE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Hashu-Zharr Rockets | Ranged | 24" | 3 | 4+ | 2+ | 2 | D3+2 | Anti-Monster (+1 Rend), Anti-War Machine (+1 Rend) |
| Artillerist Weapons | Melee | - | 3 | 4+ | 4+ | - | 1 | - |

**Abilities:**

#### Hungering Flames (Activated)
- **Timing:** Once Per Turn (Army), Your Shooting Phase
- **Effect:** For the rest of the turn, the Damage characteristic of this unit's **Hashu-Zharr Rockets** is 5 if the target is a Monster or War Machine.

#### Watch Them Burn (Activated)
- **Timing:** Reaction: You declared a Shoot ability for this unit and all of its attacks targeted the same enemy unit
- **Effect:** Roll a number of dice equal to the number of **daemonic power points** this unit has for each other enemy unit within the combat range of the target of this unit's shooting attacks. For each 3+, inflict 1 mortal damage on the enemy unit being rolled for.

---

### Dominator Engine with Bane Maces
**Points:** 160 | **Move:** 6" | **Health:** 10 | **Save:** 2+ | **Control:** 2
**Keywords:** CHAOS, HELSMITHS OF HASHUT, WAR MACHINE, AUTOMATON

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Bane Maces | Melee | - | 4 | 4+ | 2+ | 1 | 3 | Charge (+1 Damage) |

**Abilities:**

#### Engine of Domination (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Effect:** If this unit is in combat with any enemy Heroes, it can use 2 Fight abilities this phase. After the first is used, however, this unit has Strike-last for the rest of the phase and can only be picked to use a second Fight ability if it is still in combat with any enemy Heroes.

#### Daemonic Strength (Passive)
- **Effect:** For each **daemonic power point** this unit has:
• Add 1" to its Move characteristic
• Add 1 to the Attacks characteristic of its weapons.

---

### Dominator Engine with Immolation Cannons
**Points:** 170 | **Move:** 6" | **Health:** 10 | **Save:** 2+ | **Control:** 2
**Keywords:** CHAOS, HELSMITHS OF HASHUT, WAR MACHINE, AUTOMATON

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Immolation Cannons | Ranged | 8" | 5 | 2+ | 4+ | 1 | D3 | Anti-Cavalry (+1 Rend), Shoot in Combat |
| Horns and Pummelling Strikes | Melee | - | 3 | 4+ | 3+ | 1 | D3 | Charge (+1 Damage) |

**Abilities:**

#### All Must Burn (Activated)
- **Timing:** Once Per Turn (Army), Your Shooting Phase
- **Declare:** Pick 2 or more visible enemy units within 8" of this unit to be the targets.
- **Effect:** For the rest of the turn, add 2 to the Attacks characteristic of this unit’s **Immolation Cannons** for each unit picked to be the target of this ability, but each time this unit uses a Shoot ability, it must target each of those units with at least 3 of its attacks.

#### Daemonic Strength (Passive)
- **Effect:** For each **daemonic power point** this unit has:
• Add 1" to its Move characteristic
• Add 1 to the Attacks characteristic of its weapons.

---

### Hobgrot Vandalz
**Points:** 70 | **Move:** 5" | **Health:** 1 | **Save:** 5+ | **Control:** 1
**Keywords:** INFANTRY, CHAMPION, STANDARD BEARER (1/10), MUSICIAN (1/10), CHAOS, HELSMITHS OF HASHUT, HOBGROT

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Scavenged Weapons | Melee | - | 2 | 4+ | 5+ | - | 1 | - |

**Abilities:**

#### Disposable Lackeys (Activated)
- **Timing:** Deployment Phase
- **Effect:** This unit can immediately use the 'Normal Move' ability as if it were your movement phase.

---

### Infernal Cohort with Hashutite Blades
**Points:** 90 | **Move:** 4" | **Health:** 1 | **Save:** 3+ | **Control:** 1
**Keywords:** INFANTRY, CHAMPION, STANDARD BEARER (1/10), MUSICIAN (1/10), CHAOS, HELSMITHS OF HASHUT, DUARDIN

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Hashutite Blade | Melee | - | 2 | 3+ | 4+ | - | 1 | Anti-Infantry (+1 Rend) |

**Abilities:**

#### Disciplined March (Passive)
- **Effect:** When making run rolls for this unit, if you roll a 1-3, you can use a value of 4 instead.

#### Daemonic Resilience (Passive)
- **Effect:** Apply the effect below that corresponds with the number of **daemonic power points** this unit has:
**1** This unit has Ward (6+).
**2** This unit has Ward (5+).
**3** This unit has Ward (4+) against damage inflicted by Spells, Prayers, and abilites used by Manifestations. Otherwise, it has Ward (5+).

#### Sacred Gongs (Passive)
- **Effect:** This unit's **Gong Carriers** are tokens. There is 1 **Gong Carrier** for every Musician in this unit. If this unit uses the 'Rally' command, as a reaction, you can remove a **Gong Carrier** to make an additional rally roll of D6.

---

### Infernal Cohort with Hashutite Blades (Scourge of Ghyran)
**Points:** 110 | **Move:** 4" | **Health:** 1 | **Save:** 3+ | **Control:** 1
**Keywords:** INFANTRY, CHAMPION, STANDARD BEARER (1/10), MUSICIAN (1/10), CHAOS, HELSMITHS OF HASHUT, DUARDIN

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Hashutite Blade | Melee | - | 2 | 3+ | 4+ | - | 1 | Anti-Infantry (+1 Rend) |

**Abilities:**

#### Take Up Their Arms (Activated)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Declare:** Pick another visible friendly non-Hobgrot Helsmiths of Hashut Infantry unit, **Deathshrieker Rocket Battery** or **Tormentor Bombard** within 12" of this unit to be the target.
- **Effect:** Remove up to 3 models from this unit. Then, if the target is an Infantry unit, return a number of slain models to the target equal to the number of models you removed from this unit. If the target is a War Machine, heal a number of damage points allocated to the target equal to the number of models you removed from this unit.
Models removed from this unit by this ability cannot be returned to
this unit.

#### Daemonic Resilience (Passive)
- **Effect:** Apply the effect below that corresponds with the number of **daemonic power points** this unit has:
**1** This unit has Ward (6+).
**2** This unit has Ward (5+).
**3** This unit has Ward (4+) against damage inflicted by Spells, Prayers, and abilites used by Manifestations. Otherwise, it has Ward (5+).

#### Sanctified Gongs (Passive)
- **Effect:** This unit's **Gong Carriers** are tokens. There is 1 **Gong Carrier** for every Musician in this unit. If this unit uses the ‘Rally’ command, make 2 additional rally rolls of D6 for each **Gong Carrier** in this unit, but for each unmodified rally roll of 1, you must remove a **Gong Carrier** from this unit after the command has been resolved.

---

### Infernal Cohort with Hashutite Spears
**Points:** 100 | **Move:** 4" | **Health:** 1 | **Save:** 3+ | **Control:** 1
**Keywords:** INFANTRY, CHAMPION, STANDARD BEARER (1/10), MUSICIAN (1/10), CHAOS, HELSMITHS OF HASHUT, DUARDIN

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Hashutite Spear | Melee | - | 2 | 3+ | 4+ | - | 1 | Anti-Cavalry (+1 Rend), Anti-charge (+1 Rend) |

**Abilities:**

#### Conquered Lands (Activated)
- **Timing:** Your Hero Phase
- **Declare:** If this unit is contesting an objective you control, pick a friendly non-Hobgrot Helsmiths of Hashut unit wholly within 12" of this unit to be the target.
- **Effect:** Roll a dice. On a 3+, give the target 1 **daemonic power point**.
**Designer's Note:** *You cannot make more than one roll for each objective in each turn regardless of how many friendly units with this ability are contesting that objective.*

#### Daemonic Resilience (Passive)
- **Effect:** Apply the effect below that corresponds with the number of **daemonic power points** this unit has:
**1** This unit has Ward (6+).
**2** This unit has Ward (5+).
**3** This unit has Ward (4+) against damage inflicted by Spells, Prayers, and abilites used by Manifestations. Otherwise, it has Ward (5+).

#### Sacred Gongs (Passive)
- **Effect:** This unit's **Gong Carriers** are tokens. There is 1 **Gong Carrier** for every Musician in this unit. If this unit uses the 'Rally' command, as a reaction, you can remove a **Gong Carrier** to make an additional rally roll of D6.

---

### Infernal Razers with Blunderbusses
**Points:** 110 | **Move:** 4" | **Health:** 1 | **Save:** 4+ | **Control:** 1
**Keywords:** CHAOS, INFANTRY, HELSMITHS OF HASHUT, DUARDIN, CHAMPION

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Grizmalok Blunderbusses | Ranged | 18" | 1 | 3+ | 2+ | - | 2 | - |
| Weapon Butt | Melee | - | 1 | 4+ | 4+ | - | 1 | - |

**Abilities:**

#### Hateful Hail (Activated)
- **Timing:** Once Per Turn (Army), Your Shooting Phase
- **Declare:** If this unit used a Shoot ability this turn and all of its attacks targeted the same enemy unit, that enemy unit is the target.
- **Effect:** Roll a dice and add the number of enemy models in the target unit slain by attacks made by this unit this turn. If the result is 6+, the target has Strike-last for the rest of the turn.

#### Manglers of Metal (Passive)
- **Effect:** Add 1 to the Rend characteristic of this unit’s ranged weapons for each **daemonic power point** this unit has.

---

### Infernal Razers with Flamehurlers
**Points:** 90 | **Move:** 4" | **Health:** 1 | **Save:** 4+ | **Control:** 1
**Keywords:** CHAOS, INFANTRY, HELSMITHS OF HASHUT, DUARDIN, CHAMPION

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Karagthrun Flamehurlers | Ranged | 12" | 3 | 2+ | 4+ | - | 1 | Shoot in Combat |
| Weapon Butt | Melee | - | 1 | 4+ | 4+ | - | 1 | - |

**Abilities:**

#### Scorched Remains (Activated)
- **Timing:** Once Per Turn (Army), Your Shooting Phase
- **Declare:** If this unit used a Shoot ability this turn and all of its attacks targeted the same enemy Infantry unit, that enemy unit is the target.
- **Effect:** Roll a dice. On a 3+, subtract an amount equal to the roll from the target's control score until the start of your next turn.

#### Manglers of Metal (Passive)
- **Effect:** Add 1 to the Rend characteristic of this unit’s ranged weapons for each **daemonic power point** this unit has.

---

### The Blood of the Bull
**Points:** 120 | **Move:** 4" | **Health:** 2 | **Save:** 4+ | **Control:** 1
**Keywords:** INFANTRY, UNIQUE, Legends, CHAMPION, CHAOS, DUARDIN, STANDARD BEARER (1/5), HELSMITHS OF HASHUT

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Daemonflame Weapons | Melee | - | 2 | 3+ | 4+ | 1 | 1 | - |
| Blades and Fists | Melee | - | 2 | 4+ | 4+ | - | 1 | - |
| Blades and Fists | Melee | - | 2 | 4+ | 4+ | - | 1 | - |
| Master-crafted Flamehurler | Ranged | 12" | 4 | 2+ | 4+ | 2 | 1 | Shoot in Combat |
| Daemonflame Weapons | Melee | - | 2 | 3+ | 4+ | 1 | 1 | - |
| Daemonflame Weapons | Melee | - | 2 | 3+ | 4+ | 1 | 1 | - |

**Abilities:**

#### Infernal Empowerment (Passive)
- **Effect:** Apply the effect(s) below that corresponds with the number of **daemonic power points** this unit has. These effects are cumulative.
**1+** Add 1" to this unit's Move characteristic.
**2+** Add 1 to wound rolls for this unit's combat attacks.
**3+** Add 1 to the Rend characteristic of this unit's melee weapons.

---

### Tormentor Bombard
**Points:** 130 | **Move:** 4" | **Health:** 8 | **Save:** 3+ | **Control:** 2
**Keywords:** CHAOS, HELSMITHS OF HASHUT, WAR MACHINE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Torrent of Ruinous Energy | Ranged | 30" | 4 | 3+ | 3+ | 1 | D3 | Anti-Cavalry (+1 Rend), Anti-Infantry (+1 Rend) |
| Artillerist Weapons | Melee | - | 3 | 4+ | 4+ | - | 1 | - |

**Abilities:**

#### Ruinous Bombardment (Activated)
- **Timing:** Once Per Turn (Army), Your Shooting Phase
- **Declare:** If this unit used a Shoot ability this turn and all of its attacks targeted the same enemy unit, that enemy unit is the target. Then, you can pick a number of enemy units within 6" of the target equal to the number of **daemonic power points** this unit has to be additional targets.
- **Effect:** Roll a dice for each target. On a 3+, that unit cannot use commands until the start of your next turn.

#### Calculated Trajectory (Passive)
- **Effect:** Add 1 to hit rolls for this unit's shooting attacks that target an enemy unit with 5 or more models that is more than 12" from this unit.

---

### Urak Taar, the First Daemonsmith
**Points:** 340 | **Move:** 10" | **Health:** 16 | **Save:** 3+ | **Control:** 5
**Keywords:** HERO, UNIQUE, WARMASTER, CHAOS, WAR MACHINE, MONSTER, FLY, HELSMITHS OF HASHUT, DUARDIN, WIZARD (2)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Dumakar | Melee | - | 4 | 4+ | 3+ | 1 | D3 | - |
| Ruinous Torrents | Ranged | 18" | 6 | 4+ | 4+ | 1 | D3 | Crit (2 Hits), Shoot in Combat |
| Ghorrakos's Horns and Hooves | Melee | - | 6 | 4+ | 2+ | 2 | 3 | Charge (+1 Damage), Companion |

**Abilities:**

#### Pitiless Trampling (Activated)
- **Timing:** Once Per Turn (Army), Any Charge Phase
- **Declare:** If this unit charged this phase, pick an enemy unit within 1" of it to be the target.
- **Effect:** Inflict D3 mortal damage on the target. Then, roll 2D6. This unit can move a distance up to the value of the roll. during that move, this unit can pass through models in the target unit but must end that move in combat.
- **Keywords:** Rampage

#### Peerless Among Daemonsmiths (Passive)
- **Effect:** Apply the effect below that corresponds with the number of **daemonic power points** this unit has:
**1** This unit has Ward (6+).
**2** Add 1 to casting rolls for this unit. In addition, this unit has Ward (5+).
**3** Add 2 to casting rolls for this unit. In addition, this unit has Ward (4+) against damage inflicted by Spells, Prayers, and abilites used by Manifestations. Otherwise, it has Ward (5+).

#### The Curse of Stone (Spell)
- **Timing:** Your Hero Phase
- **Casting Value:** 7
- **Declare:** Pick a point on the battlefield within 9" of this unit, then pick a second point on the battlefield within 9" of the first point. Draw a line between the caster and the first point, and between the first and second points. Each enemy unit the lines pass across is a target. Then, make a casting roll of 2D6.
- **Effect:** Roll a D3 for each target. On a 2+, inflict an amount of mortal damage on the target equal to the roll.
- **Keywords:** Spell

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of **Ghorrakos's Horns and Hooves** is 4.

#### Master of Daemonic Power (Activated)
- **Timing:** Enemy Hero Phase
- **Effect:** Remove up to 3 **daemonic power points** in total from any combination of friendly units wholly within 18" of this unit. Then, allocate them to a different friendly non-Hobgrot Helsmiths of Hashut unit wholly within 18" of this unit.

---

### War Despot
**Points:** 90 | **Move:** 4" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, CHAOS, HELSMITHS OF HASHUT, DUARDIN, INFANTRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Daemonflame Glaive | Melee | - | 5 | 3+ | 3+ | 1 | 2 | Crit (Mortal) |

**Abilities:**

#### 'Fight, You Scum!' (Activated)
- **Timing:** Reaction: You declared a Fight ability for this unit
- **Effect:** Pick a friendly non-Hero Helsmiths of Hashut Infantry unit that has not used a Fight ability this turn and is within this unit's combat range to be the target. The target can be picked to use a Fight ability immediately after the Fight ability used by this unit has been resolved. If it is picked to do so, add 1 to the Attacks characteristic of its melee weapons for the rest of the turn.

#### Black-Hearted Conqueror (Passive)
- **Effect:** Add 3 to the control scores of friendly Helsmith of Hashut Infantry units while they are wholly within 6" of this unit. Add 6" to the range of this ability for each **daemonic power point** this unit has.

---

### War Despot (Scourge of Ghyran)
**Points:** 130 | **Move:** 4" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, CHAOS, HELSMITHS OF HASHUT, DUARDIN, INFANTRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Daemonflame Glaive | Melee | - | 5 | 3+ | 3+ | 1 | 2 | Crit (Mortal) |

**Abilities:**

#### Triumph or Perish, You Vermin! (Passive)
- **Effect:** While this unit has any **daemonic power points**, friendly non‑Hobgrot Helsmiths of Hashut units, excluding **War Despots**, with 0 **daemonic power points** count as having 1 **daemonic power point** while they are wholly within 9" of and visible to this unit.
Add 3" to the range of this ability while this unit has 2 **daemonic power points**. Add 6" to the range of this ability instead while this unit has 3 **daemonic power points**.
**Designer’s Note:** *If a friendly unit with 0 daemonic power points is affected by this ability and subsequently gains 1 or more daemonic power points, this ability would stop affecting that unit. Also, Urak Taar’s ‘Master of Daemonic Power’ ability has no effect on units affected by this ability*

#### Daemonfire Pistol (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Declare:** Pick a visible enemy unit within 12" of this unit to be the enemy target. Then, you can pick a friendly Helsmiths of Hashut Infantry unit in combat with the enemy target to be the friendly target.
- **Effect:** Roll a D3. On a 2+, inflict an amount of mortal damage on the enemy target equal to the roll.
If there is a friendly target, for the rest of the turn:
• It can use a Retreat ability and still use Shoot and/or Charge abilities later in the turn.
• Add 2" to the distance it can move when using Retreat abilities.
• No mortal damage is inflicted on it by Retreat abilities.

---

## Points Table

| Unit | Points |
| --- | ---: |
| Hobgrot Vandalz | 70 |
| Infernal Cohort with Hashutite Blades | 90 |
| Infernal Razers with Flamehurlers | 90 |
| War Despot | 90 |
| Daemonsmith | 100 |
| Infernal Cohort with Hashutite Spears | 100 |
| Infernal Cohort with Hashutite Blades (Scourge of Ghyran) | 110 |
| Infernal Razers with Blunderbusses | 110 |
| Ashen Elder | 120 |
| The Blood of the Bull | 120 |
| Anointed Sentinels | 130 |
| Tormentor Bombard | 130 |
| War Despot (Scourge of Ghyran) | 130 |
| Deathshrieker Rocket Battery | 140 |
| Dominator Engine with Bane Maces | 160 |
| Dominator Engine with Immolation Cannons | 170 |
| Bull Centaurs | 190 |
| Daemonsmith on Infernal Taurus | 290 |
| Urak Taar, the First Daemonsmith | 340 |
