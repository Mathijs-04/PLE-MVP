# Stormcast Eternals

## Army Rules

### Anvil of Apotheosis

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of its **Mighty Flying Noble Beast's Claws** is 4.

#### Freshly Forged (Passive)
- **Effect:** When this unit earns the Aspiring rank on its Path, it gets both the abilities for that rank instead of just one of them.

#### Snatched From the Grave (Passive)
- **Effect:** Enemy units cannot use the ‘Rally’ command while they are within 18" of this unit.

#### Tribal Warleader (Activated)
- **Timing:** Your Charge Phase
- **Declare:** Pick up to 2 other friendly units wholly within 12" of this unit to be the targets.
- **Effect:** For the rest of the turn, add 1 to charge rolls for this unit and the targets.

#### Redeemed (Activated)
- **Timing:** End of Any Turn
- **Declare:** Pick up to 2 other friendly units wholly within 12" of this unit to be the targets.
- **Effect:** Heal (D3) this unit and each target.

#### Brash and Impulsive (Passive)
- **Effect:** This unit cannot contest objectives.

#### Unvanquishable Pride (Passive)
- **Effect:** This unit cannot use Retreat abilities.

#### Fractured Soul (Activated)
- **Timing:** End of Any Turn
- **Declare:** This unit must use this ability at the end of each turn.
- **Effect:** Roll a dice. On a 4+, allocate 1 damage point to this unit (a ward roll cannot be made for that damage point).

#### Fuelled by Hatred (Passive)
- **Effect:** In each combat phase, if this unit is in combat and does not have Strike-last, after friendly units with Strike-first have been picked to use a Fight ability, this unit must be the next friendly unit that is picked to use a Fight ability.

#### Mighty Roar (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick up to 3 enemy units in combat with this unit to be the targets.
- **Effect:** Roll a dice for each target. On a 4+, the target cannot use commands for the rest of the turn.
- **Keywords:** Rampage

#### Titanic Duel (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Effect:** Roll a dice. On a 3+, add 1 to the Damage characteristic of this unit’s **Companion** melee weapons for attacks that target enemy Monsters for the rest of the turn.
- **Keywords:** Rampage

#### Loyal Companion (Passive)
- **Effect:** This unit's **Loyal Companion** is a token. This unit has Ward (5+) while its **Loyal Companion** is on the battlefield. If you make an unmodified save roll of 1 for this unit, remove its **Loyal Companion** from the battlefield.

### Paths

#### Strength of the Tempest (Passive)
- **Effect:** Add 1 to wound rolls for this unit's attacks.

#### Indomitable Soul (Passive)
- **Effect:** Add 1 to this unit's Health characteristic.

#### Noble Defender (Activated)
- **Timing:** Once Per Battle, Any Hero Phase
- **Effect:** For the rest of the turn, if a friendly Stormcast Eternals unit wholly within 12" of this unit uses the 'All-out Defence' command, no command points are spent.

#### Lightning-Wreathed Weapons (Passive)
- **Effect:** This unit's melee weapons have **Crit (Mortal)**.

#### Celestial Halo (Activated)
- **Timing:** End of Any Turn
- **Declare:** Pick up to 3 enemy units in combat with this unit to be targets.
- **Effect:** Roll a D3 for each target, On a 2+, inflict an amount of mortal damage on the target equal to the roll.

#### The God-King's Favour (Passive)
- **Effect:** This unit has Ward (5+)

#### Sigmar's Judgement (Activated)
- **Timing:** Once Per Battle, Any Combat Phase
- **Effect:** The unit can use 2 Fight abilities this phase. After the first is used, however, the target has Strike-last for the rest of the turn.

#### Bold Tactician (Activated)
- **Timing:** Reaction: You declared the 'Counter-charge' command for this unit
- **Effect:** Pick another friendly unit wholly within 12" of this unit to be the target. After the 'Counter-charge' command for this unit has been resolved, if this unit charged, the target can immediately use the 'Counter-charge' command without any command points being spent.

#### Heralds of the Tempest (Activated)
- **Timing:** Once Per Battle, Your Movement Phase
- **Effect:** If this unit was set up on the battlefield using the 'Scions of the Storm' ability this turn, add 1 to charge rolls for this unit for the rest of the turn.

#### Blaze of Glory (Passive)
- **Effect:** Each time a model in this unit is slain by a combat attack and that model was in combat with the attacking unit, roll a dice. For each 4+, inflict 1 mortal damage on the attacking unit after the Fight ability has been resolved.

#### Speed of the Storm (Passive)
- **Effect:** Add 2" to the Move characteristic of this unit.

#### Blessed by Azyr (Activated)
- **Timing:** Any Hero Phase
- **Effect:** **Heal (D3)** this unit.

#### Defensive Mastery (Activated)
- **Timing:** Any Combat Phase
- **Effect:** For the rest of the turn, this unit has Strike-last and Ward (4+)

#### Indomitable Souls (Passive)
- **Effect:** Add 1 to this unit's Health characteristic.

#### Lightning-Wreathed Weapons (Passive)
- **Effect:** This unit’s melee weapons have **Crit (Mortal)**.

### Battle Traits

#### Heavens-sent (Command)
- **Timing:** Once Per Battle, Your Movement Phase
- **Cost:** 1
- **Declare:** Pick a friendly non-Unique Stormcast Eternals Infantry or Cavalry unit that started the battle with 2 or more models and that has been destroyed to be the target.
- **Effect:** Set up a replacement unit with half the number of models from the target unit (rounding up) more than 9" from all enemy units.

#### Their Finest Hour (Activated)
- **Timing:** Once Per Turn (Army), Your Hero Phase
- **Declare:** Pick a friendly Stormcast Eternals unit that has not used this ability this battle to use this ability
- **Effect:** For the rest of the turn, add 1 to wound rolls for that unit’s combat attacks and add 1 to save rolls for that unit.

#### Scions of the Storm (Activated)
- **Timing:** Your Movement Phase
- **Declare:** Pick a friendly Stormcast Eternals unit that is **in the Celestial Realm**.
- **Effect:** Set up that unit anywhere on the battlefield more than 9" from all enemy units.

#### The Celestial Realm (Activated)
- **Timing:** Deployment Phase
- **Declare:** If there are more friendly Stormcast Eternals units on the battlefield than there are set up in reserve, pick a friendly Stormcast Eternals unit that has not been deployed.
- **Effect:** Set up that unit in reserve **in the Celestial Realm**. It has now been deployed.
- **Keywords:** Deploy

### Artefacts of Power

#### Null Pendant (Activated)
- **Timing:** Once Per Battle, End of Any Turn
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Subtract 5 from the target’s control score for the rest of the turn.

#### Quicksilver Draught (Activated)
- **Timing:** Once Per Battle, Any Combat Phase
- **Effect:** This unit has Strike-first for the rest of the turn.

#### Mirrorshield (Passive)
- **Effect:** This unit cannot be targeted by shooting attacks unless the attacking model is within 9" of it.

#### Beacon of Azyr (Activated)
- **Timing:** Once Per Battle, Any Hero Phase
- **Declare:** Pick a visible friendly Stormcast Eternals Wizard or Priest wholly within 12" of this unit to be the target.
- **Effect:** The next time this turn that the target uses a Prayer or Spell ability, instead of making a casting roll or chanting roll for them, you can use a value of 7 for the roll that cannot be modified. After that ability is resolved, if the target has any ritual points, reset their ritual points total to 0.
**Designer’s Note:** *Ritual points apply modifiers so cannot be used with this ability to increase the chanting roll to more than 7.*

#### Sigil of Morrda (Activated)
- **Timing:** Once Per Turn, Reaction: Opponent declared a command or a Rampage ability for a unit in combat with this unit
- **Effect:** Roll a dice. On a 3+, that command or Rampage ability has no effect, it still counts as having been used and any command points spent to use that ability are still lost.

#### Banner of Sigmar (Activated)
- **Timing:** Once Per Battle, Your Hero Phase
- **Declare:** Pick an objective within 18" of this unit to be the target.

### Battle Formations

#### Peerless Manoeuvres (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Declare:** Pick a friendly Vanguard Chamber unit that did not use a Move ability this turn and is not in combat.
- **Effect:** Roll a dice. On a 3+, remove that unit from the battlefield and set it up again anywhere on the battlefield more than 9" from all enemy units.

#### Ancient Aura (Activated)
- **Timing:** Once Per Turn (Army), Any Hero Phase
- **Declare:** Pick a friendly Ruination Chamber unit.
- **Effect:** That unit has Ward (5+) for the rest of the turn.

#### Synchronised Strikes (Passive)
- **Effect:** Add 1 to hit rolls for combat attacks made by friendly Warrior Chamber units while they are wholly within 12" of any friendly non-Hero non-Beast Stormcast Eternals units that do not have the Warrior Chamber keyword.

#### Oncoming Storm (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick a friendly Extremis Chamber unit that charged this turn.
- **Effect:** Roll a dice. On a 3+, that unit has Strike-first for the rest of the turn.

#### Cycle of the Storm (Activated)
- **Timing:** Once Per Turn (Army), End of Your Turn
- **Declare:** Pick a friendly Sacrosanct Chamber unit that had models slain in this turn to be the target.
- **Effect:** Return 1 slain model to the target unit.

### Heroic Traits

#### Staunch Defender (Passive)
- **Effect:** While this unit is contesting an objective in friendly territory, add 3 to the control scores of friendly Stormcast Eternals units contesting that objective.

#### Envoy of the Heavens (Activated)
- **Timing:** Your Hero Phase
- **Declare:** If a friendly Stormcast Eternals unit was destroyed in the previous turn, pick a visible friendly Stormcast Eternals unit wholly within 12" of this unit to be the target.
- **Effect:** The target has Ward (5+) until the start of your next turn.

#### Shock and Awe (Passive)
- **Effect:** Each time this unit is set up using the ’Scions of the Storm’ ability, subtract 1 from hit rolls for attacks that target this unit for the rest of the battle round.
In addition, each time a friendly Stormcast Eternals unit is set up wholly within 12" of this unit using the ‘Scions of the Storm’ ability, subtract 1 from hit rolls for attacks that target that unit for the rest of the battle round.

#### Intense Piety (Activated)
- **Timing:** Your Hero Phase
- **Declare:** Pick a visible enemy Priest or Wizard within 18" of this unit to be the target.
- **Effect:** Roll a dice. On a 3+, subtract 1 from casting rolls and chanting rolls for the target until the start of your next turn.

#### Legendary Tenacity (Passive)
- **Effect:** The first time this unit would be destroyed, before removing it from play, roll a dice. On a 3+, this unit is not destroyed and any remaining damage points inflicted on it have no effect. Then, **Heal (D3)** this unit.

#### Hour of Glory (Passive)
- **Effect:** This unit can be picked to use the ‘Their Finest Hour’ ability even if it has already used it this battle. In addition, when this unit is picked to use the ‘Their Finest Hour’ ability, add 5 to its control score for the rest of the turn.

## Subfactions

### Astral Templars

**Battle Traits**

#### Thrive on the Attack (Passive)
- **Effect:** Friendly Astral Templars units cannot use the 'All-out Defence' command. However, up to 2 friendly Astral Templars units can use the 'All-out Attack' command each phase.

#### Live off the Land (Activated)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Declare:** Pick a friendly Astral Templars unit that is wholly within enemy territory and not in combat to be the target.
- **Effect:** **Heal (D3)** the target.

#### Thundrous Roar (Command)
- **Timing:** Your Movement Phase
- **Cost:** 1
- **Declare:** Pick a friendly Astral Templars unit to be the target.
- **Effect:** The next time you make a run roll for the target this phase, roll 1 additional dice, to a maximum of 2.

#### Hunter's Prowess (Passive)
- **Effect:** Each time you declare an Attack ability for a friendly Astral Templars unit, you gain 1 **hunter's prowess point**.
Each time a combat attack made by a friendly Astral Templars unit scores a critical hit, you can spend 1 **hunter's prowess point**. If you do so, inflict 1 mortal damage on the target after the Attack ability has been resolved.

**Artefacts of Power**

#### Godbeast Plate (Passive)
- **Effect:** If the unmodified wound roll for an attack made with a **Companion** weapon that targets this unit is 1-3, the attack fails and the attack sequence ends.

**Heroic Traits**

#### Champion of Ursricht (Passive)
- **Effect:** Add 1 to the Damage characteristic of this unit's melee weapons for attacks that target an enemy Hero or Monster.

### Draconith Skywing

**Battle Traits**

#### Exemplars of Fury (Passive)
- **Effect:** Add 1 to the Attacks characteristic of **Companion** weapons used by friendly Draconith Skywing Heroes while they are within 6" of 2 or more friendly **Stormdrake Guard** units.

#### Aetheric Cyclone (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick a friendly Draconith Skywing Monster that has not used any Rampage abilities this turn to use this ability. Then, pick an enemy unit in combat with it to be the target.
- **Effect:** Roll a dice. On a 3+, subtract 1 from wound rolls for the target’s attacks for the rest of the turn.
- **Keywords:** Rampage

#### Thunderous Roar (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick a friendly Draconith Skywing Monster that has not used any Rampage abilities this turn to use this ability. Then, pick up to 3 enemy units in combat with it to be the targets.
- **Effect:** Roll a dice for each target. On a 3+, subtract 5 from that target’s control score for the rest of the turn.
- **Keywords:** Rampage

#### Shields of the Warden (Passive)
- **Effect:** While any friendly **Stormdrake Guard** units are within a friendly **Ionus Cryptborn**’s combat range:
• That friendly **Ionus Cryptborn** has Ward (4+).
• Each time you make a successful ward roll for that friendly **Ionus Cryptborn**, allocate 1 damage point to a friendly **Stormdrake Guard** unit within its combat range after the damage sequence for that friendly **Ionus Cryptborn** has been resolved (ward rolls cannot be made for those damage points).

**Artefacts of Power**

#### Celestium Ensign (Activated)
- **Timing:** Once Per Battle (Army), End of Any Turn
- **Effect:** **Heal (3)** each friendly Draconith Skywing unit wholly within 12" of this unit

**Heroic Traits**

#### Fearless Fliers (Activated)
- **Timing:** End of Any Turn
- **Effect:** If this unit is not in combat, it can immediately make a D6" move and can end that move in combat

### Heroes of the First-Forged

**Battle Traits**

#### Summon the Storm (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Declare:** If a friendly **Bastian Carthalos** is on the battlefield and was not set up this turn, you can pick a friendly Stormcast Eternals unit that is in the Celestial Realm.
- **Effect:** Set up that unit wholly within 6" of a friendly **Bastian Carthalos** and more than 3" from all enemy units. If the target is a unit of **Annihilators** or **Annihilators with Meteoric Grandhammers**, for the rest of the turn, the target counts as having used the ‘Scions of the Storm’ ability for the purposes of the ‘Blazing Impact’ ability

#### The Celestial Realm (Activated)
- **Timing:** Deployment Phase
- **Declare:** If there are more friendly Stormcast Eternals units on the battlefield than there are set up in reserve, pick a friendly Stormcast Eternals unit that has not been deployed.
- **Effect:** Set up that unit in reserve **in the Celestial Realm**. It has now been deployed.
- **Keywords:** Deploy

#### Punch Through (Activated)
- **Timing:** Once Per Turn (Army), Your Charge Phase
- **Declare:** Pick a friendly Infantry unit in combat that has not charged this turn to use this ability.
- **Effect:** That unit can immediately use the ‘Charge’ ability and can move through enemy models when it does so.

#### Scions of the Storm (Activated)
- **Timing:** Your Movement Phase
- **Declare:** Pick a friendly Stormcast Eternals unit that is **in the Celestial Realm**.
- **Effect:** Set up that unit anywhere on the battlefield more than 9" from all enemy units.

**Artefacts of Power**

#### Shield of Skjordamar (Activated)
- **Timing:** Any Combat Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a dice. On a 4+, ward rolls cannot be made for the target for the rest of the battle.

**Heroic Traits**

#### Thegn of Bellicos (Passive)
- **Effect:** Add 1 to the Attacks characteristic of this unit’s melee weapons while it is wholly within 6" of a friendly **Bastian Carthalos**.

### Ruination Brotherhood

**Battle Traits**

#### Hallowed Presence (Passive)
- **Effect:** While friendly Warrior Chamber units are wholly within 12" of any friendly Ruination Chamber units, those friendly Warrior Chamber units have Ward (6+).

#### Entropic Screech (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick a friendly Ruination Chamber Monster that has not used any Rampage abilities this turn to use this ability. Then, pick up to 3 enemy units in combat with it to be the targets.
- **Effect:** Roll a D3 for each target. On a 2+, inflict an amount of mortal damage on the target equal to the roll. The unit using this ability cannot use any more Rampage abilities for the rest of the turn.
- **Keywords:** Rampage

#### Temper the Flaw (Activated)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Declare:** You must use this ability at the end of each turn. Each friendly non-Hero Ruination Chamber unit that used a Fight ability this turn is a target.
- **Effect:** For each target, pick 1 of the following effects:
• ***Guided:*** If the target is wholly within 12" of any friendly Ruination Chamber Heroes, return 1 slain model to it.
• ***Tormented:*** If the target is not wholly within 12" of any friendly Ruination Chamber Heroes, it becomes tormented until the end of the next turn. While a unit is tormented, add 1 to the Attacks characteristic of its melee weapons but it cannot contest objectives or use commands.

#### Ancient Aura (Activated)
- **Timing:** Once Per Turn (Army) Any Hero Phase
- **Declare:** Pick a friendly Ruination Chamber unit.
- **Effect:** That unit has Ward (5+) for the rest of the turn.

**Artefacts of Power**

#### Hallowed Scrolls (Passive)
- **Effect:** If this unit is Infantry, it has Ward (5+). Otherwise, it has Ward (6+).

**Heroic Traits**

#### Corven Lord (Passive)
- **Effect:** If this unit has the ‘Ruination Chamber’, ability, it can use that ability once per turn regardless of whether another friendly unit has used it during the same turn.

## Units

### Aetherwings
**Points:** 80 | **Move:** 14" | **Health:** 2 | **Save:** 6+ | **Control:** 1
**Keywords:** ORDER, STORMCAST ETERNALS, VANGUARD CHAMBER, BEAST, FLY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Beak and Claws | Melee | - | 2 | 4+ | 4+ | - | 1 | Companion |

**Abilities:**

#### Marked for Destruction (Passive)
- **Effect:** Add 1 to hit rolls for shooting attacks made by friendly Vanguard Chamber units if the target is within 9" of this unit.

---

### Annihilators
**Points:** 130 | **Move:** 4" | **Health:** 3 | **Save:** 2+ | **Control:** 1
**Keywords:** Warrior Chamber Infantry, INFANTRY, CHAMPION, ORDER, STORMCAST ETERNALS, WARRIOR CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Meteoric Hammer | Melee | - | 3 | 3+ | 3+ | 1 | 2 | - |

**Abilities:**

#### Brace for Impact (Activated)
- **Timing:** Any Charge Phase
- **Declare:** Pick an enemy unit that charged this turn and is in combat with this unit to be the target.
- **Effect:** Roll a dice for each model in this unit. If any of the rolls are a 6, the target has Strike-last for the rest of the turn.

#### Blazing Impact (Activated)
- **Timing:** Your Movement Phase
- **Declare:** If this unit was set up this turn using the ‘Scions of the Storm’ ability, pick up to 3 enemy units within 10" of it to be the targets.
- **Effect:** Roll a D3 for each target. On a 2+, inflict an amount of mortal damage on the target equal to the roll.

---

### Annihilators with Meteoric Grandhammers
**Points:** 180 | **Move:** 4" | **Health:** 3 | **Save:** 3+ | **Control:** 1
**Keywords:** Warrior Chamber Infantry, INFANTRY, CHAMPION, ORDER, STORMCAST ETERNALS, WARRIOR CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Meteoric Grandhammer | Melee | - | 3 | 3+ | 2+ | 2 | 3 | - |

**Abilities:**

#### Force of a Falling Star (Activated)
- **Timing:** Any Charge Phase
- **Declare:** If this unit charged this phase and the unmodified charge roll was 8+, pick an enemy unit within 1" of it to be the target.
- **Effect:** The target has Strike-last for the rest of the turn.

#### Blazing Impact (Activated)
- **Timing:** Your Movement Phase
- **Declare:** If this unit was set up this turn using the ‘Scions of the Storm’ ability, pick up to 3 enemy units within 10" of it to be the targets.
- **Effect:** Roll a D3 for each target. On a 2+, inflict an amount of mortal damage on the target equal to the roll.

---

### Astreia Solbright
**Points:** 220 | **Move:** 10" | **Health:** 8 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, UNIQUE, ORDER, STORMCAST ETERNALS, SACROSANCT CHAMBER, CAVALRY, WIZARD (1), Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Astreia’s Aetherstave | Melee | - | 4 | 3+ | 3+ | 1 | 2 | Crit (Mortal) |
| Kazra’s Monstrous Claws | Melee | - | 3 | 3+ | 3+ | 1 | 2 | Companion |

**Abilities:**

#### Lightning Pulse (Spell)
- **Timing:** Your Hero Phase
- **Casting Value:** 6
- **Declare:** Pick up to 3 enemy units in combat with this unit to be the targets, then make a casting roll of 2D6.
- **Effect:** If you picked 1 unit, inflict 3 mortal damage on the target. If you picked 2 units, inflict 2 mortal damage on each target. If you picked 3 units, inflict 1 mortal damage on each target.
- **Keywords:** Spell

#### Thunderous Pounce (Activated)
- **Timing:** Your Charge Phase
- **Effect:** Add 2 to charge rolls for friendly Sacrosanct Chamber Cavalry units while they are wholly within 12" of this unit.

#### Soul Energy of the First Host (Activated)
- **Timing:** Any Combat Phase
- **Declare:** Pick a visible friendly Sacrosanct Chamber unit wholly within 12" of this unit to be the target.
- **Effect:** Add 1 to wound rolls for the target’s attacks for the rest of the turn.

---

### Aventis Firestrike, Magister of Hammerhal
**Points:** 310 | **Move:** 14" | **Health:** 12 | **Save:** 3+ | **Control:** 5
**Keywords:** HERO, FLY, UNIQUE, ORDER, STORMCAST ETERNALS, MONSTER, WIZARD (2), SACROSANCT CHAMBER, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Staff of Hammerhal | Melee | - | 4 | 3+ | 3+ | 2 | 2 | - |
| Loithar’s Stamping Hooves | Melee | - | 3 | 4+ | 3+ | 1 | D3 | Companion |

**Abilities:**

#### Pyroelectric Blast (Spell)
- **Timing:** Your Hero Phase
- **Casting Value:** 6
- **Declare:** Pick a visible enemy unit within 12" of this unit to be the target.
- **Effect:** Inflict 3 mortal damage on the target, then inflict 1 mortal damage on each other enemy unit within 3" of the target.
- **Keywords:** Spell

#### Comet Trail (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** For the rest of the phase, add 1 to hit rolls for combat attacks that target that enemy unit.
- **Keywords:** Rampage

#### Thunderhead Crown (Activated)
- **Timing:** Your Hero Phase
- **Effect:** **Heal (3)** this unit.

---

### Castigators
**Points:** 110 | **Move:** 5" | **Health:** 2 | **Save:** 3+ | **Control:** 1
**Keywords:** Sacrosanct Chamber Infantry, CHAMPION, ORDER, STORMCAST ETERNALS, SACROSANCT CHAMBER, INFANTRY, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Thunderhead Greatbow | Ranged | 15" | 3 | 3+ | 3+ | 1 | 1 | Anti-Cavalry (+1 Rend), Crit (2 Hits) |
| Greatbow Stock | Melee | - | 1 | 3+ | 3+ | - | 1 | - |

**Abilities:**

#### Dependable Gryph-hound (Passive)
- **Effect:** This unit’s **Gryph-hound** is a token. If this unit’s **Gryph-hound** has been removed, it cannot use the ‘Scouting Companion’ ability.

#### Castigator Aetheric Channelling (Activated)
- **Timing:** Your Shooting Phase
- **Effect:** If this unit has not used a Move ability this turn and was not set up this turn, this unit’s shooting attacks score critical hits on unmodified hit rolls of 5+ for the rest of the turn.

#### Scouting Companion (Activated)
- **Timing:** Reaction: You declared a Shoot ability for this unit
- **Effect:** Until the Shoot ability is resolved, add 3" to the Range characteristic of this unit’s **Thunderhead Greatbows** and add 1 to hit rolls for this unit’s shooting attacks. Remove this unit’s **Gryph-hound** from the battlefield after the Shoot ability has been resolved.

---

### Celestant-Prime, Hammer of Sigmar
**Points:** 250 | **Move:** 12" | **Health:** 10 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, FLY, WARD (5+), WARMASTER, UNIQUE, ORDER, STORMCAST ETERNALS, INFANTRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Cometstrike Sceptre | Ranged | 18" | 1 | 2+ | ⟝See 'Cometstrike Sceptre' ability⟞ | ⟝See 'Cometstrike Sceptre' ability⟞ | ⟝See 'Cometstrike Sceptre' ability⟞ | - |
| Ghal Maraz | Melee | - | 6 | 3+ | 2+ | 2 | 4 | Crit (Mortal) |

**Abilities:**

#### Cometstrike Sceptre (Passive)
- **Effect:** Each time this unit attacks with its Cometstrike Sceptre, if the attack scores a hit, inflict D3 mortal damage on the target.

#### Retribution From on High (Activated)
- **Timing:** Once Per Battle, Your Movement Phase
- **Declare:** Pick this unit if it is **in the Celestial Realm**.
- **Effect:** Subtract the current battle round number from 10 to determine this unit’s strike zone. Set up this unit on the battlefield more than a number of inches equal to its strike zone from all enemy units.

---

### Celestar Ballista
**Points:** 120 | **Move:** 3" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** Sacrosanct Chamber War Machine, ORDER, STORMCAST ETERNALS, SACROSANCT CHAMBER, WAR MACHINE, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Celestar Stormbolts | Ranged | 18" | D6+6 | 4+ | 2+ | 1 | 1 | Anti-Monster (+1 Rend), Crit (2 Hits) |
| Sacristan Engineers’ Blades | Melee | - | 2 | 3+ | 3+ | 1 | 1 | - |

**Abilities:**

#### Stormbolt Tempest (Activated)
- **Timing:** Once Per Battle (Army), Any Combat Phase
- **Declare:** Pick an enemy unit in combat with this unit and that charged this turn to be the target.
- **Effect:** Roll a dice. On a 2+, inflict an amount of mortal damage on the target equal to the roll.

---

### Decimators
**Points:** 240 | **Move:** 4" | **Health:** 3 | **Save:** 3+ | **Control:** 1
**Keywords:** Warrior Chamber Infantry, CHAMPION, ORDER, STORMCAST ETERNALS, INFANTRY, WARRIOR CHAMBER, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Starsoul Mace | Melee | - | 1 | 3+ | 3+ | 2 | 3 | Crit (Mortal) |
| Decimator Thunderaxe | Melee | - | 4 | 3+ | 3+ | 1 | 1 | Crit (2 Hits) |

**Abilities:**

#### Decimating Blow (Passive)
- **Effect:** Add 1 to the Damage characteristic of this unit’s Decimator Thunderaxes if the target unit has 10 or more models.

---

### Domitan's Stormcoven
**Points:** 210 | **Move:** 5" | **Health:** 3 | **Save:** 3+ | **Control:** 1
**Keywords:** Warrior Chamber Infantry, ORDER, INFANTRY, UNIQUE, Legends, STORMCAST ETERNALS, WIZARD (1), WARRIOR CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Azyrite Bolts | Ranged | 18" | 2 | 3+ | 4+ | 1 | D3 | - |
| Valedictor Weapons | Melee | - | 3 | 3+ | 3+ | 1 | D3 | - |
| Azyrite Bolts | Ranged | 18" | 2 | 3+ | 4+ | 1 | D3 | - |
| Valedictor Weapons | Melee | - | 3 | 3+ | 3+ | 1 | D3 | - |
| Azyrite Bolts | Ranged | 18" | 2 | 3+ | 4+ | 1 | D3 | - |
| Valedictor Weapons | Melee | - | 3 | 3+ | 3+ | 1 | D3 | - |

**Abilities:**

#### Gather the Aether (Passive)

#### Aethershock (Spell)
- **Timing:** Your Hero Phase
- **Casting Value:** 7
- **Declare:** Pick a visible unit within 12" of the caster to be the target, then make a casting roll of 2D6.
- **Effect:** If the target is a friendly unit, it has Strike-first for the rest of the turn. If the target is an enemy unit, it has Strike-last for the rest of the turn.
- **Keywords:** Spell

---

### Dracothian Guard Concussors
**Points:** 210 | **Move:** 10" | **Health:** 5 | **Save:** 3+ | **Control:** 2
**Keywords:** ORDER, STORMCAST ETERNALS, EXTREMIS CHAMBER, CAVALRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Stormblast | Ranged | 10" | 1 | 3+ | 3+ | 2 | D3 | Crit (2 Hits), Companion |
| Lightning Hammer | Melee | - | 3 | 3+ | 2+ | 1 | 2 | Crit (Mortal) |
| Dracoth’s Claws and Fangs | Melee | - | 3 | 4+ | 2+ | 2 | 2 | Companion |

**Abilities:**

#### Thunderstruck (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a dice. On a 3+, the target cannot use commands for the rest of the turn.

---

### Dracothian Guard Desolators
**Points:** 170 | **Move:** 10" | **Health:** 5 | **Save:** 3+ | **Control:** 2
**Keywords:** ORDER, STORMCAST ETERNALS, EXTREMIS CHAMBER, CAVALRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Stormblast | Ranged | 10" | 1 | 3+ | 3+ | 2 | D3 | Crit (2 Hits), Companion |
| Thunderaxe | Melee | - | 3 | 3+ | 3+ | 1 | 2 | Anti-Infantry (+1 Rend) |
| Dracoth’s Claws and Fangs | Melee | - | 3 | 4+ | 2+ | 2 | 2 | Companion |

**Abilities:**

#### Cleaving Blow (Passive)
- **Effect:** Add 1 to the Damage characteristic of this unit’s **Thunderaxes** if the target unit has 10 or more models.

---

### Dracothian Guard Fulminators
**Points:** 200 | **Move:** 10" | **Health:** 5 | **Save:** 3+ | **Control:** 2
**Keywords:** ORDER, STORMCAST ETERNALS, EXTREMIS CHAMBER, CAVALRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Stormblast | Ranged | 10" | 1 | 3+ | 3+ | 2 | D3 | Crit (2 Hits), Companion |
| Stormstrike Glaive | Melee | - | 3 | 3+ | 3+ | 1 | 2 | Charge (+1 Damage) |
| Dracoth’s Claws and Fangs | Melee | - | 3 | 4+ | 2+ | 2 | 2 | Companion |

**Abilities:**

#### Impaling Strikes (Activated)
- **Timing:** Any Charge Phase
- **Declare:** If this unit charged this phase, pick an enemy unit within 1" of it to be the target.
- **Effect:** Roll a D3. On a 2+, inflict an amount of mortal damage on the target equal to the roll.

---

### Dracothian Guard Tempestors
**Points:** 170 | **Move:** 10" | **Health:** 5 | **Save:** 3+ | **Control:** 2
**Keywords:** ORDER, STORMCAST ETERNALS, EXTREMIS CHAMBER, CAVALRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Volleystorm Crossbow | Ranged | 12" | 3 | 3+ | 3+ | 1 | 1 | Crit (2 Hits) |
| Warblade | Melee | - | 2 | 3+ | 3+ | 1 | 1 | - |
| Dracoth’s Claws and Fangs | Melee | - | 3 | 4+ | 2+ | 2 | 2 | Companion |
| Stormblast | Ranged | 10" | 1 | 3+ | 3+ | 2 | D3 | Crit (2 Hits), Companion |

**Abilities:**

#### Overwhelming Volleystorm (Activated)
- **Timing:** Any Shooting Phase
- **Declare:** Pick an enemy unit that had any damage points allocated to it this turn by attacks made with this unit’s **Volleystorm Crossbows** to be the target.
- **Effect:** Roll a dice. On a 2+, the target cannot use the ‘All-out Defence’ command in the following combat phase.

---

### Drakesworn Templar
**Points:** 390 | **Move:** 12" | **Health:** 18 | **Save:** 3+ | **Control:** 5
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, MONSTER, FLY, EXTREMIS CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Skybolt Bow | Ranged | 18" | 2 | 3+ | 2+ | 1 | 1 | Crit (Auto-wound) |
| Templar Weapons | Melee | - | 5 | 3+ | 3+ | 1 | 2 | Anti-Hero (+1 Rend), Anti-Monster (+1 Rend) |
| Stardrake’s Cavernous Jaws | Melee | - | 7 | 4+ | 2+ | 2 | 3 | Companion |
| Rain of Stars | Ranged | 24" | 4 | 3+ | 2+ | 2 | 2 | Companion |

**Abilities:**

#### Monstrous Challenge (Activated)
- **Timing:** Once Per Turn (Army), Your Combat Phase
- **Declare:** Pick an enemy Monster in combat with this unit to be the target.
- **Effect:** Roll a dice. On a 2+, for the rest of the turn:
• Add 1 to the Damage characteristic of this unit’s **Stardrake’s Cavernous Jaws** for attacks that target that enemy Monster.
• If that enemy Monster is also a Hero, add 1 to the Damage characteristic of this unit’s **Templar Weapons** for attacks that target that unit.
- **Keywords:** Rampage

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of its Stardrake’s Cavernous Jaws is 4.

---

### Evocators
**Points:** 180 | **Move:** 5" | **Health:** 3 | **Save:** 3+ | **Control:** 2
**Keywords:** Sacrosanct Chamber Infantry, CHAMPION, ORDER, STORMCAST ETERNALS, SACROSANCT CHAMBER, INFANTRY, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Evocator Weapons | Melee | - | 3 | 3+ | 3+ | 1 | 1 | Crit (Mortal) |

**Abilities:**

#### Warrior Mages (Passive)
- **Effect:** This unit can use Unbind abilities as if it had Wizard (1).

#### Empower (Activated)
- **Timing:** Any Combat Phase
- **Effect:** Add 1 to wound rolls for the target’s combat attacks for the rest of the turn.

---

### Evocators on Celestial Dracolines
**Points:** 240 | **Move:** 10" | **Health:** 5 | **Save:** 3+ | **Control:** 2
**Keywords:** Sacrosanct Chamber Cavalry, CHAMPION, ORDER, STORMCAST ETERNALS, SACROSANCT CHAMBER, CAVALRY, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Evocator Weapons | Melee | - | 3 | 3+ | 3+ | 1 | 1 | Crit (Mortal) |
| Celestial Dracoline’s Claws | Melee | - | 3 | 3+ | 3+ | 1 | 2 | Companion |

**Abilities:**

#### Warrior Mages (Passive)
- **Effect:** This unit can use Unbind abilities as if it had Wizard (1).

#### Dracoline Fury (Passive)
- **Effect:** While this unit is contesting an objective you do not control, it has Strike-first.

---

### Gardus Steel Soul
**Points:** 160 | **Move:** 5" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, WARD (5+), UNIQUE, ORDER, STORMCAST ETERNALS, INFANTRY, WARRIOR CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Sigmarite Runeblade and Warhammer | Melee | - | 5 | 3+ | 3+ | 1 | 2 | Crit (Auto-wound) |

**Abilities:**

#### Aura of Purity (Passive)
- **Effect:** If this unit charged this turn, for the rest of the turn, friendly Stormcast Eternals units have Ward (5+) while they are wholly within 12" of this unit.

#### Saintly Assault (Activated)
- **Timing:** Once Per Battle, Your Charge Phase
- **Declare:** Pick up to 3 visible friendly Stormcast Eternals Infantry units wholly within 6" of this unit to be the targets.
- **Effect:** For the rest of the turn:
• Add 1 to charge rolls for the targets.
• Add 1 to the Attacks characteristic of the targets’ melee weapons.

---

### Gavriel Sureheart
**Points:** 120 | **Move:** 5" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, INFANTRY, WARRIOR CHAMBER, UNIQUE, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Starbound Blade | Melee | - | 5 | 3+ | 3+ | 1 | 2 | Crit (Mortal) |

**Abilities:**

#### Once More, For Sigmar, Charge! (Activated)
- **Timing:** Your Charge Phase
- **Declare:** Pick this unit and another visible friendly Warrior Chamber unit within 3" of it to be the targets.
- **Effect:** For the rest of the phase, when making charge rolls for the targets, add 1 to the number of dice rolled, then remove 1 dice of your choice and use the remaining dice as the charge roll.

#### Sigmarite Thundershield (Passive)
- **Effect:** Each time you make an unmodified save roll of 6 for a combat attack that targets this unit, inflict 1 mortal damage on the attacking unit after the Fight ability has been resolved.

---

### Gryph-hounds
**Points:** 90 | **Move:** 9" | **Health:** 2 | **Save:** 6+ | **Control:** 1
**Keywords:** ORDER, STORMCAST ETERNALS, BEAST, CHAMPION

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Vicious Beak and Claws | Melee | - | 4 | 4+ | 4+ | - | 1 | Companion |

**Abilities:**

#### Warning Cry (Activated)
- **Timing:** Any Shooting Phase
- **Declare:** Pick an enemy unit within 12" of this unit that was set up this turn. Then, pick up to 3 friendly Stormcast Eternals units armed with ranged weapons that are within 12" of that enemy unit to be the targets.
- **Effect:** Roll a D3 for each target. On a 2+, inflict an amount of mortal damage on that enemy unit equal to the roll.

---

### Hamilcar Bear-Eater
**Points:** 230 | **Move:** 5" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** ASTRAL TEMPLARS, HERO, WARD (5+), UNIQUE, ORDER, STORMCAST ETERNALS, INFANTRY, WARRIOR CHAMBER, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Hamilcar's Halberd | Melee | - | 5 | 3+ | 3+ | 1 | 2 | Anti-Monster, Crit (Mortal) |

**Abilities:**

#### 'Because I'm Hamilcar!' (Activated)
- **Timing:** Start of Any Turn
- **Effect:** Roll a dice. You gain a number of **charmed fate points** equal to the roll.
During the turn, you can spend your **charmed fate points**. For each **charmed fate point** you spend, you can re-roll 1 hit roll, 1 wound roll, 1 save roll, or 1 ward roll for this unit. For every 2 **charmed fate points** you spend, you can re-roll 1 charge roll for this unit.
At the end of the turn, any unspent **charmed fate points** are lost.

#### 'That One's Mine!' (Activated)
- **Timing:** Any Combat Phase
- **Declare:** This unit can only use this ability if it is in combat with an enemy Monster and no other friendly units are in combat with that enemy Monster
- **Effect:** Double the Attacks characteristic of this unit's melee weapons for the rest of the turn.

---

### Ionus Cryptborn, Warden of Lost Souls
**Points:** 360 | **Move:** 12" | **Health:** 16 | **Save:** 3+ | **Control:** 5
**Keywords:** HERO, FLY, WARMASTER, UNIQUE, ORDER, STORMCAST ETERNALS, MONSTER, PRIEST (2), RUINATION CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Spirit-scouring Flames | Ranged | 10" | 9 | 2+ | 4+ | 2 | 1 | Shoot in Combat, Companion |
| Soulbreaker | Melee | - | 4 | 3+ | 3+ | 1 | 2 | - |
| Cthorak’s Ancient Claws | Melee | - | 6 | 4+ | 2+ | 2 | 3 | Companion |

**Abilities:**

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of **Cthorak’s Ancient Claws** is 4.

#### Lightning Tempest (Prayer)
- **Timing:** Your Hero Phase
- **Chanting Value:** 5
- **Declare:** Pick a visible enemy unit within 12" of this unit to be struck by lightning, then make a chanting roll of D6.
- **Effect:** Inflict D3 mortal damage on that unit, then roll a dice. On a 1-2, the sequence ends. On a 3+, pick another enemy unit within 3" of that unit to be struck by lightning and inflict D3 mortal damage on it. If the chanting roll was 10+, you can pick another enemy unit within 6" of that unit instead of 3". Keep rolling dice in this way until the sequence ends or there are no other enemy units eligible to be struck by lightning. A unit cannot be struck by lightning more than once per turn.
- **Keywords:** Prayer

#### Spirit-scouring Flames (Activated)
- **Timing:** Once Per Turn (Army), Any Shooting Phase
- **Declare:** Pick an enemy unit that had any damage points allocated to it this turn by attacks made with this unit’s **Spirit-scouring Flames** to be the target.
- **Effect:** The target has the Soulburned keyword for the rest of the battle.
- **Keywords:** Rampage

#### Soulburned (Activated)
- **Timing:** Any Shooting Phase
- **Declare:** This unit can use this ability even if it has been destroyed. Pick any number of Soulburned enemy units to be the targets.
- **Effect:** Roll a D3 for each target. On a 2+, inflict an amount of mortal damage on the target equal to the roll.

---

### Iridan the Witness
**Points:** 260 | **Move:** 12" | **Health:** 12 | **Save:** 3+ | **Control:** 5
**Keywords:** HERO, FLY, UNIQUE, ORDER, STORMCAST ETERNALS, MONSTER, PRIEST (1), RUINATION CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Axe of the Final Threshold | Melee | - | 4 | 3+ | 3+ | 2 | D3 | Crit (Mortal) |
| Ariax's Talons | Melee | - | 6 | 4+ | 2+ | 2 | 2 | Crit (2 Hits), Companion |

**Abilities:**

#### Prayer of the Witness (Prayer)
- **Timing:** Your Hero Phase
- **Chanting Value:** 4
- **Declare:** Make a chanting roll of D6.
- **Effect:** For the rest of the turn, each time a friendly Ruination Chamber unit uses a Fight ability while it is wholly within 18" of this unit, after that ability has been resolved:
• **Heal (D3)** that unit. If the chanting roll was 8+, **Heal (3)** that unit instead.
• That unit has Ward (6+) for the rest of the turn.
- **Keywords:** Prayer

#### Dark Flight (Activated)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a dice. If the roll exceeds the target’s Health characteristic:
• 1 model in the target unit is slain.
• This unit can immediately use the ‘Retreat’ ability without any mortal damage being inflicted upon it.
- **Keywords:** Rampage

---

### Iridan the Witness (Scourge of Ghyran)
**Points:** 320 | **Move:** 12" | **Health:** 12 | **Save:** 3+ | **Control:** 5
**Keywords:** HERO, FLY, UNIQUE, ORDER, STORMCAST ETERNALS, MONSTER, PRIEST (1), RUINATION CHAMBER, WARD (6+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Axe of the Final Threshold | Melee | - | 4 | 3+ | 3+ | 2 | 3 | Crit (Mortal) |
| Ariax's Talons | Melee | - | 6 | 4+ | 2+ | 2 | 2 | Charge (+1 Damage), Companion |

**Abilities:**

#### Demands of the Witness (Prayer)
- **Timing:** Your Hero Phase
- **Chanting Value:** 3
- **Declare:** Pick a visible friendly Ruination Chamber unit wholly within 12" of this unit to be the target, then make a chanting roll of D6.
- **Effect:** Until the start of your next turn, add 1 to the Rend characteristic of the target’s melee weapons. If the chanting roll was 7+, you can pick another eligible unit to be a second target.
- **Keywords:** Prayer

#### On Wings of Death (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a dice. On a 3+:
• You can remove this unit from the battlefield and set it up again on the battlefield within 1" of the target.
• If the target charged this turn, subtract 1 from hit rolls for the target’s attacks for the rest of the turn.
- **Keywords:** Rampage

#### The Flames of Hope (Passive)
- **Effect:** You can re-roll resistance rolls for friendly Ruination Chamber units while they are wholly within 12" of this unit.

#### Guardian of the Final Threshold (Activated)
- **Timing:** Once Per Turn, Reaction: This unit was picked as the target of a non-Core ability
- **Effect:** Make a resistance roll of D6. On a 3+, that ability has no effect on this unit.

---

### Judicators with Boltstorm Crossbows
**Points:** 160 | **Move:** 5" | **Health:** 2 | **Save:** 3+ | **Control:** 1
**Keywords:** Warrior Chamber Infantry, CHAMPION, ORDER, STORMCAST ETERNALS, INFANTRY, WARRIOR CHAMBER, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Storm Gladius | Melee | - | 1 | 3+ | 3+ | 1 | 1 | - |
| Boltstorm Crossbow | Ranged | 15" | 3 | 3+ | 3+ | - | 1 | Anti-Infantry (+1 Rend) |
| Thunderbolt Crossbow | Ranged | 15" | 3 | 3+ | 3+ | - | 2 | Anti-Infantry (+1 Rend) |

**Abilities:**

#### Hail of Bolts (Activated)
- **Timing:** Once Per Turn (Army), Any Shooting Phase
- **Declare:** Pick a visible enemy Infantry unit that had any models slain this phase by this unit’s shooting attacks to be the target.
- **Effect:** Roll a dice. On a 3+, for the rest of the turn, subtract 1 from the number of dice rolled when making charge rolls for the target, to a minimum of 1.

---

### Judicators with Skybolt Bows
**Points:** 140 | **Move:** 5" | **Health:** 2 | **Save:** 3+ | **Control:** 1
**Keywords:** Warrior Chamber Infantry, CHAMPION, ORDER, STORMCAST ETERNALS, INFANTRY, WARRIOR CHAMBER, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Storm Gladius | Melee | - | 1 | 3+ | 3+ | 1 | 1 | - |
| Shockbolt Bow | Ranged | 18" | D3+3 | 3+ | 3+ | 1 | 1 | Anti-Monster (+1 Rend) |
| Skybolt Bow | Ranged | 18" | 2 | 3+ | 3+ | 1 | 1 | Crit (Auto-wound) |

**Abilities:**

#### Subduing Shots (Activated)
- **Timing:** Once Per Turn (Army), Any Shooting Phase
- **Declare:** Pick a visible enemy Monster that had any damage points allocated to it this phase by this unit’s shooting attacks to be the target.
- **Effect:** Roll a dice. On a 3+, the target cannot use Rampage abilities until the start of your next turn.

---

### Karazai the Scarred
**Points:** 440 | **Move:** 12" | **Health:** 20 | **Save:** 3+ | **Control:** 5
**Keywords:** HERO, UNIQUE, ORDER, STORMCAST ETERNALS, WARMASTER, MONSTER, FLY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Blazing Tempest | Ranged | 12" | 1 | 2+ | 3+ | 2 | D3+3 | Shoot in Combat |
| Annihilating Jaws | Melee | - | 4 | 3+ | 2+ | 2 | 4 | Anti-Infantry (+1 Rend) |
| Drake-lord’s Talons | Melee | - | 6 | 3+ | 2+ | 2 | 2 | - |

**Abilities:**

#### Calamitous Tail Sweep (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a dice for each model in the target unit. For each 5+, inflict 1 mortal damage on the target.
- **Keywords:** Rampage

#### Fires of Retribution (Passive)
- **Effect:** While this unit has 5-9 damage points, add 1 to the Attacks characteristic of its melee weapons.
While this unit has 10 or more damage points, add 2 to the Attacks characteristic of its melee weapons instead.

#### Ancient Master of War (Activated)
- **Timing:** Any Combat Phase
- **Declare:** Pick an enemy unit within this unit’s combat range to be the target.
- **Effect:** Subtract 1 from the Attacks characteristic of the target’s melee weapons for the rest of the turn.

---

### Knight-Arcanum
**Points:** 120 | **Move:** 5" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, INFANTRY, WARRIOR CHAMBER, WIZARD (1)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Valedictor’s Stave | Melee | - | 3 | 3+ | 3+ | 1 | D3 | Anti-Manifestation (+1 Rend) |

**Abilities:**

#### Indomitable Loreseekers (Passive)
- **Effect:** Add 1 to casting rolls for this unit when it uses a Summon ability and add 1 to banishment rolls for this unit.
Enemy Manifestations cannot pass through or end any move within this unit’s combat range unless they started that move within this unit’s combat range.
This unit has Ward (4+) against mortal damage inflicted by Manifestations.

---

### Knight-Azyros
**Points:** 120 | **Move:** 12" | **Health:** 6 | **Save:** 3+ | **Control:** 5
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, RUINATION CHAMBER, FLY, INFANTRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Star-blessed Weapon | Melee | - | 5 | 3+ | 3+ | 1 | 2 | - |

**Abilities:**

#### Light in the Dark (Activated)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Declare:** Pick a friendly **Prosecutors** unit wholly within 12" of this unit to be the target.
- **Effect:** Roll a dice. On a 3+, pick 1 of the following effects:
• If the target is in combat, the target can immediately use the ‘Retreat’ ability as if it were your movement phase without any mortal damage being inflicted on it.
• If the target is not in combat, the target can immediately use the ‘Normal Move’ ability as if it were your movement phase.

#### The Light of Sigmar (Activated)
- **Timing:** Any Combat Phase
- **Effect:** Roll a dice. On a 3+, pick 1 of the following effects:
• For the rest of the turn, subtract 1 from hit rolls for attacks made by enemy units while they are in combat with this unit.
• Heal (D3) each friendly Stormcast Eternals unit within this unit’s combat range.

---

### Knight-Draconis
**Points:** 230 | **Move:** 12" | **Health:** 11 | **Save:** 3+ | **Control:** 5
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, MONSTER, FLY, EXTREMIS CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Draconic Flamestream | Ranged | 10" | 4 | 2+ | 3+ | 2 | 1 | Shoot in Combat, Companion |
| Aeonfire Blade | Melee | - | 5 | 3+ | 2+ | 2 | 2 | - |
| Draconith’s Fangs and Talons | Melee | - | 4 | 4+ | 2+ | 2 | 2 | Companion |

**Abilities:**

#### Arcane Heritage (Activated)
- **Timing:** Reaction: Opponent declared a Spell ability
- **Effect:** If this unit or a friendly **Stormdrake Guard** unit wholly within 12" of this unit was picked to be the target of that spell, roll a dice. On a 4+, ignore the effect of that spell on this unit or that friendly unit. This unit can use this ability more than once per phase but only once per **Spell** ability

#### Wrath of the Draconith (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick a friendly **Stormdrake Guard** unit within this unit’s combat range to be the target.
- **Effect:** Roll a dice. On a 2+, add 1 to the Attacks characteristic of both this unit’s and the target’s **Draconith’s Fangs and Talons** for the rest of the turn.
- **Keywords:** Rampage

---

### Knight-Heraldor
**Points:** 110 | **Move:** 5" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, INFANTRY, WARRIOR CHAMBER, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Sigmarite Broadsword | Melee | - | 3 | 3+ | 3+ | 1 | 2 | - |

**Abilities:**

#### Thunderblast (Activated)
- **Timing:** Your Shooting Phase
- **Declare:** Pick a visible faction terrain feature within 18" of this unit to be the target.
- **Effect:** Roll a dice. On a 2+, inflict an amount of mortal damage on the target equal to the roll.

#### Call to Battle (Passive)
- **Effect:** If a friendly Warrior Chamber unit wholly within 12" of this unit uses the ‘Rally’ command, you can make 3 additional rally rolls of D6.

---

### Knight-Incantor
**Points:** 140 | **Move:** 5" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, SACROSANCT CHAMBER, WIZARD (1), INFANTRY, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Incantor’s Staff | Melee | - | 3 | 3+ | 3+ | 1 | 2 | - |

**Abilities:**

#### Voidstorm Scroll (Activated)
- **Timing:** Once Per Battle, Reaction: Opponent declared a Spell ability for a Wizard within 30" of this unit
- **Effect:** The spell is unbound and its effect is not resolved.
- **Keywords:** Unbind

#### Aetheric Destabilisation (Passive)
- **Effect:** While they are within 30" of this unit, subtract 1 from casting rolls for enemy units when they use Summon abilities.

---

### Knight-Judicator with Gryph-hounds
**Points:** 130 | **Move:** 6" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, INFANTRY, WARRIOR CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Terminus Greatbow | Ranged | 18" | 3 | 3+ | 3+ | 2 | 3 | - |
| Judicator’s Warblade | Melee | - | 3 | 3+ | 3+ | 1 | 1 | - |

**Abilities:**

#### Faithful Gryph-hounds (Passive)
- **Effect:** This unit’s **Gryph-hounds** are tokens. After setting up this unit on the battlefield for the first time, place both **Gryph-hounds** next to it.

#### Hunting Hounds (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Declare:** If this unit was not set up this turn and any of this unit’s **Gryph-hounds** are on the battlefield, pick an enemy unit within 12" of this unit to be the target.
- **Effect:** Roll a dice. On a 1, remove 1 of this unit’s **Gryph-hounds** from the battlefield. On a 2+, pick 1 of this unit’s **Gryph-hounds** that is next to this unit or next to an enemy unit and place it next to the target instead.
While any of this unit’s **Gryph-hounds** are next to an enemy unit, add 1 to hit rolls and wound rolls for this unit’s shooting attacks that target that unit.
If an enemy unit is destroyed or removed from the battlefield while any of this unit’s **Gryph-hounds** are next to it, place those **Gryph-hounds** next to this unit.

---

### Knight-Questor
**Points:** 110 | **Move:** 5" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, INFANTRY, WARRIOR CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Questor Warblade | Melee | - | 5 | 3+ | 3+ | 1 | 2 | Anti-Hero (+1 Rend),Crit (Mortal) |

**Abilities:**

#### His Will Be Done (Passive)

#### Heroic Retribution (Activated)
- **Timing:** Reaction: You declared a Fight ability for this unit
- **Effect:** Pick a friendly **Questor Soulsworn** unit that has not used a **Fight** ability this turn and is within this unit’s combat range to be the target. The target can be picked to use a **Fight** ability immediately after the **Fight** ability used by this unit has been resolved.

#### Ordained Quest (Activated)
- **Timing:** Once Per Battle (Army), Deployment Phase
- **Effect:** Pick an objective wholly outside friendly territory. That objective is considered by you to be questmarked.

---

### Knight-Relictor
**Points:** 120 | **Move:** 5" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, INFANTRY, WARRIOR CHAMBER, PRIEST (1)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Relic Maul | Melee | - | 3 | 3+ | 3+ | 1 | 2 | - |

**Abilities:**

#### Relic Censer (Activated)
- **Timing:** Your Shooting Phase
- **Declare:** Pick an enemy unit within 12" of this unit to be the target.
- **Effect:** Roll a dice. On a 2+, subtract 1 from ward rolls made by the target for the rest of the turn.

---

### Knight-Venator
**Points:** 170 | **Move:** 12" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, INFANTRY, WARRIOR CHAMBER, FLY, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Realmhunter’s Bow | Ranged | 24" | 6 | 3+ | 3+ | 1 | 1 | Anti-Monster (+1 Rend), Crit (2 Hits) |
| Bowstave | Melee | - | 3 | 3+ | 3+ | - | 1 | - |
| Star-eagle’s Celestial Talons | Melee | - | 2 | 4+ | 3+ | - | 1 | Companion |

**Abilities:**

#### Star-fated Arrow (Activated)
- **Timing:** Once Per Battle (Army), Your Shooting Phase
- **Declare:** Pick a visible enemy unit within 24" of this unit to be the target.
- **Effect:** If the target is a non-Monster unit, inflict D6 mortal damage on it. If the target is a Monster, inflict D6+3 mortal damage on it.
- **Keywords:** Core, Attack, Shoot

---

### Knight-Vexillor
**Points:** 100 | **Move:** 5" | **Health:** 6 | **Save:** 3+ | **Control:** 5
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, INFANTRY, WARRIOR CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Sigmarite Warblade | Melee | - | 4 | 3+ | 3+ | 1 | 2 | - |

**Abilities:**

#### The Banner of the Reforged (Activated)
- **Timing:** Any Hero Phase
- **Declare:** Pick up to D3 friendly Stormcast Eternals units wholly within 12" of this unit to be the targets.
- **Effect:** Add 3 to each target’s control score for the rest of the turn. In addition, **Heal (D3)** each target.

---

### Knight-Zephyros
**Points:** 130 | **Move:** 7" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, INFANTRY, VANGUARD CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Stormshot Pistol | Ranged | 12" | 3 | 3+ | 3+ | - | 1 | Anti-Hero (+1 Rend) |
| Tempest Axes | Melee | - | 6 | 3+ | 3+ | 1 | 1 | Anti-Hero (+1 Rend), Crit (2 Hits) |

**Abilities:**

#### Tireless Hunter (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Effect:** Remove this unit from the battlefield and set it up again on the battlefield more than 9" from all enemy units.

---

### Krondys, Son of Dracothion
**Points:** 500 | **Move:** 12" | **Health:** 20 | **Save:** 3+ | **Control:** 5
**Keywords:** HERO, UNIQUE, ORDER, STORMCAST ETERNALS, WARMASTER, MONSTER, FLY, WIZARD (2)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Blazing Tempest | Ranged | 12" | 1 | 2+ | 3+ | 2 | D3+3 | Shoot in Combat |
| Apex Maw | Melee | - | 4 | 3+ | 2+ | 2 | 4 | Anti-Infantry (+1 Rend) |
| Drake-lord’s Talons | Melee | - | 6 | 3+ | 2+ | 2 | 2 | - |

**Abilities:**

#### Soul-shaking Roar (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a dice. On a 3+, the target cannot use commands for the rest of the turn.
- **Keywords:** Rampage

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of its **Apex Maw** is 2.

#### Regalia Fulmentarus (Passive)
- **Effect:** Add 2 to casting rolls for this unit.

#### Atavistic Tempest (Spell)
- **Timing:** Your Hero Phase
- **Casting Value:** 8
- **Declare:** Pick a visible enemy unit within 18" of the caster to be the target, then make a casting roll of 2D6.
- **Effect:** Pick 1 of the following effects to apply to the target until the start of your next turn:
***Blinding Gales:*** Subtract 1 from hit rolls for the target’s attacks.
***Meteor-hail:*** Subtract 1 from the Rend characteristic of the target’s weapons.
***Flash-freezing Blizzard:*** Subtract 1 from save rolls for the target.
- **Keywords:** Spell

---

### Liberators
**Points:** 90 | **Move:** 5" | **Health:** 2 | **Save:** 3+ | **Control:** 1
**Keywords:** Warrior Chamber Infantry, CHAMPION, INFANTRY, ORDER, STORMCAST ETERNALS, WARRIOR CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Grandhammer | Melee | - | 2 | 3+ | 3+ | 1 | 2 | Crit (Mortal) |
| Warhammer | Melee | - | 2 | 3+ | 3+ | 1 | 1 | Crit (Mortal) |

**Abilities:**

#### Stalwart Defenders (Passive)

---

### Lord-Aquilor
**Points:** 140 | **Move:** 14" | **Health:** 8 | **Save:** 3+ | **Control:** 3
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, CAVALRY, VANGUARD CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Heavy Boltstorm Pistol | Ranged | 12" | 4 | 3+ | 3+ | 1 | 1 | Shoot in Combat |
| Starbound Blade and Shock Handaxe | Melee | - | 5 | 3+ | 3+ | 1 | 2 | - |
| Gryph-charger’s Razor Beak | Melee | - | 3 | 4+ | 3+ | 1 | 1 | Companion |

**Abilities:**

#### Ride the Winds Aetheric (Activated)
- **Timing:** Your Movement Phase
- **Declare:** If this unit is not in combat, you can pick up to 2 friendly Vanguard-Palladors units that are not in combat to be the targets.
- **Effect:** Remove this unit and the targets (if any) from the battlefield. Set this unit up again on the battlefield more than 9" from all enemy units. Then, set up each target wholly within 12" of this unit and more than 9" from all enemy units.

---

### Lord-Arcanum
**Points:** 140 | **Move:** 5" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, SACROSANCT CHAMBER, WIZARD (1), INFANTRY, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Aetherstave | Melee | - | 4 | 3+ | 3+ | 1 | 2 | - |

**Abilities:**

#### Magical Mending (Activated)
- **Timing:** End of Any Turn
- **Declare:** Pick a visible friendly Manifestation wholly within 12" of this unit to be the target.
- **Effect:** **Heal (D6)** the target.

#### Aetheric Manipulation (Passive)
- **Effect:** Add 1 to casting rolls for this unit when it uses a SUMMON ability.

---

### Lord-Arcanum on Celestial Dracoline
**Points:** 210 | **Move:** 10" | **Health:** 8 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, SACROSANCT CHAMBER, CAVALRY, WIZARD (1), Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Astreia’s Aetherstave | Melee | - | 4 | 3+ | 3+ | 1 | 2 | - |
| Celestial Dracoline’s Claws | Melee | - | 3 | 3+ | 3+ | 1 | 2 | Companion |

**Abilities:**

#### Storm Wave (Activated)
- **Timing:** Once Per Turn (Army), Any Charge Phase
- **Declare:** Pick a visible friendly Sacrosanct Chamber Cavalry unit that charged this phase and is wholly within 12" of this unit to be the target.
- **Effect:** Inflict D3 mortal damage on each enemy unit in combat with the target.

#### Pack Alpha (Activated)
- **Timing:** Any Combat Phase
- **Declare:** If this unit is in combat, pick this unit and up to 1 other visible friendly Sacrosanct Chamber Cavalry unit within its combat range to be the targets.
- **Effect:** Add 1 to the Attacks characteristic of the targets’ **Companion** melee weapons for the rest of the turn.

---

### Lord-Arcanum on Gryph-charger
**Points:** 220 | **Move:** 12" | **Health:** 8 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, SACROSANCT CHAMBER, CAVALRY, WIZARD (1), Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Aetherstave | Melee | - | 4 | 3+ | 3+ | 1 | 2 | - |
| Gryph-charger’s Razor Beak | Melee | - | 3 | 4+ | 3+ | 1 | 1 | Companion |

**Abilities:**

#### Healing Light (Activated)
- **Timing:** End of Any Turn
- **Declare:** Pick a visible friendly Sacrosanct Chamber unit that is not in combat and is within 12" of this unit to be the target.
- **Effect:** If the target is damaged, **Heal (D6)** the target. If the target is not damaged, return a slain model to it.

#### On the Winds Aetheric (Activated)
- **Timing:** Your Movement Phase
- **Effect:** If this unit is not in combat, remove it from the battlefield and set it up again on the battlefield more than 9" from all enemy units.

---

### Lord-Arcanum on Tauralon
**Points:** 280 | **Move:** 14" | **Health:** 12 | **Save:** 3+ | **Control:** 5
**Keywords:** HERO, FLY, ORDER, STORMCAST ETERNALS, MONSTER, SACROSANCT CHAMBER, WIZARD (1), Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Tauralon’s Hooves | Melee | - | 3 | 4+ | 3+ | 1 | D3 | Companion |
| Aetherstave | Melee | - | 4 | 3+ | 3+ | 1 | 2 | - |

**Abilities:**

#### Trails of Astral Light (Passive)
- **Effect:** Add 1 to casting rolls for friendly Stormcast Eternals Wizards while they are wholly within 12" of this unit.

#### Starlight Mantle (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** If this unit is in combat, pick each visible friendly Stormcast Eternals unit within its combat range to be the targets.
- **Effect:** For the rest of the turn, the targets have Ward (5+) and this unit has Strike-last.
- **Keywords:** Rampage

#### Aetheric Deflection (Activated)
- **Timing:** Once Per Turn (Army), Enemy Hero Phase
- **Declare:** Pick a visible enemy Wizard that used a Spell ability this phase and is within 30" of this unit to be the target.
- **Effect:** Roll **X**+1 dice, where **X** is an amount equal to the target’s power level. For each 2+, inflict 1 mortal damage on the target.

---

### Lord-Castellant
**Points:** 150 | **Move:** 5" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, INFANTRY, WARRIOR CHAMBER, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Castellant’s Halberd | Melee | - | 3 | 3+ | 3+ | 1 | 2 | Crit (Auto-wound) |
| Gryph-hound’s Beak | Melee | - | 4 | 4+ | 4+ | - | 1 | Companion |

**Abilities:**

#### Loyal Companion (Activated)
- **Timing:** Once Per Battle, Any Combat Phase
- **Effect:** For the rest of the turn, this unit has Ward (4+). Remove this unit’s **Gryph-hound** from the battlefield at the end of the turn.

#### Dutiful Gryph-hound (Passive)
- **Effect:** This unit’s **Gryph-hound** is a token. If this unit’s **Gryph-hound** has been removed, it can no longer attack with its **Gryph-hound’s Beak** and it cannot use the ‘Loyal Companion’ ability

#### Warding Lantern (Activated)
- **Timing:** Your Hero Phase
- **Declare:** Pick an objective within 12” of this unit to be the target.
- **Effect:** For the rest of the turn:
• Friendly Stormcast Eternals units have Ward (6+) while they are contesting the target.
• Ward rolls cannot be made for enemy units while they are contesting the target.

---

### Lord-Celestant
**Points:** 100 | **Move:** 5" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, INFANTRY, WARRIOR CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Weapons of Legend | Melee | - | 5 | 3+ | 3+ | 1 | 2 | Crit (Auto-wound) |

**Abilities:**

#### Furious Retribution (Activated)
- **Timing:** Reaction: You declared a Fight ability for this unit
- **Effect:** Pick a friendly non-Hero Warrior Chamber Infantry unit that has not used a Fight ability this turn and is within this unit’s combat range to be the target. The target can be picked to use a Fight ability immediately after the Fight ability used by this unit has been resolved. If it is picked to do so, add 1 to wound rolls for the target’s attacks for the rest of the turn.

---

### Lord-Celestant on Dracoth
**Points:** 160 | **Move:** 10" | **Health:** 8 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, CAVALRY, EXTREMIS CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Stormblast | Ranged | 10" | 1 | 3+ | 3+ | 2 | D3 | Crit (2 Hits), Companion |
| Dracoth’s Claws and Fangs | Melee | - | 3 | 4+ | 2+ | 2 | 2 | Companion |
| Weapons of Legend | Melee | - | 5 | 3+ | 3+ | 1 | 2 | Crit (Auto-wound) |

**Abilities:**

#### Righteous Avengers (Activated)
- **Timing:** Reaction: You declared a Fight ability for this unit
- **Effect:** Pick a friendly non-Hero Extremis Chamber Cavalry unit that has not used a Fight ability this turn and is within this unit’s combat range to be the target. The target can be picked to use a Fight ability immediately after the Fight ability used by this unit has been resolved.

---

### Lord-Celestant on Stardrake
**Points:** 480 | **Move:** 12" | **Health:** 18 | **Save:** 3+ | **Control:** 5
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, MONSTER, FLY, EXTREMIS CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Rain of Stars | Ranged | 24" | 4 | 3+ | 2+ | 2 | 2 | Companion |
| Stardrake's Cavernous Jaws | Melee | - | 7 | 4+ | 2+ | 2 | 3 | Companion |
| Weapons of Legend | Melee | - | 5 | 3+ | 3+ | 1 | 2 | Crit (Auto-wound) |

**Abilities:**

#### Cavernous Jaws (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll 3 dice. For each roll that equals or exceeds the target’s Health characteristic, 1 model in the target unit is slain.
- **Keywords:** Rampage

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of its **Stardrake's Cavernous Jaws** is 4.

#### Lord of the Host (Activated)
- **Timing:** Once Per Battle, Any Combat Phase
- **Effect:** Add 1 to the Attacks characteristic of other friendly Extremis Chamber units wholly within 12" of this unit for the rest of the turn.

---

### Lord-Commander Bastian Carthalos
**Points:** 190 | **Move:** 5" | **Health:** 8 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, UNIQUE, ORDER, STORMCAST ETERNALS, INFANTRY, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Thunderbolts of Azyr | Ranged | 18" | 2 | 2+ | 2+ | 2 | D3 | - |
| Uskavar, the Sunderer | Melee | - | 4 | 3+ | 2+ | 2 | 4 | Crit (Auto-wound) |

**Abilities:**

#### Voice of Thunder (Passive)
- **Effect:** Each time a friendly Stormcast Eternals unit wholly within 12" of this unit uses the ‘All-out Attack’ command, no command points are spent.

#### Castellan of Azyr (Activated)
- **Timing:** Deployment Phase
- **Declare:** Pick up to 3 friendly Stormcast Eternals Infantry units to be the targets.
- **Effect:** Each target can use the ‘Normal Move’ ability as if it were your movement phase.

#### Mantle of the First Storm (Activated)
- **Timing:** End of Any Turn
- **Effect:** If any enemy models were slain by this unit’s attacks this turn, **Heal (4)** this unit.

---

### Lord-Exorcist
**Points:** 150 | **Move:** 5" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, SACROSANCT CHAMBER, WIZARD (1), INFANTRY, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Redemption Stave | Melee | - | 3 | 3+ | 3+ | 1 | 2 | - |

**Abilities:**

#### Purifying Blast (Activated)
- **Timing:** Your Hero Phase
- **Declare:** Pick a visible enemy Priest within 18" of this unit to be the target.
- **Effect:** Roll a dice. On a 2+:
• If the target has any ritual points, remove a number of ritual points from it equal to the roll.
• If the target has no ritual points, inflict D3 mortal damage on it.

#### Redemptor Casket (Passive)
- **Effect:** Models cannot be returned to enemy units within 12" of this unit.

---

### Lord-Imperatant
**Points:** 110 | **Move:** 5" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, INFANTRY, WARRIOR CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Blessed Warhammer | Melee | - | 4 | 3+ | 3+ | 1 | 2 | Crit (Auto-wound) |

**Abilities:**

#### Guided by Lightning (Activated)
- **Timing:** Once Per Battle (Army), Your Movement Phase
- **Declare:** Pick up to 3 friendly Stormcast Eternals units wholly within 12" of this unit that were set up this turn using the ‘Scions of the Storm’ ability to be the targets.
- **Effect:** Each target can immediately move D3". The targets cannot move into combat during any part of that move.

#### Loyal Gryph-hound (Passive)
- **Effect:** This unit’s **Gryph-hound** is a token. This unit has Ward (5+) while its **Gryph-hound** is on the battlefield. If you make an unmodified save roll of 1 for this unit, remove its **Gryph-hound** from the battlefield after the Attack ability has been resolved (the damage point is still inflicted).

---

### Lord-Ordinator
**Points:** 100 | **Move:** 5" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, INFANTRY, SACROSANCT CHAMBER, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Astral Weapons | Melee | - | 3 | 3+ | 3+ | 1 | 2 | Crit (Auto-wound) |

**Abilities:**

#### Arcane Engineer (Activated)
- **Timing:** Once Per Turn (Army), Any Shooting Phase
- **Declare:** Pick a visible friendly Sacrosanct Chamber War Machine wholly within 12" of this unit to be the target.
- **Effect:** The target’s attacks score critical hits on unmodified hit rolls of 4+ for the rest of the phase.

---

### Lord-Relictor
**Points:** 120 | **Move:** 5" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, INFANTRY, PRIEST (1), WARRIOR CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Relic Hammer | Melee | - | 4 | 3+ | 3+ | 1 | 2 | Crit (Auto-wound) |

**Abilities:**

#### Mortis Priest (Passive)
- **Effect:** Add 1 to chanting rolls for this unit.

---

### Lord-Terminos
**Points:** 140 | **Move:** 5" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, INFANTRY, RUINATION CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| The Blade Terminos | Melee | - | 4 | 3+ | 2+ | 2 | 3 | Crit (Mortal) |

**Abilities:**

#### Earn an Honourable Death (Activated)
- **Timing:** Reaction: You declared a Fight ability for this unit
- **Effect:** Pick a friendly non-Hero Ruination Chamber Infantry unit that has not used a Fight ability this turn and is within this unit’s combat range to be the target. The target can be picked to use a Fight ability immediately after the Fight ability used by this unit has been resolved. If it is picked to do so, add 1 to hit rolls for the target’s attacks for the rest of the turn.

#### Memorian (Passive)
- **Effect:** This unit’s **Memorian** is a token. Add 3 to the control scores of friendly Ruination Chamber units wholly within 12" of this unit while its **Memorian** is on the battlefield. If you make an unmodified save roll of 1 for this unit, remove its **Memorian** from the battlefield after the Attack ability has been resolved (the damage point is still inflicted).

---

### Lord-Veritant
**Points:** 110 | **Move:** 5" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, INFANTRY, RUINATION CHAMBER, PRIEST (1)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Staff of Abjuration | Melee | - | 1 | 3+ | 3+ | 1 | 3 | - |
| Judgement Blade | Melee | - | 3 | 3+ | 3+ | 1 | D3 | Anti-Priest (+1 Rend), Anti-Wizard (+1 Rend) |

**Abilities:**

#### Sense Unholy Sorcery (Passive)
- **Effect:** This unit’s **Gryph-crow** is a token. Subtract 1 from casting rolls and chanting rolls for enemy units within 12" of this unit while its **Gryph-crow** is on the battlefield.
If you make an unmodified save roll of 1 for this unit, remove its **Gryph-crow** from the battlefield after the Attack ability has been resolved (the damage point is still inflicted).

#### Staff of Abjuration (Passive)
- **Effect:** This unit can use Unbind abilities as if it had Wizard (1).

---

### Lord-Vigilant on Gryph-stalker
**Points:** 140 | **Move:** 12" | **Health:** 8 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, RUINATION CHAMBER, CAVALRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Gryph-stalker’s Beak and Talons | Melee | - | 3 | 4+ | 3+ | 1 | 2 | Companion |
| Hallowed Greataxe | Melee | - | 5 | 3+ | 3+ | 2 | 2 | - |

**Abilities:**

#### Deliver Judgement (Activated)
- **Timing:** Once Per Battle (Army), Any Combat Phase
- **Declare:** Pick a friendly non-Hero Ruination Chamber unit wholly within 12" of this unit.
- **Effect:** That unit can use 2 Fight abilities this phase. After the first is used, however, that unit has Strike-last for the rest of the turn.

---

### Lord-Vigilant on Morrgryph
**Points:** 180 | **Move:** 12" | **Health:** 12 | **Save:** 3+ | **Control:** 5
**Keywords:** HERO, ORDER, STORMCAST ETERNALS, RUINATION CHAMBER, MONSTER, FLY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Morrgryph's Talons | Melee | - | 6 | 4+ | 2+ | 2 | 2 | Companion |
| Hallowed Glaive | Melee | - | 5 | 3+ | 3+ | 1 | 2 | Charge (+1 Damage) |

**Abilities:**

#### Direct the Assault (Activated)
- **Timing:** Once Per Turn (Army), Your Hero Phase
- **Declare:** Pick a friendly non-Hero Ruination Chamber unit wholly within 12" of this unit to be the target.
- **Effect:** For the rest of the turn, while the target is wholly within 12" of this unit, add 5 to the target’s control score.

#### Vice-like Grip (Activated)
- **Timing:** Once Per Turn (Army), Any Charge Phase
- **Declare:** Pick an enemy non-Monster Hero in combat with this unit to be the target.
- **Effect:** Roll 2D6. If the result exceeds the target’s Health characteristic, for the rest of the turn:
• The target has Strike-last.
• The target cannot use commands.

---

### Neave Blacktalon
**Points:** 280 | **Move:** 7" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, UNIQUE, ORDER, STORMCAST ETERNALS, INFANTRY, VANGUARD CHAMBER, THE BLACKTALONS

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Relic Boltstorm Pistol | Ranged | 10" | 2 | 3+ | 3+ | 1 | 1 | Shoot in Combat |
| Whirlwind Axes | Melee | - | 7 | 2+ | 3+ | 2 | 2 | Crit (2 Hits) |

**Abilities:**

#### Justice From Azyr (Passive)
- **Effect:** Add 1 to the Damage characteristic of this unit’s **Whirlwind Axes** for attacks that target a Hero.

#### Windrider (Activated)
- **Timing:** Your Combat Phase
- **Declare:** If this unit is in combat or charged this turn, this unit can make a pile-in move. Then, if this unit is in combat, you must pick 1 or more enemy units to be the target(s) of this unit’s attacks.
- **Effect:** Resolve combat attacks against the target unit(s). Then, roll a dice. On a 3+, remove this unit from the battlefield and set it up again on the battlefield more than 3" from all enemy units and wholly within the combat range of a friendly The Blacktalons unit that is not in combat.
- **Keywords:** Core, Attack, Fight

---

### Praetors
**Points:** 120 | **Move:** 5" | **Health:** 3 | **Save:** 3+ | **Control:** 1
**Keywords:** Warrior Chamber Infantry, INFANTRY, CHAMPION, ORDER, STORMCAST ETERNALS, WARRIOR CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Soulguard’s Halberd | Melee | - | 3 | 3+ | 3+ | 1 | 2 | - |

**Abilities:**

#### Soul-bonded (Activated)
- **Timing:** Deployment Phase
- **Effect:** Pick a friendly Stormcast Eternals Hero to be this unit’s soul-bonded charge for the rest of the battle. This unit can use this ability while it is in reserve.

#### Soul-forged Guardians (Passive)
- **Effect:** While this unit’s soul-bonded charge is within this unit’s combat range, both this unit and that soul-bonded charge have Ward (5+).

---

### Prosecutors
**Points:** 150 | **Move:** 12" | **Health:** 2 | **Save:** 3+ | **Control:** 1
**Keywords:** INFANTRY, CHAMPION, ORDER, STORMCAST ETERNALS, FLY, RUINATION CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Stormcall Javelin | Melee | - | 3 | 3+ | 3+ | 1 | 1 | Charge (+1 Damage) |
| Stormcall Javelin | Ranged | 10" | 1 | 3+ | 3+ | 1 | D3 | - |

**Abilities:**

#### Dispersed Formation (Passive)
- **Effect:** This unit has a coherency range of 2".

#### Heralds of Righteousness (Passive)
- **Effect:** Add 1 to the number of dice rolled when making charge rolls for this unit, to a maximum of 3.

---

### Protectors
**Points:** 220 | **Move:** 4" | **Health:** 3 | **Save:** 3+ | **Control:** 1
**Keywords:** Warrior Chamber Infantry, CHAMPION, ORDER, STORMCAST ETERNALS, INFANTRY, WARRIOR CHAMBER, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Starsoul Mace | Melee | - | 1 | 3+ | 3+ | 2 | 3 | Crit (Mortal) |
| Protector Stormstrike Glaive | Melee | - | 4 | 3+ | 3+ | 1 | 1 | - |

**Abilities:**

#### Shield of the Storm (Passive)
- **Effect:** While any friendly Stormcast Eternals Infantry Heroes are within this unit’s combat range, both this unit and those friendly units have Ward (5+).

---

### Questor Soulsworn
**Points:** 210 | **Move:** 5" | **Health:** 3 | **Save:** 3+ | **Control:** 1
**Keywords:** Warrior Chamber Infantry, INFANTRY, ORDER, STORMCAST ETERNALS, WARRIOR CHAMBER, CHAMPION (1/6)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Questor Weapons | Melee | - | 3 | 3+ | 3+ | 1 | 2 | - |

**Abilities:**

#### Ordained Quest (Activated)
- **Timing:** Once Per Battle (Army), Deployment Phase
- **Effect:** Pick an objective wholly outside friendly territory. That objective is considered by you to be questmarked.

#### His Will Be Done (Passive)

#### Relictor Soulguide (Activated)
- **Timing:** Once Per Battle, Your Movement Phase
- **Declare:** You can pick a friendly **Knight-Questor** within this unit’s combat range to be the target.
- **Effect:** Remove this unit from the battlefield and set it up again on the battlefield more than 9" from all enemy units. Then, if you picked a target, remove it from the battlefield and set it up again within this unit’s combat range and more than 9" from all enemy units.
- **Keywords:** Core

---

### Reclusians
**Points:** 140 | **Move:** 5" | **Health:** 3 | **Save:** 3+ | **Control:** 1
**Keywords:** INFANTRY, CHAMPION, ORDER, STORMCAST ETERNALS, RUINATION CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Rune-blessed Weapons | Melee | - | 3 | 3+ | 3+ | 1 | 2 | Crit (Mortal) |

**Abilities:**

#### Memorian Descendants (Passive)
- **Effect:** This unit’s **Memorians** are tokens. There are 2 **Memorians** for every 3 models in this unit. While any of this unit’s **Memorians** are on the battlefield, add 1 to this unit’s resistance rolls when using the ‘Ruination Chamber’ ability. Each time you make an unmodified save roll of 1 for this unit, remove 1 of its **Memorians** from the battlefield after the Attack ability has been resolved (the damage point is still inflicted).

---

### Retributors
**Points:** 170 | **Move:** 4" | **Health:** 3 | **Save:** 3+ | **Control:** 1
**Keywords:** Warrior Chamber Infantry, CHAMPION, ORDER, STORMCAST ETERNALS, INFANTRY, WARRIOR CHAMBER, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Starsoul Mace | Melee | - | 1 | 3+ | 3+ | 2 | 3 | Crit (Mortal) |
| Retributor Lightning Hammer | Melee | - | 2 | 3+ | 3+ | 1 | 2 | Crit (Mortal) |

**Abilities:**

#### Crushing Counter-blow (Passive)
- **Effect:** If this unit is damaged or if any models in this unit were slain this turn, add 1 to hit rolls for this unit’s attacks for the rest of the turn.

---

### Sequitors
**Points:** 120 | **Move:** 5" | **Health:** 2 | **Save:** 3+ | **Control:** 1
**Keywords:** Sacrosanct Chamber Infantry, CHAMPION, ORDER, STORMCAST ETERNALS, SACROSANCT CHAMBER, INFANTRY, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Stormsmite Greatmace | Melee | - | 2 | 3+ | 3+ | 1 | 2 | Crit (2 Hits) |
| Sacrosanct Weapons | Melee | - | 2 | 3+ | 3+ | 1 | 1 | Crit (2 Hits) |

**Abilities:**

#### Sequitor Aetheric Channelling (Activated)
- **Timing:** Any Combat Phase

---

### Steelheart's Champions
**Points:** 110 | **Move:** 5" | **Health:** 3 | **Save:** 3+ | **Control:** 1
**Keywords:** Warrior Chamber Infantry, ORDER, INFANTRY, UNIQUE, Legends, STORMCAST ETERNALS, WARRIOR CHAMBER, CHAMPION

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Heavens-wrought Weapon | Melee | - | 3 | 3+ | 3+ | 1 | 1 | Crit (Mortal) |
| Heavens-wrought Weapon | Melee | - | 3 | 3+ | 3+ | 1 | 1 | Crit (Mortal) |
| Heavens-wrought Weapon | Melee | - | 3 | 3+ | 3+ | 1 | 1 | Crit (Mortal) |

**Abilities:**

#### Heroic Guard (Passive)
- **Effect:** Add 1 to save rolls for this unit while it is in combat with any enemy units that charged in the same turn.

#### Stalwart Defenders (Passive)

---

### Stormcoven
**Points:** 210 | **Move:** 5" | **Health:** 3 | **Save:** 3+ | **Control:** 1
**Keywords:** ORDER, INFANTRY, STORMCAST ETERNALS, WIZARD (1), WARRIOR CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Azyrite Bolts | Ranged | 18" | 2 | 3+ | 4+ | 1 | D3 | - |
| Valedictor Weapons | Melee | - | 3 | 3+ | 3+ | 1 | D3 | - |

**Abilities:**

#### Gather the Aether (Passive)

#### Aethershock (Spell)
- **Timing:** Your Hero Phase
- **Casting Value:** 7
- **Declare:** Pick a visible unit within 12" of the caster to be the target, then make a casting roll of 2D6.
- **Effect:** If the target is a friendly unit, it has Strike-first for the rest of the turn. If the target is an enemy unit, it has Strike-last for the rest of the turn.
- **Keywords:** Spell

---

### Stormdrake Guard
**Points:** 310 | **Move:** 12" | **Health:** 9 | **Save:** 3+ | **Control:** 5
**Keywords:** CHAMPION, ORDER, STORMCAST ETERNALS, FLY, MONSTER, EXTREMIS CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Draconith’s Fangs and Talons | Melee | - | 4 | 4+ | 2+ | 2 | 2 | Companion |
| Draconic Flamestream | Ranged | 10" | 4 | 2+ | 3+ | 2 | 1 | Shoot in Combat, Companion |
| Drakerider’s Lance | Melee | - | 3 | 3+ | 3+ | 1 | 1 | Charge (+1 Damage), Anti‑Infantry (+1 Rend) |
| Drakerider’s Warblade | Melee | - | 6 | 3+ | 3+ | 1 | 1 | - |

**Abilities:**

#### Dispersed Formation (Passive)
- **Effect:** This unit has a coherency range of 2".

#### Draconic Onslaught (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Effect:** If this unit charged this turn, roll a dice. On a 3+, this unit can move 2D6". It can pass through the combat ranges of enemy units and can end that move in combat. Then, pick an enemy unit that this unit passed across during that move. Inflict D3 mortal damage on that enemy unit.
- **Keywords:** Rampage

---

### Stormreach Portal
**Points:** 20 | **Move:** - | **Health:** 12 | **Save:** 4+ | **Control:** -
**Keywords:** FACTION TERRAIN, ORDER, STORMCAST ETERNALS

**Abilities:**

#### Step Into the Storm (Activated)
- **Timing:** Your Movement Phase
- **Declare:** Pick a friendly non-Monster Stormcast Eternals unit that is not in combat and is wholly within 6" of this terrain feature to be the target.
- **Effect:** Remove the target from the battlefield and set it up again on the battlefield more than 9" from all enemy units.

---

### Stormsire's Cursebreakers
**Points:** 130 | **Move:** 5" | **Health:** 3 | **Save:** 3+ | **Control:** 1
**Keywords:** Sacrosanct Chamber Infantry, ORDER, INFANTRY, UNIQUE, Legends, STORMCAST ETERNALS, SACROSANCT CHAMBER, CHAMPION

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Cursebreaker Weapons | Melee | - | 3 | 3+ | 3+ | 1 | 1 | Crit (Mortal) |
| Cursebreaker Weapons | Melee | - | 3 | 3+ | 3+ | 1 | 1 | Crit (Mortal) |
| Cursebreaker Weapons | Melee | - | 3 | 3+ | 3+ | 1 | 1 | Crit (Mortal) |

**Abilities:**

#### Averon Stormsire (Passive)
- **Effect:** While this unit’s **Averon Stormsire** is on the battlefield, it has Wizard (1).

#### Overwhelming Storm (Activated)
- **Timing:** Any Combat Phase
- **Effect:** For the rest of the phase, add 1 to wound rolls for the target’s combat attacks.

---

### Stormstrike Chariot
**Points:** 110 | **Move:** 10" | **Health:** 10 | **Save:** 3+ | **Control:** 2
**Keywords:** ORDER, STORMCAST ETERNALS, WAR MACHINE, WARRIOR CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Gryph-chargers’ Razor Beaks | Melee | - | 6 | 4+ | 3+ | 1 | 1 | Companion |
| Stormstrike Axe | Melee | - | 3 | 3+ | 3+ | 1 | 1 | Charge (+1 Damage) |
| Great Stormbow | Ranged | 18" | 2 | 3+ | 3+ | 1 | 1 | Anti-Infantry (+1 Rend), Crit (2 Hits) |
| Tempestuous Spear | Melee | - | 2 | 3+ | 3+ | 2 | 1 | - |

**Abilities:**

#### Celestial Blaze (Passive)
- **Effect:** When this unit uses the ‘Power Through’ command, inflict an additional D3 mortal damage on the target and add D6" to the distance this unit can move as part of that ability

#### Azyr Unleashed (Activated)
- **Timing:** Any Charge Phase
- **Declare:** If this unit charged this phase, pick an enemy unit within 1" of it to be the target.
- **Effect:** Roll a D3. On a 2+, inflict an amount of mortal damage on the target equal to the roll.

---

### Stormstrike Palladors
**Points:** 180 | **Move:** 12" | **Health:** 5 | **Save:** 3+ | **Control:** 2
**Keywords:** ORDER, STORMCAST ETERNALS, CAVALRY, CHAMPION, WARRIOR CHAMBER, MUSICIAN (1/3), STANDARD BEARER (1/3)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Stormstrike Lance or Blade | Melee | - | 3 | 3+ | 3+ | 1 | 1 | Anti-Infantry (+1 Rend), Charge (+1 Damage) |
| Gryph-charger’s Talons | Melee | - | 3 | 4+ | 3+ | 1 | 1 | Companion |

**Abilities:**

#### Ride for Vengeance (Passive)
- **Effect:** If any friendly Warrior Chamber units have been destroyed this battle:
• You can re-roll charge rolls for this unit.
• This unit has Strike-first if it charged in the same turn.

---

### The Emberwatch
**Points:** 140 | **Move:** 5" | **Health:** 3 | **Save:** 3+ | **Control:** 1
**Keywords:** ORDER, INFANTRY, UNIQUE, Legends, STORMCAST ETERNALS, CHAMPION, VANGUARD CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Boltstorm Weapon | Ranged | 10" | 3 | 4+ | 4+ | 1 | 1 | Shoot in Combat |
| Stormwrought Weapon | Melee | - | 3 | 3+ | 3+ | 1 | 1 | Crit (Mortal) |
| Stormwrought Weapon | Melee | - | 3 | 3+ | 3+ | 1 | 1 | Crit (Mortal) |
| Boltstorm Weapon | Ranged | 10" | 3 | 4+ | 4+ | 1 | 1 | Shoot in Combat |
| Stormwrought Weapon | Melee | - | 3 | 3+ | 3+ | 1 | 1 | Crit (Mortal) |
| Boltstorm Weapon | Ranged | 10" | 3 | 4+ | 4+ | 1 | 1 | Shoot in Combat |

**Abilities:**

#### Merciless Sentinels (Passive)
- **Effect:** Add 1 to wound rolls for this unit’s attacks while it contests an objective that is wholly outside friendly territory.

---

### The Farstriders
**Points:** 100 | **Move:** 5" | **Health:** 3 | **Save:** 3+ | **Control:** 1
**Keywords:** ORDER, INFANTRY, UNIQUE, Legends, STORMCAST ETERNALS, CHAMPION, VANGUARD CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Boltstorm Pistol | Ranged | 10" | 2 | 4+ | 4+ | 1 | 1 | Shoot in Combat |
| Stormwrought Weapon | Melee | - | 2 | 3+ | 3+ | 1 | 1 | - |
| Boltstorm Pistol | Ranged | 10" | 2 | 4+ | 4+ | 1 | 1 | Shoot in Combat |
| Stormwrought Weapon | Melee | - | 2 | 3+ | 3+ | 1 | 1 | - |
| Boltstorm Pistol | Ranged | 10" | 2 | 4+ | 4+ | 1 | 1 | Shoot in Combat |
| Stormwrought Weapon | Melee | - | 2 | 3+ | 3+ | 1 | 1 | - |

**Abilities:**

#### Star Falcon (Activated)
- **Timing:** Your Shooting Phase
- **Declare:** If this unit’s **Sanson Farstrider** is on the battlefield, pick a visible enemy unit within 18" of this unit to be the target.
- **Effect:** Roll a dice. On a 3+, inflict 1 mortal damage on the target.

---

### Tornus the Redeemed
**Points:** 150 | **Move:** 12" | **Health:** 8 | **Save:** 3+ | **Control:** 5
**Keywords:** HERO, FLY, UNIQUE, ORDER, STORMCAST ETERNALS, RUINATION CHAMBER, INFANTRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Hallowed Greatspear | Ranged | 12" | 1 | 3+ | 3+ | 2 | 2 | Crit (2 Hits) |
| Hallowed Greatspear | Melee | - | 5 | 3+ | 3+ | 2 | 2 | Crit (2 Hits) |

**Abilities:**

#### The Light of Sigmar (Activated)
- **Timing:** Any Combat Phase
- **Effect:** Roll a dice. On a 3+, pick 1 of the following effects:
• For the rest of the turn, subtract 1 from hit rolls for attacks made by enemy units while they are in combat with this unit.
• Heal (D3) each friendly Stormcast Eternals unit within this unit’s combat range.

#### Herald of Redemption (Passive)
- **Effect:** If a friendly Ruination Chamber unit wholly within 12" of this unit uses the ‘Rally’ command, you receive 3 additional rally points.

---

### Vandus Hammerhand
**Points:** 160 | **Move:** 10" | **Health:** 8 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, UNIQUE, ORDER, STORMCAST ETERNALS, WARRIOR CHAMBER, CAVALRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Stormblast | Ranged | 10" | 1 | 3+ | 3+ | 2 | D3 | Crit (2 Hits), Companion |
| Heldensen | Melee | - | 6 | 3+ | 2+ | 1 | 2 | Crit (Auto-wound) |
| Calanax’s Claws and Fangs | Melee | - | 4 | 4+ | 2+ | 2 | 2 | Companion |

**Abilities:**

#### Lord of the Hammerhands (Passive)
- **Effect:** Add 3 to the control scores of friendly Warrior Chamber units while they are wholly within 12" of this unit.

#### Vengeance Unleashed (Activated)
- **Timing:** Reaction: You declared a Fight ability for this unit
- **Effect:** Pick a friendly non-Hero Warrior Chamber unit that has not used a Fight ability this turn and is within this unit’s combat range to be the target. The target can be picked to use a Fight ability immediately after the Fight ability used by this unit has been resolved. If it is picked to do so, add 1 to hit rolls for the target’s attacks for the rest of the turn.

---

### Vanguard-Hunters
**Points:** 120 | **Move:** 5" | **Health:** 2 | **Save:** 3+ | **Control:** 1
**Keywords:** INFANTRY, CHAMPION, STANDARD BEARER (1/5), ORDER, STORMCAST ETERNALS, VANGUARD CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Boltstorm Pistol | Ranged | 10" | 2 | 4+ | 4+ | 1 | 1 | Shoot in Combat |
| Stormwrought Weapon | Melee | - | 2 | 3+ | 3+ | 1 | 1 | - |

**Abilities:**

#### Star-marked (Passive)
- **Effect:** Add 2" to this unit’s Move characteristic while a Hunted enemy unit is on the battlefield. In addition, add 1 to hit rolls for this unit’s attacks that target a Hunted enemy unit.

#### Astral Compass (Activated)
- **Timing:** Once Per Battle (Army), Deployment Phase
- **Declare:** Pick an enemy unit to be the target.
- **Effect:** The target has the Hunted keyword for the rest of the battle.

---

### Vanguard-Palladors with Shock Handaxes
**Points:** 250 | **Move:** 14" | **Health:** 5 | **Save:** 3+ | **Control:** 2
**Keywords:** ORDER, STORMCAST ETERNALS, CAVALRY, CHAMPION, VANGUARD CHAMBER, VANGUARD-PALLADORS

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Shock Handaxe | Melee | - | 2 | 3+ | 3+ | 1 | 2 | Charge (+1 Damage) |
| Boltstorm Pistol | Ranged | 10" | 2 | 4+ | 4+ | 1 | 1 | Shoot in Combat |
| Gryph-charger’s Razor Beak | Melee | - | 3 | 4+ | 3+ | 1 | 1 | Companion |

**Abilities:**

#### Shocking Strikes (Passive)
- **Effect:** If any damage points are allocated to an enemy unit by attacks made with this unit’s **Shock Handaxes**, subtract 1 from wound rolls for that enemy unit’s combat attacks for the rest of the turn.

---

### Vanguard-Palladors with Starstrike Javelins
**Points:** 250 | **Move:** 14" | **Health:** 5 | **Save:** 3+ | **Control:** 2
**Keywords:** ORDER, STORMCAST ETERNALS, CAVALRY, CHAMPION, VANGUARD CHAMBER, VANGUARD-PALLADORS

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Starstrike Javelin | Ranged | 12" | 1 | 3+ | 3+ | 1 | 2 | - |
| Starstrike Javelin | Melee | - | 1 | 3+ | 3+ | 1 | 2 | - |
| Boltstorm Pistol | Ranged | 10" | 2 | 4+ | 4+ | 1 | 1 | Shoot in Combat |
| Gryph-charger’s Razor Beak | Melee | - | 3 | 4+ | 3+ | 1 | 1 | Companion |

**Abilities:**

#### Charging Volley (Activated)
- **Timing:** Your Charge Phase
- **Declare:** If this unit charged this phase, pick an enemy unit in combat with it to be the target.
- **Effect:** This unit can immediately use a Shoot ability as if it were your shooting phase, but all of its attacks must target that enemy unit. In addition, unit’s **Starstrike Javelins** have **Shoot in Combat** this phase.

---

### Vanguard-Raptors with Hurricane Crossbows
**Points:** 110 | **Move:** 5" | **Health:** 2 | **Save:** 3+ | **Control:** 1
**Keywords:** INFANTRY, CHAMPION, ORDER, STORMCAST ETERNALS, VANGUARD CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Hurricane Crossbow | Ranged | 12" | 4 | 3+ | 3+ | - | 1 | Anti-Infantry (+1 Rend), Crit (2 Hits) |
| Heavy Stock | Melee | - | 1 | 4+ | 3+ | - | 1 | - |

**Abilities:**

#### Coordinated Strike (Activated)
- **Timing:** Your Shooting Phase
- **Declare:** Pick an enemy unit that had any damage points allocated to it this turn by attacks made with this unit’s **Hurricane Crossbows** to be the target.
- **Effect:** Add 1 to wound rolls for combat attacks made by friendly Vanguard Chamber units that target that enemy unit for the rest of the turn.

---

### Vanguard-Raptors with Longstrike Crossbows
**Points:** 200 | **Move:** 5" | **Health:** 2 | **Save:** 3+ | **Control:** 1
**Keywords:** INFANTRY, CHAMPION, ORDER, STORMCAST ETERNALS, VANGUARD CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Heavy Stock | Melee | - | 1 | 4+ | 3+ | - | 1 | - |
| Longstrike Crossbow | Ranged | 18" | 2 | 3+ | 3+ | 2 | 2 | Anti‑Hero (+1 Rend), Crit (Auto-wound) |

**Abilities:**

#### Headshot (Activated)
- **Timing:** Your Shooting Phase
- **Effect:** If this unit has not used a Move ability this turn and was not set up this turn, for the rest of the turn, this unit can ignore the effects of the ‘Guarded Hero’ ability (Core Rules, 25.0) when picking targets for its shooting attacks.

---

### Vanquishers
**Points:** 90 | **Move:** 5" | **Health:** 2 | **Save:** 3+ | **Control:** 1
**Keywords:** Warrior Chamber Infantry, INFANTRY, CHAMPION, MUSICIAN (1/5), STANDARD BEARER (1/5), ORDER, STORMCAST ETERNALS, WARRIOR CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Celestial Greatsword | Melee | - | 2 | 3+ | 3+ | 1 | 1 | Anti-Infantry (+1 Rend) |

**Abilities:**

#### Lightning Strikes (Passive)
- **Effect:** Add 1 to the Damage characteristic of this unit’s **Celestial Greatswords** for attacks that target an enemy unit that has 5 or more models.

---

### Vigilors
**Points:** 140 | **Move:** 6" | **Health:** 2 | **Save:** 3+ | **Control:** 1
**Keywords:** Warrior Chamber Infantry, INFANTRY, CHAMPION, ORDER, STORMCAST ETERNALS, WARRIOR CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Stormcaller Bow | Ranged | 18" | 2 | 3+ | 3+ | 1 | 1 | - |
| Stormblade | Melee | - | 2 | 3+ | 3+ | 1 | 1 | - |

**Abilities:**

#### Navigators of the Storm (Passive)
- **Effect:** If an enemy unit had any damage points allocated to it this turn by this unit’s shooting attacks, add 1 to hit rolls for attacks made by friendly Stormcast Eternals units that target that unit for the rest of the turn.

---

### Vindictors
**Points:** 90 | **Move:** 5" | **Health:** 2 | **Save:** 3+ | **Control:** 1
**Keywords:** Warrior Chamber Infantry, INFANTRY, CHAMPION, STANDARD BEARER (1/5), ORDER, STORMCAST ETERNALS, WARRIOR CHAMBER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Stormspear | Melee | - | 2 | 3+ | 3+ | 1 | 1 | Anti-charge (+1 Rend) |

**Abilities:**

#### Hold the Shieldwall (Activated)
- **Timing:** Any Combat Phase
- **Effect:** If this unit did not charge this turn and is in combat with an enemy unit that charged this turn, roll a dice. On a 4+, this unit has Strike-first for the rest of the turn.

---

### Xandire's Truthseekers
**Points:** 130 | **Move:** 5" | **Health:** 3 | **Save:** 3+ | **Control:** 1
**Keywords:** Warrior Chamber Infantry, ORDER, INFANTRY, UNIQUE, Legends, STORMCAST ETERNALS, WARRIOR CHAMBER, CHAMPION

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Truthseeker Weapons | Melee | - | 3 | 3+ | 3+ | 1 | 1 | Crit (Mortal) |
| Great Hammer | Melee | - | 2 | 3+ | 3+ | 1 | 2 | Crit (Mortal), Anti-Monster (+1 Rend) |
| Stormcaller Bow | Ranged | 18" | 2 | 3+ | 3+ | 1 | 1 | Crit (Mortal) |
| Truthseeker Weapons | Melee | - | 3 | 3+ | 3+ | 1 | 1 | Crit (Mortal) |

**Abilities:**

#### Taros (Passive)
- **Effect:** This unit’s **Taros** is a token. After setting up this unit on the battlefield for the first time, place **Taros** next to it.

#### Harrying Aetherwing (Activated)
- **Timing:** Your Shooting Phase
- **Declare:** If any attacks made with this unit’s **Stormcaller Bow** scored a critical hit this phase, pick an enemy unit targeted by those attacks to be the target.
- **Effect:** Until the start of your next turn, place **Taros** next to the target. While **Taros** is placed next to an enemy unit, that enemy unit cannot use commands.

---

### Yndrasta, the Celestial Spear
**Points:** 270 | **Move:** 12" | **Health:** 8 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, WARD (5+), UNIQUE, ORDER, STORMCAST ETERNALS, INFANTRY, WARMASTER, FLY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Thengavar | Ranged | 12" | 1 | 3+ | 2+ | 2 | 4 | Anti-Monster (+1 Rend) |
| Blade of the High Heavens | Melee | - | 5 | 3+ | 3+ | 2 | 3 | - |
| Thengavar | Melee | - | 2 | 2+ | 2+ | 2 | 3 | Anti-Monster (+1 Rend) |

**Abilities:**

#### The Prime Huntress (Passive)
- **Effect:** Double the Damage characteristic of this unit’s **Thengavar** for attacks that target Monsters.

#### Dazzling Radiance (Passive)
- **Effect:** Ignore negative modifiers to the control scores of friendly Stormcast Eternals units while they are wholly within 12" of this unit.

#### On Wings of Brilliance (Passive)
- **Effect:** Add 1 to the number of dice rolled when making charge rolls for this unit, to a maximum of 3.

---

## Points Table

| Unit | Points |
| --- | ---: |
| Stormreach Portal | 20 |
| Aetherwings | 80 |
| Gryph-hounds | 90 |
| Liberators | 90 |
| Vanquishers | 90 |
| Vindictors | 90 |
| Knight-Vexillor | 100 |
| Lord-Celestant | 100 |
| Lord-Ordinator | 100 |
| The Farstriders | 100 |
| Castigators | 110 |
| Knight-Heraldor | 110 |
| Knight-Questor | 110 |
| Lord-Imperatant | 110 |
| Lord-Veritant | 110 |
| Steelheart's Champions | 110 |
| Stormstrike Chariot | 110 |
| Vanguard-Raptors with Hurricane Crossbows | 110 |
| Celestar Ballista | 120 |
| Gavriel Sureheart | 120 |
| Knight-Arcanum | 120 |
| Knight-Azyros | 120 |
| Knight-Relictor | 120 |
| Lord-Relictor | 120 |
| Praetors | 120 |
| Sequitors | 120 |
| Vanguard-Hunters | 120 |
| Annihilators | 130 |
| Knight-Judicator with Gryph-hounds | 130 |
| Knight-Zephyros | 130 |
| Stormsire's Cursebreakers | 130 |
| Xandire's Truthseekers | 130 |
| Judicators with Skybolt Bows | 140 |
| Knight-Incantor | 140 |
| Lord-Aquilor | 140 |
| Lord-Arcanum | 140 |
| Lord-Terminos | 140 |
| Lord-Vigilant on Gryph-stalker | 140 |
| Reclusians | 140 |
| The Emberwatch | 140 |
| Vigilors | 140 |
| Lord-Castellant | 150 |
| Lord-Exorcist | 150 |
| Prosecutors | 150 |
| Tornus the Redeemed | 150 |
| Gardus Steel Soul | 160 |
| Judicators with Boltstorm Crossbows | 160 |
| Lord-Celestant on Dracoth | 160 |
| Vandus Hammerhand | 160 |
| Dracothian Guard Desolators | 170 |
| Dracothian Guard Tempestors | 170 |
| Knight-Venator | 170 |
| Retributors | 170 |
| Annihilators with Meteoric Grandhammers | 180 |
| Evocators | 180 |
| Lord-Vigilant on Morrgryph | 180 |
| Stormstrike Palladors | 180 |
| Lord-Commander Bastian Carthalos | 190 |
| Dracothian Guard Fulminators | 200 |
| Vanguard-Raptors with Longstrike Crossbows | 200 |
| Domitan's Stormcoven | 210 |
| Dracothian Guard Concussors | 210 |
| Lord-Arcanum on Celestial Dracoline | 210 |
| Questor Soulsworn | 210 |
| Stormcoven | 210 |
| Astreia Solbright | 220 |
| Lord-Arcanum on Gryph-charger | 220 |
| Protectors | 220 |
| Hamilcar Bear-Eater | 230 |
| Knight-Draconis | 230 |
| Decimators | 240 |
| Evocators on Celestial Dracolines | 240 |
| Celestant-Prime, Hammer of Sigmar | 250 |
| Vanguard-Palladors with Shock Handaxes | 250 |
| Vanguard-Palladors with Starstrike Javelins | 250 |
| Iridan the Witness | 260 |
| Yndrasta, the Celestial Spear | 270 |
| Lord-Arcanum on Tauralon | 280 |
| Neave Blacktalon | 280 |
| Aventis Firestrike, Magister of Hammerhal | 310 |
| Stormdrake Guard | 310 |
| Iridan the Witness (Scourge of Ghyran) | 320 |
| Ionus Cryptborn, Warden of Lost Souls | 360 |
| Drakesworn Templar | 390 |
| Karazai the Scarred | 440 |
| Lord-Celestant on Stardrake | 480 |
| Krondys, Son of Dracothion | 500 |
