# Disciples of Tzeentch

## Army Rules

### Anvil of Apotheosis

#### Illustrious Daemon (Passive)
- **Effect:** Each time a friendly Disciples of Tzeentch unit wholly within 12" of this unit uses the 'Rally' command, you can make 3 additional rally rolls of D6.

#### Tzaangor Boss (Passive)
- **Effect:** Each time a friendly Warflock unit is set up wholly within 12" of this unit, that Warflock unit can immediately move D3" but cannot end that move in combat.

#### Eldritch Artificer (Activated)
- **Timing:** Your Hero Phase
- **Declare:** Pick a friendly Disciples of Tzeentch unit within this unit's combat range to be the target.
- **Effect:** Roll a dice. On a 3+, the target's melee weapons have **Crit (2 Hits)** for the rest of the turn.

#### Third Eye (Passive)
- **Effect:** **Heal (X)** this unit, where **X** is the number of **fate points** you have.

#### Master of Corruption (Passive)
- **Effect:** Add 5 to this unit's control score if it was set up in the same turn.

#### Endless Minions (Command)
- **Timing:** Once Per Battle, Your Movement Phase
- **Cost:** 1
- **Declare:** Pick a friendly non-Hero Arcanite unit that has been destroyed to be the target.
- **Effect:** Set up a replacement unit with half the number of models from the target unit (rounding up) wholly within 9" of a battlefield edge and more than 9" from all enemy units.

#### Cursed by Tzeentch (Passive)
- **Effect:** This unit no longer has the Wizard (1) keyword.

#### Ever-shifting Mind (Passive)
- **Effect:** This unit cannot contest objectives that it contested in the previous turn.

#### Limned in Wyrdflame (Passive)
- **Effect:** This unit can only be set up using a Deploy ability.

#### Mutated Limbs (Passive)
- **Effect:** This unit's Move characteristic is D6"+2".

#### Destined to Survive (Fate)
- **Timing:** Any Hero Phase
- **Effect:** Spend any number of **fate points**. For each **fate point** you spend, **Heal (1)** this unit.
- **Fate Points:** 1+

#### Flash of Insight (Activated)
- **Timing:** Deployment Phase
- **Effect:** This unit can use the 'Normal Move' ability as if it were your movement phase.

#### Illusory Aura (Passive)
- **Effect:** If this unit was set up in the same turn, friendly Disciples of Tzeentch units have Ward (5+) while they are wholly within 12" of this unit.

#### Fated Slayer (Passive)
- **Effect:** Add 3 to the Attacks characteristic of this unit's melee weapons while you have 6 or more **fate points**.

#### Master Arcanist (Passive)
- **Effect:** Add 1 to casting rolls and banishment rolls for this unit.

#### Blazing Trail (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Effect:** This unit can move a distance up to its Move characteristic. It can pass through other models and the combat ranges of enemy units, but it cannot end that move in combat. Then, pick up to 1 enemy unit that this unit passed across during that move to be the target. Roll a dice for each model in the target unit. For each 5+, inflict 1 mortal damage on the target.
**Designer's Note:** *This unit can use this ability even if it is in combat.*
- **Keywords:** Core, Move

#### Slash and Rip (Activated)
- **Timing:** Once Per Turn (Army), Any Charge Phase
- **Declare:** If this unit charged this phase, pick an enemy unit in combat with it to be the target.
- **Effect:** Roll a number of dice equal to the unmodified charge roll. For each 4+, inflict 1 mortal damage on the target.

#### Eater of Fate (Activated)
- **Timing:** Any Hero Phase
- **Declare:** Pick a friendly Disciples of Tzeentch unit wholly within 12" of this unit to be the target.
- **Effect:** If the target is destroyed this turn, before removing it from play, gain 1 **fate point**.

### Paths

#### Spelljaunt (Passive)
- **Effect:** When using the 'Smoke and Mirrors' ability, if this unit is **masked by illusion**, you can pick a Manifestation (friendly or enemy) that was not set up in the same turn to be the target instead. If you do so, you must pick this unit to be the substitute. After setting up the substitute, the target is banished instead of being set up in reserve.

#### Master of Misdirection (Passive)
- **Effect:** Subtract 1 from hit rolls for attacks that target this unit while it is within the combat range of another friendly **Disciples of Tzeentch* unit.

#### Stream of Babble (Passive)
- **Effect:** Add 1 to unbinding rolls for this unit.

#### Agent of Change (Activated)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Effect:** If this unit is contesting an objective you do not control, gain 1 **fate point**.

#### Splinter Spell (Activated)
- **Timing:** Once Per Turn, Reaction: You declared a Spell ability for this unit.
- **Effect:** If that spell is successfully cast, roll a dice. On a 4+, after resolving the effect of that spell, this unit can immediately cast that spell a second time using the same casting roll. Do not count the second spell towards the number of Spell abilities this unit can use this turn.

#### Driven by Fate (Passive)
- **Effect:** While you have 3 or more **fate points**, add 1 to hit rolls for this unit's attacks.
While you have 6 or more **fate points**, add 1 to wound rolls for this unit's attacks.
While you have 9 or more **fate points**, add 1 to the Rend characteristic of this unit's melee weapons.

#### Concealed Sorcery (Activated)
- **Timing:** Your Hero Phase
- **Declare:** This unit can only use this ability if it is **masked by illusion**. Pick a model in a friendly Disciples of Tzeentch unit to be the target.
- **Effect:** This unit can immediately use a Spell ability. When it does so, measure the range and visibility of that Spell ability from the target.

#### Eldritch Coven (Activated)
- **Timing:** Once Per Battle, Enemy Hero Phase
- **Effect:** Each visible friendly Disciples of Tzeentch Wizard wholly within 12" of this unit can use a Spell ability as if it were your hero phase.

#### Silver Skin (Passive)
- **Effect:** Subtract 1 from the Rend characteristic of weapons used for attacks that target this unit in the first battle round.

#### Bird-like Limbs (Passive)
- **Effect:** Add 2 to run rolls for this unit.

#### Telepathic Link (Activated)
- **Timing:** Your Hero Phase
- **Declare:** Pick a visible friendly Disciples of Tzeentch Wizard wholly within 12" of this unit to be the target.
- **Effect:** The target can use an additional Spell ability this turn, but this unit must be picked to be the target of that spell.

#### Crown of Wyrdflame (Activated)
- **Timing:** Any Charge Phase
- **Declare:** If this unit charged this turn, pick an enemy unit in combat with it to be the target.
- **Effect:** Roll 9 dice. For each 5+, inflict 1 mortal damage on the target.

#### Temporal Distortion (Activated)
- **Timing:** Any Hero Phase
- **Effect:** If this unit was set up this turn, it can move D3". That move cannot pass through or end within the combat range of an enemy unit.

#### Inhuman Reflexes (Passive)
- **Effect:** This unit has Ward (5+) if it was set up in the same turn.

#### Doppelgänger (Passive)
- **Effect:** If this unit is destroyed, before removing it from play, set up a replacement unit with half the number of models from the target unit (rounding up) in reserve **masked by illusion**

#### Quicksilver Blood (Passive)
- **Effect:** Each time a model in this unit is slain, before removing the model from play, pick an enemy unit in combat with this unit and roll a dice. On a 4+, inflict 1 mortal damage on that enemy unit.

### Battle Traits

#### Eldritch Illusions (Activated)
- **Timing:** Once Per Turn (Army), Deployment Phase
- **Declare:** Pick up to 3 friendly non-Monster Disciples of Tzeentch units to be the targets.
- **Effect:** Remove the targets from the battlefield and set them up in reserve **masked by illusion**.
Friendly units that are **masked by illusion** are destroyed at the end of the fifth battle round and not at the start of the fourth battle round.

#### All Part of the Plan (Passive)
- **Effect:** You start the battle with 0 **fate points**, Gain 1 **fate point** each time:
• You lose the priority toll.
• A spell cast by a friendly Disciples of Tzeentch unit is unbound.
• A friendly Disciples of Tzeentch unit miscasts a spell.
• Your opponent gains control of an objective that you controlled at the start of the turn.
• A friendly **Argent Shard** is **demolished**.

#### Smoke and Mirrors (Activated)
- **Timing:** Once Per Turn (Army), Any Hero Phase
- **Declare:** Pick a friendly non-Monster Disciples of Tzeentch unit that was not set up this turn and was not set up using this ability in the previous turn to be the target, then pick a friendly unit that is **masked by illusion** to be the substitute.
- **Effect:** Set up the substitute wholly within 6" of the target and not in combat. Then, remove the target from the battlefield and set up in reserve **masked by illusion**.

#### Silver Simulacrum (Activated)
- **Timing:** Once Per Battle (Army), Deployment Phase
- **Effect:** If there is a friendly **Argent Shard** on the battlefield, you can set up 1 additional **Argent Shard** on the battlefield. It must be set up wholly within friendly territory and more than 3" from all objectives and other terrain features.
**Designer's Note**: *The original Argent Shard is set up in Step 1 of the deployment phase using the 'Deploy Faction Terrain' ability. This ability allows you to set up a second such terrain feature and is used in Step 3 of the deployment phase, after armies have been deployed.*

#### Destined to Serve (Fate)
- **Timing:** Passive
- **Effect:** Before allocating damage points to a friendly **Disciples of Tzeentch** unit, you can spend any number of **fate points**. For each **fate point** you spend, remove 1 damage point in that unit's damage pool.
- **Fate Points:** 1+

#### Destined Arcana (Fate)
- **Timing:** Reaction: You declared a Spell ability for a Disciples of Tzeentch unit
- **Effect:** Spend any number of **fate points**. For each **fate point** you spend, add 1 to the casting roll for that spell.
- **Fate Points:** 1+

#### Destined for Battle (Fate)
- **Timing:** Reaction: You declared a Charge ability for a Disciples of Tzeentch unit in your turn
- **Effect:** Spend any number of **fate points**. For each **fate point** you spend, add 1 to the charge roll for that Charge ability.
- **Fate Points:** 1+

### Artefacts of Power

#### Kairic Tome (Activated)
- **Timing:** Reaction: You declared a Spell ability for a Disciples of Tzeentch unit wholly within 6" of this unit
- **Effect:** Add 1 to or subtract 1 form the casting roll for that spell. This unit can use this ability more than once per phase but only once per Spell ability.

#### Fate Slicer (Activated)
- **Timing:** End of Any Turn
- **Effect:** Gain 1 **fate point** if any models were slain by this unit this turn.

#### Cloaked in Wyrdflame (Passive)
- **Effect:** Subtract 1 from wound rolls for attacks that target this unit.

#### Arcane Siphon (Activated)
- **Timing:** Once Per Turn (Army), Your Hero Phase
- **Declare:** Pick a visible Manifestation (friendly or enemy) that was not set up this turn and is within 12" of this unit to be the target.
- **Effect:** Roll a dice. On a 3+:
• The target is banished.
• **Heal (X)** this unit, where X is the target’s Health characteristic.

#### Mutating Blade (Activated)
- **Timing:** Deployment Phase
- **Effect:** Pick 1 of this unit’s melee weapons. That weapon’s Attacks characteristic is 2D6 for the rest of the battle.

#### Spell-Catcher Amulet (Activated)
- **Timing:** Once Per Turn (Army), Your Hero Phase
- **Declare:** This unit can use this ability if any enemy Manifestations were banished this phase.
- **Effect:** This unit can use a Summon ability from the manifestation lore you picked during army composition. That Summon ability does not count towards the total number of Spell or Banish abilities this unit can use this phase.

### Battle Formations

#### Cometh the Hour (Passive)
- **Effect:** If you make a charge roll of 9+ for a friendly Disciples of Tzeentch unit, add 1 to the Attacks characteristic of that unit's melee weapons for the rest of the turn.

#### Inevitable Ending (Activated)
- **Timing:** Once Per Turn (Army), Start of Any Turn
- **Declare:** Pick a unit (friendly or enemy) to be the target.
- **Effect:** If the target is destroyed this turn, gain 1 **fate point**.

#### Twist of Fate (Passive)
- **Effect:** Each time a friendly Disciples of Tzeentch unit uses an ability that requires you to spend 1 or more **fate points**, after that ability has been resoled, roll a dice. On a 4+, gain 1 **fate point**.
If you remove 1 or more damage points from the damage pool of a friendly Disciples of Tzeentch unit as a result of the 'Destined to Serve' ability, after the damage sequence for that unit has been resolved, roll a dice. On a 4+, gain 1 **fate point**.

#### Hideous Unpredictability (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Declare:** Pick a friendly unit that is **masked by illusion** to be the target.
- **Effect:** Set up the target wholly within 6" of a battlefield edge and more than 9" from all enemy units.

#### Symbiotic Madness (Passive)
- **Effect:** Friendly Arcanite units’ melee weapons have **Crit (2 Hits)** while they are wholly within 12" of a friendly Disciples of Tzeentch Daemon unit.

#### Sorcerers of War (Activated)
- **Timing:** Once Per Turn (Army), Your Combat Phase
- **Declare:** Pick a friendly Disciples of Tzeentch Wizard that is in combat to be the target.
- **Effect:** The target can use a non-Summon Spell ability as if it were your hero phase, but only the caster or enemy units in combat with the caster can be picked to be the targets of that Spell ability.

### Heroic Traits

#### Silver Summoner (Command)
- **Timing:** Your Movement Phase
- **Cost:** 1
- **Declare:** Pick a friendly non-Warflock Arcanite unit that has been destroyed to be the target.
- **Effect:** Set up a replacement unit with half the number of models from the target unit (rounding up) more than 9" away from all enemy units and wholly within 12" of a friendly **Argent Shard** that is visible to this unit.

#### Grand Illusionist (Activated)
- **Timing:** Any Hero Phase
- **Effect:** If this unit was set up this turn, it can move D3". That move cannot pass through or end within the combat range of an enemy unit.

#### Devolving Aura (Activated)
- **Timing:** Any Hero Phase
- **Declare:** Pick a visible enemy unit within 12" of this unit to be the target.
- **Effect:** Roll a dice. Add 1 to the roll if any damage points were allocated to the target this phase. On a 3+, the target has a maximum control score of 1 until the start of your next turn.

## Subfactions

### Change-cult Uprising

**Battle Traits**

#### Illusory Infiltration (Activated)
- **Timing:** Once Per Battle (Army), Deployment Phase
- **Declare:** Pick up to 3 friendly non-reinforced Change-cult **Kairic Acolytes** units to be the targets.
- **Effect:** Remove the targets from the battlefield and set them up again wholly within 3" of a terrain feature and not in combat.

#### Puppet on a String (Activated)
- **Timing:** Once Per Turn (army), Any Hero Phase
- **Declare:** Pick an enemy Hero within 24" of and visible to a friendly Change-cult Hero to be the target.
- **Effect:** For the rest of the turn, each time the target uses a Core ability, gain 1 **fate point**.
**Designer's Note:** *You can only gain fate points using this ability and the 'Spell-eater Pendant' ability.*

#### Silver Simulacrum (Activated)
- **Timing:** Once Per Battle (Army), Deployment Phase
- **Effect:** If there is a friendly **Argent Shard** on the battlefield, you can set up 1 additional **Argent Shard** on the battlefield. It must be set up wholly within friendly territory and more than 3" from all objectives and other terrain features.
**Designer's Note**: *The original Argent Shard is set up in Step 1 of the deployment phase using the 'Deploy Faction Terrain' ability. This ability allows you to set up a second such terrain feature and is used in Step 3 of the deployment phase, after armies have been deployed.*

#### Overtaken by Events (Fate)
- **Timing:** Once Per Turn (Army), Any Hero Phase
- **Declare:** Spend 1 **fate point**. Then, pick an enemy unit within 24" of and visible to a friendly Change-cult Hero to be the target.
- **Effect:** The target cannot use commands for the rest of the turn.
- **Fate Points:** 1

#### Seeds of Discord (Fate)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Declare:** Spend 1 **fate point**. Then, pick an enemy unit within 24" of and visible to a friendly Change-cult Hero to be the target, then pick another enemy unit within the target's combat range to be the victim.
- **Effect:** Pick 1 of the target's melee weapons. Immediately resolve combat attacks for the target made with that weapon against the victim.
- **Fate Points:** 1

**Artefacts of Power**

#### Spell-eater Pendant (Passive)
- **Effect:** Each time this unit unbinds a spell or banishes an enemy Manifestation, gain 1 **fate point**.

**Heroic Traits**

#### Call of Anarchy (Command)
- **Timing:** Your Movement Phase
- **Cost:** 1
- **Declare:** Pick a terrain feature within 18" of this unit, then pick a friendly non-Hero Change-cult unit that is **masked by illusion** or that has been destroyed to be the target.
- **Effect:** If you picked a unit that is **masked by illusion**, set it up wholly within 3" of that terrain feature and more than 6" from all enemy units.
If you picked a unit that has been destroyed, set up a replacement unit with half the number of models from the target unit (rounding up) wholly within 3" of that terrain feature and more than 6" from all enemy units.
**Designer's Note:** *Units can be set up in reserve masked by illusion using the Fatemaster's 'Long in the Planning' ability.

### Pyrofane Cult

**Battle Traits**

#### Wyrdflame Barrage (Passive)
- **Effect:** When picking targets for shooting attacks made by friendly units with weapons that have the **Wyrdflame** weapon ability, you can also pick terrain features that are visible to and within range of the attacking model to be targets. Non-Faction Terrain terrain features are considered to have a Save characteristic of 5+ and infinite Health for the purposes of this ability.
If any damage points were allocated to a terrain feature by those attacks, after the Shoot ability has been resolved, roll a dice. If the result is less than or equal to the number of damage points allocated to that terrain feature in this phase, it has the Wyrdpyre keyword for the rest of the battle.
**Designer's Note:** *As non-Faction Terrain features are treated as having infinite Health for the purposes of this ability, you can remove all damage points allocated to them at the end of the shooting phase.*

#### Immolation of the Realms (Activated)
- **Timing:** Once Per Battle Round (Army), Start of Battle Round
- **Declare:** You must use this ability at the start of the battle round if there are Wyrdpyres on the battlefield.
- **Effect:** Resolve the effect below that corresponds to the number of Wyrdpyres on the battlefield.
**1-2**: Inflict 1 mortal damage on each unit within 3" of any Wyrdpyres. Add 2" to the Move characteristic of friendly units that were allocated any of that damage for the rest of the battle round.
**3-4**: Inflict D3 mortal damage on each unit within 3" of any Wyrdpyres. Add 1 to wound rolls for combat attacks made by friendly units that were allocated any of that damage for the rest of the battle round.
**5-6**: Inflict D3 mortal damage on each unit within 6" of any Wyrdpyres. Add 1 to the Attacks characteristic of melee weapons used by friendly units that were allocated any of that damage for the rest of the battle round.
**7-8**: Inflict D3 mortal damage for each unit within 6" of any Wyrdpyres. For friendly units that were allocated any of that damage, for the rest of the battle round:
• Add 1 to wound rolls for their combat attacks.
• Add 2" to their Move characteristic.
• Add 1 to the Attacks characteristic of their melee weapons.
**9+**: Inflict D3+3 mortal damage on each unit within 6" of any Wyrdpyres. Then, pick a non-Faction Terrain terrain feature with the Wyrdpyre keyword to **reduce to ash**. Each player must remove all models that are on that terrain feature and set them up again within 3" of it, but not on it, starting with the active player. Those models can be set up in combat. Models that cannot be set up in this manner are slain, Then, remove that terrain feature from the battlefield.

#### Hidden Instigator (Activated)
- **Timing:** Deployment Phase
- **Declare:** Pick a friendly Wizard to use this ability if it has not been deployed.
- **Effect:** Set up that unit in reserve **hidden among cultists**. It has now been deployed. Friendly units that are **hidden among cultists** are not automatically destroyed at the start of the fourth battle round

#### Deception Revealed (Activated)
- **Timing:** Your Hero Phase
- **Declare:** Pick a friendly Wizard that is **hidden among cultists** to use this ability, then pick a friendly Infantry unit with 3 or more models to be the target.
- **Effect:** Set up that Wizard within the combat range of the target. It can be set up in combat. In addition, if that Wizard uses the 'Magical Intervention' command this phase, no command points are spent to do so.

#### Ritual of Flame (Activated)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Declare:** Pick an objective being contested by any friendly **Kairic Acolyte** units to be the target.
- **Effect:** The target objective has the Wyrdpyre keyword for the rest of the battle.

**Artefacts of Power**

#### Pyrofane Conduit (Activated)
- **Timing:** Once Per Turn, Your Movement Phase
- **Declare:** This unit can only use this ability if it is within 3" of a Wyrdpyre terrain feature. You can pick another friendly Pyrofane unit wholly within 12" of this unit to be the target.
- **Effect:** Remove this unit from the battlefield, then set it up again wholly within 8" of a Wyrdpyre terrain feature and more than 9" from all enemy units. Then, if you picked a target, remove the target from the battlefield and set it up again wholly within 9" of this unit and more than 9" from all enemy units.

**Heroic Traits**

#### Wyrdflame Aegis (Passive)
- **Effect:** You can ignore mortal damage inflicted on this unit by the 'Immolation of the Realms' ability, and if damage would have been inflicted upon it by that ability, the persisting effects from that ability apply to it as though it had been allocated damage by the ability.

### The Oracles of Fate

**Battle Traits**

#### Masters of Destiny (Activated)
- **Timing:** Deployment Phase
- **Effect:** Roll 9 dice and place them to one side. These are your **destiny dice**. **Destiny dice** can be spent during the battle to change certain dice rolls. You can never have more thna 9 **Destiny dice** at once.
During the battle, instead of making a roll from the list below for a friendly Oracle of Fate unit, you can spend your **destiny dice**. The roll you would have made is replaced with the roll on the **destiy dice** you spend. If you want to replace a 2D6 roll, 3D6 roll, etc. you must spend a number of **destiny dice** equal to the number before 'D6'. Any rolls that have been replaced count as unmodified rolls, and they cannot be re-rolled or modified by other friendly abilities except for those that apply negative modifiers (but they can be modified by enemy abilities). You cannot spend **destiny dice** to replace re-rolls.
**Destiny dice** can be spent in place of the following dice rolls:
• Casting rolls
• Unbinding rolls
• Banishment rolls
• Run rolls
• Charge rolls in your turn
• Hit rolls
• Wound rolls
• Save rolls

#### Ninefold Blessings (Activated)
- **Timing:** Once Per Battle Round, Start of Your Turn
- **Declare:** You can only use this ability if the total value of your unspent **destiny dice** is exactly 9.
- **Effect:** Re-roll all of your unspent **destiny dice**. Then, until the start of your next turn:
• Friendly Oracles of Fate units have Ward (5+).
• Subtract 1 from save rolls for enemy units.

**Artefacts of Power**

#### Corrupted Leystone (Activated)
- **Timing:** Your Movement Phase
- **Effect:** If this unit is contesting an objective, remove it from the battlefield and set it up again contesting an objective and more than 7" from all enemy units.

**Heroic Traits**

#### Nexus of Fate (Activated)
- **Timing:** Your Hero Phase
- **Effect:** Roll a dice. If you have 9 unspent **destiny dice**, you must replace 1 of your **destiny dice** with that roll. Otherwise, place that dice to one side as a **destiny dice**.

## Units

### Blue Horrors and Brimstone Horrors
**Points:** 120 | **Move:** 6" | **Health:** 1 | **Save:** 6+ | **Control:** 1
**Keywords:** INFANTRY, CHAOS, DISCIPLES OF TZEENTCH, DAEMON, WARD (6+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Arcane Flames | Ranged | 12" | 2 | 4+ | 4+ | - | 1 | Crit (Auto-wound) |
| Spiteful Talons | Melee | - | 2 | 4+ | 4+ | - | 1 | - |

**Abilities:**

#### Split Again (Activated)
- **Timing:** End of Any Turn
- **Effect:** Roll a dice for each **Blue Horror** model in this unit that is slain. On a 3+, you can add 1 **Brimstone Horrors** model to this unit.
**Designer’s Note:** *You cannot have more than 10 Blue Horror models and 10 Brimstone Horrors models in a non-reinforced unit. You cannot have more than 20 Blue Horror models and 20 Brimstone Horrors models in a reinforced unit.
Models that have been slain and returned to this unit do not count as being slain for the purposes of this ability.*

---

### Burning Chariot of Tzeentch
**Points:** 120 | **Move:** 12" | **Health:** 8 | **Save:** 5+ | **Control:** 2
**Keywords:** CHAOS, DAEMON, FLY, DISCIPLES OF TZEENTCH, WARD (6+), WAR MACHINE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Wyrdflame Blast | Ranged | 16" | 4 | 2+ | 4+ | - | D3 | Anti-Infantry (+1 Rend), Shoot in Combat |
| Flaming Maws and Blue Horrors’ Jabs | Melee | - | 6 | 4+ | 3+ | - | 1 | - |
| Screamers’ Lamprey Bites | Melee | - | 6 | 4+ | 4+ | 1 | 1 | Charge (+1 Damage), Companion |

**Abilities:**

#### Fiery Death From Above (Activated)
- **Timing:** Once Per Turn (Army), Any Charge Phase
- **Declare:** If this unit charged this turn, pick a visible enemy unit within 1" of it to be the target.
- **Effect:** Roll a D3. On a 1, gain 1 **fate point**. On a 2+:
• Inflict an amount of mortal damage on the target equal to the roll.
• This unit can immediately use a Retreat ability as if it were your movement phase. No mortal damage is inflicted on this unit by that Retreat ability.

---

### Changecaster, Herald of Tzeentch
**Points:** 130 | **Move:** 5" | **Health:** 5 | **Save:** 6+ | **Control:** 2
**Keywords:** HERO, CHAOS, DAEMON, DISCIPLES OF TZEENTCH, WARD (6+), WIZARD (1), INFANTRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Staff of Change and Ritual Dagger | Melee | - | 3 | 4+ | 3+ | 1 | D3 | - |

**Abilities:**

#### Locus of Command (Passive)
- **Effect:** Each time a friendly Disciples of Tzeentch Daemon unit wholly within 12" of this unit uses the ‘Rally’ command, you can make 3 additional rally rolls of D6.

#### Weapon Curse (Activated)
- **Timing:** Once Per Turn (Army), Your Hero Phase
- **Declare:** Pick a visible friendly Disciples of Tzeentch Daemon unit wholly within 12" of this unit to be the target.
- **Effect:** Roll a dice. Add 1 to the roll if the target was set up this turn, On a 3+, until the start of your next turn, other than the **Companion** weapon ability, weapon abilities for attacks that target that unit have no effect.

---

### Chaos Spawn of Tzeentch
**Points:** 60 | **Move:** 2D6" | **Health:** 5 | **Save:** 5+ | **Control:** 1
**Keywords:** CHAOS, DISCIPLES OF TZEENTCH, BEAST

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Freakish Mutations | Melee | - | 2D6 | 5+ | 4+ | - | 1 | Crit (2 Hits), Companion |

**Abilities:**

#### A Twisted Fate (Activated)
- **Timing:** Deployment Phase
- **Declare:** This unit must be deployed using this ability.
- **Effect:** Set up this unit in reserve **masked by illusion**. It has now been deployed.
- **Keywords:** Deploy

#### Waves of Mutation (Passive)
- **Effect:** Each time this unit or the replacement of this unit is destroyed, set up a replacement unit in reserve **masked by illusion**. Each friendly **Chaos Spawn of Tzeentch** unit can be replaced any number of times.

---

### Curseling, Eye of Tzeentch
**Points:** 150 | **Move:** 5" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, CHAOS, DISCIPLES OF TZEENTCH, WIZARD (2), INFANTRY, ARCANITE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Hurled Arcane Energy | Ranged | 18" | D6 | 3+ | 3+ | 1 | 1 | - |
| Staff of Tzeentch and Blazing Sword | Melee | - | 5 | 3+ | 4+ | 1 | 2 | - |

**Abilities:**

#### Armour-bane Curse (Spell)
- **Timing:** Your Hero Phase
- **Casting Value:** 6
- **Declare:** Pick a visible enemy unit within 12" of this unit to be the target then make a casting roll of 2D6.
- **Effect:** Subtract 1 from save rolls for combat attacks that target that unit until the start of your next turn.
- **Keywords:** Spell

#### With Spell and Sword (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Effect:** If this unit is in combat, it can use a non-Summon Spell ability as if it were your hero phase, as if that ability had the Unlimited keyword and as if this unit had not cast any spells this turn.

---

### Ephilim's Pandaemonium
**Points:** 100 | **Move:** 6" | **Health:** 2 | **Save:** 6+ | **Control:** 1
**Keywords:** INFANTRY, UNIQUE, Legends, CHAMPION, WIZARD (1), WARD (6+), DISCIPLES OF TZEENTCH, DAEMON, CHAOS

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Daemonic Talons | Melee | - | 2 | 4+ | 5+ | - | 1 | - |
| Warping Wyrdfire | Ranged | 12" | 2 | 4+ | 4+ | - | 1 | Anti-Unique (+1 Rend) |
| Daemonic Talons | Melee | - | 2 | 4+ | 5+ | - | 1 | - |
| Warping Wyrdfire | Ranged | 12" | 2 | 4+ | 4+ | - | 1 | Anti-Unique (+1 Rend) |
| Daemonic Talons | Melee | - | 2 | 4+ | 5+ | - | 1 | - |
| Warping Wyrdfire | Ranged | 12" | 2 | 4+ | 4+ | - | 1 | Anti-Unique (+1 Rend) |
| Warping Wyrdfire | Ranged | 12" | 2 | 4+ | 4+ | - | 1 | Anti-Unique (+1 Rend) |
| Daemonic Talons | Melee | - | 2 | 4+ | 5+ | - | 1 | - |
| Daemonic Talons | Melee | - | 2 | 4+ | 5+ | - | 1 | - |
| Warping Wyrdfire | Ranged | 12" | 2 | 4+ | 4+ | - | 1 | Anti-Unique (+1 Rend) |

**Abilities:**

#### Power Leech (Activated)
- **Timing:** Once Per Turn, Any Shooting Phase
- **Effect:** Roll a D3. Allocate a number of damage points to this unit equal to the roll (ward rolls cannot be made for those damage points). Then, add 2 to the Attacks characteristic of this unit’s ranged weapons for the rest of the turn.

#### Keeper of the Pandaemonium (Passive)
- **Effect:** You can return D3 slain models to this unit.

---

### Exalted Flamer of Tzeentch
**Points:** 110 | **Move:** 9" | **Health:** 5 | **Save:** 5+ | **Control:** 1
**Keywords:** CHAOS, DAEMON, FLY, DISCIPLES OF TZEENTCH, WARD (6+), INFANTRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Wyrdflame Blast | Ranged | 16" | 4 | 2+ | 4+ | - | D3 | Anti-Infantry (+1 Rend), Shoot in Combat |
| Flaming Maws | Melee | - | 3 | 3+ | 4+ | - | 1 | - |

**Abilities:**

#### Unnatural Heat (Activated)
- **Timing:** Once Per Turn (Army), Any Shooting Phase
- **Declare:** Pick an enemy unit that was targeted by this unit's shooting attacks this phase to be the target.
- **Effect:** Roll a dice. Add the number of damage points allocated to the target this phase by this unit's shooting attacks to the roll. On a 5+,subtract 1 from save rolls for the target for the rest of the turn.

#### Heart of the Inferno (Activated)
- **Timing:** Once Per Turn (Army), Your Shooting Phase
- **Declare:** Pick up to 2 visible **Flamers of Tzeentch** units wholly within 12" of this unit to be the targets.
- **Effect:** Add 1 to wound rolls for the targets' shooting attacks for the rest of the turn.

---

### Eyes of the Nine
**Points:** 100 | **Move:** 6" | **Health:** 2 | **Save:** 5+ | **Control:** 1
**Keywords:** INFANTRY, UNIQUE, Legends, CHAMPION, DISCIPLES OF TZEENTCH, ARCANITE, CHAOS

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Arcanite Weapons | Melee | - | 2 | 4+ | 4+ | - | 1 | - |
| Mighty Sorcerous Bolt | Ranged | 18" | 1 | 4+ | 3+ | 1 | 1 | - |
| Arcanite Weapons | Melee | - | 2 | 4+ | 4+ | - | 1 | - |
| Mighty Sorcerous Bolt | Ranged | 18" | 1 | 4+ | 3+ | 1 | 1 | - |
| Arcanite Weapons | Melee | - | 2 | 4+ | 4+ | - | 1 | - |
| Mighty Sorcerous Bolt | Ranged | 18" | 1 | 4+ | 3+ | 1 | 1 | - |
| Arcanite Weapons | Melee | - | 2 | 4+ | 4+ | - | 1 | - |
| Mighty Sorcerous Bolt | Ranged | 18" | 1 | 4+ | 3+ | 1 | 1 | - |
| Arcanite Weapons | Melee | - | 2 | 4+ | 4+ | - | 1 | - |
| Mighty Sorcerous Bolt | Ranged | 18" | 1 | 4+ | 3+ | 1 | 1 | - |

**Abilities:**

#### Brimstone Horror (Passive)
- **Effect:** This unit’s **Brimstone Horror** is a token. When this unit’s **Blue Horror** is slain, place this unit’s **Brimstone Horror** on the battlefield. Add 1 to the Damage characteristic of this unit’s **Mighty Sorcerous Bolt** while its **Brimstone Horror** is on the battlefield. If you make an unmodified save roll of 1 for this unit, remove this unit’s **Brimstone Horror** from the battlefield.

---

### Fatemaster
**Points:** 150 | **Move:** 5" | **Health:** 6 | **Save:** 4+ | **Control:** 2
**Keywords:** HERO, CHAOS, DISCIPLES OF TZEENTCH, ARCANITE, INFANTRY, WIZARD (1)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Cursewrought Glaive | Melee | - | 4 | 3+ | 4+ | 2 | 2 | - |

**Abilities:**

#### Bound Retinue (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Declare:** This unit can only use this ability if it is **masked by illusion**. Pick up to 1 other friendly unit that is **masked by illusion** to be the target.
- **Effect:** Set up this unit on the battlefield more than 9" from all enemy units. Then, set up the target wholly within 6" of this unit and more than 9" from all enemy units.

#### Long in the Planning (Fate)
- **Timing:** Once Per Turn (Army), Enemy Hero Phase
- **Declare:** Spend 2 **fate points**. Then, pick a visible friendly Arcanite unit wholly within 12" of this unit to be the target.
- **Effect:** For the rest of the turn, the first time an enemy unit ends a charge move within 3" of the target, you can remove the target from the battlefield and set it up in reserve **masked by illusion**.
- **Fate Points:** 2

---

### Fatemaster on Disc of Tzeentch
**Points:** 160 | **Move:** 14" | **Health:** 5 | **Save:** 4+ | **Control:** 2
**Keywords:** HERO, CHAOS, Legends, CAVALRY, FLY, DISCIPLES OF TZEENTCH, ARCANITE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Fireglaive of Tzeentch | Melee | - | 3 | 3+ | 3+ | 1 | D3 | - |
| Disc’s Teeth and Horns | Melee | - | 2 | 4+ | 3+ | 1 | D3 | Companion |

**Abilities:**

#### Soulbound Shield (Activated)
- **Timing:** Reaction: Opponent declared a Spell ability
- **Effect:** If this unit was picked to be the target of that spell, roll a dice. On a 4+, ignore the effect of that spell on this unit. This unit can use this ability more than once per phase but only once per Spell ability.

#### Lord of Fate (Passive)
- **Effect:** Add 1 to wound rolls for attacks made by friendly Arcanite units while they are wholly within 12" of this unit.

---

### Fateskimmer, Herald of Tzeentch on Burning Chariot
**Points:** 140 | **Move:** 12" | **Health:** 8 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, CHAOS, DAEMON, FLY, DISCIPLES OF TZEENTCH, WARD (6+), WAR MACHINE, WIZARD (1)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Staff of Change and Ritual Dagger | Melee | - | 3 | 4+ | 3+ | 1 | D3 | - |
| Screamers’ Lamprey Bites | Melee | - | 6 | 4+ | 4+ | 1 | 1 | Charge (+1 Damage), Companion |

**Abilities:**

#### Fuel for the Flames (Activated)
- **Timing:** Once Per Turn (Army), Any Hero Phase
- **Declare:** Pick a visible friendly Disciples of Tzeentch unit wholly within 12" of this unit to be the target.
- **Effect:** If the target is destroyed this turn, before removing it from play, gain 1 **fate point**.

#### Spread the Inferno (Passive)
- **Effect:** **Companion** weapons used by friendly Disciples of Tzeentch units have **Crit (2 Hits)** while they are wholly within 12" of this unit.

---

### Flamers of Tzeentch
**Points:** 130 | **Move:** 9" | **Health:** 2 | **Save:** 5+ | **Control:** 1
**Keywords:** INFANTRY, CHAOS, DISCIPLES OF TZEENTCH, DAEMON, WARD (6+), FLY, CHAMPION

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Flaming Maws | Melee | - | 3 | 3+ | 4+ | - | 1 | - |
| Warping Flames | Ranged | 12" | 3 | 2+ | 4+ | - | D3 | Anti-Infantry (+1 Rend), Shoot in Combat |

**Abilities:**

#### Capricious Wyrdflame (Activated)
- **Timing:** Once Per Turn (Army), Any Shooting Phase
- **Declare:** Pick an enemy unit that had any damage points allocated to it this turn by this unit's shooting attacks to be the target.
- **Effect:** The target has the Burning keyword for the rest of the battle

#### Lingering Burns (Activated)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Declare:** This unit can use this ability even if it has been destroyed. Pick any number of Burning enemy units to be the targets.
- **Effect:** Roll a D3 for each target. On a 1, the target no longer has the Burning keyword. On a 2+, inflict an amount of mortal damage on the target equal to the roll.

---

### Fluxmaster, Herald of Tzeentch on Disc
**Points:** 180 | **Move:** 14" | **Health:** 6 | **Save:** 4+ | **Control:** 2
**Keywords:** HERO, UNIQUE, CHAOS, Legends, WIZARD (1), CAVALRY, FLY, WARD (6+), DISCIPLES OF TZEENTCH, DAEMON

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Iridescent Flames | Ranged | 12" | 3 | 4+ | 4+ | 1 | D3 | - |
| Staff of Change and Ritual Dagger | Melee | - | 3 | 4+ | 3+ | 1 | D3 | - |
| Disc’s Teeth and Horns | Melee | - | 2 | 4+ | 3+ | 1 | D3 | Companion |

**Abilities:**

#### Arcane Tome (Activated)
- **Timing:** Once Per Turn (Army), Reaction: You declared a Spell ability for this unit
- **Effect:** Re-roll 1 of the dice in the casting roll.

---

### Gaunt Summoner
**Points:** 180 | **Move:** 5" | **Health:** 5 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, CHAOS, DAEMON, DISCIPLES OF TZEENTCH, WIZARD (2), INFANTRY, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Changestaff | Ranged | 12" | 3 | 4+ | 3+ | 1 | D3 | Crit (Mortal) |
| Warptongue Blade | Melee | - | 3 | 3+ | 3+ | 1 | 2 | - |

**Abilities:**

#### Leaden Limbs (Spell)
- **Timing:** Your Hero Phase
- **Casting Value:** 7
- **Declare:** Pick a visible enemy unit within 18" of this unit to be the target, then make a casting roll of 2D6.
- **Effect:** Until the start of your next turn:
• Halve the target's Move characteristic.
• The effects of the 'Fly' ability do not apply to the target.

#### Masters of the Silver Towers (Passive)
- **Effect:** Enemy units cannot end a charge move within 1/2" of a friendly **Argent Shard** that is wholly within 12" of this unit and was set up on the battlefield in the same turn.

#### All Belongs to Tzeentch (Activated)
- **Timing:** Once Per Battle (Army), Start of Any Turn
- **Declare:** Pick an objective you do not control and that you controlled earlier in the battle to be the target.
- **Effect:** You control the target objective.

---

### Gaunt Summoner on Disc of Tzeentch
**Points:** 210 | **Move:** 14" | **Health:** 7 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, CHAOS, DAEMON, DISCIPLES OF TZEENTCH, WIZARD (2), CAVALRY, DISC OF TZEENTCH, FLY, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Changestaff | Ranged | 12" | 3 | 4+ | 3+ | 1 | D3 | Crit (Mortal) |
| Warptongue Blade | Melee | - | 3 | 3+ | 3+ | 1 | 2 | - |
| Disc’s Teeth and Horns | Melee | - | 2 | 4+ | 3+ | 1 | D3 | Companion |

**Abilities:**

#### Masters of the Silver Towers (Passive)
- **Effect:** Enemy units cannot end a charge move within 1/2" of a friendly **Argent Shard** that is wholly within 12" of this unit and was set up on the battlefield in the same turn.

#### Arcane Imprisonment (Spell)
- **Timing:** Your Hero Phase
- **Casting Value:** 7
- **Declare:** Pick an enemy Hero in combat with this unit to be the target, then make a casting roll of 2D6.
- **Effect:** If the unmodified casting roll exceeds the target’s Health characteristic, it is automatically destroyed and cannot be replaced for the rest of the battle.
- **Keywords:** Spell

#### All Belongs to Tzeentch (Activated)
- **Timing:** Once Per Battle (Army), Start of Any Turn
- **Declare:** Pick an objective you do not control and that you controlled earlier in the battle to be the target.
- **Effect:** You control the target objective.

---

### Jade Obelisk
**Points:** 100 | **Move:** 5" | **Health:** 1 | **Save:** 4+ | **Control:** 1
**Keywords:** INFANTRY, CHAOS, DISCIPLES OF TZEENTCH, ARCANITE, CHAMPION (1/9)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Mason’s Tools | Melee | - | 2 | 4+ | 3+ | 1 | 1 | Anti-Faction Terrain (+1 Rend) |

**Abilities:**

#### Silver Monoliths (Passive)
- **Effect:** Subtract 1 from the Rend characteristic of weapons used for attacks that target this unit while it is wholly within 12" of a friendly **Argent Shard**.

#### Idolarc (Passive)
- **Effect:** This unit's **Idolarc** is a token. After setting up this unit on the battlefield for the first time, place its **Idolarc** next to it.

#### Chosen by the Stone (Activated)
- **Timing:** Once Per Turn (Army), Your Shooting Phase
- **Declare:** If this unit has any models carrying an **Antithete Bow**, pick a visible enemy unit within 18" of this unit to be the target.
- **Effect:** Roll a dice. On a 3+, place the **Idolarcs** of all other friendly **Jade Obelisk** units next to their respective units. Then, place this unit's **Idolarc** next to the target.
While a friendly **Idolarc** is next to an enemy unit, subtract 1" from that enemy unit's Move characteristic for each **fate point** you have. This ability cannot reduce an enemy unit's Move characteristic below half its unmodified value.

---

### Kairic Acolytes
**Points:** 90 | **Move:** 5" | **Health:** 1 | **Save:** 5+ | **Control:** 1
**Keywords:** INFANTRY, CHAMPION, CHAOS, DISCIPLES OF TZEENTCH, ARCANITE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Cursed Blade | Melee | - | 2 | 4+ | 3+ | 1 | 1 | - |

**Abilities:**

#### Driven by Destiny (Activated)
- **Timing:** Once Per Turn (Army), Enemy Movement Phase
- **Effect:** If this unit is more than 6" from all enemy units, it can move **X**", where **X** is the number of **fate points** you have. It cannot move into combat during any part of that move.
- **Keywords:** Move

---

### Kairic Acolytes (Scourge of Ghyran)
**Points:** 110 | **Move:** 5" | **Health:** 1 | **Save:** 5+ | **Control:** 1
**Keywords:** INFANTRY, CHAMPION, CHAOS, DISCIPLES OF TZEENTCH, ARCANITE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Cursed Blades and Glaives | Melee | - | 2 | 4+ | 3+ | - | 1 | - |

**Abilities:**

#### Empowered by Magic (Passive)
- **Effect:** Add 1 to the Damage characteristic of this unit’s melee weapons while this unit is wholly within 12" of any Manifestations (friendly or enemy).

#### Vulcharc (Activated)
- **Timing:** Once Per Turn (Army), Your Shooting Phase
- **Declare:** Pick a visible enemy unit within 12" of this unit to be the target.
- **Effect:** For the rest of the turn, add 1 to the Rend characteristic of melee weapons used by friendly **Kairic Acolytes** units for attacks that target that enemy unit.

---

### Kairos Fateweaver
**Points:** 400 | **Move:** 12" | **Health:** 16 | **Save:** 5+ | **Control:** 5
**Keywords:** HERO, UNIQUE, WARMASTER, CHAOS, DAEMON, MONSTER, WARD (5+), WIZARD (3), FLY, DISCIPLES OF TZEENTCH

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Staff of Tomorrow | Melee | - | 4 | 3+ | 2+ | 1 | D3 | - |
| Curved Beaks | Melee | - | 5 | 4+ | 3+ | 1 | 2 | - |

**Abilities:**

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, subtract 1 from its power level.

#### Master of Destiny (Passive)
- **Effect:** If this unit has not already **engineered the outcome** this battle round, instead of making a casting roll, unbinding roll, banishment roll, hit roll, wound roll, or save roll for a visible friendly Disciples of Tzeentch unit wholly within 12" of this unit, this unit can **engineer the outcome**.
If the roll would have been a D6 roll, you can use a value of 6 for the roll, you can use a value of 6 for it that cannot be modified. If the roll would have been a 2D6 roll, you can use a value of 9 for it that cannot be modified.

#### Mastery of Magic (Passive)
- **Effect:** When you make a casting roll for this unit, you can change the lowest D6 to match the highest D6.

#### Fateful Reverse (Spell)
- **Timing:** Your Hero Phase
- **Casting Value:** 8
- **Declare:** Pick a visible enemy unit within 18" of this unit to be the target, then make a casting roll of 2D6.
- **Effect:** Until the start of your next turn:
• If the target charged in the same turn, friendly Disciples of Tzeentch units in combat with the target have Strike-first.
• The target's weapons cannot be affected by the **Charge (+1 Damage)** weapon ability.
- **Keywords:** Spell

#### Beacon of Sorcery (Passive)
- **Effect:** Add 1 to casting rolls and unbinding rolls for friendly Disciples of Tzeentch Wizards while they are wholly within 12" of this unit.
In addition, if a friendly Disciples of Tzeentch Wizard wholly within 12" of this unit unbinds a spell, no command points are spent for that Wizard to use the 'Magical Intervention' command in that phase.

---

### Lord of Change
**Points:** 380 | **Move:** 12" | **Health:** 14 | **Save:** 5+ | **Control:** 5
**Keywords:** HERO, CHAOS, DAEMON, MONSTER, WARD (5+), FLY, DISCIPLES OF TZEENTCH, WIZARD (3)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Storm of Wyrdfire | Ranged | 18" | 5 | 3+ | 3+ | - | D3 | Crit (Auto-wound) |
| Eldritch Weapons | Melee | - | 6 | 4+ | 3+ | 1 | 3 | - |

**Abilities:**

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, subtract 1 from its power level.

#### Summon Daemons of Tzeentch (Command)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Cost:** 1
- **Declare:** Pick a friendly non-Hero non-Unique Disciples of Tzeentch Daemon unit that has been destroyed to be the target.
- **Effect:** Set up a replacement unit with half the number of models from the target unit (rounding up) wholly within 9" of this unit and more than 9" from all enemy units.

#### Temporal Manipulation (Spell)
- **Timing:** Your Hero Phase
- **Casting Value:** 6
- **Declare:** Pick another visible friendly Disciples of Tzeentch unit that was not set up this turn and that is wholly within 18" of this unit to be the target. Them, make a casting roll of 2D6.
- **Effect:** The target can use the 'Normal Move' ability as if it were your movement phase, but it must end that move closer to this unit.
- **Keywords:** Spell, Wyrdflame

#### Locus of Impossibility (Passive)
- **Effect:** Add 1 to casting rolls for this unit. In addition, subtract 1 from hit rolls for attacks that target friendly Disciples of Tzeentch Daemon units while they are wholly within 12" of this unit.

---

### Magister
**Points:** 140 | **Move:** 5" | **Health:** 6 | **Save:** 4+ | **Control:** 2
**Keywords:** HERO, CHAOS, DISCIPLES OF TZEENTCH, WIZARD (1), ARCANITE, INFANTRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Tzeentchian Runestaff | Ranged | 18" | 1 | 3+ | 4+ | - | D3 | - |
| Warpsteel Sword | Melee | - | 3 | 3+ | 4+ | - | 2 | - |

**Abilities:**

#### Imbued with Arcane Fire (Command)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Cost:** 1
- **Declare:** Pick a visible friendly Arcanite unit wholly within 12" of this unit to be the target.
- **Effect:** The target's melee weapons have **Crit (Mortal)** until the start of your next turn.

#### Spellmaster (Activated)
- **Timing:** Once Per Turn (Army), Reaction: You declared a Spell ability for a Wizard within 30" of this unit and the casting roll equalled or exceeded the spell’s casting value and that spell has not been unbound
- **Effect:** Make an unbinding roll of 2D6. If the roll exceeds the casting roll for the spell, the spell is unbound and its effect is not resolved.
If the spell is unbound, add 1 to casting rolls for this unit for the rest of the phase.
- **Keywords:** Unbind

---

### Magister on Disc of Tzeentch
**Points:** 150 | **Move:** 14" | **Health:** 7 | **Save:** 4+ | **Control:** 2
**Keywords:** HERO, CHAOS, DISCIPLES OF TZEENTCH, CAVALRY, DISC OF TZEENTCH, FLY, WIZARD (1), ARCANITE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Tzeentchian Runestaff | Ranged | 18" | 1 | 3+ | 4+ | - | D3 | - |
| Warpsteel Sword | Melee | - | 3 | 3+ | 4+ | - | 2 | - |
| Disc’s Teeth and Horns | Melee | - | 2 | 4+ | 3+ | 1 | D3 | Companion |

**Abilities:**

#### Fates Converge (Passive)
- **Effect:** While you have 6 or more **fate points**, add 1 to the Attacks characteristic of melee weapons used by friendly Arcanite units while they are wholly within 12" of this unit.

#### Spellmaster (Activated)
- **Timing:** Once Per Turn (Army), Reaction: You declared a Spell ability for a Wizard within 30" of this unit and the casting roll equalled or exceeded the spell’s casting value and that spell has not been unbound
- **Effect:** Make an unbinding roll of 2D6. If the roll exceeds the casting value, the spell is unbound and its effect is not resolved.
If the spell is unbound, add 1 to casting rolls for this unit for the rest of the phase.
- **Keywords:** Unbind

---

### Ogroid Thaumaturge
**Points:** 130 | **Move:** 6" | **Health:** 8 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, CHAOS, DISCIPLES OF TZEENTCH, WIZARD (1), ARCANITE, INFANTRY, WARD (6+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Thaumaturge Staff | Ranged | 12" | 3 | 3+ | 4+ | - | D3 | Shoot in Combat |
| Great Horns and Cloven Hooves | Melee | - | 6 | 4+ | 2+ | 1 | 2 | Charge (+1 Damage) |

**Abilities:**

#### Arcane Absorption (Activated)
- **Timing:** End of Any Turn
- **Effect:** If any damage points were allocated to an enemy Wizard or Priest by this unit's attacks this turn and that enemy unit has been destroyed:
• **Heal (7)** this unit.
• Add 1 to this unit's power level for the rest of the battle.
This unit can be affected by this ability multiple times and the effects are cumulative.

#### Thaumaturgic Hunger (Activated)
- **Timing:** Reaction: Opponent declared a Spell ability for a unit within 30" of this unit.
- **Effect:** This unit can move 6". It can move into combat and must end that move closer to the caster. If this unit was in combat at the start of the move, it must end that move in combat.
- **Keywords:** Move

---

### Ogroid Thaumaturge (Scourge of Ghyran)
**Points:** 140 | **Move:** 6" | **Health:** 8 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, CHAOS, DISCIPLES OF TZEENTCH, WIZARD (1), ARCANITE, INFANTRY, WARD (6+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Thaumaturge Staff | Ranged | 12" | 3 | 3+ | 4+ | - | D3 | - |
| Great Horns and Cloven Hooves | Melee | - | 6 | 4+ | 2+ | 1 | 2 | Charge (+1 Damage) |

**Abilities:**

#### Wrath of the Thaumaturge (Activated)
- **Timing:** Once Per Turn (Army), Any Charge Phase
- **Declare:** If this unit is not in combat, pick a friendly Arcanite unit in combat to be the target.
- **Effect:** Remove this unit from the battlefield and set it up again within 1" of the target and in combat. This unit has charged.
- **Keywords:** Core, Charge

#### Soul-Deep Devotion (Passive)
- **Effect:** While this unit is within a friendly non-Hero Arcanite unit’s combat range:
• This unit has Ward (4+).
• Each time you make a successful ward roll for this unit, allocate 1 damage point to a friendly non-Hero Arcanite unit within this unit’s combat range after the damage sequence for this unit has been resolved (ward rolls cannot be made for those damage points).

---

### Pink Horrors
**Points:** 170 | **Move:** 6" | **Health:** 2 | **Save:** 5+ | **Control:** 1
**Keywords:** INFANTRY, CHAMPION, STANDARD BEARER (1/10), MUSICIAN (1/10), CHAOS, DISCIPLES OF TZEENTCH, DAEMON, WARD (6+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Arcane Flames | Ranged | 12" | 2 | 4+ | 4+ | - | 1 | Crit (Auto-wound) |
| Taloned Hands | Melee | - | 2 | 4+ | 4+ | - | 1 | - |

**Abilities:**

#### Lunatic Demise (Passive)
- **Effect:** Each time a model in this unit is slain, before removing the model from play, pick 1 of the following effects: 
***Split:*** Pick a friendly **Blue Horrors and Brimstone Horrors** unit within 12" of this unit. Return up to 2 slain **Blue Horror** models to that unit. 
***Petty Vengeance:*** Pick an enemy unit in combat with this unit to be the target, then roll a dice. On a 5+, inflict 1 mortal damage on the target.

---

### Pyrofane Fusil-Major on Ogor Warhulk
**Points:** 180 | **Move:** 5" | **Health:** 8 | **Save:** 3+ | **Control:** 2
**Keywords:** PYROFANE, HERO, HUMAN, INFANTRY, CHAOS, DISCIPLES OF TZEENTCH, ARCANITE, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Long Fusil | Ranged | 24" | 3 | 3+ | 3+ | 1 | 2 | Crit (Auto-wound), Wyrdflame |
| Warhulk’s Mace | Melee | - | 4 | 4+ | 2+ | 2 | 2 | - |

**Abilities:**

#### Light Up Targets (Activated)
- **Timing:** Your Shooting Phase
- **Declare:** Pick an enemy unit that was targeted by this unit’s shooting attacks this turn to be the target.
- **Effect:** Add 1 to hit rolls for attacks made by friendly Pyrofane units that target that unit for the rest of the turn.

---

### Pyrofane Fusiliers
**Points:** 120 | **Move:** 5" | **Health:** 1 | **Save:** 4+ | **Control:** 1
**Keywords:** PYROFANE, INFANTRY, CHAMPION, HUMAN, STANDARD BEARER (1/10), DISCIPLES OF TZEENTCH, ARCANITE, CHAOS, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Fusil-cannon: Fortified Position | Ranged | 18" | 2 | 4+ | 4+ | 1 | 1 | Wyrdflame |
| Fusil-cannon: Mobile | Ranged | 12" | 1 | 4+ | 4+ | 1 | 1 | Shoot in Combat, Wyrdflame |
| Bayonet | Melee | - | 1 | 4+ | 4+ | - | 1 | - |

**Abilities:**

#### Fusiliers, Fire! (Passive)
- **Effect:** Each time this unit uses a Shoot ability, if it is in a fortified position, it can pick either of the ranged weapon profiles for all the attacks it makes with its **Fusil-cannons**. If it is not in a fortified position, use the **Mobile** weapon characteristics.

#### Blackpowder Squire (Passive)
- **Effect:** This unit has a **Blackpowder Squire** token. Each time this unit uses a Shoot ability, if this unit’s **Blackpowder Squire** is on the battlefield, you can re-roll 1 hit roll. If a hit roll re-rolled in this manner is 1, remove this unit’s **Blackpowder Squire** from the battlefield.

---

### Pyrofane Great Cannon
**Points:** 130 | **Move:** 3" | **Health:** 8 | **Save:** 4+ | **Control:** 2
**Keywords:** PYROFANE, HUMAN, WAR MACHINE, CHAOS, DISCIPLES OF TZEENTCH, ARCANITE, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Great Cannon: Cannonball | Ranged | 24" | 2 | 4+ | 2+ | 2 | D3+2 | Wyrdflame |
| Great Cannon: Grapeshot | Ranged | 12" | 5 | 3+ | 3+ | 1 | 2 | Wyrdflame |
| Crew’s Tools and Sidearms | Melee | - | 2 | 4+ | 4+ | - | 1 | - |

**Abilities:**

#### Shot and Shell (Passive)
- **Effect:** Each time this unit uses a Shoot ability, if it is in a fortified position, pick either the **Cannonball** or **Grapeshot** weapon characteristics for all the attacks it makes with its **Great Cannon**. If it is not in a fortified position, use the **Grapeshot** weapon characteristics.

---

### Pyrofane Steelhelms
**Points:** 100 | **Move:** 5" | **Health:** 1 | **Save:** 4+ | **Control:** 1
**Keywords:** PYROFANE, INFANTRY, CHAMPION, HUMAN, MUSICIAN (1/10), STANDARD BEARER (1/10), DISCIPLES OF TZEENTCH, ARCANITE, CHAOS, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Freeguild Weapons | Melee | - | 2 | 4+ | 4+ | - | 1 | - |

**Abilities:**

#### Pyromaniacs (Passive)
- **Effect:** This unit has Ward (5+) while it is wholly within 6" of a Wyrdpyre

---

### Screamers of Tzeentch
**Points:** 90 | **Move:** 14" | **Health:** 3 | **Save:** 5+ | **Control:** 1
**Keywords:** CHAOS, DISCIPLES OF TZEENTCH, DAEMON, WARD (6+), BEAST, FLY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Lamprey Bite | Melee | - | 3 | 4+ | 4+ | 1 | 1 | Charge (+1 Damage), Companion |

**Abilities:**

#### Drawn to Magic (Activated)
- **Timing:** Once Per Turn (Army), Any Charge Phase
- **Declare:** If this unit is not in combat, pick an enemy Manifestation to be the target. Then, make a charge roll of 3D6.
- **Effect:** This unit can move a distance up to the value of the charge roll. It can move through the combat ranges of enemy units and must end that move within 1/2" of the target. If it does so, this unit has charged.
Then, if this unit charged this turn, inflict D3 mortal damage on each enemy Manifestation in combat with this unit.
- **Keywords:** Core, Move, Charge

---

### The Blue Scribes
**Points:** 180 | **Move:** 14" | **Health:** 5 | **Save:** 4+ | **Control:** 2
**Keywords:** HERO, UNIQUE, CHAOS, Legends, CAVALRY, FLY, DISCIPLES OF TZEENTCH, DAEMON, WIZARD (1)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Sharpened Quills | Melee | - | 2 | 5+ | 5+ | - | 1 | - |
| Disc’s Teeth and Horns | Melee | - | 2 | 4+ | 3+ | 1 | D3 | Companion |

**Abilities:**

#### Treacherous Bond (Spell)
- **Timing:** Your Hero Phase
- **Casting Value:** 5
- **Declare:** Pick a friendly unit within this unit’s combat range to be the target, then make a casting roll of 2D6.
- **Effect:** Until the start of your next turn:
• This unit has Ward (4+).
• Each time you make an unmodified ward roll of 1-3 for this unit, allocate 1 damage point to the target.
- **Keywords:** Spell

#### Scrolls of Sorcery (Activated)
- **Timing:** Your Hero Phase
- **Effect:** The next time this unit uses a non-Summon Spell ability this phase, do not make a casting roll as part of that ability. Instead, roll a dice. On a 2+, the spell is successfully cast and cannot be unbound.

---

### The Changeling
**Points:** 170 | **Move:** 5" | **Health:** 6 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, CHAOS, DAEMON, DISCIPLES OF TZEENTCH, WARD (6+), INFANTRY, UNIQUE, WIZARD (2)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| The Trickster’s Staff | Melee | - | 3 | 3+ | 3+ | 1 | D3 | - |

**Abilities:**

#### Arch-Deceiver (Activated)
- **Timing:** Your Hero Phase
- **Declare:** This unit can only use this ability if it is **masked by illusion**. Pick a model in a friendly Disciples of Tzeentch unit to be the target.
- **Effect:** This unit can immediately use a Spell ability. When it does so, measure the range and visibility of that Spell ability from the target. The target is treated as the caster for the purposes of other abilities such as 'Unbind'.

#### Puckish Misdirection (Passive)
- **Effect:** While they are wholly within 12" of this unit, friendly Disciples of Tzeentch units can use the 'Redeploy' command even if they were set up in the same turn.

#### Whispers of Treachery (Spell)
- **Timing:** Your Hero Phase
- **Casting Value:** 7
- **Declare:** Pick a visible enemy unit within 18" of this unit to be the target, then make a casting roll of 2D6.
- **Effect:** Roll a dice for each model in the target unit. For each 5+, inflict 1 mortal damage on the target.
- **Keywords:** Spell

---

### Tzaangor Enlightened
**Points:** 200 | **Move:** 14" | **Health:** 4 | **Save:** 4+ | **Control:** 1
**Keywords:** CHAOS, DISCIPLES OF TZEENTCH, FLY, CAVALRY, CHAMPION, ARCANITE, WARFLOCK, DISC OF TZEENTCH

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Tzeentchian Spear and Vicious Beak | Melee | - | 3 | 4+ | 3+ | 1 | 2 | Charge (+1 Damage) |
| Disc’s Teeth and Horns | Melee | - | 2 | 4+ | 3+ | 1 | D3 | Companion |

**Abilities:**

#### All Too Predictable (Activated)
- **Timing:** Any Combat Phase
- **Effect:** If this unit charged this turn, pick 1 of the following effects to apply for the rest of the turn:
• This unit's **Tzeentchian Spear and Vicious Beak** has **Crit (2 Hits)**
• Add 1 to the Rend characteristic of this unit's **Tzeentchian Spear and Vicious Beak**.

---

### Tzaangor Enlightened on Foot
**Points:** 110 | **Move:** 6" | **Health:** 3 | **Save:** 4+ | **Control:** 1
**Keywords:** CHAOS, DISCIPLES OF TZEENTCH, CHAMPION, ARCANITE, WARFLOCK, INFANTRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Tzeentchian Spear and Vicious Beak | Melee | - | 2 | 4+ | 3+ | 1 | 2 | - |

**Abilities:**

#### Babbling Stream of Secrets (Activated)
- **Timing:** Any Charge Phase
- **Declare:** If this unit charged this phase, pick an enemy unit in combat with it to be the target.
- **Effect:** Roll a dice. On a 3+, the target cannot use commands for the rest of the turn.

---

### Tzaangor Shaman
**Points:** 130 | **Move:** 14" | **Health:** 6 | **Save:** 5+ | **Control:** 2
**Keywords:** HERO, CHAOS, DISCIPLES OF TZEENTCH, CAVALRY, FLY, WIZARD (1), ARCANITE, WARFLOCK, DISC OF TZEENTCH

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Staff of Change and Ritual Dagger | Melee | - | 3 | 4+ | 3+ | 1 | D3 | - |
| Disc’s Teeth and Horns | Melee | - | 2 | 4+ | 3+ | 1 | D3 | Companion |

**Abilities:**

#### Avian Swiftness (Activated)
- **Timing:** Your Hero Phase
- **Declare:** Pick a visible friendly Arcanite unit wholly within 12" of this unit to be the target.
- **Effect:** Roll a dice. On a 3+, for the rest of the turn, the target can use Charge abilities even if it used a Run ability in the same turn.

#### Wit of Beasts (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick a visible enemy Monster or Cavalry unit within 6" of the target.
- **Effect:** The target's melee weapons have the **Companion** ability for the rest of the turn.

---

### Tzaangor Skyfires
**Points:** 160 | **Move:** 14" | **Health:** 4 | **Save:** 4+ | **Control:** 1
**Keywords:** CHAOS, DISCIPLES OF TZEENTCH, FLY, CAVALRY, CHAMPION, ARCANITE, WARFLOCK, DISC OF TZEENTCH

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Arrow of Fate | Ranged | 18" | 2 | 4+ | 3+ | 1 | 2 | - |
| Bow Stave and Vicious Beak | Melee | - | 1 | 4+ | 3+ | - | 1 | - |
| Disc’s Teeth and Horns | Melee | - | 2 | 4+ | 3+ | 1 | D3 | Companion |

**Abilities:**

#### Guided by the Future (Passive)
- **Effect:** Ignore negative modifiers to hit rolls and wound rolls for this unit’s shooting attacks.

#### Destined Quarry (Activated)
- **Timing:** Your Shooting Phase
- **Declare:** Pick an enemy unit that had any damage points allocated to it by this unit’s attacks this phase to be the target.
- **Effect:** Add 1 to hit rolls for combat attacks made by friendly Warflock units that target that enemy unit for the rest of the turn.

---

### Tzaangors
**Points:** 170 | **Move:** 6" | **Health:** 2 | **Save:** 5+ | **Control:** 1
**Keywords:** INFANTRY, CHAMPION, STANDARD BEARER (1/10), MUSICIAN (1/10), CHAOS, DISCIPLES OF TZEENTCH, ARCANITE, WARFLOCK

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Savage Blade and Vicious Beak | Melee | - | 3 | 4+ | 3+ | 1 | 1 | Crit (2 Hits) |

**Abilities:**

#### Eldritch Raiders (Passive)
- **Effect:** Add 1 to wound rolls for this unit's attacks while it is wholly within enemy territory.

---

## Points Table

| Unit | Points |
| --- | ---: |
| Chaos Spawn of Tzeentch | 60 |
| Kairic Acolytes | 90 |
| Screamers of Tzeentch | 90 |
| Ephilim's Pandaemonium | 100 |
| Eyes of the Nine | 100 |
| Jade Obelisk | 100 |
| Pyrofane Steelhelms | 100 |
| Exalted Flamer of Tzeentch | 110 |
| Kairic Acolytes (Scourge of Ghyran) | 110 |
| Tzaangor Enlightened on Foot | 110 |
| Blue Horrors and Brimstone Horrors | 120 |
| Burning Chariot of Tzeentch | 120 |
| Pyrofane Fusiliers | 120 |
| Changecaster, Herald of Tzeentch | 130 |
| Flamers of Tzeentch | 130 |
| Ogroid Thaumaturge | 130 |
| Pyrofane Great Cannon | 130 |
| Tzaangor Shaman | 130 |
| Fateskimmer, Herald of Tzeentch on Burning Chariot | 140 |
| Magister | 140 |
| Ogroid Thaumaturge (Scourge of Ghyran) | 140 |
| Curseling, Eye of Tzeentch | 150 |
| Fatemaster | 150 |
| Magister on Disc of Tzeentch | 150 |
| Fatemaster on Disc of Tzeentch | 160 |
| Tzaangor Skyfires | 160 |
| Pink Horrors | 170 |
| The Changeling | 170 |
| Tzaangors | 170 |
| Fluxmaster, Herald of Tzeentch on Disc | 180 |
| Gaunt Summoner | 180 |
| Pyrofane Fusil-Major on Ogor Warhulk | 180 |
| The Blue Scribes | 180 |
| Tzaangor Enlightened | 200 |
| Gaunt Summoner on Disc of Tzeentch | 210 |
| Lord of Change | 380 |
| Kairos Fateweaver | 400 |
