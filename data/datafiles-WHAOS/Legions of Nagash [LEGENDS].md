# Legions of Nagash [LEGENDS]

## Army Rules

### Battle Traits

#### Ethereal (Passive)
- **Effect:** Ignore negative modifiers to save rolls for friendly Legions of Nagash Nighthaunt units, excluding **Nagash**.

#### Legions of Endless Dead (Passive)
- **Effect:** Subtract 5 from the control scores of enemy units while they are in combat with 2 or more friendly units if any of those friendly units is **Nagash** or if none of those friendly units share any of the following keywords: Soulblight Gravelords, Flesh-eater Courts, Nighthaunt or Ossiarch Bonereapers.

### Artefacts of Power

#### Soul-shrive Amulet (Activated)
- **Timing:** Once Per Battle, Any Hero Phase
- **Declare:** Pick **empowered agent** or **sacrificial pawn**.
- **Effect:** If you picked **empowered agent**, for the rest of the turn:
• Add 2 to the Attacks characteristic of this unit's melee weapons.
• Add 1 to the Rend characteristic of this unit's melee weapons.
If you picked **sacrificial pawn**, instead of making a casting roll for the next spell cast by the friendly **Nagash** this turn, you can use a value of 7 for the roll that cannot be modified and that spell cannot be unbound. Then, allocate 3 damage points to this unit (ward rolls cannot be made for those damage points).

### Heroic Traits

#### Inescapable Fate (Activated)
- **Timing:** Once Per Battle, End of Any Turn
- **Declare:** This unit can only use this ability if it has been destroyed.
- **Effect:** If the friendly **Nagash** is on the battlefield, set up a replacement of this unit wholly within 12" of that **Nagash** and more than 9" from all enemy units. If this unit is not Infantry or Cavalry, allocate 8 damage points to it (ward rolls cannot be made for those damage points).

## Units

### Abhorrant Archregent
**Points:** 140 | **Move:** 6" | **Health:** 6 | **Save:** 5+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, DEATH, HERO, FLESH-EATER COURTS, WARD (6+), INFANTRY, ABHORRANT, WIZARD (1)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Gory Talons and Fangs | Melee | - | 5 | 3+ | 3+ | 1 | 2 | - |

**Abilities:**

#### Countless Servants (Activated)
- **Timing:** Once Per Turn (Army), Your Hero Phase
- **Declare:** Pick a friendly Serfs or Knights unit wholly within 12" of this unit to be the target.
- **Effect:** Roll a dice. On a 3+:
• If the target is a Serfs unit, return up to 3 slain models to it.
• If the target is a Knights unit, return 1 slain model to it.
- **Keywords:** Core

---

### Abhorrant Cardinal
**Points:** 130 | **Move:** 6" | **Health:** 5 | **Save:** 6+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, DEATH, HERO, FLESH-EATER COURTS, WARD (6+), INFANTRY, PRIEST (1), ABHORRANT

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Cursed Crosier | Melee | - | 3 | 4+ | 3+ | - | D3 | - |

**Abilities:**

#### Insular Contemplations (Passive)
- **Effect:** While this unit is more than 6" from all enemy units, add 1 to chanting rolls for this unit.

---

### Abhorrant Ghoul King
**Points:** 120 | **Move:** 6" | **Health:** 5 | **Save:** 5+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, DEATH, HERO, FLESH-EATER COURTS, ABHORRANT, FLY, WIZARD (1), WARD (6+), INFANTRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Bloodstained Talons and Fangs | Melee | - | 5 | 3+ | 3+ | 1 | 2 | Anti-Hero (+1 Rend) |

**Abilities:**

#### Vassals, to Arms! (Activated)
- **Timing:** Once Per Turn (Army), Reaction: You declared a Fight ability for this unit
- **Effect:** Pick a friendly Serfs unit that has not used a Fight ability this turn and is within this unit's combat range to be the target. The target can be picked to use a Fight ability immediately after the Fight ability used by this unit has been resolved. If it is picked to do so, add 1 to wound rolls for the target's attacks for the rest of the turn.

---

### Abhorrant Ghoul King (Scourge of Ghyran)
**Points:** 110 | **Move:** 6" | **Health:** 5 | **Save:** 5+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, DEATH, HERO, FLESH-EATER COURTS, ABHORRANT, WIZARD (1), WARD (6+), INFANTRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Gory Talons and Fangs | Melee | - | 5 | 3+ | 3+ | 1 | 2 | Anti-Hero (+1 Rend) |

**Abilities:**

#### Long Live the King (Passive)
- **Effect:** If this unit would be destroyed, before removing it from play, friendly non-Unique Flesh-eater Courts units wholly within 12" of this unit become enraged until the start of your next turn. Melee weapons used by friendly enraged units have **Crit (Auto-wound)** and score critical hits on unmodified hit rolls of 5+, even if this unit has been destroyed.

#### The King's Court (Passive)
- **Effect:** Add 1 to the Attacks characteristic of melee weapons used by other friendly non-Monster Flesh-eater Courts Heroes while they are wholly within 12" of this unit. In addition, add 1 to the Attacks characteristic of this unit’s melee weapons for each other friendly non-Monster Flesh-eater Courts Hero wholly within 12" of this unit, to a maximum of +3.

---

### Abhorrant Ghoul King on Royal Terrorgheist
**Points:** 380 | **Move:** 12" | **Health:** 16 | **Save:** 4+ | **Control:** 5
**Keywords:** LEGIONS OF NAGASH, Restrict General, DEATH, HERO, MONSTER, FLESH-EATER COURTS, ABHORRANT, FLY, WIZARD (1), WARD (6+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Death Shriek | Ranged | 10" | 1 | 4+ | 2+ | 2 | D6 | Shoot in Combat, Companion |
| Gory Talons and Fangs | Melee | - | 5 | 3+ | 3+ | 1 | 2 | - |
| Terrorgheist’s Fanged Maw | Melee | - | 3 | 4+ | 2+ | 2 | D3+3 | Crit (Mortal), Companion |
| Terrorgheist’s Skeletal Talons | Melee | - | 6 | 4+ | 2+ | 1 | 2 | Crit (Mortal), Companion |

**Abilities:**

#### Necromantic Limits (Passive)
- **Effect:** This unit's **Terrorgheist's Skeletal Talons** have a maximum Attacks characteristic of 10.
While this unit has 10 or more damage points, the Attacks characteristic of its **Terrorgheist's Skeletal Talons** is 4.

#### Wicked Predator (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** For each champion, standard bearer, and musician in the target unit, add 1 to the Attacks characteristic of this unit's **Terrorgheist's Skeletal Talons**, to a maximum of 10, for the rest of the phase. Those additional attacks must target that enemy unit.

#### Ferocious Hunger (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Effect:** Roll a dice. On a 3+, add 1 to hit rolls for attacks made with this unit's **Companion** melee weapons for the rest of the turn.
- **Keywords:** Rampage

---

### Abhorrant Ghoul King on Royal Zombie Dragon
**Points:** 380 | **Move:** 12" | **Health:** 16 | **Save:** 4+ | **Control:** 5
**Keywords:** LEGIONS OF NAGASH, Restrict General, DEATH, HERO, MONSTER, FLESH-EATER COURTS, ABHORRANT, FLY, WIZARD (1), WARD (6+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Gory Talons and Fangs | Melee | - | 5 | 3+ | 3+ | 1 | 2 | - |
| Draconic Jaws | Melee | - | 4 | 4+ | 2+ | 2 | 4 | Crit (Auto-wound), Companion |
| Draconic Claws | Melee | - | 7 | 4+ | 2+ | 1 | 2 | Crit (Auto-wound), Companion |
| Baneful Breath | Ranged | 10" | D6 | 2+ | 4+ | 1 | 3 | Shoot in Combat, Companion |

**Abilities:**

#### Necromantic Limits (Passive)
- **Effect:** This unit's **Draconic Claws** have a maximum Attacks characteristic of 10.
While this unit has 10 or more damage points, the Attacks characteristic of its **Draconic Claws** is 5.

#### Monstrous Hunger (Passive)
- **Effect:** While they are wholly within 12" of this unit, friendly non-Hero Flesh-eater Courts Monsters can use Charge abilities even if they used a Run ability in the same turn.

#### Draconic Terror (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a dice. If the roll exceeds the target’s Control characteristic, the target cannot use commands for the rest of the turn.
- **Keywords:** Rampage

---

### Abhorrant Gorewarden
**Points:** 170 | **Move:** 12" | **Health:** 7 | **Save:** 5+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, DEATH, HERO, FLESH-EATER COURTS, ABHORRANT, FLY, WIZARD (1), WARD (6+), INFANTRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Gory Talons and Fangs | Melee | - | 5 | 3+ | 3+ | 1 | 2 | - |

**Abilities:**

#### Lord of the Marches (Activated)
- **Timing:** Deployment Phase
- **Declare:** Pick this unit and up to 2 friendly Flesh-eater Courts units in this unit's regiment that have not been deployed to be the targets.
- **Effect:** Set up the target units in reserve **patrolling the borders**. They have now been deployed.
- **Keywords:** Deploy

#### Trespassers! (Activated)
- **Timing:** Your Movement Phase
- **Declare:** Pick this unit if it is **patrolling the borders**, then pick an enemy unit to be the target.
- **Effect:** Set up this unit more than 6" from the target and more than 9" from all other enemy units. Then, set up the units that were **patrolling the borders** with this unit wholly within 12" of this unit and more than 9" from all enemy units.
- **Keywords:** Core

---

### Abhorrant Gorewarden (Scourge of Ghyran)
**Points:** 130 | **Move:** 12" | **Health:** 7 | **Save:** 5+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, DEATH, HERO, FLESH-EATER COURTS, ABHORRANT, FLY, WIZARD (1), WARD (6+), INFANTRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Gory Talons and Fangs | Melee | - | 5 | 3+ | 3+ | 1 | 2 | - |

**Abilities:**

#### Swooping Descent (Activated)
- **Timing:** Once Per Turn (Army), End of Enemy Turn
- **Effect:** Remove this unit from the battlefield, then set it up again more than 9" from all enemy units and either:
• Wholly within 6" of the edge of the battlefield, or
• Within 3" of an objective you do not control.

#### Glorious Charge (Passive)
- **Effect:** While this unit is within the combat range of a friendly Knights unit and has charged this turn, this unit has Ward (5+) and its **Gory Talons and Fangs** have **Charge (+1 Damage)**.

---

### Askurgan Trueblades
**Points:** 140 | **Move:** 6" | **Health:** 2 | **Save:** 5+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, INFANTRY, WARD (6+), DEATH, CHAMPION (1/8), SOULBLIGHT GRAVELORDS, VAMPIRE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Askurgan Weapons | Melee | - | 2 | 3+ | 3+ | 1 | 1 | Anti-Monster (+1 Rend), Crit (2 Hits) |
| Elongated Claws and Slavering Maw | Melee | - | 5 | 4+ | 3+ | 2 | 1 | - |
| Askurgan Weapons | Melee | - | 2 | 3+ | 3+ | 1 | 1 | Anti-Monster (+1 Rend), Crit (2 Hits) |

**Abilities:**

#### Gut-wrenching Howl (Activated)
- **Timing:** Any Combat Phase
- **Declare:** Pick an enemy unit in combat with this unit’s **Curseblood** to be the target.
- **Effect:** Roll a dice. On a 4+, the target has Strike-Last for the rest of the turn.

#### Creed of the Askurga Renkai (Passive)
- **Effect:** Subtract 1 from hit rolls for combat attacks made by enemy Monsters that target this unit. 
Add 1 to hit rolls for combat attacks made by this unit that target enemy Monsters.

---

### Barrow Guard
**Points:** 130 | **Move:** 4" | **Health:** 1 | **Save:** 4+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, DEATH, INFANTRY, WARD (6+), CHAMPION, SOULBLIGHT GRAVELORDS, DEATHRATTLE, STANDARD BEARER (1/5), MUSICIAN (1/5)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Wight Blade | Melee | - | 2 | 3+ | 3+ | 1 | 1 | Crit (Mortal) |

**Abilities:**

#### Guardians of the King (Passive)
- **Effect:** While any friendly Soulblight Gravelords Infantry Heroes are within this unit’s combat range, both this unit and those friendly units have Ward (5+).

---

### Barrow Knights
**Points:** 200 | **Move:** 10" | **Health:** 3 | **Save:** 4+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, DEATH, WARD (6+), CAVALRY, CHAMPION, STANDARD BEARER (1/5), SOULBLIGHT GRAVELORDS, DEATHRATTLE, MUSICIAN (1/5)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Skeletal Steed’s Hooves and Teeth | Melee | - | 2 | 5+ | 3+ | - | 1 | Companion |
| Barrow Lance | Melee | - | 2 | 4+ | 3+ | 1 | 1 | Crit (Mortal), Charge (+1 Damage) |

**Abilities:**

#### Cursed to Rise Once More (Activated)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Effect:** You can return 1 slain model to this unit.

#### Knights of the King (Passive)
- **Effect:** While any friendly Deathrattle Cavalry Heroes are within this unit’s combat range, both this unit and those friendly units have Ward (5+).

---

### Black Coach
**Points:** 220 | **Move:** 10" | **Health:** 12 | **Save:** 5+ | **Control:** 5
**Keywords:** LEGIONS OF NAGASH, DEATH, FLY, NIGHTHAUNT, WAR MACHINE, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Soulreach Grasp | Ranged | 10" | 2 | 4+ | 3+ | 2 | 2 | Crit (Auto-wound), Shoot In Combat |
| Wraith's Spectral Weapons | Melee | - | 5 | 3+ | 3+ | 1 | 2 | Anti-Infantry (+1 Rend), Crit (Auto-wound) |
| Relic Bearers’ Claws and Ethereal Steeds’ Hooves | Melee | - | 14 | 4+ | 4+ | - | 1 | Crit (Auto-wound), Companion |

**Abilities:**

#### Runaway Coach (Activated)
- **Timing:** Any Charge Phase
- **Declare:** If this unit charged this phase, pick an enemy unit within 1" of it to be the target.
- **Effect:** Roll a D3. On a 2+, inflict an amount of mortal damage equal on the target equal to the roll.

#### Nimbus of Power (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Effect:** If this unit is not in combat, remove it from the battlefield and set it up again on the battlefield more than 9" from all enemy units.
- **Keywords:** Core

#### Evocation of Death (Activated)
- **Timing:** End of Any Turn
- **Declare:** If any damage points were allocated to an enemy unit by this unit's combat attacks this turn and that enemy unit was destroyed this turn, pick a friendly non-Unique Nighthaunt Hero that has been destroyed.
- **Effect:** Set up a replacement unit wholly within 12" of this unit. The replacement unit can only be set up in combat with enemy units that are in combat with this unit.

---

### Black Coach (Scourge of Ghyran)
**Points:** 290 | **Move:** 10" | **Health:** 12 | **Save:** 5+ | **Control:** 5
**Keywords:** LEGIONS OF NAGASH, DEATH, FLY, NIGHTHAUNT, WAR MACHINE, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Wraiths' Spectral Weapons | Melee | - | 5 | 3+ | 3+ | 2 | 2 | Charge (+1 Damage), Crit (Auto-wound) |
| Black Coach's Iron-shod Wheels | Melee | - | 8 | 4+ | 3+ | 2 | 2 | Crit (Auto-wound), Companion |

**Abilities:**

#### Spectral Carriage (Activated)
- **Timing:** Your Charge Phase
- **Effect:** For the rest of the phase, add 1 to the number of dice rolled when making charge rolls for this unit, to a maximum of 3.

#### One Stop, No Return (Activated)
- **Timing:** Once Per Turn (Army), Any Hero Phase
- **Declare:** Pick an objective you do not control to be the target.
- **Effect:** For the rest of the turn, while enemy units are contesting the target objective, they cannot score critical hits while this unit is also contesting it (hit rolls that would be critical hits score hits instead).

#### Evocation of Death (Activated)
- **Timing:** Once Per Turn (Army), Any Hero Phase
- **Effect:** Pick 1 of the following to apply for the rest of the turn:
***Noble Blade:*** This unit’s melee weapons, including **Companion weapons**, have **Crit (Mortal)** instead of **Crit (Auto-wound)**.
***Unholy Grail:*** Pick a friendly Nighthaunt unit wholly within 12" of this unit. Each time that unit uses the ‘Rally’ command, you receive 3 additional rally points.
***Tome of Undead Lore:*** This unit has Wizard (1).

---

### Bladegheist Revenants
**Points:** 210 | **Move:** 8" | **Health:** 2 | **Save:** 6+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, INFANTRY, DEATH, FLY, NIGHTHAUNT, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Tomb Greatblade | Melee | - | 2 | 3+ | 3+ | 1 | 1 | Crit (Auto-wound) |

**Abilities:**

#### Thrashing Desperation (Passive)
- **Effect:** While this unit contests an objective you control, enemy units cannot finish a charge move within 1/2" of a model in this unit unless the unmodified charge roll was 7 or more.

---

### Blood Knights
**Points:** 220 | **Move:** 12" | **Health:** 3 | **Save:** 3+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, DEATH, WARD (6+), CAVALRY, CHAMPION, STANDARD BEARER (1/5), SOULBLIGHT GRAVELORDS, VAMPIRE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Nightmare’s Hooves and Teeth | Melee | - | 3 | 5+ | 3+ | - | 1 | Companion |
| Templar Weapon | Melee | - | 3 | 3+ | 3+ | 1 | 1 | Anti-Infantry(+1 Rend), Charge (+1 Damage) |

**Abilities:**

#### Riders of Ruin (Passive)
- **Effect:** When this unit moves, it can pass through models in enemy Infantry units and can pass through the combat ranges of enemy Infantry units, but it cannot end a move in combat unless specified in the ability used.

#### Trampled to Dust (Activated)
- **Timing:** Any Charge Phase
- **Declare:** Pick an enemy unit that this unit passed across this turn to be the target.
- **Effect:** Roll a D3. On a 2+, inflict an amount of mortal damage on the target equal to the roll.

---

### Bloodseeker Palanquin
**Points:** 220 | **Move:** 10" | **Health:** 12 | **Save:** 4+ | **Control:** 5
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, WIZARD (1), WARD (6+), DEATH, SOULBLIGHT GRAVELORDS, VAMPIRE, WAR MACHINE, FLY, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Sanguinarch's Stiletto | Melee | - | 5 | 3+ | 3+ | 1 | 2 | Crit (2 Hits) |
| Spectral Host's Blades | Melee | - | 10 | 4+ | 4+ | - | 1 | Companion |

**Abilities:**

#### Wails of the Damned (Passive)
- **Effect:** Subtract 3 from the control scores of enemy units while they are within 12" of this unit.

#### A Promising Concoction (Activated)
- **Timing:** Once Per Battle (Army), Your Combat Phase
- **Declare:** Pick up to 3 friendly Vampire units wholly within 12" of this unit to be the targets.
- **Effect:** Add 1 to the Attacks characteristic of the targets’ melee weapons for the rest of the turn.

---

### Cairn Wraith
**Points:** 130 | **Move:** 8" | **Health:** 5 | **Save:** 5+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, DEATH, FLY, NIGHTHAUNT, INFANTRY, WARD (5+), Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Cairnoch Scythe | Melee | - | 5 | 4+ | 3+ | 2 | 2 | Crit (Auto-wound) |

**Abilities:**

#### Eager Death-Dealers (Activated)
- **Timing:** Once Per Battle (Army), Any Combat Phase
- **Declare:** Pick a visible friendly **Grimghast Reapers** unit wholly within 12" of this unit to be the target.
- **Effect:** Add 1 to wound rolls for the target’s combat attacks for the rest of the turn.

---

### Chainghasts
**Points:** 90 | **Move:** 8" | **Health:** 3 | **Save:** 6+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, INFANTRY, DEATH, CHAMPION, FLY, NIGHTHAUNT, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Ghastflails | Melee | - | 2 | 4+ | 3+ | 2 | 1 | Crit (Auto-wound) |
| Ghastflails | Ranged | 12" | 2 | 4+ | 3+ | 2 | 1 | Crit (Auto-wound) |

**Abilities:**

#### Another Link in the Chain (Activated)
- **Timing:** Any Combat Phase
- **Effect:** Roll a dice. Add 1 to the roll if this unit is wholly within 12" of any friendly **Spirit Torments**. On a 3+, for the rest of the turn, add 1 to hit rolls for combat attacks made by friendly Nighthaunt units while they are wholly within 12" of this unit.

---

### Chainrasps
**Points:** 100 | **Move:** 8" | **Health:** 1 | **Save:** 6+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, INFANTRY, DEATH, CHAMPION, FLY, NIGHTHAUNT, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Malignant Weapon | Melee | - | 2 | 4+ | 5+ | - | 1 | Crit (Auto-wound) |
| Malignant Weapon | Melee | - | 2 | 4+ | 5+ | - | 1 | Crit (Auto-wound) |

**Abilities:**

#### Chilling Horde (Activated)
- **Timing:** Once Per Battle (Army), Deployment Phase
- **Declare:** Pick up to 3 friendly **Chainrasp** units to be the targets,
- **Effect:** Each target can move 6" but cannot end that move in combat.

---

### Corpse Cart
**Points:** 70 | **Move:** 4" | **Health:** 7 | **Save:** 6+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, WARD (6+), DEATH, SOULBLIGHT GRAVELORDS, WAR MACHINE, DEADWALKERS

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Corpsemaster’s Weapon | Melee | - | 3 | 4+ | 4+ | - | 1 | - |
| Zombies’ Rusty Blades | Melee | - | 2D6 | 4+ | 4+ | - | 1 | Companion |

**Abilities:**

#### Locus of Undeath (Passive)
- **Effect:** Melee weapons used by other friendly Deadwalkers units have **Crit (Auto-wound)** while they are wholly within 12" of this unit.

---

### Corpse Cart (Scourge of Ghyran)
**Points:** 80 | **Move:** 4" | **Health:** 7 | **Save:** 6+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, WARD (6+), DEATH, SOULBLIGHT GRAVELORDS, WAR MACHINE, DEADWALKERS

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Corpsemaster’s Tools and Zombies' Bites | Melee | - | D6+6 | 4+ | 4+ | - | 1 | Crit (Auto-wound) |

**Abilities:**

#### Unholy Lodestone (Activated)
- **Timing:** Your Hero Phase
- **Declare:** Pick a friendly **Deadwalker Zombies** unit within this unit’s combat range to be the target.
- **Effect:** Until the start of your next turn, while the target is wholly within 12" of this unit, its ‘Mindless Ferocity’ ability inflicts mortal damage on rolls of 5+ instead of 6+.

#### Necromantic Goad (Activated)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Declare:** Pick a friendly **Deadwalker Zombies** unit that is wholly within 12" of this unit and not in combat to be the target.
- **Effect:** The target can move up to 3". It can move into combat.

---

### Coven Throne
**Points:** 230 | **Move:** 10" | **Health:** 12 | **Save:** 4+ | **Control:** 5
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, WIZARD (1), WARD (6+), DEATH, SOULBLIGHT GRAVELORDS, VAMPIRE, WAR MACHINE, FLY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Acolytes' Poniards | Melee | - | 6 | 3+ | 3+ | 1 | 1 | - |
| Matriarch's Stiletto | Melee | - | 5 | 3+ | 3+ | 1 | 2 | Crit (2 Hits) |
| Spectral Host's Blades | Melee | - | 10 | 4+ | 4+ | - | 1 | Companion |

**Abilities:**

#### Scrying Pool (Passive)
- **Effect:** If a friendly Soulblight Gravelords unit wholly within 12" of this unit uses the ‘Redeploy’ command, you can pick another friendly Soulblight Gravelords unit wholly within 12" of this unit that has not used a command this phase to use the ‘Redeploy’ command immediately after the first has been resolved (this is an exception to Commands, 1.2). No command point is spent the second time that command is used.

#### A Promising Concoction (Activated)
- **Timing:** Once Per Battle (Army), Your Combat Phase
- **Declare:** Pick up to 3 friendly Vampire units wholly within 12" of this unit to be the targets.
- **Effect:** Add 1 to the Attacks characteristic of the targets’ melee weapons for the rest of the turn.

#### Tactical Insight (Passive)
- **Effect:** Friendly Deathrattle and Deadwalkers units have Ward (5+) while they are wholly within 6" of this unit.

---

### Craventhrone Guard
**Points:** 100 | **Move:** 8" | **Health:** 2 | **Save:** 6+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, INFANTRY, DEATH, CHAMPION, FLY, NIGHTHAUNT, STANDARD BEARER (1/5), WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Wicked Sidearm | Melee | - | 1 | 4+ | 4+ | - | 1 | Crit (Auto-wound) |
| Soulhunter’s Crossbow | Ranged | 16" | 2 | 4+ | 3+ | 1 | 1 | Crit (Auto-wound) |

**Abilities:**

#### Black-hearted Lackeys (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a D3. On a 2+, this unit immediately uses the 'Retreat' ability without any mortal damage being inflicted on it. Then, inflict an amount of mortal damage on the target equal to the roll.

---

### Crypt Flayers
**Points:** 90 | **Move:** 12" | **Health:** 4 | **Save:** 5+ | **Control:** 1
**Keywords:** Undersize Unit, LEGIONS OF NAGASH, WARD (6+), DEATH, CHAMPION, FLY, FLESH-EATER COURTS, KNIGHTS, INFANTRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Piercing Talons | Melee | - | 4 | 4+ | 3+ | 1 | 1 | - |
| Death Scream | Ranged | 10" | 4 | 4+ | 3+ | 2 | 1 | Shoot in Combat |

**Abilities:**

#### To Our Liege (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Effect:** If this unit is not in combat, remove it from the battlefield and set it up again wholly within 12" of a friendly Flesh-eater Courts Hero and more than 9" from all enemy units.

---

### Crypt Ghast Courtier
**Points:** 80 | **Move:** 6" | **Health:** 4 | **Save:** 5+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, INFANTRY, Legends, WARD (6+), DEATH, FLESH-EATER COURTS, NOBLE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Bone Club and Filthy Claws | Melee | - | 5 | 4+ | 4+ | - | 2 | - |

**Abilities:**

#### Marshal of the Peasantry (Activated)
- **Timing:** Reaction: You declared a Fight ability for this unit
- **Effect:** Pick a friendly **Crypt Ghouls** unit that has not used a Fight ability this turn and is within this unit’s combat range to be the target. The target can be picked to use a Fight ability immediately after the Fight ability used by this unit has been resolved. If it is picked to do so, add 1 to hit rolls for the target’s attacks for the rest of the turn.

---

### Crypt Ghouls
**Points:** 150 | **Move:** 6" | **Health:** 1 | **Save:** 6+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, WARD (6+), DEATH, CHAMPION, FLESH-EATER COURTS, INFANTRY, SERFS

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Sharpened Teeth and Filthy Claws | Melee | - | 2 | 4+ | 4+ | - | 1 | Crit (Auto-wound) |

**Abilities:**

#### Royal Approval (Passive)
- **Effect:** Add 1 to the Rend characteristic of this unit’s melee weapons while it is wholly within 12" of any friendly Nobles or Abhorrants.

---

### Crypt Haunter Courtier
**Points:** 120 | **Move:** 7" | **Health:** 6 | **Save:** 5+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, DEATH, HERO, FLESH-EATER COURTS, WARD (6+), INFANTRY, NOBLE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Massive Bone Club and Rancid Talons | Melee | - | 5 | 4+ | 3+ | 2 | 3 | - |

**Abilities:**

#### Knightly Exemplar (Activated)
- **Timing:** Reaction: You declared a Fight ability for this unit
- **Effect:** Pick a friendly **Crypt Horrors** unit that has not used a Fight ability this turn and is within this unit’s combat range to be the target. The target can be picked to use a Fight ability immediately after the Fight ability used by this unit has been resolved. If it is picked to do so, add 1 to hit rolls for the target’s attacks for the rest of the turn.

#### Blessed Blood (Activated)
- **Timing:** Your Hero Phase
- **Effect:** Heal (D3) this unit.

---

### Crypt Horrors
**Points:** 160 | **Move:** 7" | **Health:** 4 | **Save:** 5+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, WARD (6+), DEATH, CHAMPION, FLESH-EATER COURTS, KNIGHTS, INFANTRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Club and Septic Talons | Melee | - | 4 | 4+ | 3+ | 1 | 2 | Anti-Cavalry (+1 Rend), Crit (Auto-wound) |

**Abilities:**

#### Holy Blades of Bone (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll 2 dice for each model in this unit. Add 2 to each roll if the target is Cavalry. For each 5+, inflict 1 mortal damage on the target.

---

### Crypt Infernal Courtier
**Points:** 140 | **Move:** 12" | **Health:** 6 | **Save:** 5+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, DEATH, HERO, FLESH-EATER COURTS, WARD (6+), INFANTRY, NOBLE, FLY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Piercing Death Screech | Ranged | 10" | 4 | 4+ | 3+ | 2 | D3 | Shoot in Combat |
| Skewering Claws | Melee | - | 5 | 4+ | 3+ | 1 | 2 | - |

**Abilities:**

#### Mind-shattering Cacophony (Activated)
- **Timing:** Once Per Turn (Army), Your Shooting Phase
- **Declare:** Pick an enemy unit that had any damage points allocated to it this turn by attacks made with this unit's **Piercing Death Screech** to be the target.
- **Effect:** For the rest of the turn, add 1 to the Damage characteristic of ranged weapons used by friendly **Crypt Flayers** units for attacks that target that enemy unit.

#### On Winged Steeds (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Declare:** Pick a friendly **Crypt Flayers** ability that used the 'To Our Liege' ability this turn and is wholly within 9" of this unit to be the target.
- **Effect:** The target can immediately move D3" but cannot end that move in combat.

---

### Cryptguard
**Points:** 120 | **Move:** 6" | **Health:** 1 | **Save:** 6+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, WARD (6+), DEATH, CHAMPION, FLESH-EATER COURTS, STANDARD BEARER (1/10), MUSICIAN (1/10), SERFS, INFANTRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Cursed Weapon | Melee | - | 3 | 4+ | 4+ | 1 | 1 | Anti-charge (+1 Rend), Crit (Auto-wound) |

**Abilities:**

#### Royal Bodyguard (Passive)
- **Effect:** While any friendly non-Monster Flesh-eater Courts Heroes are wholly within this unit’s combat range, both this unit and those Heroes have Ward (5+).

#### Elite Guardians (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Effect:** If this unit is in combat with any enemy units that charged this turn, roll a dice. On a 3+, for the rest of the turn, this unit has Strike-first but ward rolls cannot be made for it.

---

### Deadwalker Zombies
**Points:** 130 | **Move:** 4" | **Health:** 1 | **Save:** 6+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, DEATH, INFANTRY, WARD (6+), SOULBLIGHT GRAVELORDS, DEADWALKERS

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Crude Weapon or Infectious Bite | Melee | - | 1 | 5+ | 4+ | - | 1 | - |

**Abilities:**

#### Mindless Ferocity (Passive)
- **Effect:** Each time a model in this unit is slain by a combat attack and that model was in combat with the attacking unit, roll a dice. On a 6+, inflict 1 mortal damage on the attacking unit after the Fight ability has been resolved.

---

### Deathrattle Skeletons
**Points:** 90 | **Move:** 4" | **Health:** 1 | **Save:** 5+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, DEATH, INFANTRY, WARD (6+), CHAMPION, STANDARD BEARER (1/10), SOULBLIGHT GRAVELORDS, DEATHRATTLE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Ancient Weapon | Melee | - | 2 | 4+ | 4+ | - | 1 | - |

**Abilities:**

#### Skeleton Legion (Activated)
- **Timing:** End of Any Turn
- **Effect:** You can return D3 slain models to this unit.

---

### Dire Wolves
**Points:** 150 | **Move:** 10" | **Health:** 2 | **Save:** 5+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, DEATH, WARD (6+), SOULBLIGHT GRAVELORDS, DEADWALKERS, BEAST, CHAMPION (1/10)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Rotting Fangs and Claws | Melee | - | 2 | 4+ | 3+ | - | 1 | Companion |

**Abilities:**

#### On the Hunt (Passive)
- **Effect:** This unit can use a Run ability and still use Charge abilities later in the turn.

---

### Dreadblade Harrows
**Points:** 140 | **Move:** 12" | **Health:** 3 | **Save:** 6+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, DEATH, FLY, NIGHTHAUNT, CAVALRY, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Dreadblade | Melee | - | 4 | 4+ | 3+ | 1 | 2 | Crit (Auto-wound) |
| Ethereal Steed’s Ghostly Hooves and Teeth | Melee | - | 3 | 5+ | 3+ | - | 1 | Crit (Auto-wound), Companion |

**Abilities:**

#### Curse of Loyalty (Passive)
- **Effect:** If this unit charged this turn, add 3 to the control scores of friendly Nighthaunt units while they are wholly within 12" of this unit.

#### Phantasmal Discorporation (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Effect:** Remove this unit from the battlefield and set it up again on the battlefield more than 9" from all enemy units.
- **Keywords:** Core

---

### Dreadscythe Harridans
**Points:** 200 | **Move:** 8" | **Health:** 2 | **Save:** 6+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, INFANTRY, DEATH, FLY, NIGHTHAUNT, CHAMPION, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Scythed Limbs | Melee | - | 3 | 4+ | 4+ | 1 | 1 | Crit (Auto-wound) |

**Abilities:**

#### Harrowing Shrieks (Passive)
- **Effect:** Enemy units cannot be healed or have slain models returned to them while they are in combat with this unit.

---

### Fell Bats
**Points:** 80 | **Move:** 12" | **Health:** 3 | **Save:** 6+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, DEATH, WARD (6+), SOULBLIGHT GRAVELORDS, BEAST, DEADWALKERS, FLY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Chiropteran Fangs | Melee | - | 3 | 4+ | 4+ | - | 2 | Companion |

**Abilities:**

#### Fleet and Fell (Passive)
- **Effect:** This unit can use Charge abilities even if it used a Retreat ability in the same turn. In addition, no mortal damage is inflicted on this unit by Retreat abilities.

---

### Glaivewraith Stalkers
**Points:** 100 | **Move:** 8" | **Health:** 2 | **Save:** 6+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, INFANTRY, DEATH, FLY, NIGHTHAUNT, MUSICIAN (1/4), WARD (5+), Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Hunter’s Glaive | Melee | - | 2 | 4+ | 3+ | 1 | 1 | Charge (+1 Damage), Crit (Auto-wound) |

**Abilities:**

#### Unerring Hunters (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Effect:** For the rest of the turn, each time this unit makes a pile-in move, add 3" to the distance it can move.

---

### Gothizzar Harvester
**Points:** 200 | **Move:** 6" | **Health:** 10 | **Save:** 4+ | **Control:** 5
**Keywords:** LEGIONS OF NAGASH, OSSIARCH BONEREAPERS, WARD (6+), DEATH, WAR MACHINE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Soulcrusher Weapons | Melee | - | 6 | 4+ | 2+ | 2 | 3 | - |

**Abilities:**

#### Bone Harvest (Activated)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Effect:** Roll a dice for each enemy unit that has been destroyed this battle. Add 1 to each roll if this unit used a Fight ability this turn. If any of the rolls are a 4+, this unit is **full**.

#### Bone Cage (Passive)
- **Effect:** This unit is either **full** or **empty**. It starts the battle **full**.

#### Repair Construct (Activated)
- **Timing:** Once Per Turn (Army), Your Hero Phase
- **Declare:** If this unit is **full**, pick a visible friendly Ossiarch Bonereapers unit wholly within 12" of this unit to be the target.
- **Effect:** Return 1 slain model to the target unit. If the target has a Health characteristic of 1, return 3 slain models to it instead. Then, this unit is **empty**.

---

### Grimghast Reapers
**Points:** 200 | **Move:** 8" | **Health:** 2 | **Save:** 6+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, INFANTRY, DEATH, CHAMPION, FLY, NIGHTHAUNT, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Slasher Scythe | Melee | - | 2 | 4+ | 3+ | 1 | 1 | Anti-Infantry (+1 Rend), Crit (Auto-wound) |

**Abilities:**

#### Reaped Like Corn (Activated)
- **Timing:** Any Combat Phase
- **Declare:** Pick an enemy unit that is within the combat ranges of 5 or more models in this unit to be the target.
- **Effect:** Roll a number of dice equal to the number of models in the target unit. For each 6, inflict 1 mortal damage on the target. If the target unit has 20 or more models, inflict 1 mortal damage on the target for each 5+ instead.

---

### Guardian of Souls
**Points:** 110 | **Move:** 8" | **Health:** 5 | **Save:** 5+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, DEATH, FLY, NIGHTHAUNT, INFANTRY, WIZARD (1), WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Chill Blade | Melee | - | 3 | 4+ | 3+ | 1 | 2 | Crit (Auto-wound) |

**Abilities:**

#### Nightmare Lantern (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick a visible friendly Nighthaunt unit wholly within 12" of this unit to be the target.
- **Effect:** Roll a dice. On a 3+, add 1 to wound rolls for the target's attacks for the rest of the turn.

---

### Hexwraiths
**Points:** 150 | **Move:** 12" | **Health:** 3 | **Save:** 6+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, DEATH, CHAMPION, FLY, NIGHTHAUNT, CAVALRY, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Ethereal Steed’s Ghostly Hooves and Teeth | Melee | - | 2 | 5+ | 3+ | - | 1 | Crit (Auto-wound), Companion |
| Spectral Scythe | Melee | - | 2 | 4+ | 4+ | 1 | 1 | Charge (+1 Damage), Crit (Auto-wound) |

**Abilities:**

#### Spectral Hunters (Activated)
- **Timing:** Any Charge Phase
- **Declare:** If this unit charged this phase, pick an enemy unit within 1" of it to be the target.
- **Effect:** Roll a D3. On a 2+, inflict an amount of mortal damage on the target equal to the roll.

---

### Immortis Guard
**Points:** 170 | **Move:** 5" | **Health:** 4 | **Save:** 3+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, INFANTRY, OSSIARCH BONEREAPERS, WARD (6+), DEATH

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Dread Halberd | Melee | - | 3 | 3+ | 3+ | 1 | 2 | - |

**Abilities:**

#### Immortal Bulwark (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy unit in combat with this unit and that charged this turn to be the target.
- **Effect:** Other than the **Companion** weapon ability, weapon abilities for the target's combat attacks have no effect for the rest of the turn.

---

### Kavalos Deathriders
**Points:** 150 | **Move:** 10" | **Health:** 3 | **Save:** 4+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, OSSIARCH BONEREAPERS, WARD (6+), DEATH, CHAMPION, CAVALRY, STANDARD BEARER (1/5)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Kavalos Blades | Melee | - | 3 | 3+ | 4+ | 1 | 1 | - |
| Kavalos Steed's Hooves and Teeth | Melee | - | 2 | 5+ | 3+ | - | 1 | Companion |

**Abilities:**

#### Deathrider Wedge (Activated)
- **Timing:** Any Combat Phase
- **Declare:** If this unit charged this turn, pick an enemy unit in combat with it to be the enemy target. Then, you can pick a friendly **Liege-Kavalos** within this unit's combat range to be the friendly target.
- **Effect:** Inflict D3 mortal damage on the enemy target. Inflict an additional D3 mortal damage on the enemy target if you picked a friendly target. Then, this unit and the friendly target (if any) can move 10". They can move through the combat ranges of enemy units but cannot end that move in combat.
- **Keywords:** Core, Attack, Fight

---

### Kavalos War Chariot
**Points:** 170 | **Move:** 10" | **Health:** 10 | **Save:** 3+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, DEATH, OSSIARCH BONEREAPERS, WARD (6+), WAR MACHINE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Charioteers' Weapons | Melee | - | 4 | 3+ | 4+ | 1 | 1 | - |
| Kavalos Steed's Hooves, Teeth, and Barbed Tails | Melee | - | 6 | 5+ | 3+ | - | 1 | Companion |

**Abilities:**

#### Scything Assault (Activated)
- **Timing:** Any Combat Phase
- **Declare:** If this unit charged this turn, pick an enemy unit in combat with it to be the target.
- **Effect:** Inflict D3 mortal damage on the target. Then, you can remove this unit from the battlefield and set it up again within 1" of the target.
- **Keywords:** Core, Attack, Fight

#### Pinning Attacks (Activated)
- **Timing:** Once Per Turn (Army), Your Shooting Phase
- **Declare:** Pick a visible enemy unit within 18" of this unit to be the target.
- **Effect:** Roll a dice. On a 3+:
• Inflict 1 mortal damage on the target.
• The target cannot make pile-in moves for the rest of the turn.

---

### Knight of Shrouds
**Points:** 100 | **Move:** 8" | **Health:** 5 | **Save:** 5+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, DEATH, FLY, NIGHTHAUNT, INFANTRY, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Sword of Stolen Hours | Melee | - | 5 | 4+ | 3+ | 1 | 2 | Crit (Auto-wound) |

**Abilities:**

#### Stolen Hours (Passive)
- **Effect:** Each time this unit uses a Fight ability, after that Fight ability has been resolved, **Heal (X)** this unit, where X is the number of damage points allocated to enemy units by combat attacks made as part of that Fight ability.

#### Infantry Overseer (Activated)
- **Timing:** Reaction: You declared a Fight ability for this unit
- **Effect:** Pick a friendly non-Hero Nighthaunt Infantry unit that has not used a Fight ability this turn and is within this unit’s combat range to be the target. The target can be picked to use a Fight ability immediately after the Fight ability used by this unit has been resolved. If it is picked to do so, add 1 to hit rolls for the target’s attacks for the rest of the turn.

---

### Knight of Shrouds on Ethereal Steed
**Points:** 160 | **Move:** 12" | **Health:** 7 | **Save:** 5+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, DEATH, FLY, NIGHTHAUNT, CAVALRY, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Ethereal Steed’s Ghostly Hooves and Teeth | Melee | - | 3 | 5+ | 3+ | - | 1 | Crit (Auto-wound), Companion |
| Sword of Stolen Hours | Melee | - | 5 | 4+ | 3+ | 1 | 2 | Crit (Auto-wound) |

**Abilities:**

#### Stolen Hours (Passive)
- **Effect:** Each time this unit uses a Fight ability, after that Fight ability has been resolved, **Heal (X)** this unit, where X is the number of damage points allocated to enemy units by combat attacks made as part of that Fight ability.

#### Cavalry Overseer (Activated)
- **Timing:** Reaction: You declared a Fight ability for this unit
- **Effect:** Pick a friendly non-Hero Nighthaunt Cavalry or War Machine unit that has not used a Fight ability this turn and is within this unit’s combat range to be the target. The target can be picked to use a Fight ability immediately after the Fight ability used by this unit has been resolved. If it is picked to do so, add 1 to hit rolls for the target’s attacks for the rest of the turn.

---

### Kosargi Nightguard
**Points:** 110 | **Move:** 6" | **Health:** 4 | **Save:** 5+ | **Control:** 3
**Keywords:** LEGIONS OF NAGASH, DEATH, WARD (6+), SOULBLIGHT GRAVELORDS, DEADWALKERS, INFANTRY, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Kosargi Bardiche | Melee | - | 2 | 4+ | 2+ | 2 | 3 | - |

**Abilities:**

#### A Pact Maintained (Passive)
- **Effect:** While a friendly **Radukar the Wolf** or **Radukar the Beast** is within this unit’s combat range, both that unit and this unit have Ward (5+).

---

### Krulghast Cruciator
**Points:** 150 | **Move:** 8" | **Health:** 6 | **Save:** 5+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, DEATH, FLY, NIGHTHAUNT, INFANTRY, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Talons and Flensing Knives | Melee | - | 4 | 4+ | 3+ | 1 | 2 | Crit (Auto-wound) |
| Phantasmal Torture | Ranged | 12" | 4 | 3+ | 3+ | 2 | 1 | Crit (Auto-wound), Shoot in Combat |

**Abilities:**

#### Empowered Through Excruciation (Passive)
- **Effect:** Ignore the first damage point that would be allocated to each friendly Nighthaunt unit wholly within 12" of this unit in each phase.

---

### Liege-Kavalos
**Points:** 180 | **Move:** 10" | **Health:** 7 | **Save:** 3+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, DEATH, OSSIARCH BONEREAPERS, WARD (6+), CAVALRY, RELENTLESS DISCIPLINE (6)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Commander's Blade | Melee | - | 5 | 3+ | 4+ | 1 | 2 | Charge (+1 Damage) |
| Kavalos Steed's Hooves, Teeth, and Barbed Tails | Melee | - | 4 | 5+ | 3+ | - | 1 | Companion |

**Abilities:**

#### Deathrider Commander (Passive)
- **Effect:** This unit and visible friendly **Kavalos Deathriders** units wholly within 12" Of this unit can move 12" instead of 10" when they use the 'Deathrider Wedge' ability.

#### Master of Cavalry (Activated)
- **Timing:** Reaction: You declared a Fight ability for this unit
- **Effect:** Pick a friendly **Kavalos Deathriders** unit that has not used a Fight ability this turn and is within this unit’s combat range to be the target. The target can be picked to use a Fight ability immediately after the **Fight** ability used by this unit has been resolved. If it is picked to do so, add 1 to hit rolls for the target's combat attacks for the rest of the turn.

---

### Liege-Kavalos on War Chariot
**Points:** 230 | **Move:** 10" | **Health:** 12 | **Save:** 3+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, DEATH, OSSIARCH BONEREAPERS, WARD (6+), RELENTLESS DISCIPLINE (6), WAR MACHINE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Commander's Blade | Melee | - | 5 | 3+ | 4+ | 1 | 2 | Charge (+1 Damage) |
| Kavalos Steed's Hooves, Teeth, and Barbed Tails | Melee | - | 6 | 5+ | 3+ | - | 1 | Companion |

**Abilities:**

#### Grind Them to Dust (Activated)
- **Timing:** Once Per Turn (Army), Any Movement Phase
- **Declare:** If this unit is in combat, pick this unit and up to 2 visible friendly **Kavalos War Chariots** wholly within 12" of this unit and in combat to be the targets.
- **Effect:** For each target:
• Make a pile-in move.
• Then, pick an enemy unit in combat with the target and roll a D3. On a 2+, inflict an amount of mortal damage on that enemy unit equal to the roll.

#### Masterful Charioteer (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** If this unit is in combat and has charged this turn, pick this unit and up to 2 visible friendly **Kavalos War Chariots** that have charged this turn, that are wholly within 12" Of this unit and that are in combat to be the targets.
- **Effect:** Each target has Ward (5+) for the rest of the turn.

---

### Liege-Mortek
**Points:** 120 | **Move:** 5" | **Health:** 5 | **Save:** 3+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, DEATH, OSSIARCH BONEREAPERS, WARD (6+), INFANTRY, RELENTLESS DISCIPLINE (5)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Commander's Weapon | Melee | - | 5 | 3+ | 4+ | 1 | 2 | - |

**Abilities:**

#### Clinical Efficiency (Activated)
- **Timing:** Once Per Turn (Army), Reaction: You declared a Fight ability for this unit
- **Effect:** Pick a visible friendly **Mortek Guard**, **Mortek Triaxes** or **Mortek Crawler** unit that has not used an Attack ability this phase and is wholly within 12" Of this unit to be the target. Immediately after the Fight ability used by this unit has been resolved, the target can be picked to use either a Shoot ability as if it were your shooting phase or a Fight ability. If it is picked to do so:
• Add 1 to hit rolls for the target's attacks for the rest of the turn.
• The target's ranged weapons (if any) have **Shoot in Combat** for the rest of the turn.
• If the target uses a Shoot ability and is not in combat, all of its shooting attacks must target units in combat with this unit.

---

### Lord Executioner
**Points:** 170 | **Move:** 8" | **Health:** 5 | **Save:** 5+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, DEATH, FLY, NIGHTHAUNT, INFANTRY, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Decapitating Greataxe | Melee | - | 5 | 3+ | 3+ | 2 | 2 | Anti-Hero (+1 Rend), Crit (Auto-wound) |

**Abilities:**

#### Ghastly Descent (Activated)
- **Timing:** End of Any Turn
- **Declare:** Pick an enemy Infantry or **Cavalry Hero** that had any damage points allocated to it this turn by this unit's attacks to be the target.
- **Effect:** Roll a dice. Add 1 to the roll if the target is a Sentenced unit. On a 5+, the target is automatically destroyed.

#### Staring Death in the Face (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a dice. On a 2+, subtract 1 from the Attacks characteristic of the target’s melee weapons for the rest of the turn. In addition, if the target is a Hero, subtract 5 from the target’s control score for the rest of the turn.

---

### Lord Vitriolic
**Points:** 120 | **Move:** 8" | **Health:** 5 | **Save:** 5+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, DEATH, FLY, NIGHTHAUNT, INFANTRY, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Hurled Vial | Ranged | 10" | 1 | 3+ | 3+ | 2 | D6 | Crit (Auto-wound) |
| Culler's Sickle | Melee | - | 3 | 4+ | 3+ | 1 | D3 | Crit (Auto-wound) |

**Abilities:**

#### Spectral Alchemy (Activated)
- **Timing:** Once Per Turn (Army), Your Shooting Phase
- **Declare:** Pick a visible enemy unit within 10" of this unit to be the target.
- **Effect:** Roll a dice. On a 3+, pick 1 of the following effects or roll two dice and apply both corresponding effects in an order of your choosing, including duplicate effects:
**1 *Phantasmal Solvent:*** If the target has a Move characteristic of '-', inflict 6 mortal damage on the target.
**2 *Acidic Fug:*** Inflict D3 mortal damage on the target.
**3 *Fling Concoctions:*** Add 1 to the Attacks characteristic of this unit's **Hurled Vial** this phase but all attacks made by this unit this phase must target that enemy unit.
**4 *Choking Vapours:*** If the target is a Wizard, subtract 1 from its power level until the start of your next turn.
**5 *Fear-laced Hallucinogen:*** If the target is a Priest, remove D3 ritual points from it.
**6 *Unholy Prescription:*** Ward rolls cannot be made for the target for the rest of the turn.

---

### Marrowscroll Herald
**Points:** 100 | **Move:** 6" | **Health:** 5 | **Save:** 5+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, DEATH, HERO, FLESH-EATER COURTS, WARD (6+), INFANTRY, NOBLE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Bone Scythe | Melee | - | 5 | 4+ | 3+ | 1 | 2 | - |

**Abilities:**

#### Infected (Activated)
- **Timing:** Reaction: Opponent declared a command, Spell ability or Prayer ability for an Infected unit
- **Effect:** Roll a dice. On a 5+:
• If they declared a command, that command has no effect, it still counts as having been used and the command points spent to use the command are still lost.
• If they declared a Spell or Prayer ability, that spell or prayer fails.
This reaction can be used more than once per phase but only once per command, Spell ability or Prayer ability.

#### The King's Entreaty (Activated)
- **Timing:** Once Per Turn (Army), Enemy Charge Phase
- **Declare:** Pick a visible enemy unit within 6" of this unit to be the target.
- **Effect:** Roll a dice. On a 3+, your opponent must decide whether the target will accept or reject the entreaty offered by this unit.
***Accept:*** The target has the Infected keyword for the rest of the battle.
***Reject:*** The target has Strike-last for the rest of the turn.

---

### Morbheg Knights
**Points:** 180 | **Move:** 12" | **Health:** 4 | **Save:** 4+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, WARD (6+), DEATH, CHAMPION, FLY, CAVALRY, STANDARD BEARER (1/3), MUSICIAN (1/3), FLESH-EATER COURTS, KNIGHTS

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Nightshrieker’s Claws and Teeth | Melee | - | 3 | 4+ | 3+ | 1 | 2 | Companion |
| Grisly Lance | Melee | - | 2 | 3+ | 4+ | 1 | 1 | Charge (+1 Damage) |

**Abilities:**

#### Predator Knights (Activated)
- **Timing:** Once Per Turn (Army), End of Enemy Turn
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Inflict D3 mortal damage on the target. Then, this unit can use a Retreat ability as if it were your movement phase. No mortal damage is inflicted on this unit by that Retreat ability.

---

### Morghast Archai
**Points:** 260 | **Move:** 10" | **Health:** 6 | **Save:** 3+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, INFANTRY, OSSIARCH BONEREAPERS, WARD (6+), DEATH, FLY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Spirit Weapons | Melee | - | 3 | 3+ | 2+ | 2 | 3 | Anti-Monster (+1 Rend), Crit (2 Hits) |

**Abilities:**

#### Discharge Armour (Activated)
- **Timing:** Once Per Turn (Army), Any Hero Phase
- **Declare:** Pick a friendly Ossiarch Bonereapers Wizard wholly within 12" of this unit to be the target.
- **Effect:** Roll a dice. On a 3+, add 1 to casting rolls for the target for the rest of the turn.

#### Ebon-wrought Armour (Passive)
- **Effect:** This unit has Ward (3+) against damage points inflicted by Spell abilities, Prayer abilities and abilities used by Manifestations.

---

### Morghast Harbingers
**Points:** 250 | **Move:** 10" | **Health:** 6 | **Save:** 4+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, INFANTRY, OSSIARCH BONEREAPERS, WARD (6+), DEATH, FLY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Spirit Weapons | Melee | - | 3 | 3+ | 2+ | 2 | 3 | Anti-Monster (+1 Rend), Crit (2 Hits) |

**Abilities:**

#### Harbingers of Death (Passive)
- **Effect:** Add 1 to the number of dice rolled when making charge rolls for this unit, to a maximum of 3.

---

### Mortek Crawler
**Points:** 250 | **Move:** 5" | **Health:** 12 | **Save:** 4+ | **Control:** 5
**Keywords:** LEGIONS OF NAGASH, OSSIARCH BONEREAPERS, WARD (6+), DEATH, WAR MACHINE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Attendants' Crawler Tools | Melee | - | 6 | 4+ | 4+ | - | 1 | - |
| Dread Catapult | Ranged | 24" | 4 | 4+ | 2+ | 1 | D3+2 | Crit (2 Hits) |

**Abilities:**

#### Deathly Barrage (Activated)
- **Timing:** Once Per Turn (Army), Reaction: You declared a Shoot ability for this unit.
- **Effect:** After that Shoot ability has been resolved, if all of this unit's attacks targeted the same enemy unit, roll a dice. Add 1 to the roll if this unit is wholly within 12" of a friendly **Mortisan Ossifector**, and add 1 to the roll if any other friendly **Mortek Crawler** units picked that enemy unit to be the target of all of their shooting attacks this turn. On a 4+, the target has Strike-Last for the rest of the turn

#### Cursed Stele (Activated)
- **Timing:** Once Per Turn (Army), Your Shooting Phase
- **Effect:** For the rest of the turn:
• This unit's ranged weapons have **Anti-Faction Terrain (+1 Rend)**.
• Add 1 to the Damage characteristic of this unit's ranged weapons for attacks that target faction terrain features.

---

### Mortek Guard
**Points:** 110 | **Move:** 4" | **Health:** 1 | **Save:** 4+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, INFANTRY, OSSIARCH BONEREAPERS, WARD (6+), DEATH, CHAMPION, STANDARD BEARER (1/10)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Nadirite Blades | Melee | - | 2 | 3+ | 4+ | - | 1 | Anti-Cavalry (+1 Rend) |

**Abilities:**

#### Shields of the Legion (Activated)
- **Timing:** Once Per Phase (Army), Reaction: Opponent declared an Attack ability that targeted a visible Ossiarch Bonereapers unit within this unit's combat range.
- **Effect:** Subtract 1 from the Rend characteristic of weapons used for attacks made as part of that Attack ability that target any friendly Ossiarch Bonereapers units within this unit’s combat range.

---

### Mortek Guard (Scourge of Ghyran)
**Points:** 100 | **Move:** 4" | **Health:** 1 | **Save:** 4+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, INFANTRY, OSSIARCH BONEREAPERS, WARD (6+), DEATH, CHAMPION, STANDARD BEARER (1/10)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Nadirite Blade or Spear | Melee | - | 2 | 3+ | 4+ | - | 1 | Anti-Cavalry (+1 Rend), Crit (2 Hits) |

**Abilities:**

#### Shields of the Legion (Activated)
- **Timing:** Once Per Phase (Army), Reaction: A friendly Ossiarch Bonereapers War Machine or Wizardwithin this unit's combat range was targeted by an Attack ability
- **Effect:** For the rest of the phase, subtract 1 from the Rend characteristic of weapons used for attacks that target that friendly unit.

---

### Mortek Triaxes
**Points:** 140 | **Move:** 4" | **Health:** 1 | **Save:** 5+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, INFANTRY, OSSIARCH BONEREAPERS, WARD (6+), DEATH, CHAMPION

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Osseous Bow | Ranged | 18" | 2 | 3+ | 4+ | 1 | 1 | - |
| Nadirite Dagger | Melee | - | 1 | 3+ | 4+ | - | 1 | - |

**Abilities:**

#### Balefire Braziers (Passive)
- **Effect:** This unit's **Balefire Braziers** are tokens. There are 2 **Balefire Braziers** for every 10 models in this unit. After setting up this unit on the battlefield for the first time, place its **Balefire Braziers** next to it.

#### Cursed Flames (Activated)
- **Timing:** Once Per Turn (Army), Reaction: You declared a Shoot ability for this unit.
- **Effect:** After that Shoot ability has been resolved, if all of this unit's attacks targeted the same enemy unit and this unit has any **Balefire Braziers** next to it, place 1 of those **Balefire Braziers** next to that enemy unit.

#### Atavistic Nightmares (Passive)
- **Effect:** While any of this unit's **Balefire Braziers** are next to an enemy unit, subtract 3 from the control scores of enemy units within that enemy unit's combat range.
If an ability would heal or return slain models to an enemy unit while any of this unit's **Balefire Braziers** are next to it, that ability does not heal or return any slain models to it. Instead, place those **Balefire Braziers** next to this unit.

---

### Mortis Engine
**Points:** 230 | **Move:** 10" | **Health:** 12 | **Save:** 4+ | **Control:** 5
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, WIZARD (1), WARD (6+), DEATH, SOULBLIGHT GRAVELORDS, WAR MACHINE, FLY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Corpsemaster's Staff | Melee | - | 3 | 4+ | 3+ | 1 | D3 | - |
| Spectral Host's Blades | Melee | - | 10 | 4+ | 4+ | - | 1 | Companion |

**Abilities:**

#### Nexus of Death Energy (Passive)
- **Effect:** When a friendly Deathrattle or Deadwalkers unit wholly within 12" of this unit is picked to be the target of the ‘Deathly Invocation’ ability, add D3 to the number of damage points that can be healed or the combined Health characteristic of models that can be returned.

#### Wave of Power (Activated)
- **Timing:** Once Per Turn (Army), Your Shooting Phase
- **Declare:** Pick up to 3 enemy units within 10" of this unit to be the targets.
- **Effect:** Roll a dice for each target. On a 3+, inflict an amount of mortal damage on the target equal to the value of this unit’s **stored energy dice**. Then, change the value of the **stored energy dice** back to 1.

#### The Reliquary (Passive)
- **Effect:** When this unit is set up for the first time, place a **stored energy dice** beside it with the ‘1’ facing up. Each time a friendly Soulblight Gravelords Wizards successfully casts a spell while it is within 12" of this unit, increase the value of the **stored energy dice** beside this unit by 1 (to a maximum of 6).

---

### Mortis Reapers
**Points:** 90 | **Move:** 6" | **Health:** 2 | **Save:** 5+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, INFANTRY, OSSIARCH BONEREAPERS, WARD (6+), DEATH, CHAMPION

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Reaper' Blades | Melee | - | 3 | 3+ | 4+ | 1 | 1 | Anti-Hero (+1 Rend) |

**Abilities:**

#### Necrocaches (Activated)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Declare:** Pick each enemy unit in combat with this unit to be the targets.
- **Effect:** Roll a D3 for each target. On a 2+, inflict an amount of mortal damage on the target equal to the roll. Then, this unit can move 6". It can move through the combat ranges of enemy units but cannot end that move in combat.

#### No Escaping the Tithe (Activated)
- **Timing:** Once Per Turn (Army), Any Movement Phase
- **Declare:** If there are no enemy units within 6" of this unit, pick a terrain feature within 3" of this unit to be the target.
- **Effect:** Remove this unit from the battlefield and set it up again wholly within 3" of the target and more than 9" from all enemy units.

---

### Mortisan Boneshaper
**Points:** 140 | **Move:** 5" | **Health:** 5 | **Save:** 4+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, WIZARD (1), INFANTRY, WARD (6+), OSSIARCH BONEREAPERS, DEATH, RELENTLESS DISCIPLINE (2)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Ossified Talons | Melee | - | 3 | 4+ | 4+ | - | D3 | - |

**Abilities:**

#### Legion Quartermaster (Passive)
- **Effect:** Each time a friendly non-Hero Ossiarch Bonereapers unit wholly within 12" of this unit uses the 'Rally' command, you can make 3 additional rally rolls of D6.

#### Boneshaper (Command)
- **Timing:** Your Hero Phase
- **Cost:** 1
- **Declare:** If this unit is not in combat, pick a friendly **Mortek Guard** or **Mortek Triaxes** unit that has been destroyed to be the target.
- **Effect:** Set up a replacement unit with half the number of models from the target unit (rounding up) wholly within 12" of this unit and not in combat.

---

### Mortisan Boneshaper (Scourge of Ghyran)
**Points:** 100 | **Move:** 5" | **Health:** 5 | **Save:** 4+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, WIZARD (1), INFANTRY, WARD (6+), OSSIARCH BONEREAPERS, MORTISAN, DEATH

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Ossified Talons | Melee | - | 3 | 4+ | 4+ | - | 2 | Crit (2 Hits) |

**Abilities:**

#### Boneshaper (Command)
- **Timing:** Your Hero Phase
- **Cost:** 1
- **Declare:** If this unit is not in combat, pick a friendly **Mortek Guard**unit that has been destroyed to be the target.
- **Effect:** Set up a replacement unit with half the number of models from the target unit (rounding up) wholly within 12" of this unit and more than 9" from all enemy units.

#### Legion Quartermaster (Passive)
- **Effect:** Each time a friendly non-Hero Ossiarch Bonereapers unit wholly within 12" of this unit uses the 'Rally' command, you can make 3 additional rally rolls of D6.

---

### Mortisan Ossifector
**Points:** 110 | **Move:** 5" | **Health:** 5 | **Save:** 4+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, WIZARD (1), INFANTRY, WARD (6+), OSSIARCH BONEREAPERS, DEATH, RELENTLESS DISCIPLINE (2), FLY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Ossified Talons | Melee | - | 3 | 4+ | 4+ | - | D3 | - |

**Abilities:**

#### Refined Creations (Activated)
- **Timing:** Once Per Turn (Army), Your Hero Phase
- **Declare:** Pick a friendly **Mortek Crawler**, **Gothizzar Harvester**, **Morghast Archai** or **Morghast Harbingers** unit wholly within 12" of this unit to be the target.
- **Effect:** Roll a dice. On a 3+, add 1 to the Rend characteristic of the target’s weapons until the start of your next turn.

---

### Mortisan Soulmason
**Points:** 150 | **Move:** 6" | **Health:** 6 | **Save:** 4+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, INFANTRY, WARD (6+), OSSIARCH BONEREAPERS, DEATH, WIZARD (2), RELENTLESS DISCIPLINE (2)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Soulmason's Staff | Melee | - | 3 | 4+ | 4+ | - | D3 | - |

**Abilities:**

#### Soul-guide (Spell)
- **Timing:** Your Hero Phase
- **Casting Value:** 7
- **Declare:** Pick a visible friendly Ossiarch Bonereapers unit wholly within 12" of the caster to be the target, then make a casting roll of 2D6.
- **Effect:** The target has Strike-first for the rest of the turn.
- **Keywords:** Spell

---

### Mortisan Soulreaper
**Points:** 130 | **Move:** 5" | **Health:** 5 | **Save:** 4+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, WIZARD (1), INFANTRY, WARD (6+), OSSIARCH BONEREAPERS, DEATH, RELENTLESS DISCIPLINE (2), FLY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Soulreaper Scythe | Melee | - | 4 | 3+ | 4+ | 2 | D3 | Crit (2 Hits) |

**Abilities:**

#### Soulreaper (Passive)
- **Effect:** Subtract 1 from wound rolls for combat attacks made by enemy units while they are in combat with this unit.

#### Necrotic Impetus (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick each enemy unit in combat with this unit to be the targets.
- **Effect:** Roll a D3 for each target. On a 2+, inflict an amount of mortal damage on the target equal to the roll. After this ability has been resolved, you gain a number of **relentless discipline** points equal to the number of total damage points allocated by this ability this turn.

---

### Myrmourn Banshees
**Points:** 100 | **Move:** 8" | **Health:** 2 | **Save:** 6+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, INFANTRY, DEATH, FLY, NIGHTHAUNT, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Chill Dagger | Melee | - | 2 | 4+ | 3+ | 1 | D3 | Anti-Wizard (+1 Rend), Crit (Auto-wound) |

**Abilities:**

#### Spell-eaters (Activated)
- **Timing:** Reaction: Opponent declared a Spell ability
- **Effect:** If a friendly Nighthaunt unit wholly within 12" of this unit was picked to be the target of that spell, this unit can use the ‘Unbind’ ability as if it had Wizard (1). Add 1 to the unbinding roll for that ability. If the spell is unbound, inflict D3 mortal damage on the caster

---

### Nagash, Supreme Lord of the Underworlds
**Points:** 670 | **Move:** 10" | **Health:** 18 | **Save:** 3+ | **Control:** 10
**Keywords:** DEATH, UNIQUE, HERO, MONSTER, WIZARD (9), WARD (5+), FLY, WARMASTER, FLESH-EATER COURTS, NIGHTHAUNT, SOULBLIGHT GRAVELORDS, OSSIARCH BONEREAPERS, LEGIONS OF NAGASH, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Alakanash, the Staff of Power | Melee | - | 4 | 3+ | 3+ | 2 | D6 | - |
| Zefet-nebtar, the Mortis Blade | Melee | - | 4 | 3+ | 3+ | 2 | 3 | - |

**Abilities:**

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, subtract 3 from its power level.

#### The Staff of Power (Passive)
- **Effect:** Add 2 to casting rolls for this unit while it has not miscast any spells this turn. If this unit miscasts a spell, ignore the restriction that would stop this unit from casting any more spells this turn.

#### Soul Devourer (Activated)
- **Timing:** End of Any Turn
- **Declare:** Pick any number of units (friendly or enemy) within this unit's combat range to be the targets.
- **Effect:** Inflict D6 mortal damage on each target. For each model slain by this ability, **Heal (1)** this unit.
- **Keywords:** Rampage

#### Father of Necromancy (Activated)
- **Timing:** Your Hero Phase
- **Declare:** Pick 2 visible friendly Legions of Nagash units that are wholly within 18" of this unit. Pick 1 of those units to be the target, then pick the other to be **drained**.
- **Effect:** Up to 3 models in the **drained** unit are automatically slain. Then, return a number of slain models to the target unit with a combined Health characteristic of models slain in the **drained** unit.

#### Necromantic Empowerment (Activated)
- **Timing:** Onec Per Turn (Army), Any Combat Phase
- **Declare:** Pick a viisble friendly **Legions of Nagash** unit within this unit's combat range to be the target.
- **Effect:** Allocate D3 mortal damage to the target (ward rolls cannot be made for those damage points). If any models are slain by this ability, for the rest of the turn:
• This unit's **Alakanash** has **Crit (Mortal)**.
• This unit's **Zefet-nebtar** has **Crit (2 Hits)**.
- **Keywords:** Rampage

---

### Necromancer
**Points:** 120 | **Move:** 5" | **Health:** 5 | **Save:** 6+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, WARD (6+), DEATH, SOULBLIGHT GRAVELORDS, INFANTRY, WIZARD (1)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Mortis Staff | Melee | - | 3 | 4+ | 4+ | 1 | 2 | - |

**Abilities:**

#### Vanhel's Danse Macabre (Command)
- **Timing:** Once Per Turn (Army), Your Combat Phase
- **Cost:** 1
- **Declare:** Pick a friendly Deathrattle or Deadwalkers unit wholly within 12" of this unit to be the target.
- **Effect:** The target can use 2 Fight abilities this phase. After the first is used, however, the target has Strike-last for the rest of the turn.

#### Undead Minions (Passive)
- **Effect:** While this unit is within the combat range of a friendly Deathrattle or Deadwalkers unit:
• This unit has Ward (4+).
• Each time you make a successful ward roll for this unit, allocate 1 damage point to a friendly Deathrattle or Deadwalkers unit within this unit’s combat range after the damage sequence for this unit has been resolved (ward rolls cannot be made for those damage points).

---

### Necropolis Stalkers
**Points:** 140 | **Move:** 7" | **Health:** 4 | **Save:** 4+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, INFANTRY, OSSIARCH BONEREAPERS, WARD (6+), DEATH

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Stalker Blades | Melee | - | 4 | 3+ | 3+ | 2 | 1 | - |

**Abilities:**

#### Fourfold Slayers (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a number of dice equal to the number of models in the target unit. For each 6, inflict 1 mortal damage on the target. If this unit has 3 or more models, inflict 1 mortal damage on the target for each 5+ instead.

---

### Pyregheists
**Points:** 180 | **Move:** 8" | **Health:** 2 | **Save:** 6+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, INFANTRY, DEATH, CHAMPION (1/8), FLY, NIGHTHAUNT, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Flaming Weapons | Melee | - | 2 | 4+ | 4+ | - | 1 | Crit (Auto-wound) |
| Gout of Balefire | Ranged | 10" | 2 | 2+ | 4+ | 1 | 1 | Crit (Auto-wound), Shoot in Combat |

**Abilities:**

#### Light a Pyre (Activated)
- **Timing:** Once Per Turn (army), End of Any Turn
- **Declare:** If this unit is contesting an
objective, pick a visible enemy unit within
6" of this unit to be the target.
- **Effect:** Roll a D3. On a 2+:
• Inflict an amount of mortal damage on the target equal to the roll.
• Add a number equal to the roll to this unit’s control score for the rest of the turn.

---

### Revenant Draconith
**Points:** 190 | **Move:** 12" | **Health:** 16 | **Save:** 5+ | **Control:** 5
**Keywords:** LEGIONS OF NAGASH, WARD (6+), DEATH, MONSTER, FLY, SOULBLIGHT GRAVELORDS

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Pestilential Miasma | Ranged | 10" | 5 | 3+ | 3+ | 1 | 2 | Shoot in Combat, Companion |
| Draconic Maw | Melee | - | 3 | 4+ | 2+ | 2 | 3 | Companion |
| Draconic Claws | Melee | - | 7 | 4+ | 2+ | 1 | 2 | Companion |

**Abilities:**

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of its **Draconic Claws** is 5.

#### Death on Tattered Wings (Activated)
- **Timing:** Deployment Phase
- **Declare:** Pick this unit if it has not been deployed.
- **Effect:** Set up this unit in reserve **high above the battlefield**. It has now been deployed.
- **Keywords:** Deploy

#### Loathsome Descent (Activated)
- **Timing:** Your Movement Phase
- **Declare:** Pick this unit if it is **high above the battlefield**.
- **Effect:** Set up this unit anywhere on the battlefield more than 9" from all enemy units.

#### Red Ruin (Activated)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a D3. On a 2+, resolve the following effects:
• Inflict an amount of mortal damage on the target equal to the roll.
• **Heal (X)** this unit where **X** is an amount equal to the roll.
This unit can be removed from the battlefield and set up in reserve **high above the battlefield**.
- **Keywords:** Rampage

---

### Royal Beastflayers
**Points:** 110 | **Move:** 6" | **Health:** 1 | **Save:** 6+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, WARD (6+), DEATH, FLESH-EATER COURTS, INFANTRY, SERFS, CHAMPION (1/10)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Beastflayer Weapons | Melee | - | 3 | 4+ | 4+ | - | 1 | Anti-Beast (+1 Rend), Anti-Monster (+1 Rend) |

**Abilities:**

#### Hunter's Instincts (Passive)
- **Effect:** Enemy Monsters cannot use Rampage abilities while they are in combat with this unit. In addition, subtract 1 from the Damage characteristic of melee weapons used by enemy Monsters and Beast units while they are in combat with this unit.

---

### Royal Decapitator
**Points:** 100 | **Move:** 6" | **Health:** 5 | **Save:** 5+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, DEATH, HERO, FLESH-EATER COURTS, WARD (6+), INFANTRY, NOBLE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Headsman’s Axe | Melee | - | 3 | 4+ | 3+ | 2 | 3 | - |

**Abilities:**

#### Executioner's Entourage (Activated)
- **Timing:** Reaction: You declared a Fight ability for this unit
- **Effect:** Pick a friendly Serfs unit that has not used a Fight ability this turn and is within this unit’s combat range to be the target. The target can be picked to use a Fight ability immediately after the Fight ability used by this unit has been resolved. If it is picked to do so, the target's melee weapons have **Crit (2 Hits)** for the rest of the turn.

#### Off With Their Head! (Activated)
- **Timing:** Reaction: You declared a Fight ability for this unit
- **Effect:** Immediately after that Fight ability has been resolved, pick an enemy Infantry Hero in combat with this unit to be the target. Roll 2D6. If the roll exceeds the target's Health characteristic, it is automatically destroyed.

---

### Royal Terrorgheist
**Points:** 230 | **Move:** 12" | **Health:** 14 | **Save:** 4+ | **Control:** 5
**Keywords:** LEGIONS OF NAGASH, DEATH, MONSTER, FLESH-EATER COURTS, FLY, WARD (6+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Death Shriek | Ranged | 10" | 1 | 4+ | 2+ | 2 | D6 | Shoot in Combat, Companion |
| Terrorgheist’s Fanged Maw | Melee | - | 3 | 4+ | 2+ | 2 | D3+3 | Crit (Mortal), Companion |
| Terrorgheist’s Skeletal Talons | Melee | - | 6 | 4+ | 2+ | 1 | 2 | Crit (Mortal), Companion |

**Abilities:**

#### Shriek of Terror (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy Infantry unit in combat with this unit to be the target.
- **Effect:** Roll a dice for each model in the target unit. For each 6, inflict 1 mortal damage on the target. If 3 or more damage points are allocated to the target by this ability, subtract 1 from hit rolls for the target’s attacks for the rest of the turn.
- **Keywords:** Rampage

#### Necromantic Limits (Passive)
- **Effect:** This unit's **Skeletal Talons** have a maximum Attacks characteristic of 10.
While this unit has 10 or more damage points, the Attacks characteristic of its **Skeletal Talons** is 4.

#### Wicked Predator (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** For each champion, standard bearer, and musician in the target unit, add 1 to the Attacks characteristic of this unit's **Terrorgheist's Skeletal Talons**, to a maximum of 10, for the rest of the phase. Those additional attacks must target that enemy unit.

---

### Royal Zombie Dragon
**Points:** 250 | **Move:** 12" | **Health:** 14 | **Save:** 4+ | **Control:** 5
**Keywords:** LEGIONS OF NAGASH, DEATH, MONSTER, FLESH-EATER COURTS, WARD (6+), FLY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Baneful Breath | Ranged | 10" | D6 | 2+ | 4+ | 1 | 3 | Shoot in Combat, Companion |
| Draconic Jaws | Melee | - | 4 | 4+ | 2+ | 2 | 4 | Crit (Auto-wound), Companion |
| Draconic Claws | Melee | - | 7 | 4+ | 2+ | 1 | 2 | Crit (Auto-wound), Companion |

**Abilities:**

#### Soaring Terror (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Effect:** If this unit charged this turn, roll a dice. On a 3+, this unit can move 2D6". It can pass through the combat ranges of enemy units but must end that move in combat. Then, pick an enemy unit that this unit passed across during that move. Inflict D3 mortal damage on that enemy unit.
- **Keywords:** Rampage

#### On Rotten Wings (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Effect:** Remove this unit from the battlefield and set it up again on the battlefield more than 9" from all enemy units.

#### Necromantic Limits (Passive)
- **Effect:** This unit's **Draconic Claws** have a maximum Attacks characteristic of 10.
While this unit has 10 or more damage points, the Attacks characteristic of its **Draconic Claws** is 5.

---

### Scriptor Mortis
**Points:** 120 | **Move:** 8" | **Health:** 5 | **Save:** 5+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, DEATH, FLY, NIGHTHAUNT, INFANTRY, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Malicious Quill | Melee | - | 3 | 4+ | 3+ | - | 1 | Crit (Auto-wound) |

**Abilities:**

#### Sentenced to Eternal Torment (Activated)
- **Timing:** Once Per Turn (Army), Your Hero Phase
- **Declare:** Pick a visible enemy unit within 18" of this unit to be the target.
- **Effect:** Roll a dice. On a 3+, inflict 1 mortal damage on the target and the target has the Sentenced keyword until the start of your next turn.

#### Judge, Jury and Executioner (Passive)
- **Effect:** Each time a friendly Nighthaunt Infantry unit uses a Fight ability, if all of its attacks target the same Sentenced enemy unit, that friendly unit's melee weapons have **Crit (Mortal)** for that Fight ability.

---

### Spirit Hosts
**Points:** 110 | **Move:** 8" | **Health:** 3 | **Save:** 6+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, INFANTRY, DEATH, FLY, NIGHTHAUNT, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Spectral Claws and Daggers | Melee | - | 6 | 4+ | 4+ | - | 1 | Crit (Auto-wound) |

**Abilities:**

#### Drawn to War (Passive)
- **Effect:** Friendly Nighthaunt Infantry Heroes have Ward (4+) while they are wholly within this unit’s combat range.

---

### Spirit Torment
**Points:** 110 | **Move:** 8" | **Health:** 6 | **Save:** 5+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, DEATH, FLY, NIGHTHAUNT, INFANTRY, WARD (5+)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Shacklegheist Chains | Melee | - | 5 | 4+ | 3+ | 1 | 2 | Crit (Auto-wound) |

**Abilities:**

#### Captured Soul Energy (Passive)
- **Effect:** Each time a friendly Nighthaunt unit wholly within 12" of this unit uses the ‘Rally’ command, you can make 3 additional rally rolls of D6.

---

### Teratic Cohort
**Points:** 90 | **Move:** 7" | **Health:** 1 | **Save:** 5+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, OSSIARCH BONEREAPERS, WARD (6+), DEATH, CHAMPION (1/8), BEAST

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Cohort Weapons | Melee | - | 2 | 3+ | 4+ | 1 | 1 | - |

**Abilities:**

#### Bone-deep Savagery (Activated)
- **Timing:** Once Per Turn (Army), Any Hero Phase
- **Effect:** This unit can move 3". If it can move into combat, it must. If it was in combat at the start of the move, it must end that move in combat.

#### Hunters Unleashed (Activated)
- **Timing:** Deployment Phase
- **Effect:** This unit can use the 'Normal Move' ability as if it were your movement phase.

---

### Terrorgheist
**Points:** 220 | **Move:** 12" | **Health:** 14 | **Save:** 4+ | **Control:** 5
**Keywords:** LEGIONS OF NAGASH, WARD (6+), DEATH, MONSTER, FLY, SOULBLIGHT GRAVELORDS, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Death shriek | Ranged | 10" | 1 | 4+ | 2+ | 2 | D6 | Shoot in Combat, Companion |
| Fanged Maw | Melee | - | 3 | 4+ | 2+ | 2 | D6 | Companion |
| Skeletal Talons | Melee | - | 6 | 4+ | 2+ | 1 | 2 | Companion |

**Abilities:**

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of its **Skeletal Talons** is 5.

#### Gaping Maw (Passive)
- **Effect:** Each time an attack made with this unit’s **Fanged Maw** scores a critical hit, that attack inflicts 6 mortal damage on the target unit and the attack sequence ends.

#### Sonic Assault (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** If this unit charged this turn, pick an enemy unit in combat with it to be the target.
- **Effect:** Roll a dice for each model in the target unit.
If 3 or more of the dice have the same value, for the rest of the turn, this unit has Strike-first. 
If 6 or more of the dice have the same value, for the rest of the turn, this unit has Strike-first and the target cannot use commands.

---

### Tomb Banshee
**Points:** 130 | **Move:** 8" | **Health:** 5 | **Save:** 5+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, DEATH, FLY, NIGHTHAUNT, INFANTRY, WARD (5+), Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Piercing Scream | Ranged | 12" | 2 | 4+ | 3+ | 2 | D3 | Shoot in Combat |
| Ancient Chill Dagger | Melee | - | 3 | 4+ | 3+ | 2 | D3 | Crit (Auto-wound) |

**Abilities:**

#### Cacophony of Sorrow (Passive)
- **Effect:** While there are any friendly **Myrmourn Banshees** units wholly within 12" of this unit, ignore positive modifiers to the control scores of enemy units within 12" of this unit.

#### Ghostly Howl (Activated)
- **Timing:** Once Per Battle(Army), Any Shooting Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Inflict D3 mortal damage on the target. If any models in the target unit are slain by this ability, the target cannot use commands for the rest of the turn.

---

### Vampire Lord
**Points:** 140 | **Move:** 6" | **Health:** 5 | **Save:** 3+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, WIZARD (1), WARD (6+), DEATH, SOULBLIGHT GRAVELORDS, INFANTRY, VAMPIRE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Dynastic War-relic | Melee | - | 5 | 3+ | 3+ | 1 | 2 | Anti-Hero(+1 Rend), Crit (2 Hits) |

**Abilities:**

#### Sanguine Blur (Activated)
- **Timing:** Once Per Turn (Army), Your Hero Phase
- **Effect:** Roll a dice. On a 3+, remove this unit from the battlefield and set it up again on the battlefield more than 9" from all enemy non-Hero units and more than 3" from all enemy Heroes.

---

### Vampire Lord on Nightmare Steed
**Points:** 190 | **Move:** 12" | **Health:** 7 | **Save:** 3+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, WIZARD (1), WARD (6+), DEATH, SOULBLIGHT GRAVELORDS, VAMPIRE, CAVALRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Dynastic Cavalier Weapon | Melee | - | 5 | 3+ | 3+ | 2 | 2 | Charge (+1 Damage) |
| Nightmare’s Hooves and Teeth | Melee | - | 3 | 5+ | 3+ | - | 1 | Companion |

**Abilities:**

#### Strike at the Heart (Activated)
- **Timing:** Any Combat Phase
- **Declare:** Pick a friendly **Blood Knights** unit that charged this turn and is wholly within 12" of this unit to be the target.
- **Effect:** Add 1 to the Rend characteristic of that unit's **Templar Weapons** for the rest of the turn.

#### 'For Glory! For Blood!' (Activated)
- **Timing:** Reaction: You declared a Charge ability for this unit
- **Effect:** You can re-roll the charge roll for that Charge ability. Then, before the charge move is made, you can pick a friendly **Blood Knights** unit that is not in combat, is wholly within 12" of this unit and has not used a Charge ability this turn to be the target. If this unit charges, then, immediately after the Charge ability used by this unit has been resolved, the target can immediately use the ‘Charge’ ability even if it is not your charge phase. In addition, you can re-roll charge rolls for the target this phase.

---

### Vampire Lord on Zombie Dragon
**Points:** 400 | **Move:** 12" | **Health:** 16 | **Save:** 3+ | **Control:** 5
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, WIZARD (1), WARD (6+), DEATH, MONSTER, FLY, SOULBLIGHT GRAVELORDS, VAMPIRE, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Pestilential Breath | Ranged | 10" | D6 | 2+ | 4+ | 1 | 2 | Shoot in Combat, Companion |
| Dynastic Weapon | Melee | - | 4 | 3+ | 3+ | 2 | 2 | Charge (+1 Damage) |
| Draconic Maw | Melee | - | 3 | 4+ | 2+ | 2 | 3 | Companion |
| Draconic Claws | Melee | - | 7 | 4+ | 2+ | 1 | 2 | Companion |

**Abilities:**

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of its Draconic Claws is 5.

#### Bloodthirsty Dominance (Activated)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Declare:** If this unit charged this turn, pick an enemy unit in combat with it to be the target.
- **Effect:** Roll a number of dice equal to the target’s Health characteristic. For each 6, inflict 1 mortal damage on the target.
- **Keywords:** Rampage

#### That Which Should Not Be (Passive)
- **Effect:** If this unit charged this turn, for the rest of the turn, subtract 5 from the control scores of enemy units while they are in combat with this unit.

---

### Vargheists
**Points:** 120 | **Move:** 12" | **Health:** 4 | **Save:** 5+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, DEATH, WARD (6+), CHAMPION, SOULBLIGHT GRAVELORDS, VAMPIRE, INFANTRY, FLY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Murderous Fangs and Talons | Melee | - | 3 | 4+ | 3+ | 1 | 2 | Crit (2 Hits) |

**Abilities:**

#### Death's Descent (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Effect:** If this unit is not in combat, remove it from the battlefield and set it up again on the battlefield more than 9" from all enemy units.
- **Keywords:** Core

---

### Varghulf Courtier
**Points:** 100 | **Move:** 10" | **Health:** 8 | **Save:** 5+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, DEATH, FLESH-EATER COURTS, WARD (6+), BEAST

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Immense Claws | Melee | - | 7 | 4+ | 3+ | 1 | 2 | Anti-Infantry (+1 Rend) |
| Dagger-like Fangs | Melee | - | 1 | 3+ | 2+ | 2 | 3 | Anti-Infantry (+1 Rend) |

**Abilities:**

#### Victory Feast (Activated)
- **Timing:** End of Any Turn
- **Effect:** If any models were slain by this unit this turn:
• **Heal (D6)** this unit.
• If this unit is in combat, this unit can immediately move 10". It can move through the combat ranges of enemy units but must end that move in combat with the units it was already in combat with at the start of the move.
• If that unit is not in combat, this unit can immediately move 10" but cannot end that move in combat.

---

### Vargskyr
**Points:** 140 | **Move:** 8" | **Health:** 8 | **Save:** 5+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, DEATH, WARD (6+), SOULBLIGHT GRAVELORDS, VAMPIRE, BEAST, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Lacerating Fangs | Melee | - | 2 | 4+ | 2+ | 2 | 3 | Companion |
| Raking Talons | Melee | - | 4 | 4+ | 2+ | 1 | 2 | Companion |

**Abilities:**

#### Beast (Passive)
- **Effect:** This unit has a maximum control score of 1.

#### Bounding Haste (Passive)
- **Effect:** Add 1 to the number of dice rolled when making charge rolls for this unit, to a maximum of 3. In addition, when this unit uses a Charge ability, it can pass through terrain features and enemy models as if it had Fly.

---

### Vengorian Lord
**Points:** 220 | **Move:** 10" | **Health:** 11 | **Save:** 4+ | **Control:** 5
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, WIZARD (1), WARD (6+), DEATH, MONSTER, FLY, SOULBLIGHT GRAVELORDS, VAMPIRE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Gore-drenched Talons | Melee | - | 3 | 3+ | 2+ | 3 | 3 | - |
| Nightmare Sabre | Melee | - | 4 | 3+ | 3+ | 1 | 2 | - |

**Abilities:**

#### Festering Feast (Activated)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Effect:** If any damage points were allocated to enemy units by this unit's attacks this turn, **Heal (D3)** this unit and each other friendly Soulblight Gravelords Monster wholly within 12" of this unit.

#### Scything Talons (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** If this unit charged this turn, pick an enemy unit in combat with it to be the target.
- **Effect:** Roll a dice. On a 3+, roll a dice for each model in the target unit. For each 5+, inflict 1 mortal damage on the target.
- **Keywords:** Rampage

#### Nightmare's Miasma (Passive)
- **Effect:** Subtract 1 from the Rend characteristic of melee weapons used by enemy units while they are in combat with this unit.

---

### Vyrkos Blood-born
**Points:** 150 | **Move:** 8" | **Health:** 3 | **Save:** 6+ | **Control:** 1
**Keywords:** LEGIONS OF NAGASH, DEATH, WARD (6+), SOULBLIGHT GRAVELORDS, INFANTRY, VAMPIRE, Legends

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Piercing Blade | Melee | - | 3 | 3+ | 3+ | 1 | 2 | Crit (2 Hits) |

**Abilities:**

#### Shadowfast (Activated)
- **Timing:** Once Per Turn (Army), End of Enemy Turn
- **Effect:** This unit can use a Move ability as if it were your movement phase.

#### Cling to the Shadows (Passive)
- **Effect:** While each model in this unit is within 1" of any terrain features, this unit cannot be targeted by shooting attacks unless the attacking model is within 9" of it.

---

### Wight King
**Points:** 80 | **Move:** 4" | **Health:** 5 | **Save:** 3+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, WARD (6+), DEATH, SOULBLIGHT GRAVELORDS, INFANTRY, DEATHRATTLE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Baleful Tomb Blade | Melee | - | 5 | 4+ | 3+ | 1 | 2 | Crit (Mortal) |

**Abilities:**

#### Ancient Strategies (Activated)
- **Timing:** Reaction: You declared a Fight ability for this unit
- **Effect:** Pick a friendly non-Hero Deathrattle Infantry unit that has not used a Fight ability this turn and is within this unit’s combat range to be the target. The target can be picked to use a Fight ability immediately after the Fight** ability used by this unit has been resolved. If it is picked to do so, add 1 to hit rolls for the target’s attacks for the rest of the turn.

#### King of Shambling Bones (Activated)
- **Timing:** Once Per Turn (Army), Your Hero Phase
- **Declare:** Pick a friendly Deathrattle unit within this unit's combat range to be the target.
- **Effect:** Roll a dice. On a 3+, the target's melee weapons have **Crit (2 Hits)** for the rest of the turn.

---

### Wight King on Skeletal Steed
**Points:** 180 | **Move:** 10" | **Health:** 8 | **Save:** 3+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, WARD (6+), DEATH, SOULBLIGHT GRAVELORDS, DEATHRATTLE, CAVALRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| King's Relic Weapon | Melee | - | 5 | 4+ | 3+ | 1 | 2 | Charge (+1 Damage), Crit (Mortal) |
| Skeletal Steed’s Hooves and Teeth | Melee | - | 2 | 5+ | 3+ | - | 1 | Companion |

**Abilities:**

#### Immortal Conqueror (Activated)
- **Timing:** Your Hero Phase
- **Declare:** Pick an objective or terrain feature within enemy territory.
- **Effect:** For the rest of the turn, add 1 to hit rolls for attacks made by friendly Deathrattle units while they are wholly within 6" of that objective or terrain feature.

#### Ancient Barrow Curse (Passive)
- **Effect:** If an attack made with this unit's **King's Relic Weapon** scores a critical hit, the target unit becomes **cursed** for the rest of the battle. Subtract 1 from save rolls for **cursed** units even if this unit has been destroyed.
**Designer's Note:** *Multiple enemy units can become cursed by this ability but the same enemy unit cannot become cursed more than once.*

---

### Wight Lord on Skeletal Steed
**Points:** 110 | **Move:** 10" | **Health:** 7 | **Save:** 3+ | **Control:** 2
**Keywords:** LEGIONS OF NAGASH, Restrict General, HERO, WARD (6+), DEATH, SOULBLIGHT GRAVELORDS, DEATHRATTLE, CAVALRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Lord's Tomb Blade | Melee | - | 4 | 4+ | 3+ | 1 | 2 | Charge (+1 Damage), Crit (Mortal) |
| Skeletal Steed’s Hooves and Teeth | Melee | - | 2 | 5+ | 3+ | - | 1 | Companion |

**Abilities:**

#### Royal Companions (Activated)
- **Timing:** Reaction: You declared a Fight ability for this unit
- **Effect:** Pick a friendly non-Hero Deathrattle Cavalry unit that has not used a Fight ability this turn and is within this unit’s combat range to be the target. The target can be picked to use a Fight ability immediately after the Fight ability used by this unit has been resolved.

#### Lord of Trampling Bones (Passive)
- **Effect:** Add 1 to charge rolls for friendly Deathrattle Cavalry units while they are wholly within 12" of this unit.

---

## Points Table

| Unit | Points |
| --- | ---: |
| Corpse Cart | 70 |
| Corpse Cart (Scourge of Ghyran) | 80 |
| Crypt Ghast Courtier | 80 |
| Fell Bats | 80 |
| Wight King | 80 |
| Chainghasts | 90 |
| Crypt Flayers | 90 |
| Deathrattle Skeletons | 90 |
| Mortis Reapers | 90 |
| Teratic Cohort | 90 |
| Chainrasps | 100 |
| Craventhrone Guard | 100 |
| Glaivewraith Stalkers | 100 |
| Knight of Shrouds | 100 |
| Marrowscroll Herald | 100 |
| Mortek Guard (Scourge of Ghyran) | 100 |
| Mortisan Boneshaper (Scourge of Ghyran) | 100 |
| Myrmourn Banshees | 100 |
| Royal Decapitator | 100 |
| Varghulf Courtier | 100 |
| Abhorrant Ghoul King (Scourge of Ghyran) | 110 |
| Guardian of Souls | 110 |
| Kosargi Nightguard | 110 |
| Mortek Guard | 110 |
| Mortisan Ossifector | 110 |
| Royal Beastflayers | 110 |
| Spirit Hosts | 110 |
| Spirit Torment | 110 |
| Wight Lord on Skeletal Steed | 110 |
| Abhorrant Ghoul King | 120 |
| Crypt Haunter Courtier | 120 |
| Cryptguard | 120 |
| Liege-Mortek | 120 |
| Lord Vitriolic | 120 |
| Necromancer | 120 |
| Scriptor Mortis | 120 |
| Vargheists | 120 |
| Abhorrant Cardinal | 130 |
| Abhorrant Gorewarden (Scourge of Ghyran) | 130 |
| Barrow Guard | 130 |
| Cairn Wraith | 130 |
| Deadwalker Zombies | 130 |
| Mortisan Soulreaper | 130 |
| Tomb Banshee | 130 |
| Abhorrant Archregent | 140 |
| Askurgan Trueblades | 140 |
| Crypt Infernal Courtier | 140 |
| Dreadblade Harrows | 140 |
| Mortek Triaxes | 140 |
| Mortisan Boneshaper | 140 |
| Necropolis Stalkers | 140 |
| Vampire Lord | 140 |
| Vargskyr | 140 |
| Crypt Ghouls | 150 |
| Dire Wolves | 150 |
| Hexwraiths | 150 |
| Kavalos Deathriders | 150 |
| Krulghast Cruciator | 150 |
| Mortisan Soulmason | 150 |
| Vyrkos Blood-born | 150 |
| Crypt Horrors | 160 |
| Knight of Shrouds on Ethereal Steed | 160 |
| Abhorrant Gorewarden | 170 |
| Immortis Guard | 170 |
| Kavalos War Chariot | 170 |
| Lord Executioner | 170 |
| Liege-Kavalos | 180 |
| Morbheg Knights | 180 |
| Pyregheists | 180 |
| Wight King on Skeletal Steed | 180 |
| Revenant Draconith | 190 |
| Vampire Lord on Nightmare Steed | 190 |
| Barrow Knights | 200 |
| Dreadscythe Harridans | 200 |
| Gothizzar Harvester | 200 |
| Grimghast Reapers | 200 |
| Bladegheist Revenants | 210 |
| Black Coach | 220 |
| Blood Knights | 220 |
| Bloodseeker Palanquin | 220 |
| Terrorgheist | 220 |
| Vengorian Lord | 220 |
| Coven Throne | 230 |
| Liege-Kavalos on War Chariot | 230 |
| Mortis Engine | 230 |
| Royal Terrorgheist | 230 |
| Morghast Harbingers | 250 |
| Mortek Crawler | 250 |
| Royal Zombie Dragon | 250 |
| Morghast Archai | 260 |
| Black Coach (Scourge of Ghyran) | 290 |
| Abhorrant Ghoul King on Royal Terrorgheist | 380 |
| Abhorrant Ghoul King on Royal Zombie Dragon | 380 |
| Vampire Lord on Zombie Dragon | 400 |
| Nagash, Supreme Lord of the Underworlds | 670 |
