# Kruleboyz

## Army Rules

### Anvil of Apotheosis

#### Out of the Mists (Passive)
- **Effect:** Subtract 1 from hit rolls and wound rolls for ranged attacks that target friendly Kruleboyz units while they are wholly within 12" of this unit.

#### Only Da Best (Passive)
- **Effect:** Add 3" to the Range characteristic of ranged weapons used by friendly Kruleboyz units. In addition, you can re-roll 1 hit roll per turn for shooting attacks made by each friendly Kruleboyz unit.

#### Super Kreepy (Activated)
- **Timing:** Once Per Battle, Any Charge Phase
- **Effect:** Subtract 1 from the Attacks characteristic of melee weapons for attacks that target this unit.

#### Extremely Loud (Passive)
- **Effect:** Enemy Heroes cannot use commands while they are in combat with this unit.

#### A Knack for Killin' (Passive)
- **Effect:** Ward rolls cannot be made for damage points inflicted by this unit’s combat attacks.

#### Monsta-killa (Activated)
- **Timing:** Once Per Battle (Army), Your Combat Phase
- **Declare:** Pick an enemy Monster in combat with this unit to be the target.
- **Effect:** Roll a dice. On a 1, nothing happens. On a 2-5, inflict D6 mortal damage on the target. On a 6, inflict 2D6 mortal damage on the target.

#### Obsessed With Violence (Passive)
- **Effect:** Add 1 to hit rolls for this unit’s attacks while it is in combat. Add 1 to this unit’s Move characteristic while it is not in combat.

#### Pig-headed (Passive)
- **Effect:** This unit cannot use Retreat abilities.

#### Clumsy Rider (Passive)
- **Effect:** Subtract 1 from the Attacks characteristic of this unit’s **Companion** melee weapons.

#### Roaring Bully (Passive)
- **Effect:** This unit cannot contest objectives that are being contested by enemy Unique Heroes.

#### Far Too Eager (Passive)
- **Effect:** In your charge phase, if this unit is not in combat and there are any enemy units within 12" of this unit, this unit must use the ‘Charge’ ability.

#### Scarred 'Un (Passive)
- **Effect:** While this unit has 10 or more damage points:

#### Mean 'Un (Activated)
- **Timing:** Once Per Turn (Army), Any Charge Phase
- **Declare:** If this unit charged this phase, pick an enemy Infantry unit within 1" of it to be the target.
- **Effect:** Roll a dice. On a 3+, inflict an amount of mortal damage on the target equal to the roll.
- **Keywords:** Rampage

#### Loud 'Un (Activated)
- **Timing:** Once Per Turn, Reaction: Opponent declared a command for a unit within 12" of this unit
- **Effect:** Roll a dice. On a 4+, unless your opponent spends 1 additional command point to use that command, the command has no effect, it still counts as having been used and the command points spent to use the command are still lost.

#### Weird 'Un (Activated)
- **Timing:** Reaction: Opponent declared a Spell ability
- **Effect:** If this unit was picked to be the target of that spell, roll a dice. On a 4+, ignore the effect of that spell on this unit. This unit can use this ability more than once per phase but only once per Spell ability.

#### Smelly 'Un (Passive)
- **Effect:** Subtract 1 from hit rolls for combat attacks that target this unit if it has not charged this turn.

#### Touched by da Waaagh! (Passive)
- **Effect:** Add 1 to casting rolls for this unit.

### Paths

#### Slippery (Passive)
- **Effect:** Add 1 to run rolls and charge rolls for this unit.

#### Highly Acquisitive (Passive)
- **Effect:** While this unit is contesting an objective, add 1 to hit rolls for this unit's attacks.

#### Morkish Knowledge (Passive)
- **Effect:** You can re-roll casting rolls for this unit.

#### I'll Stab da 'Urty Bitz (Passive)
- **Effect:** This unit's melee weapons have **Anti-Hero (+1 Rend)**

#### You Stab da Soft Bitz (Activated)
- **Timing:** Your Hero Phase
- **Declare:** Pick a visible friendly unit wholly within 12" of this unit to be the target.
- **Effect:** Roll a dice. One a 2+ the target's melee weapons have **Anti-Infantry (+1 Rend)** until the start of your next turn.

#### Just You Try It (Activated)
- **Timing:** Once Per Turn (Army), Any Movement Phase
- **Declare:** Pick a friendly unit wholly within 12" of this unit to be the target.
- **Effect:** For the rest of the turn, each time an enemy unit uses a Charge ability and ends the move within 1" of the target, inflict D3 mortal damage on that enemy unit.

#### Utterly Unscrupulous (Passive)
- **Effect:** Each time your opponent makes an unmodified hit roll of 1 for a combat attack that targets this unit, inflict 1 mortal damage on the attacking unit after the Fight ability has been resolved.

#### Disconcerting Stare (Passive)
- **Effect:** If this unit is picked as the target of a Fight ability and this unit’s Control characteristic is equal to or higher than the Control characteristic of the enemy unit using that Fight ability, that enemy unit cannot use commands for the rest of the turn.

#### Boo! Haw Haw Haw... (Passive)
- **Effect:** Subtract 2 from the control scores of enemy units while they are in combat with this unit.

#### Merely Pretending (Activated)
- **Timing:** End of Any Turn
- **Effect:** Heal (D3) this unit.

#### Hit da Dirt! (Passive)
- **Effect:** Subtract 1 from the Rend characteristic of ranged weapons used for shooting attacks that target this unit.

#### Feigned Flight (Passive)
- **Effect:** This unit can use a Run ability and still use Shoot and/or Charge abilities later in the same turn.

#### Let's Get 'Em, Boss! (Passive)
- **Effect:** While any friendly Infantry Heroes are within this unit's combat range, both this unit and those Infantry Heroes have Ward (5+).

#### We'll Look After Dis! (Passive)
- **Effect:** While each model in this unit is contesting an objective you control, add 1 to save rolls for this unit.

#### Cull 'Em All (Passive)
- **Effect:** Add 1 to the Attacks characteristic of this unit's melee weapons for attacks that target an enemy unit that has 5 or more models.

#### Practically Gorkish (Passive)
- **Effect:** If this unit charged this turn, for the rest of the turn, add 1 to charge rolls for friendly non-Hero Infantry units while they are wholly within 18" of this unit.

### Battle Traits

#### Dirty Tricks (Passive)
- **Effect:** Your army can use 1 Dirty Trick ability per phase. The effect of a Dirty Trick ability is only applied if you make a successful dirty trick roll. The success of a dirty trick roll depends on the number of Dirty Trick abilities your army has already used this battle round:
**First:** 2+
**Second:** 3+
**Third:** 4+
**Fourth or subsequent:** 5+

#### Venom-encrusted Weapons (Activated)
- **Timing:** Once Per Turn (Army), Your Hero Phase
- **Declare:** Pick a friendly Kruleboyz Infantry unit to be the target, then make a dirty trick roll.
- **Effect:** Until the start of your next turn, the target’s attacks score critical hits on unmodified hit rolls of 5+.
- **Keywords:** Dirty Trick

#### Lethal Surprise (Activated)
- **Timing:** Once Per Turn (Army), Any Charge Phase
- **Declare:** Pick an enemy unit that charged this turn and is in combat with any friendly Kruleboyz units to be the target, then make a dirty trick roll.
- **Effect:** Inflict D3 mortal damage on the target. In addition, for the rest of the turn, the target’s weapons cannot be affected by the Charge (+1 Damage) weapon ability
- **Keywords:** Dirty Trick

#### Sneaky Sneakin' (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Declare:** Pick a friendly Kruleboyz Infantry unit that has 10 or fewer models, is not in combat and has not been the target of a Dirty Trick ability this turn to be the target, then make a dirty trick roll.
- **Effect:** Remove the target from the battlefield and set it up again wholly within 3" of a terrain feature and more than 9" from all enemy units.
- **Keywords:** Dirty Trick

#### Noisy Racket (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy unit in combat with any friendly Kruleboyz units to be the target, then make a dirty trick roll.
- **Effect:** The target has Strike-last for the rest of the turn.
- **Keywords:** Dirty Trick

### Artefacts of Power

#### Swamp Staff (Passive)
- **Effect:** If this unit is not a Wizard, it can use the ‘Unbind’ ability as if it had Wizard (1). Otherwise, add 1 to this unit’s power level.

#### Mork's Eye Pebble (Activated)
- **Timing:** Once Per Battle, Reaction: Opponent declared an Attack ability
- **Effect:** For the rest of the turn, friendly Kruleboyz units have Ward (5+) while they are wholly within 12" of this unit.

#### Eye-biter Ash (Activated)
- **Timing:** Once Per Battle, Any Combat Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Subtract 1 from hit rolls and wound rolls for the target's attacks for the rest of the turn.

### Battle Formations

#### Ded Sneaky (Passive)
- **Effect:** You can use the ‘Sneaky Sneakin'’ Dirty Trick ability twice per turn instead of only once per turn.

#### Meanest Beasts (Passive)
- **Effect:** **Companion** weapons used by friendly Kruleboyz units have **Crit (2 Hits)**.

#### Swamp Shroud (Passive)
- **Effect:** Friendly Kruleboyz units that are wholly within 3" of a terrain feature are not visible to enemy units that are more than 12" away from them.

#### Far-killa Bolts (Activated)
- **Timing:** Your Shooting Phase
- **Declare:** Pick up to 3 friendly **Man-skewer Boltboyz** or **Beast skewer Killbow** units to be the targets.
- **Effect:** For the rest of the turn, add 3" to the Range characteristic of the targets’ ranged weapons.

#### Gangin' Up (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick a friendly Kruleboyz unit to be the target. That unit must be in combat with an enemy unit that is in combat with another friendly Kruleboyz unit. **Kragnos** cannot be picked to be the target.
- **Effect:** For the rest of the turn, add 1 to the Attacks characteristic of the target's melee weapons.

#### Krule Mockery (Activated)
- **Timing:** Once Per Turn (Army), Your Shooting Phase
- **Declare:** Pick a friendly Kruleboyz unit to be the **Insult Hurla**, pick a visible enemy unit within 18" of it to be the target, then make a dirty trick roll.
- **Effect:** Until the start of your next turn, each time the target uses a Move ability and does not end that move closer to the **Insult Hurla** than it was at the start of the move, inflict D6 mortal damage on the target after that Move ability has been resolved.
- **Keywords:** Dirty Trick

### Heroic Traits

#### Slippery Skumbag (Activated)
- **Timing:** Any Combat Phase
- **Effect:** If this unit is in combat, roll a dice. On a 3+, it can immediately use the ‘Retreat’ ability as if it were your movement phase without any mortal damage being inflicted on it.

#### Egomaniak (Passive)
- **Effect:** While this unit is within the combat range of a friendly non-Hero Kruleboyz Infantry unit:
• This unit has Ward (4+).
• Each time you make a successful ward roll for this unit, allocate 1 damage point to a friendly non-Hero Kruleboyz Infantry unit within this unit’s combat range after the damage sequence for this unit has been resolved (ward rolls cannot be made for those damage points).

#### Backstabba (Activated)
- **Timing:** End of Any Turn
- **Declare:** Pick an enemy Hero in combat with both this unit and another friendly Kruleboyz unit to be the target.
- **Effect:** Inflict D3 mortal damage on the target. Add 1 to the mortal damage inflicted for each friendly Kruleboyz unit in combat with the target in addition to the first two Kruleboyz units.

### Monstrous Traits

#### Mean 'Un (Passive)
- **Effect:** Add 1 to hit rolls for attacks made with this unit's **Companion** weapons.

#### Sneaky 'Un (Activated)
- **Timing:** Once Per Battle, Enemy Hero Phase
- **Effect:** This unit can use the 'Normal Move' or 'Retreat' ability as if it were your movement phase. In addition, no mortal damage is inflicted on this unit by Retreat abilities this phase.

#### Tough 'Un (Passive)
- **Effect:** Subtract 1 from the Rend characteristic of weapons used for attacks that target this unit.

## Subfactions

### Big Waaagh!

### Murkvast Menagerie

**Battle Traits**

#### After It, Ladz! (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Declare:** Pick a friendly Murkvast Menagerie Monster unit that was not set up this turn to be the target.
- **Effect:** The target can move up to 3". It can move into combat. If it was in combat at the start of the move, it must end that move in combat.

#### Crawly Swarm (Passive)
- **Effect:** Subtract 1 from hit rolls for combat attacks that target friendly Murkvast Menagerie units while they are wholly within 12" of a friendly Murkvast Menagerie Monster that has not charged this turn.

#### Propa Grisly (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick a friendly Murkvast Menagerie Monster unit that has not used any Rampage abilities this turn to use this ability. Then, pick an enemy unit in combat with it to be the target.
- **Effect:** Roll a number of dice equal to the Health characteristic of that Murkvast Menagerie Monster. For each 6+, inflict 1 mortal damage on the target.
After this ability has been resolved, this unit cannot use any other Rampage abilities for the rest of the turn.
- **Keywords:** Rampage

**Artefacts of Power**

#### Skinwriggla Larvae (Activated)
- **Timing:** Once Per Battle, Your Hero Phase
- **Declare:** Pick an objective within 6" of this unit to be **infested with bugs** for the rest of the battle.

**Heroic Traits**

#### Grim Diet (Passive)
- **Effect:** This unit has Ward (5+)

## Units

### Beast-skewer Killbow
**Points:** 140 | **Move:** 5" | **Health:** 5 | **Save:** 5+ | **Control:** 2
**Keywords:** DESTRUCTION, KRULEBOYZ, WAR MACHINE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Beast-skewer Bolts | Ranged | 24" | 2 | 4+ | 2+ | 2 | D6 | Anti-Monster (+1 Rend), Crit (Auto-wound) |
| Jaggedy Blades | Melee | - | 2 | 4+ | 3+ | - | 1 | Crit (Mortal) |

**Abilities:**

#### Skewering Bolts (Activated)
- **Timing:** Once Per Turn (Army), Your Shooting Phase
- **Effect:** For the rest of the turn, the Damage characteristic of this unit’s **Beast-skewer Bolts** is 6 if the target is a Monster.

#### Pick 'Em Off (Activated)
- **Timing:** Your Shooting Phase
- **Effect:** If this unit has not used a Move ability this turn and was not set up this turn, add 1 to hit rolls for this unit’s shooting attacks for the rest of the turn.

---

### Breaka-boss on Mirebrute Troggoth
**Points:** 180 | **Move:** 5" | **Health:** 12 | **Save:** 4+ | **Control:** 5
**Keywords:** HERO, DESTRUCTION, KRULEBOYZ, MONSTER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Bident-goad | Melee | - | 5 | 4+ | 3+ | 1 | 2 | Crit (Mortal) |
| Mirebrute’s Clubs | Melee | - | 4 | 4+ | 2+ | 2 | 3 | Companion |

**Abilities:**

#### Tear Limb From Limb (Activated)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Declare:** Pick an enemy Infantry unit within 1" of this unit to be the target.
- **Effect:** Roll a dice. If the roll exceeds the target’s Health characteristic, 1 model in the target unit is slain.
- **Keywords:** Rampage

#### Breaka-harness (Activated)
- **Timing:** Any Combat Phase
- **Effect:** Make a breaka-roll of D3.
• Inflict an amount of mortal damage on this unit equal to the breaka-roll.
• Double the breaka-roll and add that value to the Attacks characteristic of this unit’s **Mirebrute’s Clubs** for the rest of the turn.

#### Regeneration (Activated)
- **Timing:** Start of Any Turn
- **Effect:** **Heal (D3)** this unit.

---

### Da Kunnin' Krew
**Points:** 120 | **Move:** 5" | **Health:** 3 | **Save:** 5+ | **Control:** 1
**Keywords:** UNIQUE, Legends, CHAMPION, DESTRUCTION, KRULEBOYZ, INFANTRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Krew Weapons | Melee | - | 3 | 4+ | 3+ | 1 | 1 | Crit (Mortal) |
| Krew Weapons | Melee | - | 3 | 4+ | 3+ | 1 | 1 | Crit (Mortal) |
| Krew Weapons | Melee | - | 3 | 4+ | 3+ | 1 | 1 | Crit (Mortal) |
| Krew Weapons | Melee | - | 3 | 4+ | 3+ | 1 | 1 | Crit (Mortal) |
| Krew Weapons | Melee | - | 3 | 4+ | 3+ | 1 | 1 | Crit (Mortal) |

**Abilities:**

#### Easy Meat (Activated)
- **Timing:** Once Per Turn (Army), Enemy Hero Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a dice. If the roll is at least double the target’s Health characteristic, 1 model in the target unit is slain.

---

### Daggok's Stab-ladz
**Points:** 120 | **Move:** 5" | **Health:** 3 | **Save:** 5+ | **Control:** 1
**Keywords:** UNIQUE, Legends, CHAMPION, DESTRUCTION, KRULEBOYZ, INFANTRY, MUSICIAN (1/4)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Wicked Weapons | Melee | - | 3 | 4+ | 3+ | 1 | 1 | Crit (Mortal) |
| Wicked Weapons | Melee | - | 3 | 4+ | 3+ | 1 | 1 | Crit (Mortal) |
| Wicked Weapons | Melee | - | 3 | 4+ | 3+ | 1 | 1 | Crit (Mortal) |
| Wicked Weapons | Melee | - | 3 | 4+ | 3+ | 1 | 1 | Crit (Mortal) |

**Abilities:**

#### Roar of Kragnos (Activated)
- **Timing:** Any Combat Phase
- **Declare:** If this unit’s **Hurrk da Howla** is on the battlefield, pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a dice. On a 3+, subtract 1 from the Attacks characteristic of the target’s melee weapons for the rest of the turn.

---

### Gobsprakk, the Mouth of Mork
**Points:** 240 | **Move:** 12" | **Health:** 14 | **Save:** 5+ | **Control:** 5
**Keywords:** HERO, UNIQUE, MONSTER, WARMASTER, DESTRUCTION, KRULEBOYZ, WIZARD (2), WARD (6+), FLY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Bogbark Staff | Melee | - | 3 | 4+ | 3+ | - | D3 | Anti-Wizard (+1 Rend), Crit (Mortal) |
| Killabeak’s Stinger | Melee | - | 1 | 3+ | 2+ | 1 | D6 | Crit (Mortal), Companion |
| Killabeak’s Talons and Beak | Melee | - | 7 | 4+ | 2+ | 1 | 2 | Anti-Wizard (+1 Rend), Companion |

**Abilities:**

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of **Killabeak’s Talons and Beak** is 5.

#### Screamin' Mandrakk (Activated)
- **Timing:** Once Per Battle, Reaction: Opponent declared a Spell ability
- **Effect:** This unit uses the ‘Unbind’ ability but the unbinding roll is 3D6 instead of 2D6.

#### Crush to Death (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a dice. If the roll exceeds the target’s Health characteristic, 1 model in the target unit is slain.
- **Keywords:** Rampage

#### Mork Sez No! (Passive)
- **Effect:** Each time this unit unbinds a spell, inflict D3 mortal damage on the caster. If the unbinding roll was 10+, inflict D6 mortal damage on the caster instead.

#### Strangle Hex (Spell)
- **Timing:** Your Hero Phase
- **Casting Value:** 6
- **Declare:** Pick a visible enemy unit within 18" of this unit to be the target, then make a casting roll of 2D6.
- **Effect:** Roll a number of dice equal to the target’s Health characteristic, to a maximum of 10. For each 4+, inflict 1 mortal damage on the target.
- **Keywords:** Spell

---

### Gutrippaz
**Points:** 160 | **Move:** 5" | **Health:** 2 | **Save:** 5+ | **Control:** 1
**Keywords:** INFANTRY, DESTRUCTION, KRULEBOYZ, CHAMPION, MUSICIAN (1/10), STANDARD BEARER (1/10)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Stikka or Hacka | Melee | - | 2 | 4+ | 3+ | - | 1 | Crit (Mortal) |

**Abilities:**

#### Scare Taktikz (Activated)
- **Timing:** Any Combat Phase
- **Effect:** Roll a dice. On a 3+, subtract 1 from hit rolls for attacks made by non-Hero units that target this unit for the rest of the turn.

---

### Hobgrot Slittaboss
**Points:** 70 | **Move:** 5" | **Health:** 5 | **Save:** 6+ | **Control:** 2
**Keywords:** HERO, DESTRUCTION, KRULEBOYZ, INFANTRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Toxin-smeared Slitta-sword | Melee | - | 4 | 4+ | 4+ | 1 | D3 | Crit (Mortal) |

**Abilities:**

#### Toxic Stash (Activated)
- **Timing:** Once Per Battle, Any Combat Phase
- **Declare:** Pick an enemy Infantry Hero in combat with this unit to be the target.
- **Effect:** Roll 2D6. If the roll exceeds the target's Health characteristic, it is automatically destroyed.

#### Kompany Taktikz (Activated)
- **Timing:** Reaction: You declared a Fight ability for this unit
- **Effect:** Pick a friendly **Hobgrot Slittaz** unit that has not used a Fight ability this turn and is within this unit's combat range to be the target. The target can be picked to use a Fight ability immediately after the Fight ability used by this unit has been resolved. If it is picked to do so, add 1 to wound rolls for the target's attacks for the rest of the turn.

---

### Hobgrot Slittaz
**Points:** 70 | **Move:** 5" | **Health:** 1 | **Save:** 6+ | **Control:** 1
**Keywords:** INFANTRY, CHAMPION, DESTRUCTION, STANDARD BEARER (1/10), MUSICIAN (1/10), KRULEBOYZ

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Scrap-grenades | Ranged | 8" | 1 | 4+ | 3+ | 1 | 1 | Shoot in Combat |
| Slitta-knives | Melee | - | 2 | 4+ | 5+ | - | 1 | - |

**Abilities:**

#### Scrap-bang (Activated)
- **Timing:** End of Any Turn
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a dice. On a 3+:
• Inflict 1 mortal damage on the target.
• This unit can immediately use the ‘Retreat’ ability as if it were your movement phase without any mortal damage being inflicted on it.

---

### Killaboss on Corpse-rippa Vulcha
**Points:** 240 | **Move:** 12" | **Health:** 14 | **Save:** 4+ | **Control:** 5
**Keywords:** HERO, MONSTER, DESTRUCTION, KRULEBOYZ, FLY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Boss-stikka or Boss-hacka | Melee | - | 5 | 4+ | 3+ | 1 | 2 | Crit (Mortal) |
| Vulcha’s Stinger | Melee | - | 1 | 3+ | 2+ | 1 | D6 | Crit (Mortal), Companion |
| Vulcha’s Talons and Beak | Melee | - | 7 | 4+ | 2+ | 1 | 2 | Companion |

**Abilities:**

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of **Vulcha’s Talons and Beak** is 5.

#### Commanding View (Activated)
- **Timing:** Reaction: You declared the ‘Redeploy’ command for a friendly **Gutrippaz** unit wholly within 12" of this unit
- **Effect:** If you roll a 1-3 when determining the distance that unit can move, you can use a value of 4 instead.

#### Vulcha Dive (Activated)
- **Timing:** Once Per Turn (Army), Any Charge Phase
- **Declare:** If this unit charged this phase, pick an enemy unit in combat with it to be the target.
- **Effect:** Roll a number of dice equal to the unmodified charge roll. For each 4+, inflict 1 mortal damage on the target.
- **Keywords:** Rampage

---

### Killaboss on Great Gnashtoof
**Points:** 140 | **Move:** 10" | **Health:** 10 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, DESTRUCTION, KRULEBOYZ, CAVALRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Boss-stikka | Melee | - | 5 | 4+ | 3+ | 1 | 2 | Crit (Mortal) |
| Great Gnashtoof’s Jaws | Melee | - | 5 | 4+ | 3+ | 1 | 2 | Companion |

**Abilities:**

#### That's Ours, Ya Gitz (Passive)
- **Effect:** While this unit is contesting an objective you do not control, add 1 to hit rolls for combat attacks made by friendly Kruleboyz units wholly within 12" of this unit.

#### All Part of da Plan (Activated)
- **Timing:** Your Hero Phase
- **Declare:** Pick another friendly Kruleboyz unit wholly within 9" of this unit to be the target.
- **Effect:** Add 3 to the target’s control score until the start of your next turn.

---

### Killaboss on Great Gnashtoof (Scourge of Ghyran)
**Points:** 150 | **Move:** 10" | **Health:** 10 | **Save:** 3+ | **Control:** 2
**Keywords:** HERO, DESTRUCTION, KRULEBOYZ, CAVALRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Boss-stikka | Melee | - | 5 | 4+ | 3+ | 1 | 2 | Anti-Hero (+1 Rend), Crit (Mortal) |
| Great Gnashtoof’s Jaws | Melee | - | 5 | 4+ | 3+ | 1 | 2 | Companion |

**Abilities:**

#### Drag 'Em Out (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy Infantry Hero in combat with this unit to be the target.
- **Effect:** Roll 2D6. This unit can move a distance in inches up to the value of the roll. Then, if possible, your opponent must remove the target from the battlefield and set it up again within 1" of this unit.

#### Prowling the Flanks (Activated)
- **Timing:** Once Per Turn (Army), Your Movement Phase
- **Effect:** If this unit is wholly within 6" of the battlefield edge, remove this unit from the battlefield and set it up again wholly within 6" of the battlefield edge and more than 9" from all enemy units.

---

### Killaboss with Stab-grot
**Points:** 80 | **Move:** 5" | **Health:** 6 | **Save:** 4+ | **Control:** 2
**Keywords:** HERO, DESTRUCTION, KRULEBOYZ, INFANTRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Boss-hacka | Melee | - | 5 | 4+ | 3+ | 1 | 2 | Crit (Mortal) |
| Stab-grot’s Shiv | Melee | - | 2 | 4+ | 5+ | - | 1 | Crit (Mortal), Companion |

**Abilities:**

#### Keep 'Em in Line (Activated)
- **Timing:** Reaction: You declared the ‘All-out Attack’ command for a friendly Kruleboyz Infantry unit
- **Effect:** If this unit is wholly within 12" of the unit using the ‘All-out Attack’ command, add 1 to wound rolls for combat attacks made by that unit for the rest of the phase.

#### Unleash the Stab-grot! (Activated)
- **Timing:** Once Per Battle, Any Combat Phase
- **Effect:** Pick 1 of the following effects:
• Pick an enemy unit in combat with this unit to be the target. Inflict D3 mortal damage on the target.
• This unit has Ward (5+) for the rest of the turn.
After resolving either effect, remove this unit’s **Stab-grot** from the battlefield.

#### Stab-grot (Passive)
- **Effect:** This unit’s **Stab-grot** is a token. If this unit’s **Stab grot** has been removed, it can no longer attack with its **Stab-grot’s Shiv** and it cannot use the ‘Unleash the Stab-grot!’ ability.

---

### Kragnos, the End of Empires
**Points:** 580 | **Move:** 10" | **Health:** 18 | **Save:** 4+ | **Control:** 15
**Keywords:** HERO, UNIQUE, MONSTER, WARMASTER, WARD (5+), DESTRUCTION, IRONJAWZ, KRULEBOYZ

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| The Dread Mace | Melee | - | 6 | 3+ | 2+ | 3 | 4 | - |
| Tuskbreaker | Melee | - | 3 | 4+ | 2+ | 2 | D3 | Crit (2 Hits) |
| Hooves of Wrack and Ruin | Melee | - | 6 | 3+ | 2+ | 1 | 2 | - |

**Abilities:**

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of **The Dread Mace** is 4 and this unit has a Control characteristic of 10.

#### Avatar of Destruction (Passive)
- **Effect:** If this unit would be automatically destroyed, it is not automatically destroyed. Instead, allocate 6 damage points to it (ward rolls cannot be made for those damage points).

#### Rampaging Destruction (Activated)
- **Timing:** Once Per Turn (Army), Any Charge Phase
- **Effect:** If this unit charged this phase, pick 1 of the following effects:
• Roll a dice for each enemy unit within 1" of this unit. On a 2+, inflict an amount of mortal damage on that unit equal to the roll.
• Pick an enemy Monster in combat with this unit and roll 2D6. On a 7, this ability has no effect. Otherwise, inflict an amount of mortal damage on that unit equal to the results on the dice used for the 2D6 roll multiplied together. For example, a 2D6 roll of 2 and 6 would inflict 12 mortal damage (2 × 6).
- **Keywords:** Rampage

#### The End of Empires (Activated)
- **Timing:** Your Charge Phase
- **Effect:** For the rest of the turn, add 1 to the number of dice rolled when making charge rolls for friendly Destruction units while they are wholly within 12" of this unit, to a maximum of 3.

#### The Shield Inviolate (Activated)
- **Timing:** Reaction: Opponent declared a Spell ability
- **Effect:** If this unit was picked to be the target of that spell, roll a dice. On a 3+, ignore the effect of that spell on this unit. This unit can use this ability more than once per phase but only once per Spell ability.

---

### Kruleboyz Monsta-killaz
**Points:** 120 | **Move:** 5" | **Health:** 2 | **Save:** 5+ | **Control:** 1
**Keywords:** INFANTRY, DESTRUCTION, KRULEBOYZ, CHAMPION (1/7), MUSICIAN (1/7)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Monsta-killa Weapons | Melee | - | 3 | 4+ | 3+ | 1 | 1 | Anti-Monster (+1 Rend), Crit (Mortal) |

**Abilities:**

#### Klutcha-grot (Passive)
- **Effect:** This unit’s **Klutcha-grot** is a token. If it is removed from the battlefield, this unit cannot use the ‘A Tough Grot to Swallow’ ability.

#### Bait and Trap (Activated)
- **Timing:** Any Combat Phase
- **Declare:** Pick an enemy Monster in combat with this unit to be the target.
- **Effect:** Roll a dice. Add 1 to the roll if the target is damaged. On a 3+, the target has Strike-last for the rest of the turn.

#### A Tough Grot to Swallow (Activated)
- **Timing:** Once Per Turn, Reaction: Opponent declared a Rampage ability for an enemy Monster within 6" of this unit
- **Effect:** Roll a dice. On a 2+, that Rampage ability has no effect. On a 1, remove this unit’s **Klutcha-grot** from the battlefield.

---

### Man-skewer Boltboyz
**Points:** 100 | **Move:** 5" | **Health:** 2 | **Save:** 5+ | **Control:** 1
**Keywords:** INFANTRY, CHAMPION, DESTRUCTION, KRULEBOYZ

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Man-skewer Crossbow: Hasty Shot | Ranged | 12" | 2 | 4+ | 3+ | 1 | 2 | Crit (Auto-wound) |
| Man-skewer Crossbow: Aimed Shot | Ranged | 18" | 1 | 3+ | 3+ | 2 | 2 | Crit (Auto-wound) |
| Jaggedy Blades | Melee | - | 1 | 4+ | 3+ | - | 1 | Crit (Mortal) |

**Abilities:**

#### Man-skewer Crossbows (Passive)
- **Effect:** Each time this unit uses a Shoot ability, pick either the **Hasty Shot** or **Aimed Shot** weapon characteristics for all the attacks it makes with its **Man‑skewer Crossbows**.

#### Pick 'Em Off (Activated)
- **Timing:** Your Shooting Phase
- **Effect:** If this unit has not used a Move ability this turn and was not set up this turn, add 1 to hit rolls for this unit’s shooting attacks for the rest of the turn.

---

### Marshcrawla Sloggoth
**Points:** 130 | **Move:** 6" | **Health:** 10 | **Save:** 4+ | **Control:** 2
**Keywords:** DESTRUCTION, KRULEBOYZ, WAR MACHINE

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Nets and Snatcha‑stikks | Melee | - | 4 | 4+ | 5+ | - | 1 | - |
| Sloggoth’s Claws | Melee | - | 6 | 4+ | 2+ | 1 | 2 | Companion |

**Abilities:**

#### On da March (Passive)
- **Effect:** While they are wholly within 12" of this unit, friendly Kruleboyz Infantry units can use Charge abilities even if they used a Run ability in the same turn.

#### Regeneration (Activated)
- **Timing:** Start of Any Turn
- **Effect:** **Heal (D3)** this unit.

---

### Murknob with Belcha-banna
**Points:** 90 | **Move:** 5" | **Health:** 6 | **Save:** 4+ | **Control:** 5
**Keywords:** HERO, DESTRUCTION, KRULEBOYZ, INFANTRY

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Murknob Cleaver | Melee | - | 4 | 4+ | 3+ | 1 | 2 | Crit (Mortal) |

**Abilities:**

#### Power of Kragnos (Passive)
- **Effect:** Friendly Kruleboyz units have Ward (6+) while they are wholly within 12" of this unit.

#### Breath of the Mire-drakes (Activated)
- **Timing:** Any Combat Phase
- **Declare:** Pick each enemy unit in combat with this unit to be the targets.
- **Effect:** Roll a dice for each target. On a 1, this ability has no effect. On a 2-5, inflict 1 mortal damage on the target. On a 6, inflict D3 mortal damage on the target.

---

### Snatchaboss on Sludgeraker Beast
**Points:** 200 | **Move:** 8" | **Health:** 14 | **Save:** 4+ | **Control:** 5
**Keywords:** HERO, DESTRUCTION, KRULEBOYZ, MONSTER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Grappling Hook | Melee | - | 3 | 3+ | 3+ | 1 | 3 | Crit (Mortal) |
| Sludgeraker’s Talons | Melee | - | 6 | 4+ | 2+ | 1 | 2 | Companion |
| Sludgeraker’s Bite | Melee | - | 1 | 3+ | 2+ | 1 | D3+3 | Companion |

**Abilities:**

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of its **Sludgeraker’s Talons** is 4.

#### Festering Wounds (Activated)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Declare:** Pick up to 3 enemy units that had any damage points allocated to them this turn by attacks made by this unit or a friendly unit with the Sludgeraker Venom keyword to be the targets.
- **Effect:** Roll a D3 for each target. On a 2+:
• Inflict an amount of mortal damage on the target equal to the roll.
• Subtract 1 from wound rolls for the target’s attacks until the end of the next turn.
- **Keywords:** Rampage

#### Sludgeraker Venom (Activated)
- **Timing:** Your Hero Phase
- **Declare:** Pick a friendly Kruleboyz unit wholly within 12" of this unit to be the target.
- **Effect:** That unit has the Sludgeraker Venom keyword for the rest of the turn.

---

### Swampboss Skumdrekk
**Points:** 180 | **Move:** 8" | **Health:** 15 | **Save:** 4+ | **Control:** 5
**Keywords:** HERO, DESTRUCTION, KRULEBOYZ, UNIQUE, MONSTER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Sloppklaw’s Bite | Melee | - | 1 | 4+ | 2+ | 1 | D3+3 | Companion |
| Snatcha-stikk | Melee | - | 5 | 4+ | 3+ | 1 | 2 | Crit (Mortal) |
| Sloppklaw’s Talons | Melee | - | 6 | 4+ | 2+ | 1 | 2 | Companion |

**Abilities:**

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of **Sloppklaw’s Talons** is 4.

#### Kountin' Krew (Activated)
- **Timing:** Deployment Phase
- **Declare:** Pick a friendly **Hobgrot Slittaz** unit within this unit’s combat range to be the target.
- **Effect:** For the rest of the battle, the target’s **Slitta-knives** have **Crit (Mortal)** while they are wholly within 12" of this unit.

#### Aggravate Wounds (Activated)
- **Timing:** Once Per Turn (Army), End of Any Turn
- **Declare:** Pick an enemy Monster in combat with this unit to be the target.
- **Effect:** Roll a number of dice equal to the number of damage points the target has. For each 5+, inflict 1 mortal damage on the target.
- **Keywords:** Rampage

#### Bet-master (Activated)
- **Timing:** Deployment Phase
- **Declare:** Pick an enemy unit on the battlefield to be **the bet**.
- **Effect:** Add 1 to wound rolls for attacks made by this unit and friendly **Hobgrot Slittaz** units that target **the bet**.

---

### Swampboss Skumdrekk (Scourge of Ghyran)
**Points:** 210 | **Move:** 8" | **Health:** 15 | **Save:** 4+ | **Control:** 5
**Keywords:** HERO, DESTRUCTION, KRULEBOYZ, UNIQUE, MONSTER

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Sloppklaw’s Bite and Talons | Melee | - | 6 | 4+ | 2+ | 1 | 3 | Crit (Mortal), Companion |
| Snatcha-stikk | Melee | - | 5 | 4+ | 3+ | 1 | 2 | Crit (Mortal) |

**Abilities:**

#### Battle Damaged (Passive)
- **Effect:** While this unit has 10 or more damage points, the Attacks characteristic of **Sloppklaw’s Bite and Talons** is 4.

#### Rigged Odds (Passive)
- **Effect:** The first time you fail a dirty trick roll each turn, give this unit 1 **betting chip**.
Each time you make a dirty trick roll for an ability that targets a unit wholly within 12" of this unit, you can spend any number of **betting chips** after seeing the result of the roll. For each **betting chip** you spend, add 1 to that dirty trick roll.

#### Thrashing Tail (Activated)
- **Timing:** Once Per Turn (Army), Any Combat Phase
- **Declare:** Pick an enemy unit in combat with this unit to be the target.
- **Effect:** Roll a dice. If the roll is less than or equal to the number of models in the target unit, for the rest of the turn, each time this unit uses an Attack ability, if all of the attacks target that enemy unit, those attacks score critical hits on unmodified hit rolls of 5+. This ability also affects **Companion** weapons.
- **Keywords:** Rampage

---

### Swampcalla Shaman with Pot-grot
**Points:** 120 | **Move:** 5" | **Health:** 6 | **Save:** 6+ | **Control:** 2
**Keywords:** HERO, DESTRUCTION, KRULEBOYZ, INFANTRY, WIZARD (1)

**Weapons:**
| Weapon | Type | Rng | Atk | Hit | Wnd | Rnd | Dmg | Ability |
|--------|------|-----|-----|-----|-----|-----|-----|---------|
| Bogbark Staff | Melee | - | 3 | 4+ | 3+ | - | D3 | Crit (Mortal) |

**Abilities:**

#### Pot-grot (Passive)
- **Effect:** This unit’s **Pot-grot** is a token. Add 1 to casting rolls for this unit while its **Pot-grot** is on the battlefield. If you make an unmodified casting roll of 4 or less for this unit, remove its **Pot-grot** from the battlefield.

#### Foul Elixirs (Activated)
- **Timing:** Once Per Turn (Army), Your Hero Phase
- **Declare:** Pick a friendly Kruleboyz Infantry unit wholly within 12" of this unit to be the target.
- **Effect:** Roll a dice. On a 2+, add 1 to save rolls for the target for the rest of the turn. On a 1, inflict D3 mortal damage on the target.

---

## Points Table

| Unit | Points |
| --- | ---: |
| Hobgrot Slittaboss | 70 |
| Hobgrot Slittaz | 70 |
| Killaboss with Stab-grot | 80 |
| Murknob with Belcha-banna | 90 |
| Man-skewer Boltboyz | 100 |
| Da Kunnin' Krew | 120 |
| Daggok's Stab-ladz | 120 |
| Kruleboyz Monsta-killaz | 120 |
| Swampcalla Shaman with Pot-grot | 120 |
| Marshcrawla Sloggoth | 130 |
| Beast-skewer Killbow | 140 |
| Killaboss on Great Gnashtoof | 140 |
| Killaboss on Great Gnashtoof (Scourge of Ghyran) | 150 |
| Gutrippaz | 160 |
| Breaka-boss on Mirebrute Troggoth | 180 |
| Swampboss Skumdrekk | 180 |
| Snatchaboss on Sludgeraker Beast | 200 |
| Swampboss Skumdrekk (Scourge of Ghyran) | 210 |
| Gobsprakk, the Mouth of Mork | 240 |
| Killaboss on Corpse-rippa Vulcha | 240 |
| Kragnos, the End of Empires | 580 |
